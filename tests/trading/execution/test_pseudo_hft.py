...
class TestPseudoHFTExecutionModel:
    ...
    def test_interface_compatibility(self, model):
        ...
        assert (
            result.executed_price > 1000000.0
    ...
    def test_invalid_action_type(self, model):
        ...
        with pytest.raises(ValueError, match="Invalid action_type"):
            model.simulate_execution(
                action_type="hold", requested_price=1000000.0, requested_size=0.1
            )
    ...
    def test_slippage_components(self, model):
        ...
        expected_slippage = (
            5.0
            + (0.2 * 5.0 * (1.0 / 60.0) ** 0.5)
            + (0.5 * 5.0 * (0.1 / 10000.0) ** 0.5)
        )
    ...
    def test_missing_market_data_proxy(self, model):
        ...
        expected_slippage = (
            5.0
            + (0.2 * 10.0 * (1.0 / 60.0) ** 0.5)
            + (0.5 * 10.0 * (0.1 / 1000.0) ** 0.5)
        )
    ...
    def test_data_glitch_high_low(self, model):
        """Test that high < low (data glitch) is handled by clamping spread to 0, then using ATR proxy."""
        market_data: MarketState = {
        ...
        # Spread proxy:
        # 1. max(90-100, 0) = 0
        # 2. Fallback: if spread <= EPSILON and atr > EPSILON, use ATR (5.0)
        # 3. Proxy = c_spread * 5.0 = 0.5 * 5.0 = 2.5

        slippage = model.calculate_slippage_one_way(market_data, 0.1)

        # Expected: Spread(2.5) + Vol + Impact
        expected_slippage = (
            2.5
            + (0.2 * 5.0 * (1.0 / 60.0) ** 0.5)
            + (0.5 * 5.0 * (0.1 / 10000.0) ** 0.5)
    ...
    def test_market_data_zeros_fallback(self, model):
        """Test fallback when market_data is provided but contains zeros."""
        # Provided market data has 0 ATR and 0 Volume
        market_data: MarketState = {
            'high': 0.0, 'low': 0.0, 'atr': 0.0, 'volume': 0.0, 
            'close': 10000.0, 'timestamp': None
        }
        
        slippage = model.calculate_slippage_one_way(market_data, 0.1)
        
        # Should use fallback ATR = 10000 * 0.0005 = 5.0
        # Spread proxy (from ATR) = 0.5 * 5.0 = 2.5
        # Vol risk = 0.2 * 5.0 * sqrt(1/60) = 0.129
        # Impact (using min_volume=1.0 from fixture) = 0.5 * 5.0 * (0.1/1.0)^0.5 = 2.5 * 0.316 = 0.79
        
        expected = 2.5 + 0.129 + 0.79
        assert slippage > 0.0
        assert abs(slippage - expected) < 0.1

    def test_nan_inf_handling(self, model):
        """Test that NaN/Inf values are handled gracefully (treated as missing)."""
        market_data: MarketState = {
            'high': float('nan'), 'low': float('inf'), 
            'atr': float('nan'), 'volume': float('nan'), 
            'close': 10000.0, 'timestamp': None
        }
        
        slippage = model.calculate_slippage_one_way(market_data, 0.1)
        
        # Should behave same as zeros fallback
        expected = 2.5 + 0.129 + 0.79
        assert slippage > 0.0
        assert abs(slippage - expected) < 0.1
