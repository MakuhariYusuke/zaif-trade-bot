...
class TestCalibrationMap:
    ...
    def calibration_map(self):
        config = {"ewma_tau": 10.0, "n_min": 5.0}
...
class TestCalibrationGate:
    ...
    def test_order_size_nan(self, gate):
        """Test that NaN order_size triggers fail-closed."""
        fused: FusedSignal = {
            "rl_action": 0.8,
            "regime": "trending",
            "pattern_score": 0.0,
        ...
        res = gate.evaluate(fused, market, order_size=float("nan"))
        assert res["cost"] == float("inf")
    ...
    def test_blending_logic(self, gate):
        ...
        assert (
            res["ev"]
            == res["lambda_val"] * res["ev_l1"]
            + (1.0 - res["lambda_val"]) * res["ev_fb"]
        )
