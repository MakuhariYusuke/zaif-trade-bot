...
_FFD_BOOST_SOURCE = inspect.getsource(MakerPriceCalculator._apply_ffd_boost)
_CHECK_AND_RELOAD_MODEL_SOURCE = inspect.getsource(SkipGateEvaluator._check_and_reload_model)
_UPDATE_INVENTORY_SOURCE = inspect.getsource(MakerPriceCalculator.update_inventory)
_ORDER_MONITOR_MONITOR_SIG = inspect.signature(OrderMonitor.monitor)
...
class TestFFDBoostClamp:
    ...
    def test_boost_clamped_to_max(self) -> None:
        ...
        # FFD boost セクションが helper 経由で max_ratio clamp を使うことを確認
        # 405#: side-aware intermediate cap (_effective_max_ratio) に移行
        assert "boost_mult" in _FFD_BOOST_SOURCE
        assert "_scale_offset_ratio" in _FFD_BOOST_SOURCE
        assert "max_ratio=" in _FFD_BOOST_SOURCE, "FFD boost 後に helper clamp が必要"
...
class TestCalibratorNotDoubled:
    ...
    def test_no_double_inject(self) -> None:
        ...
        code_lines = [
            line.strip()
            for line in _CHECK_AND_RELOAD_MODEL_SOURCE.split("\n")
...
class TestDeadCodeRemoved:
    ...
    def test_no_n_equals_zero_check(self) -> None:
        assert "n == 0" not in _UPDATE_INVENTORY_SOURCE
...
class TestOrderMonitorProtocols:
    ...
    def test_monitor_signature_typed(self) -> None:
        params = _ORDER_MONITOR_MONITOR_SIG.parameters
        # object ではなく型付きであること
    ...
    def test_pending_order_setter_callable(self) -> None:
        ann = str(_ORDER_MONITOR_MONITOR_SIG.parameters["pending_order_setter"].annotation)
        assert "Callable" in ann

    def test_get_mid_price_awaitable(self) -> None:
        ann = str(_ORDER_MONITOR_MONITOR_SIG.parameters["get_mid_price"].annotation)
