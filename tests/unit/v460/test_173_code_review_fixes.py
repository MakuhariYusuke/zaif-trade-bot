...
_COMPUTE_SIG = inspect.signature(MakerPriceCalculator.compute)
...
class TestOrderbookProviderType:
    ...
    def test_compute_imbalance_type_annotation(self) -> None:
        assert "OrderbookProvider" in str(
            _COMPUTE_IMBALANCE_SIG.parameters["adapter"].annotation
        )

    def test_get_mid_price_type_annotation(self) -> None:
        assert "OrderbookProvider" in str(
            _GET_MID_PRICE_SIG.parameters["adapter"].annotation
        )

    def test_compute_type_annotation(self) -> None:
        assert "OrderbookProvider" in str(
            _COMPUTE_SIG.parameters["adapter"].annotation
        )
