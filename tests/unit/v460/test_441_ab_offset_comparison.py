...
class TestLoadRecordsFilter:
    """451# P0-4: _load_records の git_sha/run_id フィルタテスト."""

    def _write_jsonl(self, tmpdir: Path, records: list[dict[str, object]]) -> None:
        path = tmpdir / "fill_records_20260316.jsonl"
    ...
    def test_no_filter(self, tmp_path: Path) -> None:
        recs = [
            {"cycle_id": "c1", "git_sha": "abc123", "run_id": "r1", "filled": True},
            {"cycle_id": "c2", "git_sha": "def456", "run_id": "r2", "filled": True},
        ]
        self._write_jsonl(tmp_path, recs)
        loaded = _load_records(tmp_path)
        assert len(loaded) == 2

    def test_git_sha_filter(self, tmp_path: Path) -> None:
        recs = [
            {"cycle_id": "c1", "git_sha": "abc123", "run_id": "r1", "filled": True},
            {"cycle_id": "c2", "git_sha": "def456", "run_id": "r2", "filled": True},
            {"cycle_id": "c3", "git_sha": "abc999", "run_id": "r3", "filled": True},
        ]
        self._write_jsonl(tmp_path, recs)
        loaded = _load_records(tmp_path, git_sha="abc")
        assert len(loaded) == 2
        assert all(str(r["git_sha"]).startswith("abc") for r in loaded)
    ...
    def test_run_id_filter(self, tmp_path: Path) -> None:
        ...
        self._write_jsonl(tmp_path, recs)
        loaded = _load_records(tmp_path, run_id="r1")
        assert len(loaded) == 1
    ...
    def test_combined_filter(self, tmp_path: Path) -> None:
        recs = [
            {"cycle_id": "c1", "git_sha": "abc123", "run_id": "r1", "filled": True},
            {"cycle_id": "c2", "git_sha": "abc456", "run_id": "r2", "filled": True},
            {"cycle_id": "c3", "git_sha": "def789", "run_id": "r1", "filled": True},
        ]
        self._write_jsonl(tmp_path, recs)
        loaded = _load_records(tmp_path, git_sha="abc", run_id="r1")
        assert len(loaded) == 1
        assert loaded[0]["cycle_id"] == "c1"
