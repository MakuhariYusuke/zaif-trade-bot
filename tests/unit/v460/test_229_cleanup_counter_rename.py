...
_APPLY_REGIME_BOOSTS_SOURCE = inspect.getsource(MakerPrice._apply_regime_boosts)
import scripts.v460.lib.fill_loop_orchestrator as orch_mod
_FILL_LOOP_ORCHESTRATOR_SOURCE = inspect.getsource(orch_mod)
...
class TestMakerPriceNoHasattr:
    ...
    def test_no_hasattr_in_apply_regime_boosts(self):
        ...
        assert "hasattr" not in _APPLY_REGIME_BOOSTS_SOURCE
...
class TestOrchestratorNoGetattr:
    ...
    def test_no_getattr_soft_drawdown(self):
        ...
        assert 'getattr(self, "_soft_drawdown_interval_multiplier"' not in _FILL_LOOP_ORCHESTRATOR_SOURCE

    def test_no_inline_import_time_as_time(self):
        """H-5: fill_loop_orchestrator に inline import time as _time が残っていない."""
        assert "import time as _time" not in _FILL_LOOP_ORCHESTRATOR_SOURCE
