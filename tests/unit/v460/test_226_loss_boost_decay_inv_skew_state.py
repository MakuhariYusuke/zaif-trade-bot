...
class TestHaltMCBSADFeedContinuation:
    ...
    def test_orchestrator_source_has_halt_mcb_update(self) -> None:
        """halt 中の continue 前に MCB update が存在する (ソースコード検証)."""
        from scripts.v460.lib.fill_loop_orchestrator import FillLoopOrchestratorMixin
        src = inspect.getsource(FillLoopOrchestratorMixin.run_continuous)
        assert "226# S5" in src
        # "is_halted" → MCB update が halt ブロック内にある
        halted_section = src[src.index("daily_drawdown_guard.is_halted()"):]
        # halt ブロックの continue 前に mcb.update がある
        continue_idx = halted_section.index("continue")
        mcb_update_in_halt = "self._mcb.update" in halted_section[:continue_idx]
        assert mcb_update_in_halt, "MCB update should appear before continue in halt block"

    def test_orchestrator_source_has_halt_sad_update(self) -> None:
        """halt 中の continue 前に SAD update が存在する."""
        from scripts.v460.lib.fill_loop_orchestrator import FillLoopOrchestratorMixin
        src = inspect.getsource(FillLoopOrchestratorMixin.run_continuous)
        halted_section = src[src.index("daily_drawdown_guard.is_halted()"):]
        continue_idx = halted_section.index("continue")
        sad_update_in_halt = "self._sad.update" in halted_section[:continue_idx]
        assert sad_update_in_halt, "SAD update should appear before continue in halt block"
