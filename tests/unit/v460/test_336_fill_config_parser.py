...
class TestParseCrossVenueSection:
    ...
    def test_cross_venue_449_config_fields(self) -> None:
        """449# depth_imbalance_threshold + confidence_floor のパース確認."""
        result = _parse_cross_venue_section({
            "cross_venue_lead_lag": {
                "enabled": True,
                "depth_imbalance_threshold": 0.15,
                "confidence_floor": 0.25,
            },
        })
        assert result["cross_venue_depth_imbalance_threshold"] == 0.15
        assert result["cross_venue_confidence_floor"] == 0.25
