...
async def _run_place_order_capture(
    ...
    **order_kwargs: object,
    ...
    def mock_make_request(method, url, data=None):
        del method, url
        capture.update(data or {})
        return {"success": True, "id": 123}

    with patch.object(adapter, "_make_api_request", side_effect=mock_make_request):
        with patch.object(adapter, "_check_rate_limit", _noop_async):
            with patch.object(adapter, "_simulate_delay", _noop_async):
                await adapter.place_order(**order_kwargs)
