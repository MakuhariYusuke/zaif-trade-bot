...
class TestR1MethodExtraction:
    ...
    def test_run_single_cycle_under_400_lines(self) -> None:
        """run_single_cycle が 550 行以下 (R1 目標 + 162# Inv-Skewing + 181# StopCond + 187# guard_trace + 195# vel_offset)."""
        from scripts.v460.run_fill_test import FillTestRunner
        source = inspect.getsource(FillTestRunner.run_single_cycle)
        line_count = len(source.splitlines())
        assert line_count <= 550, f"run_single_cycle is {line_count} lines (> 550)"
