...
_MAYBE_REGISTER_PHANTOM_SOURCE = inspect.getsource(
    FillCycleExecutorMixin._maybe_register_phantom
)
...
class TestGetAttrRemoval:
    ...
    def test_no_getattr_in_maybe_register_phantom(self) -> None:
        ...
        code_lines = [
            line for line in _MAYBE_REGISTER_PHANTOM_SOURCE.splitlines()
            if not line.strip().startswith("#")
        ]
        code_only = "\n".join(code_lines)
        assert "getattr" not in code_only
        # 新しい直接参照が存在すること
        assert "last_btc_free" in _MAYBE_REGISTER_PHANTOM_SOURCE
        assert "last_jpy_free" in _MAYBE_REGISTER_PHANTOM_SOURCE

    def test_balance_jpy_passed_to_register_unknown(self) -> None:
        """_maybe_register_phantom が balance_jpy も渡していること."""
        assert "balance_jpy=" in _MAYBE_REGISTER_PHANTOM_SOURCE
