...
def _select_real_enriched_training_df(
    *,
    ...
    initial_fill_df = _load_recent_fill_records_df(sample_rows=initial_rows)
    if initial_fill_df.empty:
        return initial_fill_df

    initial_enriched = enrich_fill_records(initial_fill_df)
    X_initial, _ = build_pnl_features(initial_enriched)
    if len(X_initial) >= min_train_samples:
        return initial_enriched

    fallback_fill_df = _load_recent_fill_records_df(sample_rows=fallback_rows)
    if fallback_fill_df.empty:
        return initial_enriched
    return enrich_fill_records(fallback_fill_df)
