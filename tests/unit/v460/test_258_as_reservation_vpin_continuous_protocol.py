...
def _make_as_reservation_stub(
    config: FillTestConfig,
    *,
    imbalance: float = 0.0,
    vol_ratio: float = 1.0,
) -> SimpleNamespace:
    def _dynamic_tau(base_tau: float, current_vol_ratio: float) -> float:
        if not config.as_tau_dynamic_enabled or current_vol_ratio <= 0:
            return base_tau
        return base_tau / current_vol_ratio

    return SimpleNamespace(
        _config=config,
        _fill_prob_model=None,
...
def _make_vpin_guard_stub(
    config: FillTestConfig,
    *,
    last_vpin: float | None = None,
    last_inv_skew_factor: float = 0.0,
) -> SimpleNamespace:
    return SimpleNamespace(
        _config=config,
        _last_vpin=last_vpin,
        _last_inv_skew_factor=last_inv_skew_factor,
        _last_vg_triggered=False,
        _last_vg_velocity_bps=None,
        _last_vg_vpin=None,
        _last_vg_boost_factor=None,
        _scale_offset_ratio=lambda current, boost, *, max_ratio: MakerPrice._scale_offset_ratio(
            current,
            boost,
            max_ratio=max_ratio,
        ),
...
class TestASReservationShift:
    ...
    def test_disabled_by_default(self) -> None:
        ...
        stub = _make_as_reservation_stub(_make_config())
        result = MakerPrice._apply_as_reservation_shift(
            stub,
            "buy",
            1000.0,
            10_000_000.0,
    ...
    def test_no_shift_when_inventory_neutral(self) -> None:
        ...
        stub = _make_as_reservation_stub(cfg)
        result = MakerPrice._apply_as_reservation_shift(
            stub,
            "buy",
            1000.0,
    ...
    def test_buy_offset_increases_when_long(self) -> None:
        ...
        stub = _make_as_reservation_stub(cfg, imbalance=1.0)
        initial = 0.05
        result = MakerPrice._apply_as_reservation_shift(
            stub,
            "buy",
    ...
    def test_shift_proportional_to_spread(self) -> None:
        ...
        stub_narrow = _make_as_reservation_stub(cfg, imbalance=1.0)
        stub_wide = _make_as_reservation_stub(cfg, imbalance=1.0)

        r_narrow = MakerPrice._apply_as_reservation_shift(
            stub_narrow,
            "buy",
            1000.0,
            10_000_000.0,
        ...
        r_wide = MakerPrice._apply_as_reservation_shift(
            stub_wide,
            "buy",
            5000.0,
            10_000_000.0,
...
class TestVPINContinuousModulator:
    ...
    def test_binary_mode_unchanged_when_disabled(self) -> None:
        ...
        stub = _make_vpin_guard_stub(cfg, last_vpin=0.50)
        result = MakerPrice._apply_volatility_guard(stub, "buy", None, 0.05)
    ...
    def test_binary_mode_triggers_above_threshold(self) -> None:
        ...
        stub = _make_vpin_guard_stub(cfg, last_vpin=0.80)
        result = MakerPrice._apply_volatility_guard(stub, "buy", None, 0.05)
    ...
    def test_continuous_mode_no_effect_below_min(self) -> None:
        ...
        stub = _make_vpin_guard_stub(cfg, last_vpin=0.30)
        result = MakerPrice._apply_volatility_guard(stub, "buy", None, 0.05)
        assert result == 0.05
    ...
    def test_continuous_mode_partial_boost_between_min_and_threshold(self) -> None:
        ...
        stub = _make_vpin_guard_stub(cfg, last_vpin=0.55)
        initial = 0.05
        result = MakerPrice._apply_volatility_guard(stub, "buy", None, initial)
        expected_boost = 1.25
    ...
    def test_continuous_mode_full_boost_at_threshold(self) -> None:
        ...
        stub = _make_vpin_guard_stub(cfg, last_vpin=0.70)
        initial = 0.05
        result = MakerPrice._apply_volatility_guard(stub, "buy", None, initial)
    ...
    def test_continuous_mode_capped_above_threshold(self) -> None:
        ...
        stub = _make_vpin_guard_stub(cfg, last_vpin=0.90)
        initial = 0.05
        result = MakerPrice._apply_volatility_guard(stub, "buy", None, initial)
    ...
    def test_continuous_quadratic_shape(self) -> None:
        ...
        for vpin in [0.45, 0.50, 0.55, 0.60, 0.65]:
            stub = _make_vpin_guard_stub(cfg, last_vpin=vpin)
            r = MakerPrice._apply_volatility_guard(stub, "buy", None, initial)
            results.append(r)
    ...
    def test_velocity_trigger_takes_priority(self) -> None:
        ...
        result = MakerPrice._apply_volatility_guard(stub, "buy", 20.0, initial)
        expected = initial * 2.0
        assert abs(result - expected) < 0.001
...
class TestVPINBuyExtraMult:
    ...
    def test_default_mult_1_no_change(self) -> None:
        ...
        buy_stub = _make_vpin_guard_stub(cfg, last_vpin=0.60)
        sell_stub = _make_vpin_guard_stub(cfg, last_vpin=0.60)
        buy_result = MakerPrice._apply_volatility_guard(buy_stub, "buy", None, 0.05)
        sell_result = MakerPrice._apply_volatility_guard(sell_stub, "sell", None, 0.05)
    ...
    def test_sell_unaffected_by_buy_extra_mult(self) -> None:
        ...
        base_stub = _make_vpin_guard_stub(cfg_base, last_vpin=0.60)
        extra_stub = _make_vpin_guard_stub(cfg_extra, last_vpin=0.60)
        sell_base = MakerPrice._apply_volatility_guard(base_stub, "sell", None, 0.05)
        sell_extra = MakerPrice._apply_volatility_guard(extra_stub, "sell", None, 0.05)
