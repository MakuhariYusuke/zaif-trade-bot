...
class TestSchemaEnvironmentFactory:
    ...
    def test_create_env_from_schema_success(
        self, mock_env_class, mock_manager_class, sample_df, mock_schema_metadata
    ):
    ...
    @patch("ztb.trading.environment.schema_env_factory.FeatureSchemaManager")
    def test_create_env_from_schema_missing_features(
        self, mock_manager_class, sample_df
    ):
    ...
    @patch("ztb.trading.environment.schema_env_factory.FeatureSchemaManager")
    def test_create_env_from_schema_with_scaler(
        self, mock_manager_class, sample_df, mock_schema_metadata
    ):
        ...
        with patch(
            "ztb.trading.environment.schema_env_factory.HeavyTradingEnv"
        ) as mock_env_class:
            mock_env = MagicMock()
            mock_env_class.return_value = mock_env
            ...
            np.testing.assert_array_equal(
                config["scaler_mean"], mock_scaler_data["mean"]
            )
    ...
    def test_create_env_from_model_path(self, sample_df):
        ...
        config = ZTBConfig()
        with patch(
            "ztb.trading.environment.schema_env_factory.create_env_from_schema"
        ) as mock_create_env:
            mock_env = MagicMock()
            mock_create_env.return_value = mock_env

            # Execute
            result = create_env_from_model_path(
                config.get_model_path("test_model.zip"), sample_df
            )
