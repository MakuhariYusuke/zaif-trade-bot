...
class TestNoiseFilter:
    ...
    def sample_dataframe(self):
        ...
        # Create clean data
        clean_data = np.sin(np.linspace(0, 4*np.pi, n_samples)) + 0.1 * np.random.randn(n_samples)
...
class TestAnomalyDetector:
    ...
    def test_detect_anomalies_statistical(self, sample_dataframe):
        ...
        result_df, anomaly_mask = detector.detect_anomalies(df_with_anomalies, method='statistical')
    ...
    def test_detect_anomalies_isolation_forest(self, mock_iso_forest, sample_dataframe):
        ...
        mock_instance.fit_predict.return_value = np.array([1] * 98 + [-1, -1])  # Last 2 are anomalies
        mock_iso_forest.return_value = mock_instance

        detector = AnomalyDetector()
        result_df, anomaly_mask = detector.detect_anomalies(sample_dataframe, method='isolation_forest')
    ...
    def test_detect_anomalies_lof(self, mock_lof, sample_dataframe):
        ...
        # Mock sklearn components
        mock_instance = MagicMock()
        mock_instance.fit_predict.return_value = np.array([1] * 95 + [-1] * 5)  # Last 5 are anomalies
        mock_lof.return_value = mock_instance

        detector = AnomalyDetector()
        result_df, anomaly_mask = detector.detect_anomalies(sample_dataframe, method='local_outlier_factor')
    ...
    def test_detect_anomalies_unknown_method(self, sample_dataframe):
        ...
        result_df, anomaly_mask = detector.detect_anomalies(sample_dataframe, method='unknown_method')
...
class TestSyntheticDataGenerator:
    ...
    def test_generate_gaussian_noise(self, sample_dataframe):
        ...
        result = generator.generate_gaussian_noise(sample_dataframe, ['price'], noise_level=0.1)
...
class TestPreprocessData:
    ...
    def test_preprocess_data_all_enabled(self, mock_generator, mock_detector, mock_filter, sample_dataframe):
        ...
        mock_detector_instance.detect_anomalies.return_value = (sample_dataframe, pd.Series(False, index=sample_dataframe.index))
