...
class TestMultiTimeframeFeatures:
    ...
    def setup_registry(self):
        ...
        from ztb.features import momentum, volatility, trend
        yield
    ...
    def test_registry_registration(self):
        ...
        expected_features = [
            # RSI multi-timeframe
            'RSI_M1', 'RSI_M5', 'RSI_M15', 'RSI_H1', 'RSI_H4', 'RSI_D1',
            # ATR multi-timeframe
            'ATR_M1', 'ATR_M5', 'ATR_M15', 'ATR_H1', 'ATR_H4', 'ATR_D1',
            # ADX multi-timeframe
            ...
            'EMACross_Diff_M1', 'EMACross_Diff_M5', 'EMACross_Diff_M15',
            'EMACross_Diff_H1', 'EMACross_Diff_H4', 'EMACross_Diff_D1',
            'EMACross_Signal_M1', 'EMACross_Signal_M5', 'EMACross_Signal_M15',
    ...
    def test_feature_count_in_curated_set(self):
        ...
        timeframe_features = [f for f in features if any(tf in f for tf in ['_M1', '_M5', '_M15', '_H1', '_H4', '_D1'])]

        # Should have 78 multi-timeframe features based on implementation
        assert len(timeframe_features) == 78, f"Expected 78 multi-timeframe features, got {len(timeframe_features)}"
    ...
    def test_heikinashi_multi_timeframe_calculation(self, sample_data):
        ...
        for tf in timeframes:
            ...
            # Check result properties
            assert len(result) == len(sample_data), f"HeikinAshi_Color_{tf.name} length mismatch"
            ...
            if len(valid_values) > 0:
                ...
                assert len(unique_values) <= 5, f"HeikinAshi_Color_{tf.name} has too many unique values: {unique_values}"
    ...
    def test_emacross_multi_timeframe_calculation(self, sample_data):
        ...
        for tf in timeframes:
            ...
            for variant in ['Diff', 'Signal']:
                ...
                func = FeatureRegistry._registry.get(feature_name)
                assert func is not None, f"EMACross_{variant} feature for {tf.name} not found"

                result = func(sample_data)

                # Check result properties
                assert len(result) == len(sample_data), f"EMACross_{variant}_{tf.name} length mismatch"
                assert not result.isna().all(), f"EMACross_{variant}_{tf.name} is all NaN"
                ...
                if len(valid_values) > 0:
                    if variant == 'Signal':
                        ...
                        assert valid_values.abs().max() < 10, \
    ...
    def test_timeframe_params(self):
        ...
        for tf in [Timeframe.M1, Timeframe.M5, Timeframe.M15, Timeframe.H1, Timeframe.H4, Timeframe.D1]:
            ...
            assert isinstance(params, dict), f"Timeframe {tf.name} params should be dict"
            assert 'short_period' in params, f"Timeframe {tf.name} missing 'short_period' param"
            assert 'medium_period' in params, f"Timeframe {tf.name} missing 'medium_period' param"
            assert 'long_period' in params, f"Timeframe {tf.name} missing 'long_period' param"

            # Values should be reasonable
            assert params['short_period'] > 0, f"Timeframe {tf.name} short_period should be positive"
            assert params['medium_period'] > 0, f"Timeframe {tf.name} medium_period should be positive"
            assert params['long_period'] > 0, f"Timeframe {tf.name} long_period should be positive"
