...
class TestAdaptiveFeatureSelector(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        with patch.object(
            ...
            return_value=list(self.test_data.columns),
        ):
    ...
    def test_importance_based_selection(self):
        ...
        with patch.object(
            ...
            result = selector._importance_based_selection(
                self.test_data, self.test_target, MarketCondition.TRENDING
            )

            self.assertIsInstance(result, FeatureSelectionResult)
            self.assertEqual(
                result.selection_method, FeatureSelectionMethod.IMPORTANCE_BASED
            )
            self.assertLessEqual(
                len(result.selected_features), self.config.target_features
            )
    ...
    def test_correlation_based_selection(self):
        ...
        with patch.object(
            ...
            result = selector._correlation_based_selection(
                self.test_data, self.test_target, MarketCondition.RANGING
            )

            self.assertIsInstance(result, FeatureSelectionResult)
            self.assertEqual(
                result.selection_method, FeatureSelectionMethod.CORRELATION_BASED
            )
            self.assertLessEqual(
                len(result.selected_features), self.config.target_features
            )
    ...
    def test_market_condition_based_selection(self):
        ...
        with patch.object(
            ...
            self.assertIsInstance(result, FeatureSelectionResult)
            self.assertEqual(
                result.selection_method, FeatureSelectionMethod.MARKET_CONDITION_BASED
            )
