...
class TestKellyPositionSizer:
    ...
    def test_calculate_dynamic_position_size_insufficient_trades(self, kelly_sizer):
        ...
        # デフォルト値が返される
        assert (
            decision.position_size_fraction == 0.01
        )  # デフォルト値（max_risk_fractionの半分）
    ...
    def test_risk_tolerance_application(self, kelly_sizer, sample_trades):
        ...
        # ハーフはフルより小さいはず
        assert (
            half_decision.position_size_fraction <= full_decision.position_size_fraction
        )
        # クォーターはハーフより小さいはず
        assert (
            quarter_decision.position_size_fraction
            <= half_decision.position_size_fraction
        )
    ...
    def test_confidence_adjustment(self, kelly_sizer, sample_trades):
        ...
        if len(high_confidence_trades) >= 3:
            ...
            assert (
                high_decision.position_size_fraction
                >= low_decision.position_size_fraction
            )
