...
class TestIntegratedOptimizer:
    ...
    def test_create_integrated_strategy_evaluator(self, sample_market_data, mock_strategy_func):
        """統合戦略評価関数作成テスト"""
        optimizer = IntegratedParameterOptimizer()
        integrated_evaluator = optimizer.create_integrated_strategy_evaluator(mock_strategy_func)
    ...
    def test_run_integrated_optimization(self, sample_market_data, mock_strategy_func):
        ...
        walk_forward_results = optimizer.walk_forward_analyzer.walk_forward_optimization(
            ...
            test_days=10,
            step_days=15,
            parameter_sets=[optimizer.walk_forward_analyzer.parameter_space.get_conservative_defaults()],
            min_samples=10
        )
        ...
        result = optimizer.run_integrated_optimization(
            ...
            base_strategy_func=mock_strategy_func
        )
...
class TestIntegrationWithComponents:
    ...
    def test_kelly_integration(self, sample_trades):
        ...
        optimizer = IntegratedParameterOptimizer()
        kelly_params = optimizer.kelly_sizer.calculate_kelly_parameters(sample_trades, 10000)
