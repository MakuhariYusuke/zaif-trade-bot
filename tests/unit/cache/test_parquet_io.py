...
class TestParquetIO:
    ...
    def test_analyze_column_dependencies(self):
        ...
        expected = {"open", "high", "low", "close", "volume", "timestamp", "close", "high", "low"}
    ...
    def test_read_parquet_with_features(self, mock_read_parquet, mock_smart_detection, 
                                       mock_load_features, mock_load_config):
        ...
        mock_read_parquet.return_value = pd.DataFrame({"close": [1, 2], "volume": [3, 4]})
        
        result = read_parquet_with_features(Path("test.parquet"), target_features=["rsi"])
    ...
    @patch("ztb.cache.parquet_io.print")
    def test_convert_to_parquet_csv(self, mock_print, mock_write_parquet, mock_load_config):
