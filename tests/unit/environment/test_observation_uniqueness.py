...
class TestObservationUniqueness:
    ...
    def obs_delta_norm(self, obs1: NDArray[np.float32], obs2: NDArray[np.float32]) -> float:
        """Compute L2 norm of observation difference."""
        return float(np.linalg.norm(obs1 - obs2))

    def test_observation_uniqueness_across_steps(self, test_env: HeavyTradingEnv) -> None:
        ...
        assert len(set(obs_hashes[:10])) > 1, \
    ...
    def test_observation_delta_norms(self, test_env: HeavyTradingEnv) -> None:
        ...
        assert zero_delta_rate < 0.9, \
            f"Zero delta rate {zero_delta_rate:.1%} too high - observations mostly static"

    def test_observation_changes_with_different_actions(self, test_env: HeavyTradingEnv) -> None:
        ...
        assert delta_hold_buy > 1e-6 or delta_hold_hold2 > 1e-6, \
    ...
    def test_observation_schema_consistency(self, test_env: HeavyTradingEnv) -> None:
        ...
        for step in range(20):
            ...
            assert obs.dtype == initial_dtype, \
