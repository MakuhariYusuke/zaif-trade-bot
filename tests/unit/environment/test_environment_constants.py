...
def test_continuous_to_discrete_basic():
    ...
    assert (
        continuous_to_discrete_action(CONTINUOUS_TO_DISCRETE_THRESHOLD + 0.1)
        == ACTION_BUY
    )
    # clearly below negative threshold -> SELL
    assert (
        continuous_to_discrete_action(CONTINUOUS_TO_DISCRETE_THRESHOLD_NEG - 0.1)
        == ACTION_SELL
    )
    # exactly zero -> HOLD
...
def test_continuous_to_discrete_at_thresholds():
    ...
    res_at_neg = continuous_to_discrete_action(
        float(CONTINUOUS_TO_DISCRETE_THRESHOLD_NEG)
    )
...
def test_type_coercion_and_invalid_inputs():
    ...
    assert continuous_to_discrete_action(0.0) == ACTION_HOLD
    assert continuous_to_discrete_action(0.3333) in (
        ACTION_HOLD,
        ACTION_BUY,
        ACTION_SELL,
    )
