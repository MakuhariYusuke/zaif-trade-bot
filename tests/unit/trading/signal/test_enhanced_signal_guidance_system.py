...
class TestEnhancedSignalGuidanceSystem:
    ...
    def test_initialization(self, system):
        ...
        assert len(system.regime_adaptation_params) == 17  # All 17 regimes
    ...
    def test_sell_regime_bias(self, system):
        ...
        sell_regimes = [RegimeType.SELL_BREAKDOWN, RegimeType.SELL_DIVERGENCE,
                       RegimeType.SELL_MOMENTUM_WEAK, RegimeType.SELL_VOLUME_SURGE]
        ...
        for regime in sell_regimes:
            ...
            assert params['confidence_multiplier'] >= 1.0, f"{regime} should have confidence boost"

    def test_bull_regime_bias(self, system):
        """Test bull trend regimes have positive bias"""
        bull_regimes = [RegimeType.STRONG_BULL_TREND, RegimeType.MODERATE_BULL_TREND,
    ...
    def test_regime_adaptation_application(self, system, sample_market_data):
        ...
        with patch.object(system.regime_classifier, 'process_signal') as mock_regime:
            ...
            with patch.object(system.quality_scorer, 'process_signal') as mock_quality:
                ...
                assert adaptation['regime_bias_applied'] < 0  # Negative bias for SELL regime
    ...
    def test_strategic_guidance_generation(self, system, sample_market_data):
        ...
        required_fields = [
            'primary_action', 'regime_context', 'confidence_level',
            'risk_assessment', 'position_sizing', 'time_horizon',
            'stop_loss_guidance', 'take_profit_guidance'
        ]
    ...
    def test_position_sizing_recommendation(self, system):
        ...
        result_high = system._recommend_position_sizing(RegimeType.CONSOLIDATION, Mock(confidence=0.9))
        assert 'LARGE' in result_high

        result_low = system._recommend_position_sizing(RegimeType.CONSOLIDATION, Mock(confidence=0.3))
    ...
    def test_stop_loss_guidance(self, system, sample_market_data):
        ...
        guidance = system._generate_stop_loss_guidance(RegimeType.EXTREME_VOLATILITY, context)
        assert guidance['type'] == 'PERCENTAGE'
        assert guidance['value'] == 0.05

        # Test normal regime stop loss
        guidance = system._generate_stop_loss_guidance(RegimeType.MODERATE_BULL_TREND, context)
        assert guidance['type'] == 'PERCENTAGE'
    ...
    def test_take_profit_guidance(self, system, sample_market_data):
        ...
        guidance = system._generate_take_profit_guidance(RegimeType.STRONG_BULL_TREND, context)
        assert guidance['type'] == 'PERCENTAGE'
        assert guidance['value'] == 0.08

        # Test consolidation take profit
        guidance = system._generate_take_profit_guidance(RegimeType.CONSOLIDATION, context)
        assert guidance['type'] == 'PERCENTAGE'
        assert guidance['value'] == 0.03
    ...
    def test_get_system_status(self, system, sample_market_data):
        ...
        required_fields = [
            'regime_classifier_status', 'performance_history_length',
            'active_regimes', 'total_regime_adaptations', 'system_config'
        ]
    ...
    def test_adapted_action_determination(self, system):
        ...
        action = system._determine_adapted_action(85, {'strong_buy': 80, 'buy': 65, 'sell': 35, 'strong_sell': 20})
        assert action == 2

        # Test BUY signal
        action = system._determine_adapted_action(70, {'strong_buy': 80, 'buy': 65, 'sell': 35, 'strong_sell': 20})
        assert action == 1

        # Test SELL signal
        action = system._determine_adapted_action(30, {'strong_buy': 80, 'buy': 65, 'sell': 35, 'strong_sell': 20})
        assert action == -1

        # Test strong SELL signal
        action = system._determine_adapted_action(15, {'strong_buy': 80, 'buy': 65, 'sell': 35, 'strong_sell': 20})
        assert action == -2

        # Test HOLD signal
        action = system._determine_adapted_action(50, {'strong_buy': 80, 'buy': 65, 'sell': 35, 'strong_sell': 20})
        assert action == 0
    ...
    def test_threshold_adjustment_application(self, system):
        ...
        adapted = system._apply_regime_adaptation(
            ...
            Mock(confidence=0.8, metadata={'regime_type': RegimeType.STRONG_BULL_TREND})
        )
