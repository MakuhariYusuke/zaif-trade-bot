...
class TestCompositeIndicator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = {
            ...
            'thresholds': {'rsi': 50, 'macd': 0, 'bollinger': 0.5}
        }

        self.indicator = CompositeIndicator(self.config)
    ...
    def test_calculate_with_custom_weights(self):
        ...
        custom_config['weights'] = {'rsi': 0.6, 'macd': 0.3, 'bollinger': 0.1}

        indicator = CompositeIndicator(custom_config)
    ...
    def test_calculate_with_missing_indicators(self):
        ...
        incomplete_config = {
            'indicators': ['rsi'],  # Only RSI
            'weights': {'rsi': 1.0},
            'thresholds': {'rsi': 50}
        }

        indicator = CompositeIndicator(incomplete_config)
        result = indicator.calculate(self.df)
    ...
    def test_weight_normalization(self):
        ...
        unnormalized_config = self.config.copy()
        unnormalized_config['weights'] = {'rsi': 2, 'macd': 2, 'bollinger': 1}  # Don't sum to 1

        indicator = CompositeIndicator(unnormalized_config)
        result = indicator.calculate(self.df)
...
class TestAdaptiveIndicator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = {
            'base_indicator': 'rsi',
            'adaptation_factors': {
                'trending': {'period': 21, 'weight': 1.2},
                'ranging': {'period': 14, 'weight': 0.8},
                'volatile': {'period': 9, 'weight': 0.6}
            }
        }

        self.indicator = AdaptiveIndicator(self.config)
    ...
    def test_calculate_adaptive_trending_market(self):
        ...
        result = self.indicator.calculate_adaptive(self.df, 'trending')

        self.assertIsInstance(result, dict)
        self.assertIn('adaptive_value', result)
        self.assertIn('market_regime', result)
        self.assertIn('adaptation_factor', result)
        self.assertEqual(result['market_regime'], 'trending')

    def test_calculate_adaptive_ranging_market(self):
        """Test adaptive calculation in ranging market"""
        result = self.indicator.calculate_adaptive(self.df, 'ranging')
    ...
    def test_calculate_adaptive_volatile_market(self):
        ...
        result = self.indicator.calculate_adaptive(self.df, 'volatile')

        self.assertIsInstance(result, dict)
        self.assertIn('adaptive_value', result)
        self.assertEqual(result['market_regime'], 'volatile')

    def test_calculate_adaptive_unknown_regime(self):
        """Test adaptive calculation with unknown market regime"""
        result = self.indicator.calculate_adaptive(self.df, 'unknown')
    ...
    def test_adaptation_factor_application(self):
        ...
        # Test trending (weight = 1.2)
        trending_result = self.indicator.calculate_adaptive(self.df, 'trending')

        # Test ranging (weight = 0.8)
        ranging_result = self.indicator.calculate_adaptive(self.df, 'ranging')

        # Results should be different due to different adaptation factors
        self.assertIsInstance(trending_result['adaptation_factor'], (int, float))
        self.assertIsInstance(ranging_result['adaptation_factor'], (int, float))
...
class TestMACDIndicator(unittest.TestCase):
    ...
    def test_calculate_macd(self):
        ...
        self.assertIsInstance(result, dict)
        expected_keys = ['macd', 'macd_signal', 'histogram']
