...
class TestCalculateCompositeScore:
    ...
    def test_calculate_composite_score_balanced(self):
        ...
        components = {"trend": 0.7, "momentum": 0.6, "volatility": 0.5, "volume": 0.8}
        weights = {"trend": 0.25, "momentum": 0.25, "volatility": 0.25, "volume": 0.25}
    ...
    def test_calculate_composite_score_weighted(self):
        ...
        components = {"trend": 0.8, "momentum": 0.4}
        weights = {"trend": 0.7, "momentum": 0.3}  # Higher weight for trend

        score = calculate_composite_score(components, weights)

        # Expected: (0.8*0.7 + 0.4*0.3) * 100 = 68.0
        assert score == 68.0
    ...
    def test_calculate_composite_score_missing_weights(self):
        ...
        components = {"trend": 0.7, "momentum": 0.6, "volatility": 0.5}
        weights = {
            "trend": 0.5,
            "momentum": 0.5,
    ...
    def test_calculate_composite_score_zero_weights(self):
        ...
        components = {"trend": 0.8, "momentum": 0.4}
        weights = {"trend": 0.0, "momentum": 0.0}
    ...
    @pytest.mark.parametrize(
        "components,weights,expected",
        [
            ({"a": 1.0}, {"a": 1.0}, 100.0),
            ({"a": 0.0}, {"a": 1.0}, 0.0),
            ({"a": 0.5, "b": 0.5}, {"a": 0.5, "b": 0.5}, 50.0),
            ({"a": 0.8, "b": 0.4}, {"a": 0.3, "b": 0.7}, 52.0),
        ],
    )
    def test_calculate_composite_score_parametrized(
        self, components, weights, expected
    ):
...
def test_confidence_to_score_thresholds_edge_cases():
    """Test mapping of confidence thresholds to score thresholds to ensure min_gap and clamping"""
    from ztb.trading.signal.common.utilities import confidence_to_score_thresholds

    # Edge case: confidence 0.0 -> buy near default/min, sell near max
    buy, sell = confidence_to_score_thresholds(
        0.0, default_buy=75.0, default_sell=25.0, min_gap=10
    )
    assert 0.0 <= buy <= 75.0
    assert 25.0 <= sell <= 100.0
    assert buy >= sell + 10 or (buy == sell + 10)

    # Midpoint: confidence 0.5 should map to buy and sell that are at least min_gap apart
    buy, sell = confidence_to_score_thresholds(
        0.5, default_buy=75.0, default_sell=25.0, min_gap=10
    )
    assert buy >= sell + 10

    # Edge case: confidence 1.0 -> buy near max, sell near min
    buy, sell = confidence_to_score_thresholds(
        1.0, default_buy=75.0, default_sell=25.0, min_gap=10
    )
    assert 75.0 <= buy <= 100.0
    assert 0.0 <= sell <= 25.0
    assert buy >= sell + 10
