...
def test_compute_weights_imbalanced():
    ...
    assert weights["BUY"] < weights["SELL"] or weights["BUY"] == pytest.approx(
        weights["SELL"], abs=1e-6
    )
...
def test_safety_guards_entropy():
    ...
    should_revert, reason = calc.check_safety_guards(
        entropy=0.03, kl_violations_rate=0.0
    )
...
def test_safety_guards_kl_violations():
    ...
    should_revert3, reason = calc.check_safety_guards(
        entropy=0.5, kl_violations_rate=0.05
    )
...
def test_cosine_warmup_midpoint():
    ...
    multiplier = cosine_warmup_schedule(
        current_step=10000, warmup_start=5000, warmup_end=15000
    )
