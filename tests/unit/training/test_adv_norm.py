...
class TestPerActionAdvantageNormalizer:
    ...
    def test_basic_normalization(self):
        ...
        advantages = np.array(
            [
                ...
                1.2,
                0.8,
                1.1,  # HOLD (4 samples, mean≈1.0)
                0.5,
                0.6,  # BUY (2 samples, mean≈0.55)
                -0.3,
                -0.2,  # SELL (2 samples, mean≈-0.25)
            ]
        )
    ...
    def test_statistics_tracking(self):
        ...
        # Check counts
        assert stats["action_counts"] == [3, 2, 2], "Action counts mismatch"
        ...
        np.testing.assert_allclose(stats["action_means"], expected_means, rtol=1e-4)
    ...
    def test_single_action_sample(self):
        ...
        normalizer = PerActionAdvantageNormalizer(n_actions=3, min_samples_per_action=1)
    ...
    def test_gradient_flow_simulation(self):
        ...
        actions = np.concatenate(
            [np.zeros(70, dtype=int), np.ones(20, dtype=int), np.full(10, 2, dtype=int)]
        )
...
def test_comparison_with_traditional():
    ...
    actions = np.concatenate(
        [np.zeros(80, dtype=int), np.ones(15, dtype=int), np.full(5, 2, dtype=int)]
    )
