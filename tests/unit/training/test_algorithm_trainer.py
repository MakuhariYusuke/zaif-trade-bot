...
class TestAlgorithmFactoryIntegration:
    ...
    @pytest.mark.skip(reason="PPOAlgorithmTrainer class no longer exists; now using AlgorithmFactory pattern")
    def test_ppo_uses_algorithm_factory(self, algorithm_trainer, mock_config_manager):
        ...
        with patch("ztb.training.core.algorithm_trainer.PPOAlgorithmTrainer") as mock_ppo_trainer:
    ...
    @pytest.mark.skip(reason="PPOAlgorithmTrainer class no longer exists; now using AlgorithmFactory pattern")
    def test_case_insensitive_algorithm_name(self, algorithm_trainer, mock_config_manager):
...
class TestAlgorithmTrainerIntegrationScenarios:
    ...
    def test_algorithm_switching(self, algorithm_trainer):
        ...
        with patch.object(algorithm_trainer.base_ml_trainer, 'train', return_value={"status": "base_ml"}), \
             patch.object(algorithm_trainer.iterative_trainer, 'train', return_value={"status": "iterative"}), \
             patch.object(algorithm_trainer.ensemble_trainer, 'train', return_value={"status": "ensemble"}):
