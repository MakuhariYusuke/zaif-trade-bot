...
class TestTargetEntropyController:
    ...
    def test_alpha_decreases_on_high_entropy(self):
        ...
        controller = TargetEntropyController(
            n_actions=3,
            target_entropy_ratio=0.7,
            initial_temperature=0.1,  # Start higher
            lr_temperature=1e-2
        )
    ...
    def test_alpha_stabilizes_at_target(self):
        ...
        alpha_std = np.std(alphas[-10:])  # Last 10 updates
        assert alpha_std < 0.01, \
            f"Alpha should stabilize (std={alpha_std:.6f})"
...
def test_convergence_simulation():
    ...
    controller = TargetEntropyController(
        n_actions=3,
        target_entropy_ratio=0.7,
        initial_temperature=0.001,  # Start very low
        lr_temperature=1e-2
    )
    
    target = controller.target_entropy
    print(f"Target Entropy: {target:.4f}")
    
    # Simulate 3 phases
    phases = [
        ("Collapsed", [[10.0, 0.0, 0.0]] * 10, 20),  # Low entropy
        ("Recovery", [[2.0, 1.0, 0.5]] * 10, 30),    # Moderate entropy
        ("Ideal", [[1.0, 0.8, 0.6]] * 10, 20)        # Near target
    ]
    
    for phase_name, logits_template, n_steps in phases:
        print(f"\nPhase: {phase_name}")
        ...
        for i in range(n_steps):
            ...
            if i % 10 == 0 or i == n_steps - 1:
                print(f"  Step {i}: H={entropy:.4f}, α={alpha:.6f}")
    ...
    print("\n✅ Convergence simulation complete!")
