...
class TestUnifiedTrainer:
    ...
    def test_run_multi_period_backtest(self, trainer, sample_backtest_data):
        ...
        with patch.object(trainer, '_create_backtest_environment') as mock_env, \
             patch.object(trainer, '_load_backtest_data', return_value=sample_backtest_data), \
             patch.object(trainer, '_run_single_period_backtest', return_value={
                 "metrics": {
                     "total_return": 0.05,
                     "win_rate": 0.6,
                     "total_trades": 100
    ...
    def test_run_single_period_backtest(self, trainer):
        ...
        with patch.object(trainer, '_create_backtest_environment') as mock_env, \
             patch('ztb.training.unified_trainer.trainer.evaluate_policy') as mock_eval:
...
with open('tests/unit/training/test_unified_trainer.py', 'w', encoding='utf-8') as f:
    f.write(content)
