...
class Phase2IntegrationTest:
    ...
    def test_regime_detection(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for idx in test_points:
            ...
            logger.info(
                f"Regime at {data.index[idx]}: {regime_info['regime']} "
    ...
    def test_signal_adaptation(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for i in range(window_size, len(data), 20):  # Test every 20 periods
            ...
            adaptation_info = {
                ...
                "confidence": result.confidence,
                "action": result.discrete_action,
                "regime_bias": result.metadata.get("regime_adaptation", {}).get(
                    "regime_bias_applied", 0
                ),
            }
    ...
    def test_performance_tracking(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "avg_system_confidence": np.mean(
                    [p["confidence"] for p in self.system.performance_history]
                )
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "unique_regimes_detected": len(
                    set(r["regime"] for r in regime_results if r["regime"])
                ),
                "avg_regime_confidence": np.mean(
                    [r["regime_confidence"] for r in regime_results]
                ),
                "avg_signal_quality": np.mean(
                    [r["quality_score"] for r in adaptation_results]
                ),
            ...
            results["test_summary"] = {"test_completed": False, "error": str(e)}
    ...
    def validate_system_requirements(self) -> Dict[str, Any]:
        ...
        validation_results["regime_classification"]["requirement_met"] = (
            actual_regimes >= 16
        )

        # Validate architecture
        validation_results["architecture_requirements"]["validation"] = {
            "has_base_classes": hasattr(self.system, "regime_classifier")
            and hasattr(self.system, "quality_scorer"),
...
def main():
    ...
    print(
        f"   Regime count: {validation['regime_classification']['actual_regimes']}/16 ✓"
        if validation["regime_classification"]["requirement_met"]
        else "   Regime count: FAILED"
    )
    ...
    if results["test_summary"]["test_completed"]:
        ...
        for result in regime_results:
            print(
                f"   {result['timestamp']}: {result['regime']} "
                ".2f"
                f"(Action: {result['action']}, Quality: {result['quality_score']:.1f})"
            )
        print()
        ...
        print(
            f"   ✗ Test failed: {results['test_summary'].get('error', 'Unknown error')}"
        )
        return 1
