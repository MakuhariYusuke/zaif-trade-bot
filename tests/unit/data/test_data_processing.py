...
class TestOutlierDetection(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_data = pd.DataFrame(
            {"price": all_data, "volume": np.random.normal(1000, 100, 100)}
        )
    ...
    def test_iqr_detection(self):
        """IQR法による異常値検出のテスト。"""
        flags = self.detector._detect_iqr(self.sample_data, ["price"], multiplier=1.5)
...
class TestDataProcessingPipeline(unittest.TestCase):
    ...
    def test_pipeline_execution(self):
        ...
        result = self.pipeline.process_data(
            self.sample_data, steps=["validation", "outlier_detection"]
        )
    ...
    def test_config_update(self):
        """設定更新のテスト。"""
        custom_config = {"augmentation": {"enabled": False}}
        ...
        # 設定が更新されていることを確認
        self.assertNotEqual(
            self.pipeline.config["augmentation"]["enabled"], original_setting
        )
