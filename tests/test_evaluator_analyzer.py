    ...
    for action in actions:
        ...
        if action == 1:  # BUY
            ...
            portfolio_values.append(
                portfolio_values[-1] * (1 + random.uniform(-0.005, 0.005))
            )
        else:  # SELL
            portfolio_values.append(
                portfolio_values[-1] * (1 + random.uniform(-0.005, 0.005))
        ...
        "continuous_action_stats": {
            "max_buy_streak": max(
                (
                    ...
                    for action, group in itertools.groupby(actions)
                    if action == 1
                ),
                default=0,
            ),
...
def test_sac_v420_anomalies():
    ...
    config = {
        "model_path": str(
            ...
            / "checkpoints"
            / "sac_session"
            / "sac_v414_balanced_trading_final.zip"
        ),  # Use latest available model
        "data_path": str(
            Path(__file__).parent.parent / "data" / "btc_jpy_featured_dataset.csv"
    ...
    try:
        ...
        if actions:
            ...
            if max_buy_streak >= 942:
                print(
                    "WARNING: Detected 942+ consecutive BUY actions - production unsuitable!"
                )
            if max_sell_streak >= 942:
                print(
                    "WARNING: Detected 942+ consecutive SELL actions - production unsuitable!"
                )
