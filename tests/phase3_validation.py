...
def run_baseline_backtest(model_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"✅ Baseline backtest completed. Final value: {baseline_result['final_value']:.2f}"
        )
...
def run_phase3_backtest(model_name: str) -> Dict[str, Any]:
    ...
    """
    logger.info(
        "🚀 Running Phase 3 integrated backtest (with risk management & statistical validation)"
    )
...
def calculate_sharpe_ratio(portfolio_values: list) -> float:
    ...
    return sharpe_ratio(pd.Series(returns), rf=0.0, period_per_year=252)
...
def calculate_max_drawdown(portfolio_values: list) -> float:
    ...
    return max_drawdown(pd.Series(portfolio_values))


def compare_results(
    baseline_result: Dict[str, Any], phase3_result: Dict[str, Any]
) -> Dict[str, Any]:
    """
    結果比較
    ...
    Returns:
        比較結果
    """
    comparison = {"baseline": baseline_result, "phase3": {}, "improvements": {}}
    ...
    comparison["improvements"] = {
        ...
        "return_std_reduction": phase3_summary.get(
            "std_total_return", 0
        ),  # Phase 3の標準偏差
...
def main():
    ...
    try:
        ...
        if phase3_result.get("success"):
            ...
            print(
                f"  Mean Final Value: {phase3_summary.get('mean_final_value', 0):.2f}"
            )
            print(
                f"  Mean Total Return: {phase3_summary.get('mean_total_return', 0):.2%}"
            )
            print(
                f"  Mean Sharpe Ratio: {phase3_summary.get('mean_sharpe_ratio', 0):.2f}"
            )
            print(
                f"  Mean Max Drawdown: {phase3_summary.get('mean_max_drawdown', 0):.2%}"
            )
        ...
        if improvements:
            ...
            print(
                f"  Return Improvement: {improvements.get('return_improvement', 0):.2%}"
            )
            print(
                f"  Sharpe Improvement: {improvements.get('sharpe_improvement', 0):.2f}"
            )
            print(
                f"  Drawdown Reduction: {improvements.get('drawdown_reduction', 0):.2%}"
            )
            print(
                f"  Return Std Dev: {improvements.get('return_std_reduction', 0):.4f}"
            )
            print(
                f"  Sharpe Std Dev: {improvements.get('sharpe_std_reduction', 0):.4f}"
            )
