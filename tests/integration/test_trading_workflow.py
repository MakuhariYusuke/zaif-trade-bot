...
class TradingStrategy:
    ...
    def generate_signals(self, data):
        ...
        signals['signal'] = np.where(signals['combined_signal'] >= 1, 1,
...
class PositionManager:
    ...
    def get_pnl(self):
        ...
        realized_pnl = sum(p.get('profit', 0) for p in self.positions if p['type'] == 'sell')
        unrealized_pnl = 0
...
class TestTradingWorkflowIntegration:
    ...
    def test_complete_trading_cycle(self):
        ...
        assert all(col in result['market_data'].columns for col in ['open', 'high', 'low', 'close', 'volume'])
    ...
    def test_position_management(self):
        ...
        assert self.workflow.position_manager.balance > 10000  # Balance increased (profit)
    ...
    def test_error_handling_in_workflow(self):
        ...
        with patch.object(self.workflow.data_generator, 'generate_synthetic_data') as mock_gen:
            # Create empty DataFrame with required columns
            mock_gen.return_value = pd.DataFrame(columns=['open', 'high', 'low', 'close', 'volume'])
