...
class TestMarketRegimeAdaptationIntegration:
    ...
    def test_complete_regime_adaptation_workflow(self, trainer_config, env_config, adaptive_classifier, sample_market_data):
    ...
    def test_regime_adaptation_performance_impact(self, trainer_config, env_config, adaptive_classifier, sample_market_data):
        ...
        env_without_adaptation = HeavyTradingEnv(df=sample_market_data, config=env_config.copy())
        ...
        next_state_with, reward_with, done_with, truncated_with, info_with = env_with_adaptation.step(action)
        next_state_without, reward_without, done_without, truncated_without, info_without = env_without_adaptation.step(action)
        ...
        assert reward_with != reward_without or abs(reward_with - reward_without) < 0.1  # Allow small differences

    def test_regime_transition_handling(self, trainer_config, env_config, adaptive_classifier, sample_market_data):
        ...
        for step in range(10):  # Reduced steps for testing
            ...
            if hasattr(env, 'regime_statistics') and 'regime_transitions' in env.regime_statistics:
    ...
    def test_regime_adaptation_stability(self, trainer_config, env_config, adaptive_classifier, sample_market_data):
    ...
    def test_regime_statistics_accuracy(self, trainer_config, env_config, adaptive_classifier, sample_market_data):
    ...
    def test_error_handling_integration(self, trainer_config, env_config, adaptive_classifier, sample_market_data):
        ...
        with patch('ztb.trading.environment.heavy_env.core.logger') as mock_env_logger:
            ...
            adaptive_classifier.detect_regime = Mock(side_effect=Exception("Detection failed"))

            env.enable_market_regime_adaptation(adaptive_classifier)
            trainer = SACTrainer(trainer_config, env)

            # Mock trainer logger
            with patch('ztb.training.unified_trainer.algorithms.sac_trainer.logger') as mock_trainer_logger:
                ...
                try:
                    ...
                    next_state, reward, done, truncated, info = env.step(np.array([0.0, 0.0, 0.0]))  # Dummy action
