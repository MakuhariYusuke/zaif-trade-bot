...
class TestCalibrationIntegration:
    ...
    def test_full_calibration_workflow(self):
        ...
        # Model 1
        if (
            model1_sharpe < min_sharpe_threshold
            or model1_masked_rate >= disqualification_threshold
        ):
            disqualified.append(0)

        # Model 2
        if (
            model2_sharpe < min_sharpe_threshold
            or model2_masked_rate >= disqualification_threshold
        ):
            disqualified.append(1)

        # Model 3
        if (
            model3_sharpe < min_sharpe_threshold
            or model3_masked_rate >= disqualification_threshold
        ):
