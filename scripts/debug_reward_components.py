...
def debug_reward_components():
    ...
    for case in test_cases:
        ...
        # Position size bonus
        position_size_bonus = env.reward_calculator._calculate_position_size_bonus(case["position"], case["old_position"])
        print(f"3. Position size bonus: {position_size_bonus:+.6f}")

        # Convert action to discrete
        discrete_action = env.reward_calculator._convert_continuous_to_discrete_action(case["action"])
        print(f"4. Discrete action: {discrete_action}")

        # Action balance bonus
        action_balance_bonus = env.reward_calculator._calculate_action_balance_bonus(discrete_action)
        print(f"5. Action balance bonus: {action_balance_bonus:+.6f}")

        # Win rate bonus
        win_rate_bonus = env.reward_calculator._calculate_win_rate_bonus(discrete_action, case["pnl"])
