...
def continuous_to_discrete_action(continuous_action: float, buy_threshold: float = 0.1, sell_threshold: float = -0.1) -> int:
    """
...
class TrainingProgressCallback(BaseCallback):
    ...
    def _log_progress(self):
        ...
        if self.discrete_actions:
            ...
            self._logger.info(f"Step {self.n_calls}: Action Distribution - "
            ...
            if recent_continuous:
                ...
                self._logger.info(f"Step {self.n_calls}: Continuous Action Stats - "
                               f"Mean: {mean_action:.3f}, Std: {std_action:.3f}")

        if self.reward_history:
            recent_rewards = self.reward_history[-1000:]  # Last 1000 rewards
            avg_reward = np.mean(recent_rewards)
            self._logger.info(f"Step {self.n_calls}: Recent Avg Reward: {avg_reward:.4f}")
...
def create_environment(config_path: str) -> HeavyTradingEnv:
    ...
    print("DEBUG: use_continuous_actions:", config_data.get('environment', {}).get('use_continuous_actions'))
...
def validate_training(model: SAC, env: HeavyTradingEnv, n_episodes: int = 10) -> Dict[str, Any]:
    ...
    logger.info(f"  Action Distribution: HOLD={action_distribution['HOLD']:.1%}, "
               f"BUY={action_distribution['BUY']:.1%}, SELL={action_distribution['SELL']:.1%}")
    logger.info(f"  Continuous Action Stats: Mean={continuous_stats['mean']:.3f}, "
...
def main():
    ...
    model_save_path = "checkpoints/sac_continuous_v1.zip"
    total_timesteps = 50000  # Moderate training for initial testing
    ...
    try:
        ...
        with open("results/sac_continuous_training_validation.json", 'w', encoding='utf-8') as f:
