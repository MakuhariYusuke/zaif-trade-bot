...
def compute_action_weights(
    ...
    # Compute frequencies
    frequencies = {
        action: max(count / total, epsilon)
...
def generate_dryrun_report(
    ...
    frequencies = {
        action: count / total if total > 0 else 0.0
    ...
    report = {
        ...
        "weight_statistics": {
            ...
            "max_ratio": max(weights.values()) / min(weights.values()) if min(weights.values()) > 0 else float('inf'),
        },
        "gradient_scaling_estimate": {
            action: f"{weight:.2f}x baseline"
