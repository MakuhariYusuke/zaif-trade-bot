    ...
    for config_name, result in all_results.items():
        ...
        if result.get("status") == "success":
            ...
            if analysis:
                ...
                report.append(
                    f"  • Total Return: {analysis.get('total_return', 'N/A')}"
                )
        else:
...
def main():
    ...
    parser.add_argument(
        "--config", help="Test specific config (scale_200, scale_300, scale_500)"
    )
    parser.add_argument(
        "--skip-training", action="store_true", help="Skip training, only analyze"
    )
    ...
    if not args.skip_training:
        for config_name, config_info in configs_to_test.items():
            result = run_training(
                config_info["config_path"], config_info["description"]
            )
