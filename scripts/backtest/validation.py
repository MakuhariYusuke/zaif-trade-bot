...
def create_short_term_enhanced_env(data_df, config):
    ...
    # 🔧 CRITICAL FIX: Ensure no zero prices
    # Fill NaNs first to avoid dropping everything if some non-critical feature is NaN
    enhanced_df = enhanced_df.fillna(method="ffill").fillna(0)

    if (enhanced_df["close"] <= 0).any():
        logger.warning("Found zero or negative prices in enhanced_df. Fixing...")
        # Replace 0 or negative prices with a small positive value or previous value
        enhanced_df.loc[enhanced_df["close"] <= 0, "close"] = 1000.0  # Fallback

    logger.info(f"✅ Enhanced features after cleaning: {len(enhanced_df)} rows")
...
def run_short_term_backtest(model_path=None, config_path=None, n_episodes=1):
    ...
    if all_actions:
        # Map actions to counts
        # 0: HOLD, 1: BUY, 2: SELL (Discrete(3)) or -1: SELL (Internal)
        hold_count = all_actions.count(0)
        buy_count = all_actions.count(1)
        sell_count = all_actions.count(2) + all_actions.count(-1)

        action_counts = {"SELL": sell_count, "HOLD": hold_count, "BUY": buy_count}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Short-term Enhanced Features Backtest"
    )
    ...
    results = run_short_term_backtest(
        model_path=args.model, config_path=args.config, n_episodes=args.episodes
    )
