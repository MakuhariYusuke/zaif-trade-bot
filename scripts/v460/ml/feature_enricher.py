...
_RAW_ORDERBOOK_CACHE: OrderedDict[
    tuple[str, tuple[str, ...] | None],
    _RawLoadCacheEntry,
] = OrderedDict()
_RAW_TRADES_CACHE: OrderedDict[
    tuple[str, tuple[str, ...] | None],
    _RawLoadCacheEntry,
] = OrderedDict()


def clear_raw_load_caches() -> None:
    """板/約定 raw ロードの module-level cache を明示的に解放する."""
    _RAW_ORDERBOOK_CACHE.clear()
    _RAW_TRADES_CACHE.clear()


def get_raw_load_cache_stats() -> dict[str, int]:
    """raw ロード cache の件数を返す."""
    return {
        "orderbook_cache_entries": len(_RAW_ORDERBOOK_CACHE),
        "trades_cache_entries": len(_RAW_TRADES_CACHE),
        "total_cache_entries": len(_RAW_ORDERBOOK_CACHE) + len(_RAW_TRADES_CACHE),
    }
...
def _load_cached_raw_entry(
    cache: OrderedDict[tuple[str, tuple[str, ...] | None], _RawLoadCacheEntry],
...
def _store_raw_cache(
    ...
    cache.move_to_end(cache_key)
    while len(cache) > _RAW_LOAD_CACHE_MAX_ENTRIES:
