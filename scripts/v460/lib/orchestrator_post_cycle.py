...
class OrchestratorPostCycleMixin:
    ...
    def _process_post_cycle(
        ...
        if record.skip_gate_skipped is False and record.effective_offset_used is not None:
            ...
            # 467#: hour_ceiling_mult 反映 (record の timestamp 基準)
            from scripts.v460.lib.hour_rules import utc_hour_from_timestamp
            _record_utc_hour = utc_hour_from_timestamp(record.timestamp)
            _ceil = self.config.resolve_offset_ceiling(record.side, utc_hour=_record_utc_hour)
