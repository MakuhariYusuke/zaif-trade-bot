...
def _load_records(results_dir: Path) -> list[dict[str, object]]:
    """fill_records を全ファイルからロード."""
    records: list[dict[str, object]] = []
    for rec in iter_fill_record_objects_glob(str(results_dir)):
        records.append(rec)
    return records
...
def _save_baseline(results_dir: Path, output_path: Path) -> None:
...
def _show_baseline(results_dir: Path) -> None:
...
def main() -> None:
    ...
    if args.save_baseline:
        _save_baseline(results_dir, Path(args.baseline_path))
    elif args.show_baseline:
        _show_baseline(results_dir)
    elif args.compare:
        _run_comparison(results_dir, args.split_date, Path(args.output))
    else:
        parser.print_help()
