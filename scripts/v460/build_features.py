...
def build_proxy_features(df: pd.DataFrame, window: int = 20) -> pd.DataFrame:
    ...
    volume_roll_sum = rolling_volume.sum()
    volume_roll_mean = rolling_volume.mean()

    # ---- 3. trade_flow_imbalance ----
    # Proxy: signed volume — CLV × volume, normalized
    signed_vol = clv * volume
    rolling_signed_vol = signed_vol.rolling(window, min_periods=1)
    sv_std = rolling_signed_vol.std() + eps
