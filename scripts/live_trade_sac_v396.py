...
class SACLiveTrader:
    ...
    def execute_trade(self, action: int, price: float):
        ...
        if action == 1:  # BUY
            if self.current_position >= 0:  # No position or long, increase long
                ...
                self.balance -= cost
                self.logger.info(f"BUY - Position: {self.current_position:.4f}, Cost: {cost:.2f}")
            ...
            if self.current_position <= 0:  # No position or short, increase short
                ...
                self.logger.info(f"SELL - Position: {self.current_position:.4f}, Revenue: {revenue:.2f}")
    ...
    def check_risk_limits(self) -> bool:
        ...
        if daily_loss > self.max_daily_loss:
            self.logger.error(f"Daily loss limit exceeded: {daily_loss:.1%} > {self.max_daily_loss:.1%}")
            return False

        # Check emergency stop
        if self.balance < self.emergency_stop_balance:
            self.logger.error(f"Emergency stop triggered: balance {self.balance:.2f} < {self.emergency_stop_balance:.2f}")
            return False

        # Check hourly trade limit
        if self.trade_count_hour >= self.max_trades_per_hour:
            self.logger.warning(f"Hourly trade limit reached: {self.trade_count_hour}/{self.max_trades_per_hour}")
            return False
    ...
    def run(self, duration_minutes: int = 60):
        ...
        while time.time() < end_time:
            try:
                ...
                self.logger.info(f"Price: {current_price:.0f}, Continuous Action: {continuous_action:.3f}, Discrete Action: {discrete_action}")
                ...
                if not self.dry_run:
                    ...
                    self.logger.info(f"DRY RUN - Would execute action {discrete_action}")

                # Log status
                self.logger.info(f"Balance: {self.balance:.2f}, Position: {self.current_position:.4f}")
        ...
        self.logger.info(f"Trading completed. Final balance: {self.balance:.2f}, Trades executed: {trade_count}")
