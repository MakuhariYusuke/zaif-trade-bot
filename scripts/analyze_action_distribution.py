...
def extract_action_distribution_from_events(event_file: Path) -> ActionDistributionResults:
    ...
    scalar_tags: List[str] = scalar_tags_raw if isinstance(scalar_tags_raw, list) else []
...
def analyze_action_distribution(results: ActionDistributionResults) -> ActionDistributionAnalysis:
    """アクション分布を分析して問題を診断"""
    ...
    if sell_rate > (ideal_rate + tolerance):
        analysis['issues'].append(f"🔴 SELL bias detected: {sell_rate*100:.2f}% (target: 33.33%)")
        analysis['status'] = 'sell_bias'
        analysis['recommendations'].append("Increase Lagrange constraint strength (lagrange_eta)")
    ...
    if hold_rate > (ideal_rate + tolerance * 2):
        analysis['issues'].append(f"🟡 HOLD bias detected: {hold_rate*100:.2f}% (target: 33.33%)")
        ...
        analysis['recommendations'].append("Check if min_holding_period is too restrictive")
    
    # BUY偏重問題のチェック
    if buy_rate > (ideal_rate + tolerance):
        analysis['issues'].append(f"🟠 BUY bias detected: {buy_rate*100:.2f}% (target: 33.33%)")
        analysis['status'] = 'buy_bias'
        analysis['recommendations'].append("Check reward function for BUY favoritism")
        analysis['recommendations'].append("Verify transaction costs are applied correctly")
    ...
    if imbalance_ratio > 5.0:
        analysis['issues'].append(f"🔴 Severe action imbalance: {imbalance_ratio:.2f}x ratio")
    ...
    if entropy is not None:
        ...
        if entropy < max_entropy * 0.7:
            analysis['issues'].append(f"🟡 Low action entropy: {entropy:.4f} (max: {max_entropy:.4f})")
            analysis['recommendations'].append("Increase entropy coefficient (ent_coef)")
    ...
    if lambda_val is not None:
        ...
        if lambda_val >= 1.9:  # lambda_max = 2.0に近い
            analysis['issues'].append(f"⚠️ Lagrange constraint saturated: λ={lambda_val:.4f}")
            analysis['recommendations'].append("Increase lagrange_lambda_max")
            analysis['recommendations'].append("Increase lagrange_eta for faster adaptation")
