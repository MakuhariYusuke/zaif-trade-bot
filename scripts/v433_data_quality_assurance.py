...
class DataQualityAssurance:
    ...
    def comprehensive_quality_check(self, df: pd.DataFrame,
                                  market: str = "BTC-JPY") -> Dict[str, Any]:
        """
        包括的なデータ品質チェックを実行
        ...
        logger.info(f"Quality check completed. Overall score: {results['overall_score']:.2f}")
        return results
    ...
    def _check_basic_integrity(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        duplicate_penalty = min(integrity['duplicate_timestamps'] / len(df), 1.0) if len(df) > 0 else 1.0

        integrity['score'] = 1.0 - (missing_penalty * 0.4 + invalid_penalty * 0.4 + duplicate_penalty * 0.2)
    ...
    def _check_statistical_properties(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            stats_check['price_distribution']['reasonable_distribution'] = reasonable_skew and reasonable_kurt
        ...
        try:
            ...
            stats_check['volatility_analysis']['realistic_volatility'] = 0.1 <= volatility <= 2.0  # 10%-200%の範囲
        ...
        try:
            if len(returns) > 10:
                ...
                stats_check['autocorrelation']['weak_autocorr'] = abs(autocorr_1) < 0.3  # 弱い自己相関
        ...
        if 'reasonable_distribution' in stats_check['price_distribution']:
            score_components.append(stats_check['price_distribution']['reasonable_distribution'])

        if 'realistic_volatility' in stats_check['volatility_analysis']:
            score_components.append(stats_check['volatility_analysis']['realistic_volatility'])
        ...
        if market == "BTC-JPY":
            ...
            if 'close' in df.columns:
                ...
                realism['price_range'] = {
                    ...
                    'in_expected_range': expected_min <= actual_min and actual_max <= expected_max
                }

            # 出来高分析
            if 'volume' in df.columns:
                avg_volume = df['volume'].mean()
                volume_variability = df['volume'].std() / df['volume'].mean() if df['volume'].mean() > 0 else 0
            ...
            if len(df) > 5:
                ...
                realism['market_behavior']['reasonable_volatility'] = extreme_changes == 0
    ...
    def _check_temporal_consistency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(df) > 1:
            ...
            expected_diff = time_diffs.mode().iloc[0] if not time_diffs.mode().empty else pd.Timedelta(days=1)
        ...
        if len(df) > 10:
            ...
            temporal['frequency_consistency'] = {
                ...
                'consistent_weekly_pattern': weekday_counts.std() / weekday_counts.mean() < 0.5
            }
        ...
        if len(df) > 30:
            ...
            temporal['business_days'] = {
                ...
                'mostly_business_days': len(weekend_data) < len(df) * 0.3  # 30%以内の週末データ
            }
        ...
        if 'consistent_weekly_pattern' in temporal['frequency_consistency']:
            score_components.append(temporal['frequency_consistency']['consistent_weekly_pattern'])
    ...
    def _detect_anomalies(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'close' in df.columns:
            ...
            if len(prices) > 0:
                ...
                anomalies['outlier_detection'] = {
                    ...
                    'acceptable_outliers': outliers / len(prices) < 0.05  # 5%以内の外れ値
                }
        ...
        if 'close' in df.columns and len(df) > 5:
            ...
            anomalies['sudden_changes'] = {
                ...
                'acceptable_changes': sudden_changes < len(returns) * 0.1  # 10%以内の突然変化
            }
        ...
        if 'acceptable_outliers' in anomalies['outlier_detection']:
            score_components.append(anomalies['outlier_detection']['acceptable_outliers'])
    ...
    def _assess_completeness(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if completeness['expected_records'] > 0:
            completeness['completeness_ratio'] = len(df) / completeness['expected_records']
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if stats.get('score', 1.0) < 0.8:
            if not stats.get('price_distribution', {}).get('reasonable_distribution', True):
                recommendations.append("価格分布の異常性を調査してください")
            if not stats.get('volatility_analysis', {}).get('realistic_volatility', True):
        ...
        if overall_score < 0.7:
            recommendations.append("データ品質が低いため、データソースの見直しを推奨します")
