...
def validate_model(model_path: str, data_path: str, num_episodes: int = 3):
    ...
    for episode in range(num_episodes):
        ...
        print(f"  Actions: HOLD={action_counts.get(0, 0)}, BUY={action_counts.get(1, 0)}, SELL={action_counts.get(2, 0)}")
        print(f"  Action %: HOLD={action_counts.get(0, 0)/len(actions)*100:.1f}%, BUY={action_counts.get(1, 0)/len(actions)*100:.1f}%, SELL={action_counts.get(2, 0)/len(actions)*100:.1f}%")
    ...
    print(f"Average reward per episode: {sum(episode_rewards) / len(episode_rewards):.4f}")
    print(f"\nAction Distribution:")
    print(f"  HOLD:  {total_actions.get(0, 0):4d} ({total_actions.get(0, 0)/total_steps*100:5.1f}%)")
    print(f"  BUY:   {total_actions.get(1, 0):4d} ({total_actions.get(1, 0)/total_steps*100:5.1f}%)")
    print(f"  SELL:  {total_actions.get(2, 0):4d} ({total_actions.get(2, 0)/total_steps*100:5.1f}%)")
    ...
    balance_score = min(sell_ratio, buy_ratio, hold_ratio) / max(sell_ratio, buy_ratio, hold_ratio)
...
def main():
    ...
    parser.add_argument(
        "--model-path",
        default="models/ppo_100k_optimized.zip",
        help="Path to model",
    )
    parser.add_argument(
        "--data-path",
