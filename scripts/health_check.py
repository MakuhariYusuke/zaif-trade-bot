...
def format_as_table(results: Dict[str, Any]) -> str:
    ...
    if "performance" in results:
        ...
        lines.append(f"  Network Sent: {current['network_bytes_sent'] / (1024**2):.1f} MB")
        lines.append(f"  Network Received: {current['network_bytes_recv'] / (1024**2):.1f} MB")
        lines.append("")
        ...
        if perf["trends"]:
            ...
            for trend in perf["trends"]:
                ...
                lines.append(f"{trend_icon} {trend['metric']}: {direction_icon} {trend['trend_direction'].title()}")
            ...
            if concerning_count > 0:
                lines.append(f"{Fore.RED}⚠ {concerning_count} concerning performance trend(s) detected{Style.RESET_ALL}")
            else:
                lines.append(f"{Fore.GREEN}✓ All performance metrics within normal ranges{Style.RESET_ALL}")
        else:
        ...
        lines.append("")
        lines.append(f"Performance Summary: {summary['data_points']} data points, {summary['concerning_trends']} concerning trends")
