...
class EnhancedFeatureAnalyzer:
    ...
    def _identify_features(self) -> List[str]:
        """特徴量列を識別"""
        exclude_cols = ["ts", "pair", "side", "pnl", "win", "source", "timestamp", "date", "datetime"]
        features = [
            col for col in self.df.columns
            if col not in exclude_cols and self.df[col].dtype in ["float64", "int64", "float32", "int32"]
        ]
        return features
        ...
        for feature in self.features:
            ...
            if len(data_clean) > 0:
                ...
                if IQR > 0:
                    ...
                    outlier_rate = ((data_clean < lower_bound) | (data_clean > upper_bound)).sum() / len(data_clean)
    ...
    def select_by_correlation(
        ...
        for i in range(len(self.features)):
            for j in range(i + 1, len(self.features)):
                ...
                if isinstance(corr_value, (int, float)) and abs(corr_value) > correlation_threshold:
                    # 高相関ペア発見
    ...
    def suggest_optimal_features(
        ...
        if remove_harmful:
            ...
            kept_after_harmful = [
                f for f in kept_after_correlation 
                if f not in harmful_features
            ]
            print(f"   Removed {len(kept_after_correlation) - len(kept_after_harmful)} harmful features")
        else:
        ...
        feature_scores = {
            f: importance_dict[f]["importance_mean"] 
            for f in kept_after_harmful
        }
        ...
        return {
            ...
            "summary": {
                ...
                "reduction_rate": (1 - len(selected_features) / len(self.features)) * 100
            }
        }
