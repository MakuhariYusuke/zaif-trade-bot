...
def analyze_rewards_and_actions(
    ...
    analysis = {
        ...
        "discrete_action_distribution": {
            ...
            "HOLD_pct": float(np.sum(np.array(discrete_actions) == 0) / len(discrete_actions) * 100),
            "BUY_pct": float(np.sum(np.array(discrete_actions) == 1) / len(discrete_actions) * 100),
            "SELL_pct": float(np.sum(np.array(discrete_actions) == 2) / len(discrete_actions) * 100),
        },
        "position_stats": {
            ...
            "zero_position_pct": float(np.sum(np.array(positions) == 0) / len(positions) * 100),
        },
        "position_change_stats": {
            ...
            "significant_changes": int(np.sum(np.array(position_changes) > 0.05)),  # >5% changes
        },
        "pnl_stats": {
    ...
    logger.info(f"Reward range: [{analysis['reward_stats']['min']:.6f}, {analysis['reward_stats']['max']:.6f}]")
    ...
    logger.info("="*60)
    logger.info(f"Continuous action mean: {analysis['continuous_action_stats']['mean']:.4f}")
    logger.info(f"Continuous action std: {analysis['continuous_action_stats']['std']:.4f}")
    logger.info(f"Continuous action range: [{analysis['continuous_action_stats']['min']:.4f}, {analysis['continuous_action_stats']['max']:.4f}]")
    ...
    logger.info("="*60)
    logger.info(f"Zero position: {analysis['position_stats']['zero_position_pct']:.2f}%")
    logger.info(f"Position range: [{analysis['position_stats']['min']:.4f}, {analysis['position_stats']['max']:.4f}]")
    logger.info(f"Significant position changes (>5%): {analysis['position_change_stats']['significant_changes']}")
    logger.info(f"Mean position change: {analysis['position_change_stats']['mean']:.6f}")
...
def main():
    parser = argparse.ArgumentParser(description="Analyze v397a_aggressive rewards and actions")
    parser.add_argument(
        "--model-path",
        type=str,
