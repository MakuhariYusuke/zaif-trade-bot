...
if os.path.exists(results_dir):
    files = [
        ...
        if f.startswith("sac_v444") and f.endswith(".json")
    ]
    if files:
        latest_file = max(
            files, key=lambda x: os.path.getctime(os.path.join(results_dir, x))
        )
        ...
        if actions:
            ...
            for action, count in sorted(action_counts.items()):
                ...
                action_name = {0: "HOLD", 1: "BUY", -1: "SELL"}.get(
            ...
            if rewards:
                ...
                print(
                    f"  Positive rewards: {len(positive_rewards)} ({len(positive_rewards)/len(rewards)*100:.1f}%)"
                )
            ...
            if balance_penalties:
                ...
                print(
                    f"  Balance penalty > 0: {len([p for p in balance_penalties if p > 0])} times"
                )
        else:
