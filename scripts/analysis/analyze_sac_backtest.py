...
def analyze_action_streaks(discrete_actions):
    ...
    return {
        ...
        "avg_sell_streak": sum(sell_streaks) / len(sell_streaks) if sell_streaks else 0.0,
...
def main():
    ...
    if 'continuous_action_stats' in data and data['continuous_action_stats']:
        ...
        if 'action_streaks' in stats:
            ...
            sorted_buy_streaks = sorted(buy_streak_counts.items(), key=lambda x: x[1], reverse=True)
            ...
            sorted_sell_streaks = sorted(sell_streak_counts.items(), key=lambda x: x[1], reverse=True)
