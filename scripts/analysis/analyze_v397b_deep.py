...
def analyze_reward_components(
    ...
    while not done:
        ...
        # 取引ボーナス
        if position_change > config["reward_settings"]["trade_execution_position_threshold"]:
        ...
        # 不活性ペナルティ（簡易推定）
        if abs(position_change) < config["reward_settings"]["inactivity_hold_threshold"]:
    ...
    results = {
        "reward_components": {
            ...
            "pnl_component_estimate": {
                ...
                "contribution_pct": float(pnl_components_arr.sum() / total_rewards_arr.sum() * 100) if total_rewards_arr.sum() != 0 else 0
            },
            "trade_bonus": {
                "mean": float(trade_bonuses_arr.mean()),
                "fired_count": int(np.sum(trade_bonuses_arr > 0)),
                "fired_pct": float(np.sum(trade_bonuses_arr > 0) / len(trade_bonuses_arr) * 100),
                "sum": float(trade_bonuses_arr.sum()),
                "contribution_pct": float(trade_bonuses_arr.sum() / abs(total_rewards_arr.sum()) * 100) if total_rewards_arr.sum() != 0 else 0
            },
            "inactivity_penalty": {
                "mean": float(inactivity_penalties_arr.mean()),
                "fired_count": int(np.sum(inactivity_penalties_arr < 0)),
                "fired_pct": float(np.sum(inactivity_penalties_arr < 0) / len(inactivity_penalties_arr) * 100),
                "sum": float(inactivity_penalties_arr.sum()),
                "contribution_pct": float(abs(inactivity_penalties_arr.sum()) / abs(total_rewards_arr.sum()) * 100) if total_rewards_arr.sum() != 0 else 0
            }
        },
        "time_series_patterns": {
            "reward_autocorrelation": float(np.corrcoef(total_rewards_arr[:-1], total_rewards_arr[1:])[0, 1]) if len(total_rewards_arr) > 1 else 0,
            "position_autocorrelation": float(np.corrcoef(positions_arr[:-1], positions_arr[1:])[0, 1]) if len(positions_arr) > 1 else 0,
            "action_position_correlation": float(np.corrcoef(continuous_actions_arr, positions_arr)[0, 1]),
            "reward_position_change_correlation": float(np.corrcoef(total_rewards_arr, position_changes_arr)[0, 1])
        },
        "trading_patterns": {
            ...
            "position_size_distribution": {
                ...
                "quartiles": [float(q) for q in np.percentile(positions_arr, [25, 50, 75])]
            },
            "position_change_distribution": {
                ...
                "small_changes_count": int(np.sum((position_changes_arr > 0) & (position_changes_arr <= 0.01)))
            }
        },
        "market_interaction": {
            "price_trend": {
                "mean_return": float(np.diff(prices_arr).mean() / prices_arr[:-1].mean()),
                "volatility": float(np.std(np.diff(prices_arr) / prices_arr[:-1])),
                "total_price_change_pct": float((prices_arr[-1] - prices_arr[0]) / prices_arr[0] * 100)
            },
            "position_vs_trend": analyze_position_vs_trend(positions_arr, prices_arr)
        },
        "action_distribution": {
            "BUY_pct": float(np.sum(discrete_actions_arr == 0) / len(discrete_actions_arr) * 100),
            "HOLD_pct": float(np.sum(discrete_actions_arr == 1) / len(discrete_actions_arr) * 100),
            "SELL_pct": float(np.sum(discrete_actions_arr == 2) / len(discrete_actions_arr) * 100),
...
def analyze_hold_periods(positions: np.ndarray) -> Dict[str, Any]:
    ...
    # 保有期間を計算
    hold_periods = np.diff(np.concatenate([[0], position_changes_indices, [len(positions)]]))
...
def analyze_position_vs_trend(positions: np.ndarray, prices: np.ndarray) -> Dict[str, Any]:
    """ポジションと価格トレンドの関係を分析"""
    # 価格変化率
    ...
    return {
        ...
        "long_in_uptrend_pct": float(long_in_uptrend / total_long * 100) if total_long > 0 else 0,
        "long_in_downtrend_pct": float(long_in_downtrend / total_long * 100) if total_long > 0 else 0,
        "short_in_uptrend_pct": float(short_in_uptrend / total_short * 100) if total_short > 0 else 0,
        "short_in_downtrend_pct": float(short_in_downtrend / total_short * 100) if total_short > 0 else 0,
        "correct_direction_pct": float((long_in_uptrend + short_in_downtrend) / len(price_returns) * 100),
        "wrong_direction_pct": float((long_in_downtrend + short_in_uptrend) / len(price_returns) * 100)
    }
...
def print_detailed_analysis(results: Dict[str, Any]):
    ...
    print(f"    Fired:        {rc['trade_bonus']['fired_count']:,} times ({rc['trade_bonus']['fired_pct']:.1f}%)")
    print(f"    Sum:          {rc['trade_bonus']['sum']:+.2f}")
    print(f"    Contribution: {rc['trade_bonus']['contribution_pct']:.1f}%")
    
    print(f"\n  Inactivity Penalty:")
    print(f"    Fired:        {rc['inactivity_penalty']['fired_count']:,} times ({rc['inactivity_penalty']['fired_pct']:.1f}%)")
    ...
    print(f"  Position Autocorrelation:            {ts['position_autocorrelation']:+.3f}")
    print(f"  Action-Position Correlation:         {ts['action_position_correlation']:+.3f}")
    print(f"  Reward-PositionChange Correlation:   {ts['reward_position_change_correlation']:+.3f}")
    
    # 取引パターン
    print("\n💼 Trading Patterns:")
    tp = results["trading_patterns"]
    print(f"  Position Hold Periods:")
    ...
    print(f"    Range:  [{tp['position_hold_periods']['min_hold_period']}, {tp['position_hold_periods']['max_hold_period']}]")
    ...
    print(f"\n  Position Changes:")
    print(f"    Large (>10%):  {tp['position_change_distribution']['large_changes_count']:,}")
    print(f"    Small (≤1%):   {tp['position_change_distribution']['small_changes_count']:,}")
