...
def instrument_reward_calculator() -> None:
    ...
    def wrapped(
        ...
        if inactivity_enabled:
            ...
            if action == 0 and abs(position) <= hold_threshold_abs and abs(old_position) <= hold_threshold_abs:
        ...
        if bonus_enabled:
            threshold = self.get_setting_float("trade_execution_position_threshold", 0.01)
            ...
            if position_change > threshold_abs:
                ...
                multiplier = self.get_setting_float("trade_execution_action_multiplier", 1.0)
...
def run_deep_analysis(
    ...
    action_distribution = (
        df_records["discrete_action"].value_counts(normalize=True)
