...
def find_convergence_point(losses: List[float], window_size: int = 1000, 
                          improvement_threshold: float = 0.01) -> int:
...
def analyze_convergence(log_dir: Path, output_path: Path):
    ...
    convergence_point = find_convergence_point(critic_losses.tolist(), 
                                               window_size=1000,
                                               improvement_threshold=0.01)
    
    print(f"収束ポイント (改善率<1%): {convergence_point:,} steps")
    print(f"収束時のCritic Loss: {critic_losses[min(convergence_point, len(critic_losses)-1)]:.6f}")
    print()
    ...
    print(f"   結果: {'✅ 定常性あり (収束)' if is_stationary else '⚠️ 定常性なし (未収束)'}")
    print()
    ...
    result = {
        ...
        "interval_analysis": {
            ...
            "10k_30k": {
                "mean": float(np.mean(critic_losses[(steps >= 10000) & (steps < 30000)])),
                "min": float(mid_min)
            },
            "30k_50k": {
...
if __name__ == '__main__':
    ...
    output_path = project_root / 'checkpoints' / 'sac_session' / 'convergence_analysis.json'
