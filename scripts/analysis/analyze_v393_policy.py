...
def main() -> None:
    ...
    print(f"P(HOLD)  mean={probs_hold_array.mean():.3f}, median={np.median(probs_hold_array):.3f}, min={probs_hold_array.min():.3f}, max={probs_hold_array.max():.3f}")
    print(f"P(BUY)   mean={probs_buy_array.mean():.3f}, median={np.median(probs_buy_array):.3f}, min={probs_buy_array.min():.3f}, max={probs_buy_array.max():.3f}")
    print(f"P(SELL)  mean={probs_sell_array.mean():.3f}, median={np.median(probs_sell_array):.3f}, min={probs_sell_array.min():.3f}, max={probs_sell_array.max():.3f}")
    ...
    for action, count in zip(unique, counts):
        ...
        print(f"  {label}: {count}/{len(argmax_actions)} ({count/len(argmax_actions)*100:.1f}%)")
