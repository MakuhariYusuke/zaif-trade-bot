#!/usr/bin/env python3
"""
SAC v424 Cost-Aware Backtest Script

Runs backtest for SAC v424 model using the trained policy.
"""
...
class SACv424Backtester:
    ...
    def run_backtest(self, data_path: str, slippage_bps: float = 5.0) -> Dict[str, Any]:
        ...
        while not done:
            ...
            action_history.append(float(action) if np.isscalar(action) else float(action[0]))
            reward_history.append(float(reward))

            # Get current price from environment
            current_price = self.env._resolve_price() if hasattr(self.env, '_resolve_price') else 0.0
            ...
            if total_steps % 1000 == 0:
                ...
                logger.info(f"Step {total_steps}: Portfolio ¥{current_value:,.0f} ({pnl_pct:+.2f}%)")

        # Calculate final results
        final_portfolio = portfolio_history[-1]
        total_return_pct = ((final_portfolio - self.initial_capital) / self.initial_capital) * 100
        ...
        logger.info(f"Final Portfolio: ¥{final_portfolio:,.0f} ({total_return_pct:+.2f}%)")
        logger.info(f"Total Trades: {results['total_trades']}")
        logger.info(f"Action Distribution: HOLD {results['action_distribution']['HOLD']:.1%}, "
...
def main():
    """Main function to run v424 backtest."""
    import argparse

    parser = argparse.ArgumentParser(description="SAC v424 Cost-Aware Backtest")
    parser.add_argument("--model", type=str, default="models/sac_v424_cost_aware.zip",
                       help="Path to trained model")
    parser.add_argument("--data", type=str, default="data/btc_jpy_real_dataset.csv",
                       help="Path to test data")
    parser.add_argument("--output", type=str, default="results/backtest_v424_cost_aware.json",
    ...
    # Create backtester
    backtester = SACv424Backtester(args.model, args.capital)

    # Run backtest
    results = backtester.run_backtest(args.data)
