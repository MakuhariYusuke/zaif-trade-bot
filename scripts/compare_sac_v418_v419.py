...
def compare_models(v418_results: Dict[str, Any], v419_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    trade_intensity_diff = v418_metrics['trades_per_step'] - v419_metrics['trades_per_step']
    balance_diff = v418_metrics['action_balance_score'] - v419_metrics['action_balance_score']
    ...
    return {
        'performance_comparison': {
            ...
            'return_improvement_pct': (return_diff / abs(v419_metrics['total_return_pct'])) * 100 if v419_metrics['total_return_pct'] != 0 else float('inf'),
        ...
        'recommendation': 'v418' if v418_metrics['total_return_pct'] > v419_metrics['total_return_pct'] else 'v419'
    }
...
def main():
    ...
    try:
        ...
        print("\n📊 PERFORMANCE COMPARISON:")
        print(f"  SAC v418 Return: {comparison['performance_comparison']['v418_return']:.2f}%")
        print(f"  SAC v419 Return: {comparison['performance_comparison']['v419_return']:.2f}%")
        print(f"  Return Difference: {comparison['performance_comparison']['return_difference']:.2f}%")
        if comparison['performance_comparison']['return_improvement_pct'] != float('inf'):
            print(f"  Return Improvement: {comparison['performance_comparison']['return_improvement_pct']:.1f}%")
        ...
        print(f"  PnL Difference: {comparison['performance_comparison']['pnl_difference']:.2f}")
        print("\n🎯 TRADING BEHAVIOR:")
        print(f"  SAC v418 Trades/Step: {comparison['trading_behavior']['v418_trades_per_step']:.4f}")
        print(f"  SAC v419 Trades/Step: {comparison['trading_behavior']['v419_trades_per_step']:.4f}")
        print(f"  Trade Intensity Difference: {comparison['trading_behavior']['trade_intensity_difference']:.4f}")
        print(f"  SAC v418 Buy Ratio: {comparison['trading_behavior']['v418_buy_ratio']:.3f}")
        print(f"  SAC v419 Buy Ratio: {comparison['trading_behavior']['v419_buy_ratio']:.3f}")
        print(f"  SAC v418 Sell Ratio: {comparison['trading_behavior']['v418_sell_ratio']:.3f}")
        print(f"  SAC v419 Sell Ratio: {comparison['trading_behavior']['v419_sell_ratio']:.3f}")
        print(f"  SAC v418 Hold Ratio: {comparison['trading_behavior']['v418_hold_ratio']:.3f}")
        print(f"  SAC v419 Hold Ratio: {comparison['trading_behavior']['v419_hold_ratio']:.3f}")
        print(f"  SAC v418 Balance Score: {comparison['trading_behavior']['v418_balance_score']:.4f}")
        print(f"  SAC v419 Balance Score: {comparison['trading_behavior']['v419_balance_score']:.4f}")
        print(f"  Balance Improvement: {comparison['trading_behavior']['balance_improvement']:.4f}")
        print("\n🏆 RECOMMENDATION:")
        print(f"Based on paper trading results, {comparison['recommendation']} shows superior performance")
