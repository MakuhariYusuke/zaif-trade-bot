...
def run_single_paper_trade(script_path: str, model_version: str) -> Dict[str, Any]:
    ...
    try:
            ...
            if dist_line:
                try:
                    ...
                    if "HOLD:" in dist_part:
                        ...
                        for part in parts:
                            if 'HOLD:' in part:
                                action_dist['HOLD'] = float(part.split(':')[1].strip().replace('%', ''))
                            elif 'BUY:' in part:
                                action_dist['BUY'] = float(part.split(':')[1].strip().replace('%', ''))
                            elif 'SELL:' in part:
                                action_dist['SELL'] = float(part.split(':')[1].strip().replace('%', ''))
                    else:
        ...
        logger.info(f"Completed {model_version}: Portfolio={metrics.get('final_portfolio', 'N/A')}")
...
def run_multiple_simulations(script_path: str, model_version: str, num_runs: int = 5) -> List[Dict[str, Any]]:
...
def perform_statistical_analysis(v418_results: List[Dict[str, Any]], v420_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        pooled_std = np.sqrt((analysis['v418']['std']**2 + analysis['v420']['std']**2) / 2)
        cohen_d = (analysis['v418']['mean'] - analysis['v420']['mean']) / pooled_std if pooled_std > 0 else 0
...
def main():
    ...
    v418_results = run_multiple_simulations(
        "scripts/paper_trade_sac_v418.py",
        "v418",
        num_runs
    )
    ...
    v420_results = run_multiple_simulations(
        "scripts/paper_trade_sac_v420.py",
        "v420",
        num_runs
    )
    ...
    print("\n📈 PERFORMANCE SUMMARY:")
    print(f"   SAC v418: {analysis['v418']['count']} runs, "
          f"Mean={analysis['v418']['mean']:.2f}, Std={analysis['v418']['std']:.2f}")
    print(f"   SAC v420: {analysis['v420']['count']} runs, "
    ...
    if analysis.get('t_test', {}).get('significant', False):
        winner = "v418" if analysis['v418']['mean'] > analysis['v420']['mean'] else "v420"
        print(f"   Statistically significant difference found!")
