...
class CorrelationAwareFeatureEngineer:
    ...
    def __init__(self, data_path: str = "data/btc_jpy_balanced_v426_dataset.csv"):
        ...
        self.output_path = self.data_path.parent / "btc_jpy_correlation_aware_v426_dataset.csv"
    ...
    def calculate_price_position_corr(self, df: pd.DataFrame) -> pd.Series:
        ...
        logger.info(f"価格位置相関計算完了: 平均={price_position_corr.mean():.4f}, 範囲=[{price_position_corr.min():.4f}, {price_position_corr.max():.4f}]")
    ...
    def calculate_action_price_corr(self, df: pd.DataFrame) -> pd.Series:
        ...
        logger.info(f"アクション価格相関計算完了: 平均={action_price_corr.mean():.4f}, 範囲=[{action_price_corr.min():.4f}, {action_price_corr.max():.4f}]")
    ...
    def calculate_regime_alignment(self, df: pd.DataFrame) -> pd.Series:
        ...
        logger.info(f"レジーム整合性計算完了: 平均={regime_alignment.mean():.4f}, 範囲=[{regime_alignment.min():.4f}, {regime_alignment.max():.4f}]")
    ...
    def validate_features(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        # 各特徴量の統計
        for feature in ['price_position_corr', 'action_price_corr', 'regime_alignment', 'market_correlation_score']:
        ...
        # 相関行列
        corr_features = ['price_position_corr', 'action_price_corr', 'regime_alignment', 'close', 'volume']
    ...
    def generate_feature_report(self, df: pd.DataFrame, validation_results: Dict) -> None:
        """特徴量エンジニアリングレポートを生成"""
    ...
    def run_phase2(self) -> None:
        ...
        try:
            ...
            logger.info(f"市場相関スコア平均: {validation_results.get('market_correlation_score_mean', 'N/A'):.4f}")
