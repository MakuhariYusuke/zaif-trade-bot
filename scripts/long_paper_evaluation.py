...
def long_paper_evaluation(
    ...
    legal_sell_rate = legal_sell_count / total_legal_steps if total_legal_steps > 0 else 0.0
    ...
    results = {
        ...
        "acceptance": {
            ...
            "overall_pass": (
                sharpe > 0.5 and
                legal_sell_rate >= 0.15 and
                max_dd < 30.0 and
                regime_metrics["min_regime_sharpe"] > 0.0
            ),
        },
    }
    ...
    print(f"  Sharpe Ratio: {sharpe:.3f} (target: >0.5) {'✅' if results['acceptance']['sharpe_pass'] else '❌'}")
    ...
    print("Risk Metrics:")
    print(f"  Max Drawdown: {max_dd:.2f}% (target: <30%) {'✅' if results['acceptance']['mdd_pass'] else '❌'}")
    print(f"  Reward Volatility: {reward_volatility:.4f}")
    print()
    
    print("Action Metrics:")
    print(f"  Legal SELL Rate: {legal_sell_rate:.1%} (target: >=15%) {'✅' if results['acceptance']['sell_pass'] else '❌'}")
    ...
    print(f"  Min Regime Sharpe: {regime_metrics['min_regime_sharpe']:.3f} {'✅' if results['acceptance']['regime_stable'] else '❌'}")
    print()
    ...
    if results["acceptance"]["overall_pass"]:
        ...
        if not results["acceptance"]["regime_stable"]:
            print(f"  - Regime instability detected")
    print()
