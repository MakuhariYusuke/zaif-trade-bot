...
class SACv426Evaluator:
        ...
        for stage in ['cost_aware', 'strong_penalty', 'correlation_focused']:
            ...
            if model_path.exists():
                try:
                    ...
                    logger.info(f"モデル読み込み成功: {stage} (ステップ: {checkpoint.get('step', 0)})")
    ...
    def evaluate_model_on_data(self, model: torch.nn.Module, df: pd.DataFrame,
        ...
        feature_cols = ['close', 'volume', 'returns', 'volatility',
                       'price_position_corr', 'action_price_corr',
                       'regime_alignment', 'market_correlation_score']
        ...
        with torch.no_grad():
            for i in range(0, len(features), batch_size):
                ...
                all_correlations.extend(df.iloc[i:i+batch_size]['market_correlation_score'].values)
        ...
        # 評価指標計算
        evaluation_results = {
            'mean_action': float(np.nanmean(actions)) if not np.all(np.isnan(actions)) else 0.0,
            'action_std': float(np.nanstd(actions)) if not np.all(np.isnan(actions)) else 0.0,
            'sell_bias': float(np.nanmean(actions < 0)) if not np.all(np.isnan(actions)) else 0.5,
            'correlation_with_market': float(np.corrcoef(actions.flatten(), correlations)[0, 1]) if not np.all(np.isnan(actions)) else 0.0,
            'action_volatility': float(np.nanstd(actions)) if not np.all(np.isnan(actions)) else 0.0,
            'stage': stage
        }
    ...
    def compare_with_v424_baseline(self, results: Dict, stage: str) -> None:
        ...
        logger.info(f"- SELLバイアス: v424={v424_sell_bias:.3f} → v426={v426_sell_bias:.3f} "
        ...
        logger.info(f"- 相関係数: v424={v424_corr:.3f} → v426={v426_corr:.3f} "
        ...
        logger.info(f"- 目標達成: 相関係数 {correlation_target}以上={corr_achieved}, "
    ...
    def create_evaluation_summary(self, evaluation_results: Dict[str, Dict]) -> Dict:
        ...
        summary = {
            'best_model': list(evaluation_results.keys())[0] if evaluation_results else None,  # 最初のステージをデフォルト
        ...
        logger.info(f"評価サマリー作成完了: ステータス={summary['overall_assessment']['status']}")
        return summary
    ...
    def generate_evaluation_report(self, evaluation_results: Dict,
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- **評価日時**: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            ...
            f.write(f"| 相関係数 | {v424_corr:.3f} | {v426_corr:.3f} | {(v426_corr-v424_corr):+.3f} |\n")

            v424_sell = self.v424_baseline['sell_bias']
            # ベストモデルのSELLバイアスを取得
            best_stage = summary['best_model']
            v426_sell = evaluation_results[best_stage]['sell_bias']
            f.write(f"| SELLバイアス | {v424_sell:.3f} | {v426_sell:.3f} | {(v424_sell-v426_sell):+.3f} |\n")
            ...
            for stage, results in evaluation_results.items():
                ...
                f.write(f"- **行動ボラティリティ**: {results['action_volatility']:.4f}\n\n")

                # レジーム別結果
                if 'regime_analysis' in results:
                    f.write("**レジーム別分析**:\n")
                    for regime, r_results in results['regime_analysis'].items():
                        f.write(f"- {regime}: 平均行動={r_results['mean_action']:.3f}, "
            ...
            f.write("**目標達成状況**:\n")
            f.write(f"- 相関係数 0.1以上: {'✓' if assessment['correlation_achieved'] else '✗'}\n")
            f.write(f"- SELLバイアス改善: {'✓' if assessment['sell_bias_improved'] else '✗'}\n\n")
            ...
            if assessment['status'] == 'SUCCESS':
                f.write("SAC v426はSAC v424の主要弱点を克服し、実用レベルの性能を達成しました。")
            else:
                f.write("SAC v426は一定の改善を示しましたが、さらなる最適化が必要です。")
