...
class EnsembleAggregator:
    ...
    def calibrate_weights(
        ...
        print(f"   - Disqualification threshold (all-masked): {self.disqualification_threshold:.1%}")
        ...
        for i, (sharpe, masked_rate) in enumerate(zip(model_performances, model_masked_counts)):
            model_name = self.model_paths[i].parent.name
            
            # 失格条件1: Sharpeが閾値以下
            if sharpe < self.min_sharpe_threshold:
                self.disqualified_models.append(i)
                print(f"  ⚠️  Model {i+1} ({model_name}) DISQUALIFIED: Sharpe {sharpe:.4f} < {self.min_sharpe_threshold}")
            
            # 失格条件2: all-masked多発
            elif masked_rate >= self.disqualification_threshold:
                self.disqualified_models.append(i)
                print(f"  ⚠️  Model {i+1} ({model_name}) DISQUALIFIED: All-masked {masked_rate:.1%} >= {self.disqualification_threshold:.1%}")
        ...
        if total > 0:
            ...
            # 全て失格の場合は均等重み（ただし警告）
            print("⚠️  WARNING: All models disqualified or zero weight, using equal weights")
    ...
    def predict(
        ...
        if self.method == "majority_vote":
            final_action, final_probs, final_confidence = self._majority_vote(individual_predictions)
        
        elif self.method == "confidence_weighted":
            final_action, final_probs, final_confidence = self._confidence_weighted(individual_predictions)
        
        elif self.method == "soft_voting":
            final_action, final_probs, final_confidence = self._soft_voting(individual_predictions)
...
def evaluate_ensemble(
    ...
    print(f"   Action distribution: BUY={action_distribution[0]:.2%}, HOLD={action_distribution[1]:.2%}, SELL={action_distribution[2]:.2%}")
    print()
