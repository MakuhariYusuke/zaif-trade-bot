...
class YahooFinanceDataPipeline:
    ...
    def __init__(self, data_dir: str = "data", cache_dir: str = "data/cache"):
        ...
        # BTC/JPYのティッカーシンボル
        self.symbol = "BTC-JPY"
        self.cryptocompare_api_key = os.getenv("CRYPTOCOMPARE_API_KEY")  # 環境変数から取得
    ...
    def fetch_historical_data(self,
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(f"Fetching {symbol} data from {start_date} to {end_date} (attempt {attempt + 1})")
    ...
    def fetch_from_cryptocompare(self,
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(f"Fetching {symbol}/{market} data from CryptoCompare (attempt {attempt + 1})")
                ...
                logger.info(f"Successfully retrieved {len(df)} records from CryptoCompare")
                return df

            except Exception as e:
                logger.error(f"Error fetching from CryptoCompare (attempt {attempt + 1}): {e}")
                if attempt < self.max_retries - 1:
                    time.sleep(self.retry_delay)
                else:
                    raise e
