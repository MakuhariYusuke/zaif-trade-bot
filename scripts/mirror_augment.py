...
def identify_mirror_features(df: pd.DataFrame) -> List[str]:
    ...
    for col in df.columns:
        if any(pattern in col.lower() for pattern in ["return", "roc", "momentum", "trend"]):
...
def main():
    parser = argparse.ArgumentParser(description="Mirror augmentation for SELL bias mitigation")
    parser.add_argument(
        "--input",
        type=Path,
        required=True,
    ...
    if "action" in df.columns:
        ...
        print(f"  Action distribution (before):")
    ...
    print(f"  Augmented size: {len(df_augmented)} rows (+{len(df_augmented) - len(df)})")
    ...
    if "action" in df_augmented.columns:
        ...
        print(f"  Action distribution (after):")
