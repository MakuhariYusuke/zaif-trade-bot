...
class SACTrainer:
    ...
    def load_reward_system(self) -> Dict:
        ...
        if not self.config_path.exists():
            raise FileNotFoundError(f"報酬システム設定ファイルが見つかりません: {self.config_path}")
    ...
    def load_training_data(self) -> pd.DataFrame:
        ...
        logger.info(f"学習データ読み込み完了: {len(df)} 行, {len(self.feature_cols)} 特徴量")
        return df

    def create_training_dataset(self, df: pd.DataFrame, stage: str = 'correlation_focused') -> TensorDataset:
        """
        学習用データセットを作成
    ...
    def create_sac_networks(self) -> Tuple[nn.Module, nn.Module, nn.Module, nn.Module, nn.Module]:
        """SACのネットワークを作成"""
    ...
    def train_sac_stage(self, stage: str, max_steps: int = 50000) -> Dict[str, List]:
        ...
        dataloader = DataLoader(dataset, batch_size=self.sac_config['batch_size'], shuffle=True)

        # ネットワーク作成
        actor, critic1, critic2, target_critic1, target_critic2 = self.create_sac_networks()

        # オプティマイザ
        actor_optimizer = Adam(actor.parameters(), lr=self.sac_config['learning_rate'])
        critic1_optimizer = Adam(critic1.parameters(), lr=self.sac_config['learning_rate'])
        critic2_optimizer = Adam(critic2.parameters(), lr=self.sac_config['learning_rate'])
        ...
        for epoch in range(max_steps // len(dataloader) + 1):
            for batch in dataloader:
                ...
                with torch.no_grad():
                    ...
                    target_q = rewards + (1 - dones) * self.sac_config['gamma'] * target_q
                ...
                actor_loss = (self.sac_config['alpha'] * actions_log_std - actor_q).mean()
                ...
                if step % self.sac_config['target_update_interval'] == 0:
                    for target_param, param in zip(target_critic1.parameters(), critic1.parameters()):
                        target_param.data.copy_(
                            self.sac_config['tau'] * param.data + (1 - self.sac_config['tau']) * target_param.data
                        )
                ...
                # 相関スコア計算（簡易版）
                correlation_score = states[:, -1].mean().item()  # market_correlation_score
                history['correlation_scores'].append(correlation_score)

                step += 1

                if step % 1000 == 0:
                    logger.info(f"ステップ {step}: Actor Loss={actor_loss.item():.4f}, "
    ...
    def save_training_report(self, training_history: Dict[str, Dict]) -> None:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            v426_final_corr = training_history['correlation_focused']['correlation_scores'][-1]
            f.write(f"| 相関係数 | {v424_correlation} | {v426_final_corr:.4f} | {(v426_final_corr-v424_correlation):.4f} |\n")
