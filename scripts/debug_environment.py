...
def debug_environment():
    ...
    for action_name, action_value in test_actions:
        ...
        print(f"{action_name:15}: {initial_position:.4f} -> {final_position:.4f} | "
    ...
    reward_test_cases = [
        {"pnl": 100.0, "position": 0.01, "old_position": 0.0, "action": 1, "desc": "BUY with profit"},
        {"pnl": 100.0, "position": 0.0, "old_position": 0.01, "action": 2, "desc": "SELL with profit"},
        {"pnl": 0.0, "position": 0.01, "old_position": 0.01, "action": 0, "desc": "HOLD neutral"},
        {"pnl": -50.0, "position": 0.01, "old_position": 0.0, "action": 1, "desc": "BUY with loss"},
        {"pnl": -50.0, "position": 0.0, "old_position": 0.01, "action": 2, "desc": "SELL with loss"},
    ]
    ...
    print(f"Random action rewards - Mean: {np.mean(rewards):+.6f}, Std: {np.std(rewards):.6f}")
    ...
    hold_actions = random_actions[(random_actions >= sell_threshold) & (random_actions <= buy_threshold)]
