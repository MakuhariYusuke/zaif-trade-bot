...
def test_balance_penalty_in_real_env():
    ...
    logger.info(
        f"Raw config balance_penalty: {behavior_opt.get('balance_penalty', 'NOT FOUND')}"
    )
    ...
    if config.reward_settings:
        logger.info(
            f"Environment config balance_penalty: {config.reward_settings.balance_penalty}"
        )
    ...
    if len(reward_calc._recent_actions) > 0:
        ...
        logger.info(
            f"Ratios: BUY={buy_ratio:.2%}, SELL={sell_ratio:.2%}, HOLD={hold_ratio:.2%}"
        )
        ...
        scale = config.reward_settings.behavior_optimization.get(
        ...
        logger.info(
            f"  Deviations: buy={dev_buy:.4f}, sell={dev_sell:.4f}, hold={dev_hold:.4f}"
        )
