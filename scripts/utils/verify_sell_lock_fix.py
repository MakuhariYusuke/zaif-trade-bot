...
def test_sell_lock_fix():
    ...
    if positions_list:
        ...
        print(
            f"  LONG:  {long_count} steps ({long_count/len(positions_list)*100:.1f}%)"
        )
        print(
            f"  FLAT:  {flat_count} steps ({flat_count/len(positions_list)*100:.1f}%)"
        )
        print(
            f"  SHORT: {short_count} steps ({short_count/len(positions_list)*100:.1f}%)"
        )
