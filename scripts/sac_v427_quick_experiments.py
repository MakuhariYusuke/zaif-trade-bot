...
def run_experiment(
    ...
    reward_scale=None,
    trading_bonus=None,
    sell_action_penalty=None,
    buy_action_penalty=None,
    action_balance_weight=None,
    optimize_reward=False,
):
    """実験実行"""
    print(f"\n{'='*50}")
    print(f"実験: {experiment_name} (SAC {version})")
    print(f"ステップ数: {timesteps}")
    if reward_scale:
        print(f"報酬スケール: {reward_scale}")
    if trading_bonus:
        print(f"取引ボーナス: {trading_bonus}")
    if sell_action_penalty:
        print(f"SELLペナルティ: {sell_action_penalty}")
    if buy_action_penalty:
        print(f"BUYボーナス: {buy_action_penalty}")
    if version == "v429" and action_balance_weight is not None:
        print(f"アクション平衡ウェイト: {action_balance_weight}")
    if optimize_reward:
        print("報酬最適化: 有効")
    print("=" * 50)
    ...
    cmd = [
        "python",
        "scripts/sac_v427_training_executor.py",
        "--version",
        version,
        "--phase",
        "1",
        ...
        str(timesteps),
        "--output-dir",
        f"results/experiments/{experiment_name}",
    ]

    if reward_scale is not None:
        cmd.extend(["--reward-scale", str(reward_scale)])

    if trading_bonus is not None:
        cmd.extend(["--trading-bonus", str(trading_bonus)])

    if sell_action_penalty is not None:
        cmd.extend(["--sell-action-penalty", str(sell_action_penalty)])

    if buy_action_penalty is not None:
        cmd.extend(["--buy-action-penalty", str(buy_action_penalty)])

    if version == "v429" and action_balance_weight is not None:
        cmd.extend(["--action-balance-weight", str(action_balance_weight)])

    if optimize_reward:
        cmd.extend(["--optimize-reward"])
    ...
    if result.returncode == 0:
        print("✅ 成功")
        # 結果ファイルを探す
        result_dir = Path(f"results/experiments/{experiment_name}")
                ...
                with open(file, "r", encoding="utf-8") as f:
                    ...
                    if "phase_1" in data:
                        ...
                        if phase_data.get("success"):
                            print("学習結果:")
                            print(
                                f"  - ステップ数: {phase_data.get('total_timesteps', 'N/A')}"
                            )
                            print(
                                f"  - 実行時間: {phase_data.get('training_time', 'N/A'):.1f}秒"
                            )
                            print("  - 成功: ✅")
    else:
        print("❌ 失敗")
        print("エラー出力:")
        print(result.stderr[-500:])  # 最後500文字
