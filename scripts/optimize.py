...
class OptunaCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.n_calls % self.eval_freq == 0:
            # 現在の報酬を取得
            # VecEnvの場合、get_attrを使用
            reward_histories = None
            if (
                hasattr(self, 'training_env') and
                self.training_env is not None and
                hasattr(self.training_env, 'get_attr')
            ):
            ...
            if reward_histories and reward_histories[0]:
                ...
                current_reward = sum(reward_histories[0][-100:])  # 直近100ステップの報酬
...
class HyperparameterOptimizer:
    ...
    def __init__(self, data_path: str, config: dict | None = None):
    ...
    def _sample_ppo_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        """PPOハイパーパラメータのサンプリング"""
        # ネットワークアーキテクチャを文字列としてサンプリング
        net_arch_str = trial.suggest_categorical('net_arch', [
    ...
    def _create_ppo_model(self, params: Dict[str, Any], env: HeavyTradingEnv) -> Tuple[PPO, HeavyTradingEnv]:
        """PPOモデルの作成"""
        # 活性化関数の変換
    ...
    def _evaluate_trial(self, model: PPO, env: HeavyTradingEnv, n_episodes: int = 5) -> Dict[str, float]:
        """トライアルの評価"""
        episode_rewards = []
        episode_pnls = []
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        # コールバックの設定
        optuna_callback = OptunaCallback(trial, eval_freq=self.config.get('eval_freq', 2000))
    ...
    def get_best_model(self, study: optuna.Study) -> Tuple[PPO, HeavyTradingEnv]:
        """最適パラメータでモデルを作成"""
    ...
    def retrain_best_model(self, study: optuna.Study, total_timesteps: int = 200000) -> PPO:
        ...
        model, _ = self.get_best_model(study)
    ...
    def analyze_optimization_results(self, study: optuna.Study) -> Dict[str, Any]:
        ...
        for param_name in study.best_params.keys():
            param_values_any = [t.params[param_name] for t in completed_trials if param_name in t.params]
            # Filter out non-numeric types for statistical analysis
            param_values = [v for v in param_values_any if isinstance(v, (int, float))]
            if param_values:
