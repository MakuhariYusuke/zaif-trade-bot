...
def test_slack_alert():
    ...
    # Create alert config
    config = AlertConfig(
        smtp_server=None,  # Disable email for this test
...
def test_alert_priority_filtering():
    ...
    alerts = [
        ...
        HealthAlert("Critical Priority", "Critical test", AlertPriority.CRITICAL, "test")
    ]
...
def main():
    ...
    if os.getenv("EMAIL_USERNAME") and os.getenv("EMAIL_PASSWORD"):
        ...
        print("⏭️  Skipping email test (no EMAIL_USERNAME/EMAIL_PASSWORD environment variables)")
    print()
    ...
    print("Enable 2FA on your Google account and generate an App Password in security settings.")
