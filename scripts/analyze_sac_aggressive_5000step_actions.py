#!/usr/bin/env python3
"""
Analyze action distribution from aggressive exploration 5000-step trained SAC model
積極的探索版5000ステップ学習済みSACモデルのアクション分布分析
"""
...
def analyze_aggressive_action_distribution(model_path, n_episodes=10):
    """Analyze action distribution from aggressive trained model"""
    logger.info(f"Analyzing action distribution from aggressive model: {model_path}")
    ...
    # Analyze action distribution with aggressive thresholds
    analysis = {
        ...
        "action_distribution": {
            "buy_signals": int(np.sum(actions > 0.02)),  # Very low threshold
            "sell_signals": int(np.sum(actions < -0.02)),  # Very low threshold
            "hold_signals": int(np.sum((actions >= -0.02) & (actions <= 0.02))),
...
def main():
    ...
    analysis, actions, rewards, obs = analyze_aggressive_action_distribution(model_path)

    # Save analysis results
    analysis_path = "analysis/sac_aggressive_5000step_action_analysis.json"
    ...
    # Print summary
    print("\n" + "="*60)
    print("AGGRESSIVE EXPLORATION SAC 5000-STEP ACTION DISTRIBUTION ANALYSIS")
    print("="*60)
    ...
    print("Action Distribution (thresholds: ±0.02):")
    print(f"  Buy Signals (>0.02): {analysis['action_distribution']['buy_signals']}")
    print(f"  Sell Signals (<-0.02): {analysis['action_distribution']['sell_signals']}")
    print(f"  Hold Signals (-0.02 to 0.02): {analysis['action_distribution']['hold_signals']}")
    ...
    # Issue detection with aggressive thresholds
    issues = []
    if analysis['bias_analysis']['sell_ratio'] > 0.8:
        issues.append("極端なSELLバイアス: SELLシグナルの割合が80%以上")
    if analysis['bias_analysis']['buy_ratio'] < 0.05:
        issues.append("極端なBUY不足: BUYシグナルの割合が5%未満")
    if analysis['action_stats']['std'] < 0.02:
        issues.append("極端な探索不足: アクションの分散が小さい（<0.02）")
    if analysis['reward_stats']['mean'] < -0.002:
        issues.append("極端な低パフォーマンス: 平均報酬が-0.002未満")
    if analysis['action_distribution']['hold_signals'] > analysis['total_actions'] * 0.98:
        issues.append("極端な保守性: HOLDシグナルが98%以上")

    # Success criteria
    success_criteria = []
    if analysis['action_distribution']['buy_signals'] > 0:
        success_criteria.append("BUYアクションが発生")
    if analysis['action_distribution']['sell_signals'] > 0:
        success_criteria.append("SELLアクションが発生")
    if analysis['action_stats']['std'] > 0.05:
        success_criteria.append("適切なアクション分散（>0.05）")
    if analysis['action_distribution']['hold_signals'] < analysis['total_actions'] * 0.9:
        success_criteria.append("HOLDシグナルが90%未満")
