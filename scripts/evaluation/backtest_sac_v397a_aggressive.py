#!/usr/bin/env python3
"""
Backtest Script for SAC v397a_aggressive Model.

Tests the aggressive trading model with:
- 100% position size
- Lower action threshold (0.15 vs 0.33)
...
def run_backtest(
    ...
    """Run backtest simulation with SAC v397a_aggressive model."""
    logging.basicConfig(level=logging.INFO if verbose else logging.WARNING)
    logger = logging.getLogger(__name__)

    # Load model
    logger.info(f"Loading SAC v397a_aggressive model from {model_path}")
    try:
    ...
    # Create environment with SAC v397a_aggressive configuration
    ...
    config = {
        ...
        "continuous_to_discrete_threshold": 0.15,  # Lower threshold!
        "reward_settings": {
            ...
            "inactivity_penalty_rate": 0.015,  # 15x stronger
            "enable_opportunity_cost": True,
            "opportunity_cost_rate": 0.003,  # 6x stronger
        },
    }
    ...
    logger.info(f"Config: position_size=100%, threshold=0.15, inactivity_penalty=0.015")
    obs, _ = env.reset()
    done = False
    trades = []
    last_position = 0
    ...
    while not done:
        ...
        discrete_action = continuous_to_discrete_action(continuous_action_value, threshold=0.15)
        ...
        # Note: Using threshold of 0.001 to detect small positions
    ...
    return {
        ...
        "trades": trades,
        "action_distribution": {
            "HOLD": discrete_dist.get(0, 0),
            "BUY": discrete_dist.get(1, 0),
            "SELL": discrete_dist.get(2, 0),
            "HOLD_pct": discrete_dist.get(0, 0)/len(discrete_actions_taken)*100,
            "BUY_pct": discrete_dist.get(1, 0)/len(discrete_actions_taken)*100,
            "SELL_pct": discrete_dist.get(2, 0)/len(discrete_actions_taken)*100,
        },
        "config": {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run backtest for SAC v397a_aggressive model")
    parser.add_argument(
        "--model-path",
    ...
    parser.add_argument(
        "--output",
        default="backtest_sac_v397a_aggressive_results.json",
    ...
    # Print results
    print("\n" + "="*60)
    print("=== SAC v397a_aggressive Backtest Results ===")
    print("="*60)
    ...
    for key, value in action_dist.items():
        if isinstance(value, float):
