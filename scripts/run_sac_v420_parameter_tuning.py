...
class SACParameterTuner:
    ...
    def __init__(self):
        ...
        self.parameter_configs = {
            "baseline": [
                "sac_v420_baseline_1k.json",
                "sac_v420_baseline_5k.json"
            ],
            "learning_rate": [
    ...
    def run_single_config(self, config_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [sys.executable, "-m", "ztb.training.unified_trainer.main", "--config", str(config_path)]
            ...
            if result.returncode == 0:
                ...
                print(f"❌ Training failed after {duration:.2f} seconds: {result.stderr}")
                return {
                    "status": "failed",
                    "config_file": config_file,
                    "duration": duration,
    ...
    def save_results_summary(self, results: Dict[str, List[Dict[str, Any]]]) -> None:
        ...
        summary = {
            "timestamp": time.time(),
            "total_configs_tested": sum(len(group_results) for group_results in results.values()),
            "successful_runs": sum(
                1 for group_results in results.values()
            ...
            "total_duration": sum(
                result.get("duration", 0) for group_results in results.values()
                for result in group_results
            ),
            "results_by_group": {}
        }
        ...
        for group_name, group_results in results.items():
            group_summary = {
                ...
                "avg_duration": sum(r.get("duration", 0) for r in group_results) / len(group_results),
