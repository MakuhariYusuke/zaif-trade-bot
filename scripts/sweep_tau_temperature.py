...
def main():
    ...
    print(f"Total experiments: {total_experiments} ({len(tau_values)} tau × {len(temp_values)} temp × {len(datasets)} datasets)")
    print()
    ...
    for i, (tau, temp) in enumerate(product(tau_values, temp_values), 1):
        ...
        for data_name, data in datasets:
            ...
            print(f"✓ (legal_sell={result['legal_sell_rate']:.1%}, sharpe={result['sharpe_proxy']:.3f})")
    ...
    print(f"  Legal SELL rate: {df_results['legal_sell_rate'].mean():.1%} ± {df_results['legal_sell_rate'].std():.1%}")
    print(f"  Sharpe proxy: {df_results['sharpe_proxy'].mean():.3f} ± {df_results['sharpe_proxy'].std():.3f}")
    print(f"  Trades: {df_results['trades'].mean():.1f} ± {df_results['trades'].std():.1f}")
    print()
    ...
    for idx, row in top3.iterrows():
        print(f"  {idx+1}. tau={row['tau']}, T={row['temperature']}, {row['data_name']}")
        print(f"     Sharpe={row['sharpe_proxy']:.3f}, legal_sell={row['legal_sell_rate']:.1%}, trades={row['trades']}")
