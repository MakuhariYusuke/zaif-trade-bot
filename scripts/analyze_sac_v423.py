...
from ztb.analysis import BacktestAnalyzer
...
class SACv423Analyzer:
        ...
        if result_path.suffix == '.json' and 'training_report' in result_path.name:
            try:
                ...
                metrics = {
                    ...
                    'best_episode_reward': training_stats.get('final_reward', 0),  # Use final_reward as best
                    'training_time_seconds': training_stats.get('training_time', 0),
                    'action_distribution': training_stats.get('action_distribution', {}),
    ...
    def load_model_config(self, result_path: Path) -> Optional[Dict[str, Any]]:
        ...
        if result_path.suffix == '.json' and 'training_report' in result_path.name:
            try:
                ...
                logger.warning(f"Failed to load config from report file {result_path}: {e}")
                return None
    ...
    def analyze_training_results(self) -> None:
        ...
        result_path = self.find_latest_training_results()
        if not result_path:
        ...
        print(f"   Final Episode Reward: {metrics.get('final_episode_reward', 'N/A')}")
        print(f"   Best Episode Reward: {metrics.get('best_episode_reward', 'N/A')}")
        print(f"   Training Time: {metrics.get('training_time_seconds', 'N/A')} seconds")
        ...
        if 'episode_rewards' in metrics:
            ...
            self.plot_reward_progression(rewards, result_path.parent if result_path.suffix == '.json' else result_path)
