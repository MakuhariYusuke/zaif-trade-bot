...
class GateLogger:
    def __init__(self, filepath) -> None:
        ...
        self.headers = [
            "step", "timestamp", "action", "threshold", "env_threshold", "regime",
            "gate_status", "gate_should_enter", "gate_ev", "gate_cost",
            "p_win_lcb", "p_win_mean", "p_win_ucb", "avg_win", "avg_loss", "n_eff", "atr", "atr_env",
            "block_reason", "gate_bin"
        ]
        self.buffer = []
...
class BacktestReporter:
    ...
    def update_step(self, step, portfolio_value, action, regime, gate_status) -> None:
        ...
        # Action Dist
        act_key = "hold"
        if action > 0: act_key = "buy"
        elif action < 0: act_key = "sell"
        self.stats["action_distribution"][act_key] = self.stats["action_distribution"].get(act_key, 0) + 1

        # Regime Dist
        self.stats["regime_distribution"][regime] = self.stats["regime_distribution"].get(regime, 0) + 1
    ...
    def finalize_stats(self) -> None:
        ...
        if self.stats["gross_loss"] > 0:
            self.stats["profit_factor"] = self.stats["gross_profit"] / self.stats["gross_loss"]
        else:
            self.stats["profit_factor"] = float('inf') if self.stats["gross_profit"] > 0 else 0.0
        ...
        if duration > 0:
            self.stats["performance"]["steps_per_second"] = self.stats["total_steps"] / duration
        ...
        if psutil:
            try:
                self.stats["system_info"]["memory_total"] = psutil.virtual_memory().total
                self.stats["system_info"]["memory_available"] = psutil.virtual_memory().available
            except:
                pass
    ...
    def print_summary(self) -> None:
        ...
        print(f"Win Rate: {self.stats['winning_trades'] / max(1, self.stats['total_trades']):.2%}")
        ...
        print(f"Max Drawdown: {self.stats['max_drawdown']:.2f} ({self.stats['max_drawdown_percent']:.2%})")
...
def run_backtest_v455() -> None:
    ...
    parser.add_argument("--warmup-steps", type=int, default=2000, help="Steps to run without Gate blocking to gather stats")
    ...
    def log_gate_decision(step, ts, action, thresh, env_thresh, regime, status, res, atr, atr_env):
        stats = res.get("stats", {})
        ...
        if status == "blocked":
            if not math.isfinite(res.get("cost", 0)): reason = "cost_inf"
            elif res.get("cost", 0) > res.get("ev", 0): reason = "cost_gt_ev"
            elif stats.get("n_eff", 0) < v455_config["n_min"]: reason = "n_eff_low"
            else:
                # Check probability based on mode
                p_val = stats.get(f"p_win_{prob_mode}", 0.0)
                if p_val <= 0.5: reason = f"{prob_mode}_low"
                else: reason = "unknown"
        elif status == "forced":
    ...
    while not done:
        ...
        if step_count % 1000 == 0:
           print(f"Step {step_count}: Action={raw_action:.3f}, Thresh={threshold:.3f}, NegThresh={negative_threshold:.3f}")
        ...
        if shadow_btc == 0.0:
            if is_buy_signal:
                ...
                if not should_enter:
                    ...
                    gate_status = "blocked"
                    if step_count % 1000 == 0:
                        print(f"DEBUG: Gate Blocked Buy. Step={step_count}, Reason={gate_res.get('reason', 'unknown')}")
                        if not math.isfinite(cost):
                             print(f"DEBUG: Cost Inf! MarketData={market_data}")
                        if step_count < args.warmup_steps:
                             print(f"DEBUG: Warmup Blocked! Cost={cost}, MarketData={market_data}")
                    ...
                    if raw_action < strict_threshold:
                         ...
                         action_to_env = np.array([strict_threshold + 1e-4], dtype=np.float32)

                    last_entry_action = raw_action
                    last_entry_regime = regime
                    last_entry_threshold = threshold
                    print(f"DEBUG: Gate Allowed Buy. Step={step_count}, Action={raw_action:.3f}, Cost={cost:.1f}")

                # LOGGING
                log_gate_decision(
                    step_count, market_data["timestamp"], raw_action, threshold, strict_threshold, regime,
                    gate_status, gate_res, market_data["atr"], market_data["atr"]
                )
                ...
                if not should_enter:
                    ...
                    gate_status = "blocked"
                    if step_count % 1000 == 0:
                        print(f"DEBUG: Gate Blocked Sell. Step={step_count}, Reason={gate_res.get('reason', 'unknown')}")
                        if not math.isfinite(cost):
                             print(f"DEBUG: Cost Inf! MarketData={market_data}")
                    action_to_env = np.array([0.0], dtype=np.float32)
                else:
                    ...
                    if raw_action > strict_negative_threshold:
                         action_to_env = np.array([strict_negative_threshold - 1e-4], dtype=np.float32)

                    last_entry_action = raw_action
                    last_entry_regime = regime
                    last_entry_threshold = negative_threshold
                    print(f"DEBUG: Gate Allowed Sell. Step={step_count}, Action={raw_action:.3f}, Cost={cost:.1f}")

                # LOGGING
                log_gate_decision(
                    step_count, market_data["timestamp"], raw_action, negative_threshold, strict_negative_threshold, regime,
                    gate_status, gate_res, market_data["atr"], market_data["atr"]
                )
        ...
        # DEBUG
        if discrete_act != 0:
           print(f"DEBUG: Shadow Logic sees Action {discrete_act}. ShadowBTC={shadow_btc}")
        ...
        # LONG EXIT
            ...
            gross_pnl = (exec_res.executed_price - last_entry_market_price) * requested_size
            ...
            entry_system.update_outcome(
                last_entry_regime, last_entry_action, market_data["close"] - last_entry_market_price, current_step,
        ...
        # SHORT EXIT (COVER)
            ...
            # Calculate PnL
            gross_pnl = (last_entry_market_price - market_data["close"]) * requested_size
            ...
            # Update Calibration Map
            entry_system.update_outcome(
                last_entry_regime, last_entry_action, last_entry_market_price - market_data["close"], current_step,
                threshold=last_entry_threshold
            )
    ...
    metrics = {
        "total_return": (shadow_history[-1] / shadow_history[0]) - 1 if shadow_history else 0,
        ...
        "win_rate": reporter.stats["winning_trades"] / max(1, reporter.stats["total_trades"]),
    ...
    metrics.update({f"action_{k}": v for k, v in reporter.stats["action_distribution"].items()})
    metrics.update({f"regime_{k}": v for k, v in reporter.stats["regime_distribution"].items()})
