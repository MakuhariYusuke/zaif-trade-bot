...
def test_harmonic_patterns():
    ...
    for pattern_name, pattern_class in patterns:
        try:
            # Create mock config
            config = {"lookback_period": 60, "tolerance": 0.05}
            ...
            signal = recognizer.recognize(
                data, index=-1, multi_timeframe_data=multi_timeframe_data
            )
            ...
            results[pattern_name] = {
                ...
                "has_mtf": "mtf" in (signal.metadata.get("mtf_confidence", 1.0) != 1.0)
                if signal
                else False,
            }
            ...
            if signal:
                ...
                print(
                    f"   Type: {signal.signal_type}, Strength: {signal.strength:.6f}, Direction: {signal.direction}, MTF: {mtf_conf:.3f}"
                )
