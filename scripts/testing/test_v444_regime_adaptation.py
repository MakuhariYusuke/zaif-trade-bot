...
def test_regime_classification():
    ...
    # Create classifier
    config = {"dynamic_thresholds": True, "adaptation_window": 50}
    ...
    for idx in test_indices:
        ...
        print(
            f"Index {idx}: {result.primary_regime.value} (confidence: {result.confidence:.3f})"
        )
        ...
        weights = classifier.get_adaptive_feature_weights(
            result.primary_regime, feature_names
        )
...
def test_dynamic_thresholds():
    ...
    classifier = V444RegimeClassifier(
        {"dynamic_thresholds": True, "adaptation_window": 20}
    )
...
def test_multi_timeframe():
    ...
    try:
        ...
        print(
            f"Multi-timeframe integrated regime: {mtf_result.integrated_regime.value}"
        )
