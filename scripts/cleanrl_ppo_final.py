...
def train_cleanrl_ppo():
    ...
    # Storage setup
    obs = torch.zeros((num_steps, num_envs) + env.observation_space.shape).to(torch.float32)
    actions = torch.zeros((num_steps, num_envs) + env.action_space.shape).to(torch.float32)
    ...
    next_obs = env.reset()
    next_obs = torch.tensor(next_obs[0] if isinstance(next_obs, tuple) else next_obs, dtype=torch.float32)
...
def test_cleanrl_ppo(agent):
    ...
    # Success criteria
    balance_ratio = min(buy_count, sell_count) / max(buy_count, sell_count) if max(buy_count, sell_count) > 0 else 0
...
def main():
    ...
    print("This indicates a fundamental issue with the RL implementation or environment.")
