...
def validate_emergency_config(config: dict) -> dict:
    ...
    scaling = config["training"]["environment"]["reward_settings"]["asymmetric_reward_scaling"]
    ...
    targets = config["training"]["environment"]["reward_settings"]["balance_penalty_targets"]
    ...
    if targets != expected:
        ...
        print(f"✅ Balance targets = 47.5/47.5/5.0 (correct)")
    
    # Forced balance min_actions check
    min_actions = config["training"]["environment"]["reward_settings"]["forced_balance_min_actions"]
    if min_actions < 100:
        issues.append(f"❌ forced_balance_min_actions too low: {min_actions} (should be ≥100)")
    else:
        print(f"✅ forced_balance_min_actions = {min_actions} (correct)")
    
    # Forced balance threshold check
    threshold = config["training"]["environment"]["reward_settings"]["forced_balance_threshold"]
    if threshold > 0.10:
        warnings.append(f"⚠️ forced_balance_threshold high: {threshold} (recommend ≤0.08)")
    else:
    ...
    if emergency_penalty < 300:
        warnings.append(f"⚠️ emergency_penalty low: {emergency_penalty} (recommend ≥300)")
    else:
    ...
    # MTF weights check
    mtf_weights = config["training"]["environment"]["multi_timeframe"]["feature_weights"]
...
def run_quick_test(config_path: Path, timesteps: int = 1000):
    """簡易テスト実行"""
    print(f"\n{'='*60}")
    print(f"SAC v448 Emergency Fix - Quick Validation")
    print(f"{'='*60}\n")
    ...
    print(f"\nCommand to run:")
    print(f"  python scripts/unified_trainer.py \\")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Training timesteps (default: 1000)"
    )
    parser.add_argument(
        "--config",
        type=str,
