...
def run_paper_trading(
    ...
    for episode in range(num_episodes):
        ...
        episode_result = {
            ...
            'initial_portfolio_value': episode_portfolio_values[0] if episode_portfolio_values else 0,
            'steps': step,
            'action_distribution': {
                'HOLD': episode_actions.count(ACTION_HOLD) / len(episode_actions) if episode_actions else 0,
                'BUY': episode_actions.count(ACTION_BUY) / len(episode_actions) if episode_actions else 0,
                'SELL': episode_actions.count(ACTION_SELL) / len(episode_actions) if episode_actions else 0
            }
        }
        ...
        logger.info(f"Episode {episode + 1} completed: Reward={episode_reward:.2f}, "
    ...
    summary = {
        ...
        'action_distribution_percent': {
            'HOLD': action_counts['HOLD'] / sum(action_counts.values()) * 100 if sum(action_counts.values()) > 0 else 0,
            'BUY': action_counts['BUY'] / sum(action_counts.values()) * 100 if sum(action_counts.values()) > 0 else 0,
            'SELL': action_counts['SELL'] / sum(action_counts.values()) * 100 if sum(action_counts.values()) > 0 else 0
        },
    ...
    logger.info(f"Average Portfolio Value: {summary['avg_portfolio_value']:.2f} ± {summary['std_portfolio_value']:.2f}")
    logger.info(f"Action Distribution: HOLD={summary['action_distribution_percent']['HOLD']:.1f}%, "
...
def main():
    ...
    try:
        ...
        print(f"Average Portfolio Value: {results['avg_portfolio_value']:.2f} ± {results['std_portfolio_value']:.2f}")
        print(f"HOLD: {results['action_distribution_percent']['HOLD']:.1f}%, "
