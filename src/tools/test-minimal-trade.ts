...
(async () => {
    ...
    // pxBid: Use override if provided, else best bid if available, else slightly below best ask
    ...
    // pxAsk: Use override if provided, else best ask if available, else slightly above best bid
    ...
    async function place(action: 'bid' | 'ask', price: number) {
        ...
        try {
            ...
            // try to derive filled from history
            let filledQty = 0;
            try {
