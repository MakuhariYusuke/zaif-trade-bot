...
interface PaperOrder { id: string; currency_pair: string; action: 'bid'|'ask'; price: number; amount: number; filled: number; ts: number; status: 'open'|'canceled'|'filled' }
...
class PaperPrivate implements PrivateApi {
  ...
  async get_info2(): Promise<GetInfo2Response> {
    ...
    const st = load();
    return { success: 1, return: { funds: st.funds, rights: { info: true, trade: true }, open_orders: st.orders.filter(o=>o.status==='open').length, server_time: Math.floor(Date.now()/1000) } };
  }
  async active_orders(p?: any): Promise<ActiveOrder[]> {
    await this.maybeDelayAndError();
    const st=load();
    return st.orders.filter(o=>o.status==='open' && (!p?.currency_pair || p.currency_pair===o.currency_pair)).map(o=>({ order_id: o.id, pair: o.currency_pair, side: o.action, price: o.price, amount: o.amount - o.filled, timestamp: Math.floor(o.ts/1000) }));
  }
  async trade_history(p?: any): Promise<TradeHistoryRecord[]> {
    await this.maybeDelayAndError();
    const st=load();
    const list = st.trades.filter(t=>!p?.currency_pair || p.currency_pair===t.currency_pair).slice(-Number(p?.count||100));
  ...
  async trade(p:any): Promise<TradeResult> {
    ...
    const order: PaperOrder = { id, currency_pair: p.currency_pair, action: p.action, price, amount: amt, filled: 0, ts: Date.now(), status: 'open' };
    ...
    if (fillQty>0){
      ...
      const tid = Date.now();
  st.trades.push({ tid, order_id: id, side: order.action, price: order.price, amount: fillQty, timestamp: Math.floor(Date.now()/1000), currency_pair: order.currency_pair });
