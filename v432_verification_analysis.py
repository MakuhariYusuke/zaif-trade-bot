...
def analyze_synthetic_vs_real(results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for version in synthetic_versions:
        if version in results:
            ...
            if trades:
                ...
                hold_count = sum(1 for action in actions if 'HOLD' in str(action).upper())
    ...
    analysis["key_differences"] = {
        ...
        "performance_gap": analysis["synthetic_summary"]["avg_return"] - analysis["real_performance"].get("total_return", 0),
        "overfitting_indicators": []
    }

    if abs(analysis["key_differences"]["win_rate_drop"]) > 10:
        analysis["key_differences"]["overfitting_indicators"].append("Significant win rate drop in real data")

    if analysis["real_performance"].get("num_trades", 0) < 1000:
        analysis["key_differences"]["overfitting_indicators"].append("Lower trade frequency in real data")
...
def analyze_reward_system_evolution(results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    """報酬システムの進化分析"""
    analysis = {
        "reward_evolution": [],
        "key_changes": [],
    ...
    reward_configs = {
        "v432.1": {"hold_penalty": -0.02, "success_bonus": 0.3, "failure_penalty": 0.15},
        "v432.2": {"hold_penalty": -0.02, "success_bonus": 0.3, "failure_penalty": 0.15},
        "v432.3": {"hold_penalty": -0.02, "success_bonus": 0.3, "failure_penalty": 0.15},
        "v432.4": {"hold_penalty": -0.02, "success_bonus": 0.3, "failure_penalty": 0.15},
        "v432.5": {"hold_penalty": -0.02, "success_bonus": 0.3, "failure_penalty": 0.15},
        "v432.6": {"hold_penalty": -0.02, "success_bonus": 0.3, "failure_penalty": 0.15},
        "v432.7": {"hold_penalty": -0.02, "success_bonus": 0.3, "failure_penalty": 0.15},
    }

    for version in ["v432.1", "v432.2", "v432.3", "v432.4", "v432.5", "v432.6", "v432.7"]:
        if version in results:
            data = results[version]
    ...
    analysis["effectiveness_assessment"] = {
        ...
        "best_win_rate_version": ["v432.1", "v432.2", "v432.3", "v432.4", "v432.5", "v432.6", "v432.7"][np.argmax(win_rates)],
        "best_return_version": ["v432.1", "v432.2", "v432.3", "v432.4", "v432.5", "v432.6", "v432.7"][np.argmax(returns)],
...
def analyze_entry_exit_logic(results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # エントリー/出口ロジックの進化
    entry_exit_configs = {
        "v432.3": {"entry": "Basic trend + volume", "exit": "Profit target 3%, Stop loss 2%"},
        "v432.4": {"entry": "Relaxed trend filters", "exit": "Profit target 2%, Stop loss 1.5%"},
        "v432.5": {"entry": "Strict trend filters (min_trend: 0.04-0.05)", "exit": "Profit target 1.5%, Stop loss 1%"},
        "v432.6": {"entry": "Ensemble-based entry", "exit": "Profit target 1%, Stop loss 0.8%"},
...
def generate_v433_recommendations(analysis_results: Dict[str, Any]) -> List[str]:
    ...
    if synthetic_real.get("key_differences", {}).get("win_rate_drop", 0) > 10:
        recommendations.append("🔴 CRITICAL: Use real market data for training and validation")
        recommendations.append("🔴 Implement cross-validation with multiple market regimes")

    # 報酬システムの洞察から
    reward_analysis = analysis_results.get("reward_system", {})
    if reward_analysis.get("effectiveness_assessment", {}).get("win_rate_trend") == "declining":
        recommendations.append("🟡 Review reward function design - current system may be causing suboptimal behavior")
    ...
    # 取引頻度と品質のバランス
    recommendations.append("🟢 Focus on entry quality rather than frequency - implement multi-factor entry scoring")
    recommendations.append("🟢 Develop adaptive exit strategies based on market regime and position holding time")
...
def main():
    ...
    print(f"合成データ平均Win Rate: {syn_real['synthetic_summary']['avg_win_rate']:.1f}%")
    ...
    print(f"最高Win Rateバージョン: {reward['effectiveness_assessment']['best_win_rate_version']}")
