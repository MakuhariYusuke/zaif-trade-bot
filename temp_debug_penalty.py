...
settings={
    ...
    'behavior': {'consistency_penalty': {'enabled': True, 'value': 0.1, 'lookback': 3}}
}
calc = BehavioralPenaltyCalculator(settings)
for a in [ACTION_BUY, ACTION_SELL]:
    calc.record_action(a)
calc.record_action(ACTION_BUY)
print('recent_actions', list(calc.recent_actions))
print('lookback', calc.lookback)
print('len>=lookback', len(calc.recent_actions) >= calc.lookback)
print('last', calc.recent_actions[-1])
...
print('calc.penalty_value', calc.penalty_value)
