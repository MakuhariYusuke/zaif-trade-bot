...
def verify_reward_calculator_fix():
    ...
    if "if curriculum_stage in balance_penalty_enabled_stages:" in content:
        print("✓ Found correct condition: 'if curriculum_stage in balance_penalty_enabled_stages:'")
    else:
        print("❌ Missing or incorrect condition")
        return False
    
    # Check 4: Verify balance_penalty calculation is present
    if "balance_penalty = abs(buy_ratio - sell_ratio) * balance_penalty_scale" in content:
...
def verify_config_files():
    ...
    for config_file in config_files:
        ...
        if stage == "balanced_penalty":
            ...
            print(f"❌ {config_file}: curriculum_stage = '{stage}' (expected 'balanced_penalty')")
            all_correct = False
