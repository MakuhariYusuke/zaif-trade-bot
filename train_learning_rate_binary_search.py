#!/usr/bin/env python3
"""
Binary search for learning_rate parameter
"""
...
def train_learning_rate_test(config_name: str = "default", reward_scaling: float = 6.0, entropy_coef: float = 0.03, learning_rate: float = 5e-4) -> str:
    ...
    model_path = PROJECT_ROOT / "models" / f"lr_test_{config_name}.zip"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=5e-4, help="Learning rate for PPO training")
    ...
    # Create config name from learning rate
    lr_str = f"{args.learning_rate:.0e}".replace("-", "").replace(".", "")
    config_name = f"lr_{lr_str}"

    print(f"\n{'='*60}")
    print(f"Training with learning_rate = {args.learning_rate}")
    print(f"Config: {config_name}")
    print(f"{'='*60}")

    try:
        model_path = train_learning_rate_test(
