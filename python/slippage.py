...
class SlippageAnalysis:
    ...
    def analyze_orderbook_liquidity(self, timestamp: datetime, pair: str = "btc_jpy",
        ...
        spread_score = max(0, 1 - (spread_bps / 100))  # Normalize spread (100bps = 0 score)
        depth_score = min(1.0, (bid_depth + ask_depth) / 10)  # Normalize depth (10 units = 1 score)
    ...
    def analyze_execution_slippage(self, trades_df: pd.DataFrame,
                                 orderbooks_dir: Optional[str] = None) -> pd.DataFrame:
        """
        ...
        for _, trade in trades_df.iterrows():
            ...
            liquidity = self.analyze_orderbook_liquidity(timestamp, trade.get('pair', 'btc_jpy'))
            ...
            size_slippage = (order_size - executed_size) / order_size if order_size > 0 else 0.0
    ...
    def generate_slippage_report(self, slippage_df: pd.DataFrame,
        ...
        report = {
            'analysis_period': {
                'start': slippage_df['timestamp'].min().isoformat() if not slippage_df.empty else None,
                'end': slippage_df['timestamp'].max().isoformat() if not slippage_df.empty else None,
                'total_trades': len(slippage_df),
                'successful_trades': len(successful_trades),
                'success_rate': len(successful_trades) / len(slippage_df) if len(slippage_df) > 0 else 0.0
            },
            'slippage_statistics': {},
            'liquidity_analysis': {},
            'recommendations': []
        ...
        if not successful_trades.empty:
            ...
            report['slippage_statistics'] = {
                'avg_price_slippage_bps': successful_trades['price_slippage_bps'].mean(),
                'median_price_slippage_bps': successful_trades['price_slippage_bps'].median(),
            ...
            report['liquidity_analysis'] = {
                ...
                'liquidity_correlation': successful_trades['price_slippage_bps'].corr(successful_trades['liquidity_score'])
            }

            # Side-specific analysis
            for side in ['buy', 'sell']:
                side_trades = successful_trades[successful_trades['side'].str.lower() == side]
            ...
            if avg_slippage is not None and avg_slippage > 50:  # > 0.5%
                report['recommendations'].append("High average slippage detected. Consider using limit orders or reducing order size.")
            elif avg_slippage is not None and avg_slippage > 20:  # > 0.2%
                report['recommendations'].append("Moderate slippage detected. Monitor market conditions closely.")

            liquidity_corr = report['liquidity_analysis'].get('liquidity_correlation')
            if liquidity_corr is not None and liquidity_corr < -0.3:  # Negative correlation between liquidity and slippage
                report['recommendations'].append("Slippage increases during low liquidity periods. Consider trading during high liquidity hours.")
    ...
    def monitor_real_time_slippage(self, execution_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        result = {
            ...
            'liquidity_score': liquidity['liquidity_score'],
            'spread_bps': liquidity['spread_bps'],
            'alert_level': self._determine_alert_level(price_slippage_bps, liquidity['liquidity_score'])
        }

        return result

    def _determine_alert_level(self, slippage_bps: float, liquidity_score: float) -> str:
        """
        ...
        Returns:
            Alert level string
        """
        if slippage_bps > 100 and liquidity_score < 0.3:  # High slippage + low liquidity
            return "critical"
