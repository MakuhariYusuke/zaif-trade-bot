...
def run_smoke_test(config_path: str = "smoke_test_10k_config.json"):
    ...
    print(
        f"  Target SELL rate (optimal): {validation['target_sell_rate_optimal']*100:.1f}%"
    )
    ...
    try:
        ...
        if "sell_rate" in probe_df.columns:
            ...
            if len(sell_rates) > 0:
                ...
                print(
                    f"  Target (optimal): {validation['target_sell_rate_optimal']*100:.1f}%"
                )

                if final_sell_rate >= validation["target_sell_rate_min"]:
                    print("  ✓ PASSED: SELL rate meets minimum target")
                else:
                    print("  ✗ FAILED: SELL rate below minimum target")
        ...
        if "grad_norm" in probe_df.columns:
            ...
            if len(grad_norms) > 0:
                ...
                print(
                    f"  Healthy gradients: {healthy_grads}/{len(grad_norms)} ({healthy_ratio*100:.1f}%)"
                )
                print(f"  Threshold: {validation['grad_norm_threshold']}")

                if healthy_ratio > 0.8:
                    print("  ✓ PASSED: Gradient flow maintained")
                else:
                    print("  ⚠ WARNING: Gradient flow degraded")
