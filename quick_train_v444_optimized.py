...
import numpy as np
from ztb.training.base_trainer import TrainingSession, TrainerConfig
...
def analyze_action_distribution(session: TrainingSession, num_samples: int = 200) -> dict:
...
def print_summary(config: dict, analysis: dict) -> None:
    ...
    print(f"  Entropy Regularization: {behavior_opt.get('entropy_regularization', 0.01)}")
    print(f"  Action Balance Target: {behavior_opt.get('action_balance_target', 0.333):.3f}")
    print(f"  Redundant Trade Penalty: {behavior_opt.get('redundant_trade_penalty', 10.0)}")
    ...
    if analysis:
        ...
        print(f"  HOLD: {discrete['HOLD']:.2%} ({analysis['discrete_action_counts']['HOLD']} actions)")
        print(f"  BUY:  {discrete['BUY']:.2%} ({analysis['discrete_action_counts']['BUY']} actions)")
        print(f"  SELL: {discrete['SELL']:.2%} ({analysis['discrete_action_counts']['SELL']} actions)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train SAC v444.2 with optimized configuration")
    parser.add_argument("--config", type=str, default="config/sac_v444_2_optimized_balance_reward.json",
