...
TrainingConfig = ConfigDict
TrainingStats = Dict[str, Any]
...
class AnomalyDetectorProtocol(Protocol):
	def fit_ml_detectors(self, training_data: "np.ndarray") -> bool:  # returns success flag
		...

	def detect_anomalies(self, data: "np.ndarray", feature_names: Optional[List[str]] = None) -> Tuple[bool, Dict[str, Any]]:
		...
...
class TrainingReporterProtocol(Protocol):
	def generate_ensemble_report(self, ensemble_stats: Dict[str, Any], decision_log: List[Dict[str, Any]]) -> Dict[str, Any]:
		...
...
class SACLikeModelProtocol(Protocol):
	...
	def predict(self, obs: Any, deterministic: bool = True) -> Any:  # pragma: no cover - runtime
		...
