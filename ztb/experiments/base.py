...
class ExperimentBase(ABC):
        ...
        try:
            ...
            if isinstance(e, (KeyboardInterrupt, SystemExit, MemoryError, OSError)):
                self.logger.critical(
                    f"Fatal error occurred: {type(e).__name__}. Terminating process."
                )
                import sys
    ...
    def aggregate_results(
        ...
        if not result_files:
            return AggregationResult(
                ...
                results=[],
                summary={
                    "error": f"No result files found matching pattern: {experiment_pattern}"
                },
            )
        ...
        if success_count > 0:
            summary["avg_execution_time"] = (
                summary["total_execution_time"] / success_count
            )
        ...
        if output_file is None:
            ...
            output_file = (
                f"reports/aggregated_results_{experiment_pattern}_{timestamp}.json"
            )
    ...
    def _rotate_old_reports(
        cls, results_dir: Path, pattern: str, max_reports: int = 10
    ):
        """古いレポートファイルをローテーション"""
        try:
            # パターンにマッチするファイルを検索
...
class ScalingExperiment(ExperimentBase):
    ...
    def run(self) -> ExperimentResult:
        ...
        try:
            ...
            self.logger.error(
                f"Scaling experiment failed at step {self.current_step}: {e}"
            )
            raise
