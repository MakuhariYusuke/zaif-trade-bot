...
class ParallelRLExperimentRunner:
                ...
                try:
                    for future in as_completed(
                        ...
                        try:
                            ...
                            # 致命的なエラーの場合は強制終了
                            if isinstance(
                                e, (KeyboardInterrupt, SystemExit, MemoryError, OSError)
                            ):
                                self.logger_manager.enqueue_notification(
            ...
            self.logger_manager.log_experiment_end(
                {
                    ...
                    "experiments_completed": len(
                        [
                            ...
                            if r.get("status") not in ["failed", "timeout"]
                        ]
                    ),
    ...
    def _run_single_experiment(self, exp_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger_manager.enqueue_notification(
            f"Starting experiment: {exp_name} (strategy: {strategy}, timeout: {timeout_seconds}s)"
        )
        ...
        log_file = (
            self.log_dir / f"{exp_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        # Log file for this experiment
        log_file = (
            self.log_dir / f"{exp_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ...
        try:
            ...
            while process.poll() is None:
                ...
                if elapsed > timeout_seconds:
                    self.logger_manager.enqueue_notification(
                        f"Experiment {exp_name} timed out after {elapsed:.1f}s. Terminating process."
                    )
                    process.terminate()
                ...
                if current_time - last_progress_time > 300:
                    self.logger_manager.enqueue_notification(
                        f"{exp_name}: Still running (elapsed: {elapsed:.1f}s)"
                    )
            ...
            self.logger_manager.enqueue_notification(
                f"Experiment {exp_name} completed with status: {result['status']}"
            )
            return result
    ...
    def _extract_metrics_from_log(self, log_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger_manager.log_error(
                f"Failed to extract metrics from {log_file}: {e}"
            )
    ...
    def _generate_summary_report(self):
        ...
        summary_file = (
            self.log_dir
            / f"parallel_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        self.logger_manager.enqueue_notification(
            f"Parallel execution summary saved to: {summary_file}"
        )
        ...
        print("=" * 80)
        print(
            f"Total execution time: {summary['execution_summary']['total_time_seconds']:.1f} seconds"
        )
        print(
            f"Experiments completed: {summary['execution_summary']['experiments_completed']}"
        )
        print(
            f"Experiments failed: {summary['execution_summary']['experiments_failed']}"
        )
        print("\nResource Usage:")
    ...
    def _terminate_all_processes(self):
        ...
        for exp_name, process in self.processes.items():
            try:
                if process.poll() is None:
                    self.logger_manager.enqueue_notification(
                        f"Terminating process for {exp_name}"
                    )
                    process.terminate()
                    time.sleep(2)
            except Exception as e:
