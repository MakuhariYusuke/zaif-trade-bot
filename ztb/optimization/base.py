    ...
    def __post_init__(self) -> None:
        ...
        if self.param_type in [ParameterType.CONTINUOUS, ParameterType.INTEGER, ParameterType.LOG_UNIFORM]:
            if self.low is None or self.high is None:
                raise ValueError(f"{self.name}: low and high must be specified for {self.param_type}")
        elif self.param_type == ParameterType.CATEGORICAL:
            if not self.choices:
                raise ValueError(f"{self.name}: choices must be specified for CATEGORICAL")
...
class OptimizerBase(ABC):
    ...
    def _create_result(self) -> OptimizationResult:
        ...
        return OptimizationResult(
            ...
            best_objective_value=self.best_trial.objective_value if self.best_trial else float('inf'),
