...
class RewardFunctionOptimizer:
    ...
    def _define_parameter_spaces(self) -> Dict[str, Dict[str, ParameterSpace]]:
        ...
        # Balanced transition stage parameters - Enhanced
        spaces["balanced_transition"] = {
            ...
            "balance_penalty": ParameterSpace(
                "balance_penalty", "float", 1.0, 20.0, log_scale=False
            ),
            "hold_penalty_rate": ParameterSpace(
                "hold_penalty_rate", "float", 0.001, 0.1, log_scale=True
            ),
            ...
            "trading_bonus": ParameterSpace(
                "trading_bonus", "float", 0.001, 0.1, log_scale=True
            ),
            # Profit bonus multipliers for each action [BUY, SELL, HOLD]
            "profit_bonus_multiplier_buy": ParameterSpace(
                "profit_bonus_multiplier_buy", "float", 0.5, 2.0, log_scale=False
            ),
            "profit_bonus_multiplier_sell": ParameterSpace(
                "profit_bonus_multiplier_sell", "float", 0.5, 2.0, log_scale=False
            ),
            "profit_bonus_multiplier_hold": ParameterSpace(
                "profit_bonus_multiplier_hold", "float", 0.1, 1.5, log_scale=False
            ),
            # ATR and portfolio-based profit bonuses
            "base_profit_bonus_atr_coeff": ParameterSpace(
                "base_profit_bonus_atr_coeff", "float", 0.5, 3.0, log_scale=False
            ),
            "base_profit_bonus_portfolio_coeff": ParameterSpace(
                "base_profit_bonus_portfolio_coeff", "float", 0.5, 3.0, log_scale=False
            ),
            # Advanced reward components
            "momentum_weight": ParameterSpace(
                "momentum_weight", "float", 0.0, 1.0, log_scale=False
            ),
            "volatility_weight": ParameterSpace(
                "volatility_weight", "float", 0.0, 1.0, log_scale=False
            ),
            "time_decay_weight": ParameterSpace(
                "time_decay_weight", "float", 0.0, 0.5, log_scale=False
            ),
    ...
    def optimize_reward_function(
        ...
        def objective(trial):
            ...
            try:
                ...
                self._handle_error(e, f"backtest evaluation (trial {trial.number})")
