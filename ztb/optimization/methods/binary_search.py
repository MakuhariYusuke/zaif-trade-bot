...
class BinarySearchOptimizer(OptimizerBase):
    ...
    def __init__(
        ...
        if not isinstance(parameter_space, ParameterSpace):
            raise ValueError("parameter_spaceはParameterSpaceインスタンスである必要があります")
        
        if parameter_space.param_type not in [ParameterType.CONTINUOUS, ParameterType.LOG_UNIFORM]:
    ...
    def _golden_section_search(self) -> OptimizationResult:
        ...
        while abs(b - a) > self.tolerance and trial_count < self.max_iterations:
            ...
            if f1 < f2:
                ...
                f2 = f1
                x1 = a + resphi * (b - a)
                f1_result = self._run_trial(trial_count, 
                                           {self.param_name: self._from_search_space(x1)})
                f1 = f1_result.objective_value if f1_result.success else float('inf')
            else:
                a = x1
                x1 = x2
                f1 = f2
                x2 = b - resphi * (b - a)
                f2_result = self._run_trial(trial_count, 
                                           {self.param_name: self._from_search_space(x2)})
                f2 = f2_result.objective_value if f2_result.success else float('inf')
            
            # 進捗表示
            current_best = min(f1, f2)
            current_range = abs(b - a)
            print(f"  区間: [{self._from_search_space(a):.6e}, "
