...
class OptimizationConfig:
    ...
    reward_objectives: List[str] = field(default_factory=lambda: ["win_rate", "profit_factor", "max_drawdown"])
...
class BaseOptimizer(ABC):
    ...
    @abstractmethod
    def optimize(self, objective_function: Callable, search_space: Dict[str, Any]) -> OptimizationResult:
        """最適化を実行"""
        pass
...
class BayesianOptimizer(BaseOptimizer):
    ...
    def _generate_recommendations(self, study, search_space: Dict[str, Any]) -> List[str]:
        """最適化結果に基づく推奨事項を生成"""
        recommendations = []
        ...
        try:
            ...
            if top_params:
                recommendations.append(f"最も重要なパラメータ: {', '.join([p[0] for p in top_params])}")
        ...
        if convergence_score > 0.8:
            ...
            recommendations.append("最適化が収束していない可能性があります。試行回数を増やすことを検討してください")
            ...
            if param_config.get("type") == "float":
                ...
                if param_values:
                    ...
                    if value_range / search_range < 0.1:
                        recommendations.append(f"{param_name}の探索範囲を狭くすることを検討してください")
    ...
    def optimize(self, objective_function: Callable, search_space: Dict[str, Any]) -> OptimizationResult:
        ...
        for param_name, param_config in search_space.items():
            ...
            if param_type == "categorical":
                ...
                if param_type == "float":
                    param_grids[param_name] = np.linspace(low, high, num_values).tolist()
                else:
                    param_grids[param_name] = list(range(low, high + 1, max(1, (high - low) // num_values)))
        ...
        convergence_info = {
            ...
            "evaluated_trials": len([h for h in optimization_history if h.get("score") is not None]),
...
class SystemOptimizer:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        """システム状態を取得"""
        status = {
            "memory_usage_history": self.memory_usage[-10:] if self.memory_usage else [],
...
class RewardFunctionOptimizer:
    ...
    def optimize_reward_structure(self, evaluation_function: Callable,
                                current_reward_config: Dict[str, Any]) -> Dict[str, Any]:
        """報酬関数構造の最適化"""
        ...
        self.logger.info(f"Reward optimization completed. Best score: {result.best_score:.4f}")

        return {
            "optimized_config": optimized_config,
            "optimization_result": result,
            "improvement": result.best_score - evaluation_function(current_reward_config)
        }
...
class UnifiedOptimizer:
    ...
    def optimize_hyperparameters(self, objective_function: Callable,
        ...
        result = self.hyperparameter_optimizer.optimize(objective_function, search_space)
    ...
    def optimize_reward_function(self, evaluation_function: Callable,
                               current_reward_config: Dict[str, Any]) -> Dict[str, Any]:
        """報酬関数の最適化"""
    ...
    def adaptive_optimize(self, current_performance: Dict[str, Any],
                         market_regime: str = "neutral") -> Dict[str, Any]:
        """適応的最適化（市場状況に応じた動的調整）"""
        if not self.config.enable_adaptive_optimization:
            return {"status": "disabled"}

        self.logger.info(f"Starting adaptive optimization for market regime: {market_regime}")
        ...
        result = {
            ...
            "recommendations": self._generate_adaptive_recommendations(strategy, performance_score)
        }
    ...
    def _generate_adaptive_recommendations(self, strategy: Dict[str, float],
...
def create_unified_optimizer(config: Optional[OptimizationConfig] = None) -> UnifiedOptimizer:
