...
class BTCDataAugmentor:
    ...
    def _load_base_data(self):
        ...
        try:
            if self.base_data_path.suffix == '.csv':
                ...
                raise ValueError(f"Unsupported file format: {self.base_data_path.suffix}")

            # Convert timestamp to datetime
            if 'timestamp' in self.base_data.columns:
                self.base_data['timestamp'] = pd.to_datetime(self.base_data['timestamp'])
                self.base_data = self.base_data.sort_values('timestamp').reset_index(drop=True)

            logger.info(f"Loaded base data: {len(self.base_data)} records from {self.base_data['timestamp'].min()} to {self.base_data['timestamp'].max()}")
    ...
    def analyze_data_bias(self) -> Dict[str, Any]:
        ...
        with self.performance_monitor:
            ...
            time_range = self.base_data['timestamp'].max() - self.base_data['timestamp'].min()
            ...
            if 'close' in self.base_data.columns:
                ...
                bias_analysis['trend_bias'] = positive_days / total_days if total_days > 0 else 0
                bias_analysis['total_return'] = total_return

                # Volatility analysis
                bias_analysis['volatility'] = returns.std()
                bias_analysis['max_drawdown'] = self._calculate_max_drawdown(self.base_data['close'])
            ...
            if 'volume' in self.base_data.columns:
                ...
                bias_analysis['volume_volatility'] = self.base_data['volume'].std() / self.base_data['volume'].mean()
    ...
    def extend_historical_data(self, years_back: int = 2) -> pd.DataFrame:
        ...
        with self.performance_monitor:
            ...
            logger.info(f"Extended data from {len(self.base_data)} to {len(combined_data)} records")
    ...
    def _generate_synthetic_day(self, date: datetime, base_stats: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        day_volatility = np.random.uniform(0.5, 1.5) * base_stats.get('return_std', 0.01)

        current_price = base_price * (1 + np.random.normal(0, 0.05))  # Start near base price

        for minute in range(minutes_per_day):
            timestamp = date + timedelta(minutes=minute)

            # Generate price movement
            return_change = np.random.normal(day_trend / minutes_per_day, day_volatility)
            current_price *= (1 + return_change)

            # Ensure price stays within reasonable bounds
            current_price = np.clip(current_price, base_stats.get('price_min', 1000000),
            ...
            # Generate volume
            volume = max(1, np.random.normal(base_stats.get('volume_mean', 100),
    ...
    def _generate_technical_indicators(self, current_record: Dict[str, Any], day_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def add_diverse_market_conditions(self, target_samples: int = 10000) -> pd.DataFrame:
        ...
        with self.performance_monitor:
            ...
            regimes = ['strong_bull', 'moderate_bull', 'sideways', 'moderate_bear', 'strong_bear', 'high_volatility', 'low_volatility']

            samples_per_regime = target_samples // len(regimes)

            for regime in regimes:
                regime_data = self._generate_regime_data(regime, samples_per_regime, base_stats)
    ...
    def _generate_regime_data(self, regime: str, num_samples: int, base_stats: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Generate data for specific market regime"""
        data = []

        # Regime-specific parameters
        regime_params = {
            'strong_bull': {'trend': 0.02, 'volatility': 0.015, 'volume_multiplier': 1.5},
            'moderate_bull': {'trend': 0.005, 'volatility': 0.01, 'volume_multiplier': 1.2},
            'sideways': {'trend': 0.0, 'volatility': 0.008, 'volume_multiplier': 0.8},
            'moderate_bear': {'trend': -0.005, 'volatility': 0.012, 'volume_multiplier': 1.1},
            'strong_bear': {'trend': -0.02, 'volatility': 0.02, 'volume_multiplier': 1.8},
            'high_volatility': {'trend': 0.0, 'volatility': 0.03, 'volume_multiplier': 2.0},
            'low_volatility': {'trend': 0.0, 'volatility': 0.003, 'volume_multiplier': 0.5}
        }
        ...
        for i in range(num_samples):
            ...
            return_change = np.random.normal(params['trend'] / 1440, params['volatility'])
            current_price *= (1 + return_change)
            ...
            high = current_price * (1 + volatility)
            low = current_price * (1 - volatility)
            open_price = current_price * (1 + np.random.normal(0, params['volatility'] * 0.5))
            close = current_price

            # Generate volume
            base_volume = base_stats.get('volume_mean', 100)
            volume = max(1, np.random.normal(base_volume * params['volume_multiplier'],
                                           base_stats.get('volume_std', 50)))
    ...
    def save_augmented_data(self, augmented_data: pd.DataFrame, output_path: str):
        ...
        logger.info(f"Saved augmented data to {output_path} ({len(augmented_data)} records)")
...
class BTCBiasDetector:
    ...
    def _check_btc_specific_bias(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'timestamp' in data.columns:
            ...
            if len(years) < 2:
                bias_issues.append("Limited to single year - missing halving cycle effects")
