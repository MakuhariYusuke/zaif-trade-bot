    ...
    def detect_regime(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for window_name, window in self.regime_windows.items():
            ...
            df[f"trend_strength_{window_name}"] = (
                df["close"] - df[f"sma_{window_name}"]
            ) / df[f"sma_{window_name}"]

            # ボラティリティ
            df[f"volatility_{window_name}"] = (
...
class AdaptiveFeatureEngineer:
    ...
    def create_features(
        self, df: pd.DataFrame, include_regime_features: bool = True
    ) -> pd.DataFrame:
        """
        包括的な特徴量生成
    ...
    def _create_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "open" in df.columns:
            ...
            df["gap_size"] = abs(df["open"] - df["close"].shift(1)) / df["close"].shift(
                1
            )
    ...
    def _create_market_structure_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 価格の位置づけ
        df["distance_from_support"] = (df["close"] - df["support_level"]) / df[
            "support_level"
        ]
        df["distance_from_resistance"] = (df["resistance_level"] - df["close"]) / df[
            "resistance_level"
        ]
    ...
    def _create_regime_adaptive_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["regime_persistence"] = (
            df["regime"]
            .groupby((df["regime"] != df["regime"].shift(1)).cumsum())
            .cumcount()
        )
    ...
    def _create_statistical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_cols:
            if col in ["close", "volume"] or "price" in col.lower():
                df[f"{col}_zscore"] = stats.zscore(
                    df[col].fillna(df[col].mean()), nan_policy="omit"
                )
    ...
    def select_features(
        self, df: pd.DataFrame, method: str = "correlation", threshold: float = 0.95
    ) -> pd.DataFrame:
        """
        特徴量選択
        ...
        if method == "correlation":
            ...
            df_selected = pd.concat(
                [df_pca, df.select_dtypes(exclude=[np.number])], axis=1
            )
...
def main():
    ...
    df_selected = engineer.select_features(
        df_featured, method="correlation", threshold=0.95
    )
    ...
    feature_stats = {
        ...
        "regime_distribution": df_featured["regime"].value_counts().to_dict()
        if "regime" in df_featured.columns
        else {},
        "feature_types": {
