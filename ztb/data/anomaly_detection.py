...
class StatisticalAnomalyDetector:
    ...
    def detect(self, data: np.ndarray, feature_names: Optional[List[str]] = None) -> AnomalyResult:
        """異常検知実行"""
        try:
            if len(data.shape) == 1:
    ...
    def _iqr_detection(self, data: np.ndarray) -> AnomalyResult:
        """IQRベースの異常検知"""
        history_flat = np.array([item for sublist in self.history[-self.window_size:] for item in sublist])
    ...
    def detect(self, data: np.ndarray) -> AnomalyResult:
        ...
        try:
            ...
            return AnomalyResult(
                ...
                details={
                    'raw_score': scores[0] if hasattr(scores, '__getitem__') else scores,
                    'prediction': predictions[0] if hasattr(predictions, '__getitem__') else predictions
                }
            )
...
class AutoencoderAnomalyDetector(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dims: List[int] = None, dropout_rate: float = 0.1):
        super().__init__()
    ...
    def fit(self, data: np.ndarray, epochs: int = 100, batch_size: int = 32,
            learning_rate: float = 0.001, device: str = 'cpu') -> bool:
        """オートエンコーダー学習"""
        try:
            self.to(device)
            ...
            for epoch in range(epochs):
                ...
                if (epoch + 1) % 20 == 0:
                    logger.info(f"Autoencoder epoch {epoch+1}/{epochs}, loss: {total_loss/len(dataloader):.6f}")

            # 閾値設定（トレーニングデータの再構成誤差の95パーセンタイル）
            self.eval()
            with torch.no_grad():
                reconstructed = self(data.to(device))
                reconstruction_errors = torch.mean((reconstructed - data.to(device)) ** 2, dim=1)
                self.threshold = torch.quantile(reconstruction_errors, 0.95).item()

            logger.info(f"Autoencoder trained successfully, threshold: {self.threshold:.6f}")
            return True
    ...
    def detect(self, data: np.ndarray, device: str = 'cpu') -> AnomalyResult:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                reconstruction_error = torch.mean((reconstructed - data_tensor) ** 2).item()

                is_anomaly = reconstruction_error > self.threshold
                anomaly_score = reconstruction_error / self.threshold if self.threshold > 0 else 0.0
...
class ComprehensiveAnomalyDetector:
    ...
    def __init__(self,
        ...
        for method in statistical_methods:
            if method == 'zscore':
                self.statistical_detectors[method] = StatisticalAnomalyDetector('zscore', threshold=3.0)
            elif method == 'iqr':
                self.statistical_detectors[method] = StatisticalAnomalyDetector('iqr', threshold=1.5)
            elif method == 'mad':
                self.statistical_detectors[method] = StatisticalAnomalyDetector('mad', threshold=3.5)

        # ML検知器初期化
        for method in ml_methods:
            if method == 'isolation_forest':
                self.ml_detectors[method] = MLAnomalyDetector('isolation_forest', contamination=0.1)
            elif method == 'elliptic_envelope':
                self.ml_detectors[method] = MLAnomalyDetector('elliptic_envelope', contamination=0.1)

        # オートエンコーダー初期化
        if enable_autoencoder:
            self.autoencoder_detector = AutoencoderAnomalyDetector(input_dim=10)  # 仮の次元
    ...
    def detect_anomalies(self, data: np.ndarray,
                        feature_names: Optional[List[str]] = None) -> Tuple[bool, Dict[str, Any]]:
        """包括的な異常検知"""
    ...
    def reset_stats(self):
        """統計情報リセット"""
        self.stats = AnomalyStats(0, 0, 0.0, list(self.statistical_detectors.keys()) +
