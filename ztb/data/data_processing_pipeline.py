...
from .data_validation import DataIntegrityChecker, DataValidator
...
class DataProcessingPipeline:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validation": {"enabled": True, "schema": {}, "strict_mode": False},
            "quality_checks": {
                "enabled": True,
        ...
        try:
            for step in steps:
                ...
                if step == "validation":
                    ...
                    if not validation_result.is_valid and self.config["validation"].get(
                        "strict_mode", False
                    ):
                        raise ValueError(
    ...
    def _run_validation(self, data: pd.DataFrame) -> Any:
        ...
        combined_result.is_valid = (
            validation_result.is_valid and integrity_result.is_valid
        )
        combined_result.errors = validation_result.errors + integrity_result.errors
        combined_result.warnings = (
            validation_result.warnings + integrity_result.warnings
        )
        combined_result.metrics = {
            **validation_result.metrics,
            **integrity_result.metrics,
        }
    ...
    def _run_outlier_detection(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Outlier detection completed. Detected outliers in {len(columns)} columns"
        )
        return result_data
    ...
    def _run_quality_checks(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        if completeness < min_completeness:
            logger.warning(
                f"Completeness {completeness:.2%} below threshold {min_completeness:.2%}"
            )
    ...
    def _generate_basic_schema(self, data: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        ...
        for col in data.columns:
            ...
            if np.issubdtype(dtype, np.number):
                col_schema["type"] = (
                    "float" if np.issubdtype(dtype, np.floating) else "int"
                )
