...
from .evaluation_types import (
    ...
    EvaluationMetrics, AlertLevel, AlertType
)
...
class ContinuousEvaluationManager:
    ...
    def perform_evaluation(self) -> EvaluationResult:
        ...
        try:
            ...
            if self.online_learning:
                futures['online_learning'] = self.executor.submit(self._evaluate_online_learning)
    ...
    def _evaluate_drift(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent_data is not None:
                ...
                # 最も深刻なドリフトを特定
                max_severity = max((result.severity.value for result in drift_results), default=0)

                return {
                    'drift_detected': any(result.drift_detected for result in drift_results),
                    'severity': max_severity,
                    'drift_types': [result.drift_type.value for result in drift_results if result.drift_detected]
                }
            else:
                return {
                    'drift_detected': False,
    ...
    def _get_recent_data_for_drift_detection(self) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for metric_value in latest_metrics.values():
                if 'feature' in metric_value.name.lower() or 'input' in metric_value.name.lower():
    ...
    def _calculate_overall_score(self, results: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if results.get('performance'):
                ...
                perf_score = (perf.accuracy * 0.3 +
                            (1 - perf.max_drawdown) * 0.3 +
                            (perf.sharpe_ratio / 3.0) * 0.4)  # Sharpe 3.0を満点とする
        ...
        try:
            ...
            if results.get('performance'):
                ...
                if perf.max_drawdown > 0.2:
                    recommendations.append("リスク管理パラメータの見直しを検討してください")
            ...
            if results.get('safety'):
                ...
                if safety.get('safety_score', 1.0) < 0.7:
                    recommendations.append("システムの安全性を向上させる検討を推奨します")

            # ドリフトベースの推奨
            if results.get('drift', {}).get('drift_detected'):
                severity = results['drift'].get('severity', 1)
                if severity >= 3:
                    recommendations.append("市場環境の大きな変化が検知されました。再学習を強く推奨します")
                else:
                    recommendations.append("市場環境の変化が検知されました。モデルの調整を検討してください")
    ...
    def add_evaluation_callback(self, callback: Callable[[EvaluationResult], None]) -> None:
        """評価コールバックの追加"""
    ...
    def get_evaluation_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            # サマリー統計の計算
            scores = [e.overall_score for e in recent_evaluations if e.overall_score is not None]
            ...
            return {
                ...
                "drift_rate": drift_detections / len(recent_evaluations) if recent_evaluations else 0.0,
                "latest_score": recent_evaluations[-1].overall_score if recent_evaluations else None,
                "latest_recommendations": recent_evaluations[-1].recommendations if recent_evaluations else []
            }
