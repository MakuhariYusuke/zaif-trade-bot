...
from ztb.utils.memory_utils import temporary_array, memory_efficient_processing, MemoryTracker
...
class ElasticWeightConsolidation:
    ...
    def consolidate_task(self, task_data: TaskData, loss_fn: Callable) -> Dict[str, Any]:
        """タスクの統合（重要パラメータの保存）"""
        ...
        return {
            ...
            'fisher_norm': torch.norm(torch.cat([f.flatten() for f in fisher_info.values()])),
            'param_count': len(current_params)
        }

    def _compute_fisher_information(self, task_data: TaskData, loss_fn: Callable) -> Dict[str, torch.Tensor]:
        """フィッシャー情報行列の計算"""
        fisher_info = {}
    ...
    def regularization_loss(self, current_params: Dict[str, torch.Tensor]) -> torch.Tensor:
        """EWC正則化損失の計算"""
...
class RehearsalBuffer:
    ...
    def __init__(self, config: ContinualLearningConfig):
        ...
        self.task_buffers = defaultdict(lambda: deque(maxlen=config.rehearsal_buffer_size // 5))  # タスクごとのバッファ
        self.memory_tracker = MemoryTracker() if config.enable_memory_tracking else None

    def add_samples(self, task_data: TaskData):
        """サンプルの追加"""
        # タスク全体のバッファ
        for i in range(min(50, task_data.num_samples)):  # メモリ節約のため最大50サンプル
            sample = {
    ...
    def get_rehearsal_batch(self, batch_size: int = 32) -> Optional[Dict[str, torch.Tensor]]:
        """リハーサル用バッチの取得"""
    ...
    def get_task_rehearsal_batch(self, task_id: str, batch_size: int = 16) -> Optional[Dict[str, torch.Tensor]]:
        """タスク固有のリハーサルバッチ取得"""
        if task_id not in self.task_buffers or len(self.task_buffers[task_id]) < batch_size:
            return None
    ...
    def get_buffer_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'task_buffers': {task_id: len(buffer) for task_id, buffer in self.task_buffers.items()},
        ...
        for name, module in expanded_network.named_modules():
            if isinstance(module, nn.Linear):
                ...
                with torch.no_grad():
                    ...
                    new_layer.weight[:min_out, :min_in] = module.weight[:min_out, :min_in]
...
class ContinualLearner:
    ...
    def learn_task(self, task_data: TaskData, loss_fn: Callable, optimizer: optim.Optimizer,
                  num_epochs: int = 10) -> Dict[str, Any]:
        """新規タスクの学習"""
        logger.info(f"Learning task: {task_data.task_id} using method: {self.config.method}")
        ...
        # 学習実行
        training_stats = self._train_task_model(task_model, task_data, loss_fn, optimizer, num_epochs)
    ...
    def _train_task_model(self, task_model: nn.Module, task_data: TaskData,
                         loss_fn: Callable, optimizer: optim.Optimizer, num_epochs: int) -> Dict[str, Any]:
        """タスクモデルの学習"""
        task_model.train()
        training_losses = []

        # データローダー作成
        dataset = TensorDataset(task_data.states, task_data.actions, task_data.rewards,
                              task_data.next_states, task_data.dones)
        ...
        for epoch in range(num_epochs):
            ...
            for batch in dataloader:
                ...
                loss = loss_fn(outputs, actions, rewards, task_model(next_states), dones)

                # 継続学習の正則化損失追加
                if self.ewc:
                    current_params = {name: param for name, param in task_model.named_parameters()}
                    regularization_loss = self.ewc.regularization_loss(current_params)
                    loss += regularization_loss

                # リハーサル損失追加
                if self.rehearsal_buffer and epoch % 2 == 0:  # 2エポックに1回
                    rehearsal_batch = self.rehearsal_buffer.get_rehearsal_batch(batch_size=16)
                    if rehearsal_batch:
                        rehearsal_outputs = task_model(rehearsal_batch['states'])
                        rehearsal_loss = loss_fn(rehearsal_outputs, rehearsal_batch['actions'],
            ...
            if (epoch + 1) % 5 == 0:
                logger.debug(f"Task {task_data.task_id}, epoch {epoch+1}/{num_epochs}, loss: {avg_loss:.6f}")
    ...
    def predict_with_continual(self, state: torch.Tensor, task_id: Optional[str] = None) -> torch.Tensor:
        """継続学習を考慮した予測"""
        if task_id is None:
