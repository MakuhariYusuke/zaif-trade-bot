...
class ABTestExecutor:
    ...
    def start_test(self, test_config: ABTestConfiguration) -> str:
        ...
        if len(self.active_tests) >= self.config.max_concurrent_tests:
            raise RuntimeError(
                f"Maximum concurrent tests ({self.config.max_concurrent_tests}) reached"
            )

        # テスト状態の初期化
        test_state = ABTestState(
            test_id=test_config.test_id, status=ABTestStatus.CREATED
        )
    ...
    def _process_test_stream(
        self, test_config: ABTestConfiguration, test_state: ABTestState
    ):
        ...
        try:
            while test_state.status == ABTestStatus.RUNNING:
                ...
                if batch_buffer:
                    ...
                    # 定期的な分析実行
                    if (
                        datetime.now() - last_analysis_time
                    ).seconds >= test_config.check_interval_minutes * 60:
        ...
        for variant_id, samples in variant_samples.items():
            if samples:
                future = self.executor.submit(
                    self._update_metrics_batch, test_state, variant_id, samples
                )
    ...
    def _perform_periodic_analysis(
        ...
        try:
            ...
            statistical_result = self.analyzer.analyze_comparison(
                test_state.metrics_a, test_state.metrics_b, test_config.statistical_test
            )
    ...
    def _check_early_stopping(
        ...
        if (
            ...
            and test_state.metrics_b.sample_count >= min_samples
        ):
            # p値が非常に小さく、効果量が十分な場合
            if (
                result.p_value < 0.01
                and result.effect_size > test_config.minimum_effect_size
            ):
                return True

        return False

    def _check_risks(self, test_config: ABTestConfiguration, test_state: ABTestState):
        """リスクをチェック"""
        # 回帰検知
    ...
    def _detect_regression(self, test_state: ABTestState) -> bool:
        ...
        if (
            test_state.metrics_a.sample_count < 100
            or test_state.metrics_b.sample_count < 100
        ):
            return False
    ...
    def _check_completion_conditions(
        ...
        # 最大時間経過
        if test_state.start_time and datetime.now() - test_state.start_time > timedelta(
            hours=test_config.max_duration_hours
        ):
            return True

        # 統計的有意性到達
        if (
            test_state.latest_statistical_result
            and test_state.latest_statistical_result.p_value
    ...
    def _generate_test_result(self, test_id: str) -> Optional[ABTestResultSummary]:
        ...
        # 勝者を決定
        if result.p_value < (
            self.config.statistics.alpha if self.config.statistics else 0.05
        ):
        ...
        return ABTestResultSummary(
            ...
            confidence_level=result.confidence_interval[1]
    ...
    def _assess_risks(self, test_state: ABTestState) -> Dict[str, Any]:
        ...
        return {
            ...
            "sample_balance": abs(
                test_state.metrics_a.sample_count - test_state.metrics_b.sample_count
            ),
            "processing_efficiency": test_state.processing_time_ms
            / max(test_state.current_sample_count, 1),
        }
    ...
    def _generate_recommendations(
        ...
        if result == ABTestResult.WINNER_A:
            recommendations.append(
                "Deploy variant A - shows statistically significant improvement"
            )
        elif result == ABTestResult.WINNER_B:
            recommendations.append(
                "Deploy variant B - shows statistically significant improvement"
            )
        else:
        ...
        if test_state.regression_detected:
            recommendations.append(
                "WARNING: Regression detected - monitor closely after deployment"
            )
