...
np.random.seed(42)
actions = np.random.choice([0, 1, 2], size=len(regime_labels), p=[0.1, 0.05, 0.85])  # SELL bias

# Analyze
result = evaluator.analyze_regime_performance({'test': 'dummy'}, df, regime_labels, {'test': actions})
