...
from ztb.features.causal_inference import CausalFeatureSelector, create_causal_engine
...
def test_causal_feature_selector():
    ...
    effect_result = selector.estimate_causal_effect(
        df, "feature_strong", "reward", confounders
    )
    print(f"Causal effect for feature_strong: {effect_result}")

    # Test feature selection
    selected_features, results = selector.select_features_causal(
        df, features, "reward", confounders
    )
...
def test_causal_inference_engine():
    ...
    # Create engine
    config = {"treatment_threshold": 0.05, "min_samples": 50, "max_features": 3}
...
def test_adaptive_selector_with_causal():
    ...
    selected_features, stats = selector.select_features(
        df, feature_names, use_causal=True, outcome_feature="reward"
    )
    ...
    selected_adaptive, stats_adaptive = selector.select_features(
        df, feature_names, use_causal=False
    )
