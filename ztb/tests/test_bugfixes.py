...
def test_ensemble_mask_provider_required():
    ...
    print(
        "✅ PASS: Ensemble mask_provider enforcement implemented (requires integration test)"
    )
...
def test_min_holding_period_with_allow_reverse():
    ...
    if env.position == 0:
        print(
            f"✅ PASS: SELL closed LONG but did NOT reverse to SHORT (position={env.position})"
        )
        print(
            "         min_holding_period correctly prevented reversal despite allow_reverse=True"
        )
        return True
    elif env.position < 0:
        print(f"❌ FAIL: SELL incorrectly reversed to SHORT (position={env.position})")
        print(
            "         min_holding_period should have prevented this with allow_reverse=True"
        )
        return False
    else:
...
def test_reward_pnl_attribution():
    ...
    print("✅ PASS: HOLD action executed (reward may include penalties)")
    ...
    print(
        f"Price changed from {old_entry:.2f} to {data.iloc[env.current_step-1]['close']:.2f}"
    )
    print(f"Unrealized PnL: {env.position_manager.calculate_unrealized_pnl():.2f}")
    print("✅ PASS: HOLD executed while holding position")
    ...
    if env.position == 0:
        print("✅ PASS: Position successfully closed, realized_pnl updated")
        return True
    else:
...
def test_forced_close_timestamp_sync():
    ...
    for _ in range(5):  # Increase iterations to ensure stop-loss triggers
        ...
        print(
            f"Step {env.current_step}: position={env.position:.2f}, _last_trade_step={env._last_trade_step}"
        )
        if env.position == 0.0:
            break
    ...
    if env.position != 0.0:
        print(
            f"❌ Test 6 failed: Position not closed by stop-loss (position={env.position})"
        )
        return False
    ...
    if steps_since_trade < min_holding:
        ...
        if legal_actions[1] == 1 or legal_actions[2] == 1:
            print(
                "❌ Test 6 failed: min_holding_period not enforced after forced close"
            )
            print(
                f"   Steps since trade: {steps_since_trade}, min required: {min_holding}"
            )
            return False

    print(
        "✅ Test 6 passed: Forced close updated _last_trade_step and min_holding_period enforced"
    )
    return True
...
def test_live_trader_pnl_calculation():
    ...
    pm = PositionManager(config=config, get_price_callback=lambda: current_price)
    ...
    if realized_pnl <= 0:
        print("❌ Test 7 failed: Closing profitable short should yield positive PnL")
...
def test_live_trader_position_closure():
    ...
    if pm.position != 0.0:
        print("❌ Test 8 failed: Closing position should go flat, not reverse")
        print(f"   Got position: {pm.position:.2f}, Expected: 0.0")
        return False

    print("✅ Test 8 passed: Position correctly closed to flat")
    return True
...
def test_entry_fee_in_reward():
    ...
    if trade_pnl >= 0:
        print(
            "❌ Test 9 failed: Opening position should return negative PnL (entry fee)"
        )
    ...
    if abs(trade_pnl - expected_fee) > 1.0:
        print("❌ Test 9 failed: Entry fee calculation incorrect")
        print(f"   Got: {trade_pnl:.2f}, Expected: ~{expected_fee:.2f}")
        return False

    print("✅ Test 9 passed: Entry fee correctly reflected in trade_pnl")
    return True
...
def test_position_size_sync():
    ...
    if abs(pm.position - 0.1) > 0.0001:
        print("❌ Test 10 failed: Position size doesn't match max_position_size")
        print(f"   Got: {pm.position:.4f}, Expected: 0.1000")
        return False

    print("✅ Test 10 passed: Position size correctly respects max_position_size")
    return True
