...
def test_attention_trainer_memory_management():
    ...
    for i in range(1200):  # More than buffer limit (10000/4 = 2500, but we use smaller)
        ...
        regime = np.random.choice(["trending", "ranging", "high_volatility", "low_volatility"])
        trainer.add_training_sample(features, reward, regime)

        if i % 200 == 0:
            current_memory = get_memory_usage()
            print(f"After {i} samples: {current_memory:.1f} MB (diff: {current_memory - initial_memory:.1f} MB)")
...
def test_causal_selector_memory_management():
    ...
    print(f"After selection: {mid_memory:.1f} MB (increase: {memory_increase_mid:.1f} MB)")
    ...
    print(f"After update: {final_memory:.1f} MB (total increase: {memory_increase_final:.1f} MB)")

    # Check memory increase is reasonable
    assert memory_increase_final < 100, f"Memory increase too large: {memory_increase_final:.1f} MB"
...
def test_memory_cleanup():
    ...
    print(f"After creating objects: {created_memory:.1f} MB (increase: {created_memory - initial_memory:.1f} MB)")
    ...
    print(f"After cleanup: {cleaned_memory:.1f} MB (cleanup: {cleanup_improvement:.1f} MB)")

    # Memory should decrease after cleanup
    assert cleanup_improvement > 0, f"No memory cleanup detected: {cleanup_improvement:.1f} MB"
