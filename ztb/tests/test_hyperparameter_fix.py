...
def test_training_config_hierarchical():
    ...
    print(f"   ppo_hyperparameters.learning_rate: {config_dict['ppo_hyperparameters']['learning_rate']}")
    print(f"   ppo_hyperparameters.batch_size: {config_dict['ppo_hyperparameters']['batch_size']}")
...
def test_hierarchical_ppo_hyperparameters():
    ...
    # 検証
    assert ppo_config["learning_rate"] == 0.007503, f"Expected 0.007503, got {ppo_config['learning_rate']}"
    assert ppo_config["batch_size"] == 256, f"Expected 256, got {ppo_config['batch_size']}"
    ...
    assert ppo_config["max_grad_norm"] == 5.05, f"Expected 5.05, got {ppo_config['max_grad_norm']}"
...
def test_top_level_hyperparameters():
    ...
    # 検証
    assert ppo_config["learning_rate"] == 0.001, f"Expected 0.001, got {ppo_config['learning_rate']}"
    assert ppo_config["batch_size"] == 64, f"Expected 64, got {ppo_config['batch_size']}"
...
def test_top_level_override():
    ...
    # 検証: トップレベルが優先
    assert ppo_config["learning_rate"] == 0.002, f"Expected 0.002 (top-level), got {ppo_config['learning_rate']}"
    assert ppo_config["batch_size"] == 256, f"Expected 256 (from ppo_hyperparameters), got {ppo_config['batch_size']}"
...
def test_default_values():
    ...
    assert ppo_config["learning_rate"] == 3e-4, f"Expected 0.0003 (default), got {ppo_config['learning_rate']}"
    assert ppo_config["batch_size"] == 64, f"Expected 64 (default), got {ppo_config['batch_size']}"
    assert ppo_config["gamma"] == 0.99, f"Expected 0.99 (default), got {ppo_config['gamma']}"
    assert ppo_config["n_steps"] == 2048, f"Expected 2048 (default), got {ppo_config['n_steps']}"
...
def test_hierarchical_environment():
    ...
    # 検証
    assert env_config["initial_balance"] == 200000, f"Expected 200000, got {env_config['initial_balance']}"
    assert env_config["max_position_size"] == 0.01, f"Expected 0.01, got {env_config['max_position_size']}"
    assert env_config["transaction_cost"] == 0.0005, f"Expected 0.0005, got {env_config['transaction_cost']}"
    assert env_config["reward_scaling"] == 2.0, f"Expected 2.0, got {env_config['reward_scaling']}"
...
def test_v392_config():
    ...
    # 検証
    assert ppo_config["learning_rate"] == 0.007503, f"v392 learning_rate mismatch"
    assert ppo_config["batch_size"] == 256, f"v392 batch_size mismatch"
    assert ppo_config["max_grad_norm"] == 5.05, f"v392 max_grad_norm mismatch"
    assert env_config["initial_balance"] == 200000, f"v392 initial_balance mismatch"
