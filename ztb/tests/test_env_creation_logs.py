...
print(
    f"Enable action masking: {getattr(env, 'enable_action_masking', 'ATTRIBUTE NOT FOUND')}"
)
...
for param_name, param in sig.parameters.items():
    if param_name == "self":
        continue
    print(
        f"  {param_name}: {param.default if param.default != inspect.Parameter.empty else 'REQUIRED'}"
    )
