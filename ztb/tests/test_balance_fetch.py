...
async def test_dry_run_balance():
    ...
    for balance in balances:
        print(
            f"  {balance.currency}: free={balance.free}, locked={balance.locked}, total={balance.total}"
        )
...
async def test_live_trader_balance():
    ...
    try:
        config = TypedConfig()
        trader = LiveTrader(
            model_path=config.get_model_path("ppo_btc_jpy_v367.zip"),
            dry_run=True,  # Use any existing model
        )
