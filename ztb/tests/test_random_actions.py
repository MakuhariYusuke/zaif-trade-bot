...
def test_random_actions():
    ...
    print(
        f"Actions: H={actions_taken.count(0)}, B={actions_taken.count(1)}, S={actions_taken.count(2)}"
    )
    ...
    expected_avg = (
        actions_taken.count(0) * -1.0
        + actions_taken.count(1) * -0.5
        + actions_taken.count(2) * 2.0
    ) / len(actions_taken)
