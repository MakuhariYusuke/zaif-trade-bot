...
def main():
    ...
    for mode_name, deterministic in [("Deterministic", True), ("Stochastic", False)]:
        ...
        for ep in range(episodes):
            ...
            print(f"Episode {ep+1:2d}: HOLD={ep_actions[0]:3d}, BUY={ep_actions[1]:3d}, SELL={ep_actions[2]:3d}")
        ...
        print(f"HOLD:  {action_counts[0]:4d} ({action_counts[0]/total_steps*100:5.1f}%)")
        print(f"BUY:   {action_counts[1]:4d} ({action_counts[1]/total_steps*100:5.1f}%)")
        print(f"SELL:  {action_counts[2]:4d} ({action_counts[2]/total_steps*100:5.1f}%)")
        print(f"")
