...
def main():
    ...
    print(
        f"   Max features: {config.get('max_features')} (None = use all with correlation filtering)"
    )
    ...
    print(
        f"   Consecutive HOLD penalty: {reward_settings.get('consecutive_hold_penalty')}"
    )
    print(
        f"   Trading frequency bonus: {reward_settings.get('trading_frequency_bonus')}"
    )
