...
class BaseFeatureTest:
    ...
    def assert_feature_names_follow_convention(self, feature_names: list, prefix: str = "auto_"):
        """Assert that feature names follow naming convention"""
        for name in feature_names:
            assert name.startswith(prefix), f"Feature name {name} doesn't start with {prefix}"
...
class TestAutoFeatureGeneration(BaseFeatureTest, ParameterizedFeatureTestMixin):
    ...
    def test_kama_feature_generation(self, auto_generator, sample_ohlc_data):
        ...
        expected_features = [
            'auto_ma_kama_10_30_30',
            'auto_ma_kama_20_30_30'
        ]
    ...
    def test_feature_naming_convention(self, auto_generator, sample_ohlc_data):
        ...
        for feature_name in features.keys():
            assert feature_name.startswith('auto_'), f"Feature {feature_name} doesn't start with 'auto_'"
            parts = feature_name.split('_')
            assert len(parts) >= 3, f"Feature {feature_name} has invalid naming structure"
    ...
    def test_evaluate_and_promote_workflow(self, auto_generator, sample_ohlc_data):
        ...
        with patch.object(auto_generator, '_calculate_basic_metrics') as mock_metrics, \
            ...
            # Run evaluation
            result = auto_generator.evaluate_and_promote_features(features, sample_ohlc_data)
    ...
    def test_feature_data_integrity(self, auto_generator, sample_ohlc_data):
        ...
        assert all(-10 <= val <= 10 for val in valid_values), f"Values out of reasonable range: {valid_values}"
    ...
    def test_promotion_rules_edge_cases(self, auto_generator):
        ...
        boundary_cases = [
            # Just meets criteria
            {'sharpe_ratio': 0.3, 'win_rate': 0.55, 'max_drawdown': -0.15, 'expected': True},
            # Just below criteria
            {'sharpe_ratio': 0.29, 'win_rate': 0.54, 'max_drawdown': -0.16, 'expected': False},
            # Mixed (good sharpe, bad win rate)
            {'sharpe_ratio': 0.5, 'win_rate': 0.4, 'max_drawdown': -0.1, 'expected': False},
            # All criteria met except max_drawdown
            {'sharpe_ratio': 0.5, 'win_rate': 0.6, 'max_drawdown': -0.05, 'expected': True},
        ]
        ...
        for case in boundary_cases:
            ...
            assert auto_generator._check_promotion_criteria(formatted_case) == expected, f"Failed for case: {case}"
    ...
    def test_naming_convention_enforcement(self, auto_generator, sample_ohlc_data):
        ...
        for feature_name in ema_features.keys():
            assert feature_name.startswith("auto_"), f"Feature name {feature_name} doesn't start with 'auto_'"
    ...
    def test_max_combinations_limit(self, auto_generator, sample_ohlc_data):
        ...
        features = auto_generator.generate_ema_cross_features(
            sample_ohlc_data,
            fast_periods=[5, 8, 12, 20],
            slow_periods=[25, 30, 40, 50]
        )
