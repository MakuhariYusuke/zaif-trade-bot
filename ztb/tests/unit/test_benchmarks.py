...
class TestPerformanceBenchmarks:
    ...
    def test_memory_usage_during_evaluation(self, medium_ohlc_data):
        ...
        try:
            result = evaluate_feature_class(MockFeature, medium_ohlc_data, 'MockFeature')
    ...
    def test_concurrent_feature_evaluation(self, medium_ohlc_data):
        ...
        def evaluate_single_feature(feature_name):
            try:
                result = evaluate_feature_class(MockFeature, medium_ohlc_data, feature_name)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [executor.submit(evaluate_single_feature, name) for name in feature_names]
            concurrent.futures.wait(futures, timeout=30)  # 30 second timeout

        elapsed = time.time() - start_time

        # Should complete within reasonable time
        assert elapsed < 30, f"Concurrent evaluation took too long: {elapsed:.2f} seconds"
    ...
    def test_regression_detection(self, medium_ohlc_data):
        ...
        for _ in range(3):
            start = time.time()
            features = generator.generate_ema_cross_features(medium_ohlc_data, [5], [20])
            end = time.time()
        ...
        assert current_time <= avg_time * 2, f"Performance regression: {current_time:.3f}s vs baseline {avg_time:.3f}s"
        assert len(result) == 1
    ...
    def test_scalability_with_data_size(self):
        ...
        if len(times) >= 2:
            ...
            assert scaling_factor < 10, f"Poor scaling: {scaling_factor:.2f}x slower for 20x more data"
    ...
    def test_indicators_performance_comparison(self, medium_ohlc_data):
        ...
        assert kalman_time < 5, f"Kalman calculation took too long: {kalman_time:.2f} seconds"
    ...
    def test_benchmark_result_validation(self, medium_ohlc_data):
        ...
        for name, data in features.items():
            ...
            if len(values) > 0:
                assert np.all(np.abs(values) <= 2), f"Unreasonable values in {name}: {values[:5]}"