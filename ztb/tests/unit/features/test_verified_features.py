...
def check_category_requirements(verified_features: List[str], config_path: str = "config/feature_params.yaml") -> bool:
    ...
    for category, required_count in category_reqs.items():
        ...
        if actual_count < required_count:
            print(f"❌ Category '{category}': {actual_count}/{required_count} (insufficient)")
            all_met = False
        else:
...
def check_performance_regression(verified_features: List[str], current_benchmarks: Dict[str, float], 
                               config_path: str = "config/feature_params.yaml") -> List[str]:
    ...
    history_path = config.get('ci', {}).get('bench_history_path', "reports/performance/feature_bench_history.json")
    ...
    for feature_name in verified_features:
        ...
        if abs(ratio - 1.0) > tolerance_ratio:
            ...
            print(f"❌ {feature_name}: {change_pct:.1f}% {direction} ({current_time:.3f}s vs {historical_time:.3f}s)")
        ...
        for feature_name in verified_features:
            try:
                ...
                if not result1.columns.equals(result2.columns):
                    print(f"✗ {feature_name}: Inconsistent column structure across scales")
                    failed_features.append(feature_name)
                    continue
                    
                # For price-based features, results should scale appropriately
                if any(col in str(feature_name).lower() for col in ['price', 'close', 'high', 'low']):
                    # Price-based features should scale
                    if not np.allclose(result1.values / scale, result2.values, rtol=1e-10, atol=1e-10, equal_nan=True):
                        print(f"✗ {feature_name}: Does not scale properly with price changes")
                        failed_features.append(feature_name)
...
def test_edge_cases(verified_features: List[str], registry: FeatureRegistryType) -> List[str]:
    ...
    nan_data = pd.DataFrame({
        'open': [np.nan if i % 5 == 0 else 100 + np.random.randn() for i in range(50)],
        'high': [np.nan if i % 5 == 0 else 105 + np.random.randn() for i in range(50)],
        'low': [np.nan if i % 5 == 0 else 95 + np.random.randn() for i in range(50)],
        'close': [np.nan if i % 5 == 0 else 100 + np.random.randn() for i in range(50)],
        'volume': [np.nan if i % 5 == 0 else np.random.randint(1000, 10000) for i in range(50)]
    }, index=dates)
    ...
    for feature_name in verified_features:
        try:
            ...
            if execution_time > max_time_per_feature:
                print(f"✗ {feature_name}: Too slow ({execution_time:.2f}s > {max_time_per_feature}s)")
                failed_features.append(feature_name)
            else:
