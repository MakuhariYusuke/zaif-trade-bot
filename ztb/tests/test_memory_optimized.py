...
def main():
    ...
    policy_kwargs = {"net_arch": {"pi": [32, 32], "vf": [32, 32]}}  # Tiny network
    ...
    logger.info("  Expected iterations: 128 / 64 = 2")
    ...
    logger.info("  Expected: 128")
    logger.info(f"  Match: {'YES' if model.num_timesteps == 128 else 'NO'}")
    logger.info("=" * 80)

    if model.num_timesteps != 128:
        logger.error(
            f"❌ BUG CONFIRMED: Expected 128 timesteps, got {model.num_timesteps}"
        )
        return False
