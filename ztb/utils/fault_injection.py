...
class FaultContext:
    ...
    def __init__(self, injector: FaultInjector, config: FaultInjectionConfig) -> None:
        """Initialize fault context."""
        self.injector = injector
        self.config = config
        self.correlation_id = f"fault_{config.name}_{int(time.time())}"

    async def __aenter__(self) -> "FaultContext":
...
def _register_default_handlers(injector: FaultInjector) -> None:
    ...
    async def ws_disconnect_handler(config: FaultInjectionConfig) -> None:
    ...
    async def corrupted_checkpoint_handler(config: FaultInjectionConfig) -> None:
        """Simulate corrupted checkpoint."""
        logger.info("Injecting corrupted checkpoint")
        # This would modify checkpoint data

    async def stream_throttle_handler(config: FaultInjectionConfig) -> None:
...
) -> AsyncGenerator[FaultContext, None]:
    """
