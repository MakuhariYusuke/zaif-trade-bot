...
class AdaptiveThresholdManager:
    ...
    def _load_historical_data(self) -> None:
        ...
        coverage_data = CoverageValidator.load_coverage_files(
            str(self.historical_data_path)
        )
        ...
        for event in coverage_data.get("events", []):
            if (
                event.get("type") == "feature_promoted"
    ...
    def get_adaptive_threshold(
        self, metric_name: str, percentile: float = 20.0
    ) -> float:
        ...
        if metric_name not in self.thresholds_cache:
            if not self.historical_successes:
                ...
                defaults = {"sharpe_ratio": 0.3, "max_drawdown": -0.2, "win_rate": 0.5}
                ...
                for feature_data in self.historical_successes.values():
                    for criterion in feature_data.get("criterion_details", []):
                        if criterion.get("name") == metric_name and criterion.get(
                            "passed"
                        ):
    ...
    def get_adaptive_gates(self) -> Dict[str, float]:
        ...
        for event in coverage_data.get("events", []):
            ...
            if event.get("details", {}).get("dataset") == "synthetic":
                continue
            if (
                event.get("type") == "feature_tested"
                and event.get("to_status") == "discarded"
            ):
                feature = event.get("feature")
        ...
        if failed_features:
            ...
            nan_rates = [
                f.get("nan_rate", 0)
                for f in failed_features.values()
