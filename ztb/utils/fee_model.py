...
class TieredFeeModel(FeeModel):
    ...
    def __init__(self, tiers: Optional[Dict[str, List[Tuple[float, float]]]] = None):
        ...
        if tiers is None:
            ...
            tiers = {
                "buy_tiers": [
                    (0.0, 0.001),  # 0+ volume: 0.1%
                    (10000.0, 0.0008),  # 10k+ volume: 0.08%
                    (50000.0, 0.0005),  # 50k+ volume: 0.05%
                    (100000.0, 0.0003),  # 100k+ volume: 0.03%
                ],
                "sell_tiers": [
                    (0.0, 0.001),
                    (10000.0, 0.0008),
...
class ExchangeFeeModel(FeeModel):
    ...
    def get_fee_rate(self, trade_type: str = "buy") -> float:
        ...
        exchange_rates = self.exchange_fees.get(
            self.current_exchange, {"buy": 0.001, "sell": 0.001}
        )
...
class FeeModelFactory:
    ...
    def create_fee_model(
        ...
        if model_type.lower() == "fixed":
            ...
            return TieredFeeModel(tiers=config.get("tiers"))

        elif model_type.lower() == "exchange":
            return ExchangeFeeModel(exchange_fees=config.get("exchange_fees"))
