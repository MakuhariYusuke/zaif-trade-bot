...
class ProcessPriorityManager:
    ...
    def set_process_priority(self, model_type: str) -> Tuple[str, int]:
        ...
        if model_type.lower() not in valid_model_types:
            raise ValueError(
                f"Invalid model_type '{model_type}'. Accepted values are: {valid_model_types}"
            )
            ...
            if hasattr(os, "nice"):
                ...
                if current_nice != 0:
                    try:
                        ...
                        logger.warning(
                            "Insufficient permissions to decrease nice value (increase priority). Run as root/administrator if needed."
                        )
                else:
                    logger.info("Process priority already at default")
            else:
                logger.info(
                    "Resetting process priority: not supported on this platform"
                )
        except OSError as e:
