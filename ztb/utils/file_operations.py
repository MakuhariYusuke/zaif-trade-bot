...
class BatchFileOperator:
    ...
    def execute(self) -> Dict:
        ...
        logger.info(f"{'[DRY-RUN] ' if self.dry_run else ''}操作を実行: {len(self.operations)}件")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._execute_single, op): op
...
def scan_files_fast(
    ...
    Returns:
        マッチしたファイルのリスト
    """
    exclude_dirs = exclude_dirs or ['.git', '.venv', 'venv', 'node_modules', '__pycache__']
    results = []
