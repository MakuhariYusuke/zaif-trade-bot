...
class DriftMonitor:
    ...
    def __init__(
        self,
        baseline_path: str = "data/baseline_stats.json",
    ...
    def update_baseline(
        self, features_df: pd.DataFrame, pnl_series: Optional[pd.Series] = None
    ) -> None:
    ...
    def _kl_divergence(
        self, p: np.ndarray, q: np.ndarray, epsilon: float = 1e-10
    ) -> float:
    ...
    def detect_data_drift(self, current_features_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for feature_name, baseline_stats in self.baseline_stats.get(
            "features", {}
        ).items():
            ...
            if baseline_hist_bins:
                ...
                current_hist, _ = np.histogram(
                    current_series, bins=baseline_hist_bins, density=True
                )
                baseline_hist = np.ones(len(current_hist)) / len(
                    current_hist
                )  # Uniform baseline
        ...
        for stat_name, weight in weights.items():
            if stat_name in baseline_pnl and stat_name in current_stats:
                ...
                if baseline_pnl.get("std", 1.0) > 0:
                    normalized_diff = (
                        abs(current_val - baseline_val) / baseline_pnl["std"]
                    )
                else:
    ...
    def check_quality_gates(
        ...
        results = {
            ...
            "model_drift": self.detect_model_drift(pnl_series)
        ...
        results["quality_gates"] = {
            ...
            "pnl_reasonableness": self._check_pnl_reasonableness(pnl_series)
            if pnl_series is not None
            else True,
        }
        ...
        results["quality_gate_pass_rate"] = cast(
            Any, sum(gate_results) / len(gate_results) if gate_results else 0.0
        )
    ...
    def _check_pnl_reasonableness(
        self, pnl: pd.Series, max_reasonable_pnl: float = 1.0
    ) -> bool:
    ...
    def get_drift_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "data_drift_incidents": sum(
                1 for entry in recent_entries if entry["data_drift"]["is_drifted"]
            ),
            "model_drift_incidents": sum(
                1
                for entry in recent_entries
                if entry.get("model_drift") and entry["model_drift"]["is_drifted"]
            ),
            "avg_quality_gate_pass_rate": np.mean(
                [entry["quality_gate_pass_rate"] for entry in recent_entries]
            ),
    ...
    def export_metrics_for_prometheus(self) -> str:
        ...
        if "latest_check" in summary and summary["latest_check"]:
            ...
            if "max_drift" in latest["data_drift"]:
                metrics.append(
                    f"# HELP data_drift_score Maximum KL divergence score for data drift detection"
                )
            ...
            if latest.get("model_drift") and "drift_score" in latest["model_drift"]:
                metrics.append(
                    f"# HELP model_drift_score Drift score for model performance"
                )
                metrics.append(f"# TYPE model_drift_score gauge")
                metrics.append(
                    f"model_drift_score {latest['model_drift']['drift_score']}"
                )

            # Quality gate pass rate
            metrics.append(
                f"# HELP quality_gate_pass_rate Overall quality gate pass rate (0.0-1.0)"
            )
