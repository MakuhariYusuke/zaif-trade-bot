...
def compute_reliability_curve(
    ...
    for i in range(n_bins):
        if bin_counts[i] > 0 and bin_predicted_prob[i] is not None and bin_observed_freq[i] is not None:
...
def compute_full_calibration_report(
    ...
    if action_names is None:
        action_names = ["HOLD", "BUY", "SELL"] if n_actions == 3 else [f"Action_{i}" for i in range(n_actions)]
