...
class FeatureCache:
    ...
    def _setup_compressor(self) -> None:
        ...
        if selected == "zstd" and HAS_ZSTD:
            ...
            self._compress_func = lambda data: zstd.ZstdCompressor(
                level=self.compression_level
            ).compress(data)
            self._decompress_func = lambda data: zstd.ZstdDecompressor().decompress(
                data
            )
        ...
        try:
            ...
            for filename in os.listdir(cache_dir_str):
                if filename.endswith(".pkl.z"):
                    ...
                    if (
                        os.path.isfile(str(f))
                        and current_time - os.path.getmtime(str(f)) > max_age_seconds
                    ):
                        os.unlink(str(f))
    ...
    def _adaptive_size_limit(self) -> None:
        ...
        try:
            ...
            if memory.percent >= 90:
                ...
                logging.warning(
                    f"[CACHE] Emergency shrink: {old_limit}MB → {self.cache_max_mb}MB (mem: {memory.percent:.1f}%)"
                )
                self._evict_lru_files()
        except ImportError:
            pass  # psutil未導入時は無視
    ...
    def monitor_cache_health(self) -> Dict[str, Any]:
        ...
        if size_mb > self.cache_max_mb * 1.2:  # 120%超過
            warnings.append(
                f"Cache oversized: {size_mb:.1f}MB > {self.cache_max_mb}MB limit"
            )
        ...
        return {"size_mb": size_mb, "warnings": warnings, "stats": stats}
