    ...
    def test_load_compressed_data_gzip(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.pkl.gz', delete=False) as tmp:
            try:
                ...
                loaded_df = self.generator.load_compressed_data(tmp.name, compression='gzip')
...
class TestImprovedDataLoader:
    ...
    def test_load_csv_memory_mapped(self):
        ...
        try:
            # Load data
            loaded_df = self.loader.load_csv_memory_mapped(tmp.name, parse_dates=['timestamp'])
    ...
    def test_compute_features_parallel(self):
        ...
        pd.testing.assert_series_equal(result_df['sma_5'], expected_sma, check_names=False)
    ...
    def test_incremental_feature_computation(self):
        ...
        feature_functions = {
            'ma_3': lambda df: simple_ma(df['close'])
        }
    ...
    async def test_load_csv_async(self):
        ...
        try:
            # Load asynchronously
            loaded_df = await self.loader.load_csv_async(tmp.name, parse_dates=['timestamp'])
