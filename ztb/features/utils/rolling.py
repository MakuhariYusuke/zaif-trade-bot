...
def optimize_dataframe_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in int_cols:
        ...
        if min_val >= np.iinfo(np.int32).min and max_val <= np.iinfo(np.int32).max:
            ...
            print(f"Warning: Column '{col}' has values outside int32 range and will remain as int64.")
...
def generate_intermediate_report(step: int, feature_times: Dict[str, float], memory_usage: float, nan_rates: Dict[str, float]) -> None:
    ...
    report = {
        ...
        "summary": {
            ...
            "avg_computation_time_ms": sum(feature_times.values()) * 1000 / len(feature_times) if feature_times else 0,
