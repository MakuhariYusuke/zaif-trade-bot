...
class UnifiedFeatureEngineer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.sac_engineer = SACv427FeatureEngineer(config_path=config_path)
    ...
    def _generate_sac_features(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Generated SAC features")
            return features_df
    ...
    def _generate_generic_features(
        ...
        **kwargs
        ...
        logger.info(f"Generated {len(feature_names)} generic features + {len(bearish_features.columns)} bearish features")
        return features_df
    ...
    def validate_features(self, df: pd.DataFrame, feature_names: List[str]) -> Dict[str, bool]:
        """
        特徴量の妥当性を検証
        ...
        for feature_name in feature_names:
            try:
                if feature_name in df.columns:
                    ...
                    if nan_count > 0:
                        logger.warning(f"Feature {feature_name} has {nan_count} NaN values")
    ...
    def _add_bearish_divergence_features(self, df: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if all(col in df.columns for col in ['MACD', 'close']):
                macd_trend = df['MACD'].rolling(5).mean() - df['MACD'].rolling(10).mean()
                price_trend = (df['close'].rolling(5).mean() - df['close'].rolling(10).mean())
    ...
    def _add_bearish_candlestick_patterns(self, df: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """弱気ローソク足パターンを追加"""
        try:
            # 弱気エングルフィングパターン
            ...
            # 現在の陰線が前の陽線を完全に包む
            bearish_engulfing = (
                (df['open'].shift(1) < df['close'].shift(1)) &  # 前のローソクは陽線
                (df['open'] > df['close']) &  # 現在のローソクは陰線
    ...
    def _add_bearish_momentum_features(self, df: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """弱気モメンタム指標を追加"""
        try:
            # 下降加速 (価格が加速して下降)
            ...
            if 'close' in df.columns:
                ...
                features['Bearish_Volatility_Surge'] = (volatility * trend_direction).fillna(0)
    ...
    def _add_sell_signal_oscillators(self, df: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """売りシグナルオシレーターを追加"""
        try:
            ...
            if 'Stochastic' in df.columns:
                ...
                stoch_trend = df['Stochastic'].rolling(5).mean() - df['Stochastic'].rolling(10).mean()
                price_trend = (df['close'].rolling(5).mean() - df['close'].rolling(10).mean())
            ...
            if 'Williams_R' in df.columns:
                features['WilliamsR_Oversold_Sell'] = np.where(df['Williams_R'] < -20, 1, 0)
    ...
    def _add_bearish_volume_features(self, df: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        """弱気ボリューム指標を追加"""
        try:
...
def get_unified_feature_engineer(config_path: Optional[str] = None) -> UnifiedFeatureEngineer:
    """
    ...
    if _unified_engineer is None:
        _unified_engineer = UnifiedFeatureEngineer(config_path)
