...
from ztb.trading.environment.constants import (
    ...
    FEATURE_NAN_RATE_THRESHOLD,

    FEATURE_OUTLIER_RATE_THRESHOLD,
...
class SACv427FeatureEngineer:
    ...
    def __init__(
        ...
        self.quality_thresholds = {
            ...
            "max_nan_rate": self.config.get("max_nan_rate", FEATURE_NAN_RATE_THRESHOLD),  # 10% max NaN rate

            "min_variance": self.config.get("min_variance", FEATURE_VARIANCE_THRESHOLD),  # Minimum variance threshold

            "max_zero_rate": self.config.get("max_zero_rate", FEATURE_ZERO_RATE_THRESHOLD),  # 99% max zero rate (increased to reduce filtering)

            "max_outlier_rate": self.config.get("max_outlier_rate", FEATURE_OUTLIER_RATE_THRESHOLD),  # 30% max outlier rate

            "max_correlation": self.config.get("max_correlation", FEATURE_CORRELATION_THRESHOLD),  # 95% max correlation
    ...
    def generate_v427_features(
        ...
        skip_quality_filtering: bool = False,
        ...
        # NOTE:
        # Some feature sets (e.g. `high_quality`) exclude raw OHLCV columns because they are
        # high-correlation inputs for the agent. However, we still need OHLCV for internal
        # feature engineering (multi-timeframe resampling, wick ratios, ATR, etc.).
        protected_cols = {"open", "high", "low", "close", "volume", "timestamp"}
        drop_cols = [
            col
            for col in excluded_features
            if col in numeric_df.columns and col not in protected_cols
        ]
        if drop_cols:
        ...
        # logger.info(f"DEBUG: features_df index type: {type(features_df.index)}")

        # logger.info(f"DEBUG: features_df index sample: {features_df.index[:5]}")
        ...
        mtf_features = self._generate_mtf_features(features_df)

        features_df = pd.concat([features_df, mtf_features], axis=1)

        logger.info(f"Added MTF features: {len(mtf_features.columns)}")
        ...
        logger.info(
            ...
            f"Generated {total_features} additional features before quality filtering"
        ...
        features_df = self._quality_filter_features(features_df, skip_quality_filtering=skip_quality_filtering)

        quality_elapsed = time.perf_counter() - quality_start

        logger.info(f"Quality filter duration: {quality_elapsed:.3f}s (skip={skip_quality_filtering})")
        ...
        # Ensure we have at least 176 additional features (182 total with 6 base features)

        min_features = 176

        if filtered_total_features < min_features:

            logger.warning(

                f"Only {filtered_total_features} features after quality filtering, padding to reach {min_features}+"

            )

            # Add synthetic features if needed

            num_padding = min_features - filtered_total_features

            logger.info(f"Generating {num_padding} padding features safely...")
        ...
        logger.info(f"Total v427 feature generation completed in {total_elapsed:.3f}s; columns={len(features_df.columns)}")

        try:

            features_df.attrs["feature_generation_time_s"] = total_elapsed
    ...
    def generate_v437_features(
        ...
        # DEBUG: Check index after normalization

        logger.info(f"DEBUG: features_df index type after normalization: {type(features_df.index)}")

        logger.info(f"DEBUG: features_df index sample after normalization: {features_df.index[:5]}")
    ...
    def _generate_mtf_features(self, df: pd.DataFrame) -> pd.DataFrame:

        """

        Generate Multi-Timeframe (MTF) features by resampling 1m data.
        ...
        Args:
            ...
            df: Input dataframe (assumed 1m timeframe)

            

        Returns:

            DataFrame with MTF features aligned to the original index

        """

        mtf_features = pd.DataFrame(index=df.index)
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            ...
            # Try to convert if 'timestamp' column exists
            ...
            if 'timestamp' in df.columns:
                ...
                temp_df = df.set_index(align_index)
 
            else:
 
                # Cannot resample without datetime index
        ...
        for tf_name, tf_code in timeframes.items():
            ...
            try:
                ...
                # Use label='right', closed='right' to avoid look-ahead bias

                resampled = temp_df.resample(tf_code, label='right', closed='right').agg({
                ...
                resampled[f'rsi_{tf_name}'] = 100 - (100 / (1 + rs))

                

                # MACD

                exp1 = resampled['close'].ewm(span=12, adjust=False).mean()

                exp2 = resampled['close'].ewm(span=26, adjust=False).mean()

                macd = exp1 - exp2

                signal = macd.ewm(span=9, adjust=False).mean()

                resampled[f'macd_{tf_name}'] = macd

                resampled[f'macd_signal_{tf_name}'] = signal

                resampled[f'macd_hist_{tf_name}'] = macd - signal

                

                # Bollinger Bands

                sma = resampled['close'].rolling(window=20).mean()

                std = resampled['close'].rolling(window=20).std()

                resampled[f'bb_upper_{tf_name}'] = sma + (std * 2)

                resampled[f'bb_lower_{tf_name}'] = sma - (std * 2)

                resampled[f'bb_width_{tf_name}'] = (resampled[f'bb_upper_{tf_name}'] - resampled[f'bb_lower_{tf_name}']) / sma

                

                # Select only the calculated features

                cols_to_keep = [c for c in resampled.columns if any(x in c for x in ['rsi_', 'macd_', 'bb_'])]

                features_subset = resampled[cols_to_keep]

                

                # DEBUG: Print tail of features_subset

                # logger.info(f"DEBUG: {tf_name} features tail:\n{features_subset.tail()}")

                

                # Reindex back to original timeframe (forward fill)
 
                # This aligns the higher timeframe features with the 1m data
 
                aligned_features = features_subset.reindex(align_index, method="ffill")
                aligned_features.index = df.index

                

                # DEBUG: Check for NaNs in aligned features

                # nan_count = aligned_features.isna().sum().sum()

                # logger.info(f"DEBUG: {tf_name} aligned NaNs: {nan_count}")

                

                # Fill remaining NaNs (at the beginning) with 0 or backfill
                ...
                mtf_features = pd.concat([mtf_features, aligned_features], axis=1)

                

            except Exception as e:

                logger.warning(f"Error generating {tf_name} features: {e}")

                continue

        

        # DEBUG: Check return index

        # logger.info(f"DEBUG: mtf_features return index type: {type(mtf_features.index)}")

        

        return mtf_features.fillna(0)
    ...
    def _generate_correlation_features_optimized(
        ...
        market_returns = market_proxy.pct_change(fill_method=None)
    ...
    def _generate_padding_features_simple(
        ...
        for i in range(num_features):
            ...
            if i % 4 == 0:
                ...
                padding_data[f"padding_noise_{i}"] = np.random.normal(0, 0.1, len(df))

            elif i % 4 == 1:

                # Sine wave patterns

                padding_data[f"padding_sine_{i}"] = np.sin(

                    np.arange(len(df)) * 0.01 * (i + 1)
                ...
                padding_data[f"padding_cosine_{i}"] = np.cos(
                ...
                padding_data[f"padding_trend_{i}"] = (

                    np.arange(len(df)) * 0.001 * (i + 1)

                )

        padding_features = pd.DataFrame(padding_data)

        logger.info(f"Generated {num_features} padding features")

        return padding_features

    def _quality_filter_features(self, features: pd.DataFrame, skip_quality_filtering: bool = False) -> pd.DataFrame:
        ...
        Args:
            ...
            skip_quality_filtering: Skip quality filtering if True
        ...
        if skip_quality_filtering:

            logger.info("Skipping quality filtering as requested")
        ...
        for col in features.columns:
            ...
            if nan_rate > self.quality_thresholds["max_nan_rate"]:
                ...
                logger.warning(
                    ...
                    f"Removing feature {col}: NaN rate {nan_rate:.3f} > {self.quality_thresholds['max_nan_rate']}"
            ...
            try:
                ...
                var_value = series.fillna(0).var()

                if (

                    pd.isna(var_value)

                    or var_value <= self.quality_thresholds["min_variance"]

                ):

                    logger.warning(

                        f"Removing feature {col}: variance {var_value} <= {self.quality_thresholds['min_variance']}"
            ...
            if zero_rate > self.quality_thresholds["max_zero_rate"]:
                ...
                logger.warning(
                    ...
                    f"Removing feature {col}: zero rate {zero_rate:.3f} > {self.quality_thresholds['max_zero_rate']}"
            ...
            if len(series.dropna()) > 10:  # Need minimum data points
                ...
                if outlier_rate > self.quality_thresholds["max_outlier_rate"]:
                    ...
                    logger.warning(
                        ...
                        f"Removing feature {col}: outlier rate {outlier_rate:.3f} > {self.quality_thresholds['max_outlier_rate']}"
        ...
        if removed_features:
            ...
            logger.info(
                ...
                f"Quality filtering removed {len(removed_features)} harmful features: {removed_features}"

            )

        logger.info(

            f"Quality filtering kept {len(filtered_features.columns)} features out of {len(features.columns)}"
    ...
    def _generate_enhanced_statistical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        tr3 = (low - close).abs()
        tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1)
        features["ATR_simplified"] = tr.rolling(14).mean()
...
def create_v437_feature_set(
    ...
    data_path: str, output_path: str | None = None, feature_set: str = "full"
...
def create_v427_feature_set(
    ...
    data_path: str, output_path: str | None = None
