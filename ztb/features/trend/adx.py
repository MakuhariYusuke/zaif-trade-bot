...
def compute_adx(df: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    dm_plus_smooth = pd.Series(dm_plus, index=df.index).ewm(span=period, adjust=False).mean()
    dm_minus_smooth = pd.Series(dm_minus, index=df.index).ewm(span=period, adjust=False).mean()
