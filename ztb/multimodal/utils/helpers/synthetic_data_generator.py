        ...
        for i in range(num_articles):
            ...
            article = {
                ...
                "category": np.random.choice(
                    ["economy", "markets", "companies", "policy"]
                ),
            }
...
class SyntheticEconomicDataGenerator:
    ...
    def generate_economic_data(
        self, start_date: datetime, end_date: datetime, frequency: str = "M"
    ) -> pd.DataFrame:
        """
        指定期間の経済指標データを生成
        ...
        for col in df.columns:
            if "index" in col:
                ...
                df[col] = savgol_filter(
                    df[col], window_length=min(31, len(df)), polyorder=2
                )
...
class GANBasedDataGenerator(nn.Module):
    ...
    def __init__(
        self, input_dim: int = 10, output_dim: int = 20, hidden_dim: int = 128
    ):
        super().__init__()
...
class MultiModalSyntheticDataset:
    ...
    def generate_comprehensive_dataset(
        ...
        # 経済指標データ生成
        economic_data = self.economic_gen.generate_economic_data(
        ...
        daily_sentiment = (
            news_df.groupby(news_df["published_at"].str[:10])["sentiment_score"]
            .agg(["mean", "std", "count"])
            .reset_index()
        )
        ...
        logger.info(
            f"Generated dataset with {len(all_news)} news articles and {len(economic_data)} economic data points"
        )
...
if __name__ == "__main__":
    ...
    dataset = generator.generate_comprehensive_dataset(
        start_date=start_date, end_date=end_date, news_per_day=3
    )
