...
from virtual_portfolio_manager import VirtualPortfolioManager, PortfolioMetrics
...
class PaperTradingManager:
    ...
    def __init__(self,
        ...
        self.validation_callbacks: List[Callable[[ValidationReport], Awaitable[None]]] = []
    ...
    async def start_session(self, session_id: Optional[str] = None) -> str:
        ...
        Returns:
            str: セッションID
        """
        if self.current_session and self.current_session.state == PaperTradingState.RUNNING:
    ...
    async def stop_session(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Paper Trading session completed: {self.current_session.session_id}")
    ...
    async def submit_order(self, order: Order) -> bool:
        ...
        Returns:
            bool: 注文成功フラグ
        """
        if not self.current_session or self.current_session.state != PaperTradingState.RUNNING:
        ...
        if success:
            ...
            # ポートフォリオ価値更新
            self.current_session.portfolio_value = self.portfolio_manager.get_portfolio_value()
    ...
    async def _monitoring_loop(self) -> None:
        """モニタリングループ"""
        while self.current_session and self.current_session.state == PaperTradingState.RUNNING:
            try:
                now = datetime.now()

                # 定期検証チェック
                if (self.current_session.last_validation is None or
                    now - self.current_session.last_validation >= timedelta(hours=self.config.validation_interval_hours)):
                ...
                if self.current_session.last_validation:
                    self.current_session.next_validation = (
                        self.current_session.last_validation + timedelta(hours=self.config.validation_interval_hours)
                    )
    ...
    async def _perform_validation(self) -> None:
        ...
        try:
            ...
            # ポートフォリオ指標取得
            portfolio_metrics = self.portfolio_manager.portfolio_history[-1000:]  # 最新1000件
            ...
            self.logger.info(f"Validation completed. Rating: {report.overall_rating.value}")
    ...
    def get_session_status(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'last_validation': self.current_session.last_validation.isoformat() if self.current_session.last_validation else None,
            'next_validation': self.current_session.next_validation.isoformat() if self.current_session.next_validation else None,
    ...
    def load_session(self, filepath: str) -> bool:
        ...
        try:
            ...
            self.current_session = PaperTradingSession(
                ...
                last_validation=datetime.fromisoformat(session_data['last_validation']) if session_data['last_validation'] else None
            )
    ...
    def add_order_callback(self, callback: Callable[[Order, bool], Awaitable[None]]) -> None:
        """
        注文コールバック追加
    ...
    def add_validation_callback(self, callback: Callable[[ValidationReport], Awaitable[None]]) -> None:
        """
        検証コールバック追加
