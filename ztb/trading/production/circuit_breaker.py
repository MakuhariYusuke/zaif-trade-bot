...
class CircuitBreaker:
    ...
    def __init__(self, config: CircuitBreakerConfig):
        ...
        self.state_change_callbacks: List[
            Callable[[CircuitState, CircuitState, str], Awaitable[None]]
        ] = []
        ...
        self.logger.info(
            f"Circuit Breaker '{config.name}' initialized in {self.state.value} state"
        )
    ...
    async def _record_success(self) -> None:
        ...
        self._add_event(
            "success",
            details={"consecutive_successes": self.metrics.consecutive_successes},
        )
    ...
    async def _record_failure(self, failure_type: FailureType, reason: str) -> None:
        ...
        if (
            ...
            await self._change_state(
                CircuitState.OPEN,
                f"Failure threshold exceeded: {self.metrics.consecutive_failures} consecutive failures",
            )

        # 半開状態での失敗
        elif self.state == CircuitState.HALF_OPEN:
            await self._change_state(
                CircuitState.OPEN, f"Failed during recovery attempt: {reason}"
            )
    ...
    async def _change_state(self, new_state: CircuitState, reason: str) -> None:
        ...
        # 状態変更イベント
        self._add_event(
            "state_change", from_state=old_state, to_state=new_state, reason=reason
        )
        ...
        self.logger.warning(
            f"Circuit breaker '{self.config.name}' state changed: {old_state.value} -> {new_state.value} ({reason})"
        )
    ...
    def add_state_change_callback(
        self, callback: Callable[[CircuitState, CircuitState, str], Awaitable[None]]
    ) -> None:
        """
        状態変更コールバック追加
...
class CircuitBreakerRegistry:
    ...
    def get_registry_metrics(self) -> Dict[str, Any]:
        ...
        open_breakers = len(
            [b for b in self.breakers.values() if b.state == CircuitState.OPEN]
        )
        closed_breakers = len(
            [b for b in self.breakers.values() if b.state == CircuitState.CLOSED]
        )
        half_open_breakers = len(
            [b for b in self.breakers.values() if b.state == CircuitState.HALF_OPEN]
        )
        ...
        for name, breaker in self.breakers.items():
            state["breakers"][name] = {
                ...
                "metrics": {
                    ...
                    "last_failure_time": breaker.metrics.last_failure_time.isoformat()
                    if breaker.metrics.last_failure_time
                    else None,
                    "last_success_time": breaker.metrics.last_success_time.isoformat()
    ...
    def load_state(self, filepath: str) -> bool:
        ...
        try:
            ...
            for name, breaker_data in state.get("breakers", {}).items():
                ...
                breaker.last_state_change = datetime.fromisoformat(
                    breaker_data["last_state_change"]
                )
                ...
                breaker.metrics = CircuitBreakerMetrics(
                    ...
                    last_failure_time=datetime.fromisoformat(
                        metrics_data["last_failure_time"]
                    )
                    if metrics_data["last_failure_time"]
                    else None,
                    last_success_time=datetime.fromisoformat(
                        metrics_data["last_success_time"]
                    )
                    if metrics_data["last_success_time"]
                    else None,
                ...
                for e_data in breaker_data.get("events", []):
                    event = CircuitBreakerEvent(
                        ...
                        from_state=CircuitState(e_data["from_state"])
                        if e_data["from_state"]
                        else None,
                        to_state=CircuitState(e_data["to_state"])
                        if e_data["to_state"]
                        else None,
