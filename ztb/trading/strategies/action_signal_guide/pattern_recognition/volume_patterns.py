    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        try:
            ...
            if index >= self.confirmation_period:
                recent_ad = chaikin_ad_series.iloc[index-self.confirmation_period:index+1]
                ad_trend = self._calculate_trend_strength(recent_ad)

                # Check for divergence with price
                price_trend = self._calculate_price_trend(data.iloc[index-self.confirmation_period:index+1])
                ...
                if ad_trend > self.divergence_threshold and price_trend < -self.divergence_threshold:
                    ...
                    return SignalResult(
                        ...
                        metadata={"ad_trend": ad_trend, "price_trend": price_trend, "divergence": "bullish"},
                        validity_period=10,
                        risk_level="medium"
                    )
                elif ad_trend < -self.divergence_threshold and price_trend > self.divergence_threshold:
                    ...
                    return SignalResult(
                        ...
                        metadata={"ad_trend": ad_trend, "price_trend": price_trend, "divergence": "bearish"},
                        validity_period=10,
                        risk_level="medium"
                    )
