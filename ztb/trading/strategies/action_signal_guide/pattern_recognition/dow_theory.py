...
class DowTheoryRecognizer(PatternRecognizer):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.primary_trend_period = self.config.get('primary_trend_period', 50)  # Primary trend
        self.secondary_trend_period = self.config.get('secondary_trend_period', 20)  # Secondary trend
        self.short_trend_period = self.config.get('short_trend_period', 10)  # Short trend

        # Confirmation thresholds
        self.trend_confirmation_threshold = self.config.get('trend_confirmation_threshold', 0.005)  # 0.5%
        ...
        self.volatility_threshold = self.config.get('volatility_threshold', 0.1)  # 10% width threshold
    ...
    def _analyze_primary_trend(self, data: pd.DataFrame, index: int) -> Dict[str, Any]:
        ...
        if self.use_supertrend and len(window) >= 15:  # Need minimum data for SuperTrend
            try:
                st_direction = compute_supertrend_direction(window)
                supertrend_direction = st_direction.iloc[-1] if not st_direction.empty else 0
            except Exception:
                supertrend_direction = 0

        # Combine slope analysis with SuperTrend confirmation
        slope_direction = 1 if normalized_slope > self.trend_confirmation_threshold else \
                         -1 if normalized_slope < -self.trend_confirmation_threshold else 0
        ...
        return {
            ...
            'strength': abs(normalized_slope) * strength_multiplier * volatility_multiplier,
    ...
    def _analyze_secondary_trend(self, data: pd.DataFrame, index: int) -> Dict[str, Any]:
        ...
        direction = 1 if normalized_slope > self.trend_confirmation_threshold else \
                   -1 if normalized_slope < -self.trend_confirmation_threshold else 0
    ...
    def _check_trend_confirmation(
        ...
        if (primary['direction'] == 1 and
            secondary['direction'] == 1 and
            short['direction'] == 1):
            ...
            strength = min(1.0, (primary['strength'] + secondary['strength'] + short['strength']) / 3)
            return {
        ...
        divergence_signal = self._check_divergence_signals(primary, secondary, short, data, index)
        if divergence_signal:
            return divergence_signal

        return None

    def _check_volume_confirmation(self, data: pd.DataFrame, index: int, direction: int) -> bool:
    ...
    def _check_reversal_signals(
        ...
        if abs(primary['slope']) > self.reversal_threshold:
            ...
            if ((primary['direction'] == 1 and secondary['direction'] <= 0 and short['direction'] <= 0) or
                (primary['direction'] == -1 and secondary['direction'] >= 0 and short['direction'] >= 0)):

                direction = ACTION_SELL if primary['direction'] == 1 else ACTION_BUY
                strength = min(0.8, abs(primary['slope']))
                trend_type = "bullish" if direction == ACTION_BUY else "bearish"
    ...
    def _check_divergence_signals(
        ...
        if (primary['direction'] == 1 and short['direction'] == -1 and
            abs(short['slope']) > self.trend_confirmation_threshold * 2):
        ...
        elif (primary['direction'] == -1 and short['direction'] == 1 and
              abs(short['slope']) > self.trend_confirmation_threshold * 2):
