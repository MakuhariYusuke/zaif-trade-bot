    ...
    def recognize(
        ...
        try:
            ...
            if (
                ...
                and current_d >= self.overbought_level
            ):
                # Overbought - potential sell signal
                ...
                # Oversold - potential buy signal
                strength = min(
                    (80 - current_k + 80 - current_d) / 160, 1.0
                )  # Normalize strength
                return SignalResult(
                ...
                if prev_k <= prev_d:
                    return SignalResult(
                        ...
                        strength=0.6,
                        direction=1.0,  # Buy signal
                        description="Stochastic bullish crossover: %K crosses above %D",
                        metadata={
                            "stoch_k": current_k,
                            "stoch_d": current_d,
                            "crossover": "bullish",
                ...
                if prev_k >= prev_d:
                    return SignalResult(
                        ...
                        strength=0.6,
                        direction=-1.0,  # Sell signal
                        description="Stochastic bearish crossover: %K crosses below %D",
                        metadata={
                            "stoch_k": current_k,
                            "stoch_d": current_d,
                            "crossover": "bearish",
    ...
    def recognize(
        ...
        try:
            ...
            if current_r >= self.overbought_level:
                ...
                strength = min(
                    current_r / 20, 1.0
                )  # Normalize strength (higher %R = stronger signal)
                return SignalResult(
                ...
                # Oversold - potential buy signal
                strength = min(
                    abs(current_r) / 80, 1.0
                )  # Normalize strength (lower %R = stronger signal)
                return SignalResult(
