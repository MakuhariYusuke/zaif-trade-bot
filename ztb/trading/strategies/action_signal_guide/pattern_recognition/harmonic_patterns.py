...
class HarmonicAnalyzer:
    ...
    @staticmethod
    def calculate_fibonacci_ratio(point1: float, point2: float, 
    ...
    def find_harmonic_pattern(self, data: pd.DataFrame, pattern_type: str, 
                            start_idx: int, tolerance: float = 0.05) -> Optional[Dict]:
        ...
        # Ensure window_data is DataFrame
        ...
        for i in range(len(pivots) - 4):
            ...
            if HarmonicAnalyzer._validate_harmonic_ratios(x.price, a.price, b.price, 
                                                        c.price, d.price, ratios, tolerance):
                ...
                return {
                    ...
                    'points': [x, a, b, c, d],
                    'completion_index': window_data.index[d.index],
                    'completion_price': completion_price,
                    'target_price': target_price,
                    'direction': direction,
                    'strength': HarmonicAnalyzer._calculate_pattern_strength(ratios, tolerance, pattern_type),
                    'ratios': ratios
                }
        
        return None
    
    def _get_pivot_points(self, data: pd.DataFrame, min_distance: int = 3) -> List[HarmonicPoint]:
        ...
        end_idx = data.index[-1] if hasattr(data.index, '__getitem__') else len(data) - 1
        ...
        for i in range(2, len(data) - 2):
            ...
            if (highs.iloc[i] > highs.iloc[i-1] and highs.iloc[i] > highs.iloc[i-2] and
                ...
                if not pivots or (i - pivots[-1].index) >= min_distance:
                    pivots.append(HarmonicPoint(i, highs.iloc[i], 'H'))
            
            # Pivot low
            elif (lows.iloc[i] < lows.iloc[i-1] and lows.iloc[i] < lows.iloc[i-2] and
    ...
    @staticmethod
    def _validate_harmonic_ratios(x: float, a: float, b: float, c: float, d: float, 
                                ratios: Dict[str, float], tolerance: float) -> bool:
    ...
    @staticmethod
    def _calculate_pattern_target(x: float, a: float, b: float, c: float, d: float, 
    ...
    @staticmethod
    def _calculate_pattern_strength(ratios: Dict[str, float], tolerance: float, pattern_type: str = "") -> float:
...
class GartleyRecognizer(PatternRecognizer):
    ...
    def __init__(self, lookback_period: int = 60, tolerance: float = 0.05):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        for start_idx in range(min(len(data) - 5, index - 4), max(0, index - self.lookback_period) - 1, -1):
...
class ButterflyRecognizer(PatternRecognizer):
    ...
    def __init__(self, lookback_period: int = 60, tolerance: float = 0.05) -> None:
        self.lookback_period = lookback_period
        self.tolerance = tolerance
        self.harmonic_analyzer = HarmonicAnalyzer()
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        for start_idx in range(min(len(data) - 5, index - 4), max(0, index - self.lookback_period) - 1, -1):
            ...
            if pattern and abs(pattern['completion_index'] - index) <= 1:
                ...
                signal_type = "butterfly_bullish" if direction == 1 else "butterfly_bearish"
