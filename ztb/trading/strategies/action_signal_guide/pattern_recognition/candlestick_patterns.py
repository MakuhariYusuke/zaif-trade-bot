...
from .base import SignalResult, CandlestickPatternRecognizer
...
class SakataFiveMethodsRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        if not (self._is_small_candle(candles[4]) and
                self._is_large_candle(candles[3]) and
                self._is_small_candle(candles[2]) and
                self._is_large_candle(candles[1]) and
                self._is_small_candle(candles[0])):
            return None
            
        # All large candles should be bullish
        if not (self.is_bullish_candle(candles[3]) and self.is_bullish_candle(candles[1])):
            return None
...
class MorningStarRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        if avg_body_size == 0:
            return None
        if not (self.is_bearish_candle(first) and 
                self.calculate_body_size(data, index-2) > avg_body_size):
            return None
        if not (self.is_bearish_candle(first) and 
                self.calculate_body_size(data, index-2) > avg_body_size):
            return None
...
class EveningStarRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        if not (self.is_bullish_candle(first) and 
                self.calculate_body_size(data, index-2) > self._get_average_body_size(data, index, 10)):
            return None
...
class HammerRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        if not (lower_shadow >= 2 * body_size and
                upper_shadow <= body_size and
                candle['close'] > (total_range * 2/3 + candle['low'])):
            return None
...
class HangingManRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        if not (lower_shadow >= 2 * body_size and
                upper_shadow <= body_size and
                candle['open'] > (total_range * 2/3 + candle['low'])):
            return None
...
class RisingThreeMethodsRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        if not (self._is_large_candle(candles[4]) and
                self._is_small_candle(candles[3]) and
                self._is_small_candle(candles[2]) and
                self._is_small_candle(candles[1]) and
                self._is_large_candle(candles[0])):
            return None
            
        # First and last candles must be bullish
        if not (self.is_bullish_candle(candles[4]) and self.is_bullish_candle(candles[0])):
            return None
...
class BullishEngulfingRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        return SignalResult(
            ...
            metadata={"pattern": "bullish_engulfing", "confidence": strength, "engulfing_ratio": engulfing_ratio}
        )
...
class BearishEngulfingRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        return SignalResult(
            ...
            metadata={"pattern": "bearish_engulfing", "confidence": strength, "engulfing_ratio": engulfing_ratio}
        )
...
class PiercingPatternRecognizer(CandlestickPatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        return SignalResult(
            ...
            metadata={"pattern": "piercing_pattern", "confidence": strength, "penetration": penetration}
        )