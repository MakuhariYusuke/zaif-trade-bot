...
class GannAnalyzer:
    ...
    @staticmethod
    def calculate_gann_angles(pivot_price: float, pivot_time: int, 
                            time_range: int = 100) -> Dict[float, np.ndarray]:
        ...
        for angle_deg in GannAnalyzer.GANN_ANGLES:
            ...
            time_diff = np.arange(time_range) - pivot_time
            price_diff = time_diff * price_per_time * (pivot_price * 0.01)  # Scale factor
    ...
    @staticmethod
    def find_gann_time_clusters(data: pd.DataFrame, index: int, 
        ...
        if len(pivot_highs) >= 2:
            intervals = [pivot_highs[i+1][0] - pivot_highs[i][0] for i in range(len(pivot_highs)-1)]
        ...
        if len(pivot_lows) >= 2:
            intervals = [pivot_lows[i+1][0] - pivot_lows[i][0] for i in range(len(pivot_lows)-1)]
            if len(set(intervals)) == 1:  # All intervals equal
                return {
        ...
        for angle_deg, angle_points in angles.items():
            if current_time_offset < len(angle_points):
                ...
                if abs(current_price - angle_price) <= tolerance:
                    ...
                    signal_type = "gann_angle_support" if direction == 1 else "gann_angle_resistance"
...
class GannSquareRecognizer(PatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        square_levels = self.gann_analyzer.calculate_gann_square(recent_high, recent_low)
        ...
        for level_name, level_price in square_levels.items():
            ...
            if abs(current_price - level_price) <= tolerance:
                ...
                signal_type = "gann_square_support" if direction == 1 else "gann_square_resistance"
...
class GannTimeClusterRecognizer(PatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        if time_cluster:
            ...
            signal_type = "gann_time_cluster_support" if direction == 1 else "gann_time_cluster_resistance"
