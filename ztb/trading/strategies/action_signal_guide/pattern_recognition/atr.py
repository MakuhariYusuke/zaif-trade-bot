...
class ATRPatternRecognizer(PatternRecognizer):
    ...
    def recognize(self, data: pd.DataFrame, index: int = -1) -> Optional[SignalResult]:
        ...
        current_atr = atr_values.iloc[index] if index < len(atr_values) else atr_values.iloc[-1]
    ...
    def _analyze_breakout(self, data: pd.DataFrame, current_atr: float, avg_atr: float, index: int) -> Optional[SignalResult]:
        """
    ...
    def _analyze_trend_strength(self, data: pd.DataFrame, atr_values: pd.Series, index: int) -> Optional[SignalResult]:
        """
