...
from ..types import MultiTimeframeData, MultiTimeframeAnalysis, AnalysisResult
...
class MultiTimeframeAnalyzer:
        ...
        for tf, tf_data in multi_timeframe_data.items():
            ...
            if tf != primary_timeframe and primary_timeframe in multi_timeframe_data:
                alignment_score = self._calculate_alignment_score(
                    df, multi_timeframe_data[primary_timeframe]['data'], tf, primary_timeframe
                )
                analysis['alignment_score'] = alignment_score
                analysis['is_aligned'] = alignment_score > self.alignment_threshold
            else:
                analysis['alignment_score'] = 1.0  # Primary timeframe aligns with itself
    ...
    def get_hierarchical_signal_strength(
        ...
        short_consensus = self._calculate_level_consensus(analysis_results, short_term, signal_type)
        medium_consensus = self._calculate_level_consensus(analysis_results, medium_term, signal_type)
        long_consensus = self._calculate_level_consensus(analysis_results, long_term, signal_type)
        
        # Hierarchical weighting: long-term has higher weight
        hierarchical_strength = (
            short_consensus * 0.2 +
            medium_consensus * 0.3 +
            long_consensus * 0.5
        )
        
        return min(hierarchical_strength, 1.0)

    def _analyze_single_timeframe(self, df: pd.DataFrame, timeframe: str) -> AnalysisResult:
    ...
    def _calculate_volatility(self, df: pd.DataFrame) -> float:
        ...
        closes = df['close'].pct_change().dropna()
        return closes.std() * np.sqrt(252)  # Annualized volatility
    ...
    def _calculate_level_consensus(
        ...
        relevant_results = [
            analysis_results[tf] for tf in timeframes
