...
def create_env_from_schema(
    ...
    if training_env_config_raw:
        ...
        if "initial_balance" in training_env_config:
            training_env_config["initial_portfolio_value"] = training_env_config.pop(
                "initial_balance"
            )
    ...
    logger.info(
        f"Final env_config: enable_correlation_reduction={env_config.get('enable_correlation_reduction')}"
    )
