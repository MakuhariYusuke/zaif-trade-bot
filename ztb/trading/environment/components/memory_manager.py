...
class MemoryManager:
    ...
    def collect_garbage(self, generation: int = 2) -> int:
        """Perform garbage collection. Returns number of unreachable objects found."""
        return gc.collect(generation=generation)

    def collect_garbage_aggressive(self) -> int:
        """Perform aggressive garbage collection (all generations).
        Returns total number of unreachable objects found."""
        collected_count = 0
        for i in range(3):
            collected_count += gc.collect(generation=i)
    ...
    # NOTE: should_collect_garbage property removed in 407# — use
    # should_collect_garbage_at_step(step) or is_gc_enabled instead.
