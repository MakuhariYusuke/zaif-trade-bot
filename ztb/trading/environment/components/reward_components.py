...
class RewardComponents:
    ...
    def calculate_drawdown_penalty(
        ...
        drawdown_window = RewardUtils.get_setting_int(self.reward_settings, "drawdown_window", 10)
        if len(portfolio_value_history) < drawdown_window or len(reward_history) < drawdown_window:
            return 0.0
        ...
        stagnation_window = RewardUtils.get_setting_int(self.reward_settings, "stagnation_window", 30)
        if len(reward_history) >= stagnation_window:
            ...
            if initial_cumulative > 0:
                ...
                if drawdown_ratio > drawdown_threshold:
                    drawdown_penalty_coeff = RewardUtils.get_setting_float(self.reward_settings, "drawdown_penalty_coeff", 0.05)
    ...
    def calculate_stagnation_penalty(self, portfolio_value_history: List[float]) -> float:
        ...
        if initial_value > 0:
            ...
            stagnation_threshold = RewardUtils.get_setting_float(self.reward_settings, "stagnation_threshold", -0.005)

            if growth_rate < stagnation_threshold:
                stagnation_penalty_max = RewardUtils.get_setting_float(self.reward_settings, "stagnation_penalty_max", 0.02)
    ...
    def calculate_growth_bonus(self, portfolio_value_history: List[float]) -> float:
        ...
        if initial_value > 0:
            growth_rate = (final_value - initial_value) / initial_value
            growth_threshold = RewardUtils.get_setting_float(self.reward_settings, "growth_threshold", 0.005)

            if growth_rate > growth_threshold:
                growth_bonus_max = RewardUtils.get_setting_float(self.reward_settings, "growth_bonus_max", 0.05)
    ...
    def calculate_win_streak_bonus(self, reward_history: List[float]) -> float:
        ...
        win_streak_window = RewardUtils.get_setting_int(self.reward_settings, "win_streak_window", 5)
        ...
        if win_count >= win_streak_min:
            win_streak_bonus_per_win = RewardUtils.get_setting_float(self.reward_settings, "win_streak_bonus_per_win", 0.01)
    ...
    def _calculate_position_penalty(self, position: float, effective_max_position: float) -> float:
        ...
        if position_ratio > 0.8:
            penalty = RewardUtils.get_setting_float(self.reward_settings, "high_position_penalty", 0.05)
        elif position_ratio > 0.5:
            penalty = RewardUtils.get_setting_float(self.reward_settings, "medium_position_penalty", 0.02)
        else:
            penalty = 0.0
    ...
    def _calculate_base_reward(
        ...
        # Profit bonus
        base_profit_bonus = max(0.0, RewardUtils.get_setting_float(self.reward_settings, "base_profit_bonus_atr_coeff", 1.5) * atr_normalised +
                               RewardUtils.get_setting_float(self.reward_settings, "base_profit_bonus_portfolio_coeff", 1.2) * portfolio_return) if pnl > 0 else 0.0
        ...
        # Action penalties
        hold_penalty = RewardUtils.get_setting_float(self.reward_settings, "hold_action_penalty", 0.0)
        buy_penalty = RewardUtils.get_setting_float(self.reward_settings, "buy_action_penalty", 0.0)
        sell_penalty = RewardUtils.get_setting_float(self.reward_settings, "sell_action_penalty", 0.0)

        base_action_penalty = RewardUtils.get_setting_float(self.reward_settings, "base_action_penalty", 0.015) if action in [ACTION_BUY, ACTION_SELL] else 0.0

        if action == ACTION_HOLD:
            position_size_factor = abs(position) / max(effective_max_position, 0.01)
            volatility_factor = min(atr / (current_price * 0.01), 1.0)
            base_action_penalty = RewardUtils.get_setting_float(self.reward_settings, "hold_penalty_base", 0.01) + (
                RewardUtils.get_setting_float(self.reward_settings, "hold_penalty_position_factor", 0.04) * position_size_factor * volatility_factor)
        ...
        # Loss penalty
        loss_penalty = RewardUtils.get_setting_float(self.reward_settings, "loss_penalty_coeff", -0.2) * abs(atr_normalised) if pnl < 0 else 0.0
    ...
    def _calculate_default_reward(
        ...
        diversity_bonus = 0.0
        if RewardUtils.get_setting_bool(self.reward_settings, "enable_forced_diversity", False):
        ...
        if observation is not None and len(observation) >= 2:
            try:
                ...
                if rsi_idx is not None and macd_idx is not None:
                    ...
                    momentum_multiplier = RewardUtils.get_setting_float(self.reward_settings, "momentum_bonus", 0.0)
        ...
        total_reward = profit_bonus - action_penalty + loss_penalty + diversity_bonus + win_rate_bonus + momentum_bonus
        return total_reward