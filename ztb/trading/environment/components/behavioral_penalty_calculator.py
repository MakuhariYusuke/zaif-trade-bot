...
class BehavioralPenaltyCalculator:
    ...
    def _load_settings(self):
        ...
        # 400# FIX: behavior_optimization dict as secondary fallback source.
        # When reward_settings is a RewardSettings dataclass AND behavior_optimization
        # is also set, keys like balance_shaping_enabled/action_entropy_shaping_enabled
        # in behavior_optimization were silently ignored because _rs_get only checked
        # the dataclass (which doesn't have these fields).
        _behavior_opt_fallback: dict[str, object] | None = None
        if not isinstance(self.config, dict) and not isinstance(reward_settings, dict):
            _behavior_opt_fallback = getattr(
                self.config, "behavior_optimization", None
            )
        ...
        def _rs_get(key, default=None):
            if reward_settings is None:
                # 400# FIX: Even if reward_settings is None, check behavior_opt_fallback
                if isinstance(_behavior_opt_fallback, dict) and key in _behavior_opt_fallback:
                    return _behavior_opt_fallback[key]
                return default
            ...
            # 400# FIX: Check behavior_optimization dict as fallback for keys
            # not present in the RewardSettings dataclass (e.g. balance_shaping_enabled,
