...
class BaseSignalScorer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.name = self.__class__.__name__.replace('Scorer', '').lower()
    ...
    def calculate_score(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if trend_slope > 0 and current_price < recent_low:
                ...
                # Trend continuation
                return 50.0 + (trend_slope * 1000)  # 0-100 range
...
class VolumeProfileScorer(BaseSignalScorer):
    ...
    def get_confidence(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            volume_variation = df['volume'].tail(20).std() / df['volume'].tail(20).mean()
...
class EnsembleSignalGenerator:
    ...
    def generate_ensemble_signal(self, market_data: Dict[str, Any]) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            logger.debug(f"Ensemble scores: {scores}, Weights: {adjusted_weights}, "
    ...
    def _adjust_weights_dynamically(self, confidences: Dict[str, float]) -> Dict[str, float]:
        ...
        if total_confidence > 0:
            for source_name, confidence in confidences.items():
                ...
                confidence_multiplier = 1.0 + (confidence - 0.5) * 0.5  # ±25% adjustment
    ...
    def get_signal_reliability(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            ensemble_score, ensemble_confidence = self.generate_ensemble_signal(market_data)

            # Calculate agreement level
            score_values = list(scores.values())
            if len(score_values) > 1:
                agreement = 1.0 - (np.std(score_values) / 50.0)  # Lower std = higher agreement
            ...
            return {
                ...
                'signal_strength': self._calculate_signal_strength(ensemble_score, ensemble_confidence),
                'recommendation': self._get_trading_recommendation(ensemble_score, ensemble_confidence)
            }
