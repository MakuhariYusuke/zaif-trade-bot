...
from ztb.trading.signal.quality.indicators.base import CompositeIndicator, AdaptiveIndicator
...
class SignalQualityScorer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.ensemble_generator = EnsembleSignalGenerator(self.config.get('ensemble', {}))
        ...
        self.ensemble_weight = self.config.get('ensemble_weight', 0.3)  # How much ensemble influences final score

    def _initialize_advanced_indicators(self):
        """Initialize advanced indicators for Phase 2 integration"""
        # Base indicators for composite and adaptive functionality
        self.rsi_indicator = RSIIndicator({'periods': self.config.get('rsi_period', 14)})
        ...
        self.adaptive_macd = AdaptiveIndicator(
            ...
            config={
                'adaptive_params': {
                    'trending': {'fast_period': 8, 'slow_period': 21, 'signal_period': 5},  # Faster for trends
                    'ranging': {'fast_period': 12, 'slow_period': 26, 'signal_period': 9}, # Standard for ranging
                    'volatile': {'fast_period': 5, 'slow_period': 13, 'signal_period': 4}  # Very fast for volatility
                }
            }
        )
    ...
    def calculate_signal_quality(self, df: pd.DataFrame,
        ...
        try:
            ...
            if self.enable_ensemble:
                final_score = self._apply_ensemble_integration(final_score, df, continuous_action, portfolio)
    ...
    def _calculate_trend_score(self, df: pd.DataFrame) -> float:
        ...
        trend_window = self.config.get('trend_window', 10)  # Default to 10 if not specified
    ...
    def _apply_ensemble_integration(self, base_score: float, df: pd.DataFrame,
                                   continuous_action: float, portfolio: Dict[str, Any]) -> float:
        """
        ...
        try:
            ...
            ensemble_score, ensemble_confidence = self.ensemble_generator.generate_ensemble_signal(market_data)
            ...
            ensemble_action = (ensemble_score - 50) / 50  # Convert 0-100 to -1 to 1 range

            # Blend ensemble signal with base score
            # Higher confidence = more weight to ensemble signal
            blended_score = (base_score * (1 - ensemble_weight) +
                           ensemble_action * ensemble_weight * 100)  # Scale to score range
            ...
            self.logger.debug(f"Ensemble integration - Base: {base_score:.2f}, "
    ...
    def _get_enhanced_signals(self, df: pd.DataFrame, market_regime: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            adaptive_rsi_result = self.adaptive_rsi.calculate_adaptive(df, market_regime)
            adaptive_macd_result = self.adaptive_macd.calculate_adaptive(df, market_regime)
