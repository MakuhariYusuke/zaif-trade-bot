...
class EnhancedSignalGuidanceSystem(BaseSignalProcessor):
    ...
    def __init__(
        ...
        # Pass threshold manager to quality scorer if provided
        self.quality_scorer = SignalQualityScorer(
            quality_config, threshold_manager=threshold_manager
        )
    ...
    def _initialize_regime_adaptation(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            RegimeType.BUY_BREAKOUT: {
                "signal_bias": 2.0,  # Strong BUY bias
                "confidence_multiplier": 1.5,
                "threshold_adjustment": 10,  # Lower threshold for BUY signals (higher score)
                "description": "Strong breakout - prioritize BUY signals",
            },
            RegimeType.BUY_DIVERGENCE: {
                "signal_bias": 1.5,
                "confidence_multiplier": 1.3,
                "threshold_adjustment": 8,
                "description": "Bullish divergence - favor BUY signals",
            },
            RegimeType.BUY_MOMENTUM_STRONG: {
                "signal_bias": 1.2,
                "confidence_multiplier": 1.2,
                "threshold_adjustment": 5,
                "description": "Strengthening momentum - reinforce BUY",
            },
            RegimeType.BUY_VOLUME_SURGE: {
                "signal_bias": 1.8,
    ...
    def process_signal(self, context: SignalContext) -> SignalResult:
        ...
        try:
            ...
            current_regime = regime_result.metadata.get(
    ...
    def _apply_regime_adaptation(
        self,
        quality_result: SignalResult,
        current_regime: str,
    ...
    def _generate_strategic_guidance(
        ...
        guidance = {
            ...
            "position_sizing": self._recommend_position_sizing(
                current_regime, adapted_result
            ),
            "time_horizon": self._recommend_time_horizon(current_regime),
            "stop_loss_guidance": self._generate_stop_loss_guidance(
                current_regime, context
            ),
            "take_profit_guidance": self._generate_take_profit_guidance(
                current_regime, context
            ),
        }
    ...
    def _assess_risk(self, regime: str, result: SignalResult) -> str:
        ...
        high_risk_regimes = [
            RegimeType.EXTREME_VOLATILITY,
            RegimeType.SELL_BREAKDOWN,
            RegimeType.BUY_BREAKOUT,  # Breakouts are also high risk/high reward
            RegimeType.STRONG_BEAR_TREND,
            RegimeType.STRONG_BULL_TREND,
        ]
    ...
    def _recommend_position_sizing(self, regime: str, result: SignalResult) -> str:
        ...
        regime_multipliers = {
            ...
            RegimeType.BUY_BREAKOUT: "INCREASED",
            RegimeType.STRONG_BULL_TREND: "INCREASED",
            RegimeType.STRONG_BEAR_TREND: "INCREASED",
        }
    ...
    def _generate_stop_loss_guidance(
        ...
        if regime in [RegimeType.SELL_BREAKDOWN, RegimeType.STRONG_BEAR_TREND]:
            # Tighter stops in strong trends (Short)
            stop_distance = (
                abs(current_price - recent_high) * 0.5
            )  # Use recent high for short stop
            return {
                "type": "PRICE_LEVEL",
                "value": current_price + stop_distance,
                "reason": "Tight stop in strong downtrend",
            }
        elif regime in [RegimeType.BUY_BREAKOUT, RegimeType.STRONG_BULL_TREND]:
            # Tighter stops in strong trends (Long)
            stop_distance = abs(current_price - recent_low) * 0.5
            return {
                "type": "PRICE_LEVEL",
                "value": current_price - stop_distance,
                "reason": "Tight stop in strong uptrend",
            }
            ...
            return {"type": "PERCENTAGE", "value": 0.03, "reason": "Standard stop loss"}
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_regime_adaptations": sum(
                len(history) for history in self.regime_performance.values()
            ),
