...
from ztb.trading.signal.types import (
    CalibrationStats,
    CalibrationStatsBundle,
    FusedSignal,
    GateResult,
)
...
class CalibrationMap:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.n_min = float(
            config.get("n_min", 30.0)
        )  # Min effective sample size for trust
    ...
    def update(self, regime: str, action: float, gross_pnl: float, step: int) -> None:
        ...
        for key in keys:
            ...
            stats["sum_w_sq"] *= (
                decay * decay
            )  # Squared weights decay with square of factor
...
class CalibrationGate:
    ...
    def _estimate_cost(
        self, market_data: MarketState, action: float, order_size: float
    ) -> float:
        """
