...
def sortino_ratio(returns: Union[pd.Series, np.ndarray], 
                 ...
                 downside_floor: float = 0.0) -> float:
    """
...
def calculate_all_metrics(returns: Union[pd.Series, np.ndarray], 
                         rf: float = 0.0, 
                         period_per_year: int = 252) -> MetricsResult:
    """
    ...
    annual_return = (1 + total_return) ** (period_per_year / len(returns)) - 1 if len(returns) > 0 else 0.0
    volatility = np.std(returns, ddof=1) * np.sqrt(period_per_year) if len(returns) > 1 else 0.0
