...
def check_training_completion():
    ...
    for name, session_id in sessions.items():
        ...
        if model_found:
            ...
            print(f"✅ {name:35s} | Session: {session_id:15s} | Model: {model_found.name} ({size_mb:.1f} MB)")
            ...
            elif "v394d" in name and ("session_9" in session_id or "session_10" in session_id):
            ...
            if event_files:
                ...
                print(f"⏳ {name:35s} | Session: {session_id:15s} | Training... ({len(event_files)} events, {total_size:.1f} KB)")
            else:
