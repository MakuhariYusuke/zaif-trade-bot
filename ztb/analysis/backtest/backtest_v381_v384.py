...
def run_backtest(
    ...
    logger.info(
        f"Environment created: {env.observation_space.shape[0]} features expected"
    )
    ...
    for episode in range(episodes):
        try:
            ...
            while not (done or truncated):
                # Predict action
                action, _states = predict_with_masks(
                    model, obs, env, deterministic=True
                )
            ...
            episode_return = (
                (episode_pnl / initial_balance) * 100 if initial_balance > 0 else 0
            )
    ...
    results = {
        ...
        "action_distribution": {
            ...
            "BUY_pct": (action_counts["BUY"] / total_actions * 100)
            if total_actions > 0
            else 0,
            "SELL_pct": (action_counts["SELL"] / total_actions * 100)
            if total_actions > 0
            else 0,
        },
    ...
    if len(all_pnls) > 0:
        ...
        if np.std(all_pnls) > 0:
            ...
            sharpe_ratio = (np.mean(all_pnls) / np.std(all_pnls)) * np.sqrt(
                periods_per_year
            )
        else:
            sharpe_ratio = 0.0
...
def print_comparison(
    ...
    for label, key, fmt in metrics:
        ...
        if "%" in fmt:
            print(
                f"{label:<30} {fmt.format(v381_val):>20} {fmt.format(v384_val):>20} {fmt.format(diff):>20}"
            )
        else:
            print(
                f"{label:<30} {fmt.format(v381_val):>20} {fmt.format(v384_val):>20} {fmt.format(diff):>20}"
    ...
    print(
        f"{'Total Actions':<30} {results_v381['total_actions']:>20,} {results_v384['total_actions']:>20,}"
    )
    print(
        f"{'Number of Trades':<30} {results_v381['num_trades']:>20,} {results_v384['num_trades']:>20,}"
    )
    ...
    if (
        results_v381["max_drawdown"] > results_v384["max_drawdown"]
...
def main() -> int:
    ...
    try:
        ...
        results_v381 = run_backtest(
            v381_model, data_path, episodes, "v381_revised_profit_focused"
        )
