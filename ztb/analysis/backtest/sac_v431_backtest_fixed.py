...
def run_backtest_simulation(config, num_steps=10000, initial_capital=10000.0):
    ...
    for step in range(1, num_steps):
        ...
        # Get SAC v431 action
        action, reward = simulate_sac_v431_action(
            current_price, prev_price, position, config
        )
...
def main():
    ...
    output_file = Path(__file__).parent / "sac_v431_backtest_results.json"
    write_json(output_file, results, indent=2, ensure_ascii=False, default=str)
