...
def clear_feature_caches() -> None:
    ...
    # 他のキャッシュもクリア
    try:
        optimize_kama.cache_clear()
        optimize_adx.cache_clear()
        optimize_kalman_filter.cache_clear()
...
def optimize_feature_pipeline(
    ...
    for feature_name, config in features_config.items():
        ...
        if hasattr(optimize_feature_pipeline, '_feature_cache'):
            cached_result = getattr(optimize_feature_pipeline, '_feature_cache', {}).get(feature_cache_key)
            if cached_result is not None:
                result = cached_result
                performance_stats[feature_name] = {"cached": True, "mean_time": 0.0}
        ...
        if feature_name in slow_features:
            ...
            # 結果をキャッシュ（メモリ使用量を制限）
            if not hasattr(optimize_feature_pipeline, '_feature_cache'):
                optimize_feature_pipeline._feature_cache = {}
            if len(optimize_feature_pipeline._feature_cache) < 50:  # キャッシュサイズ制限
                optimize_feature_pipeline._feature_cache[feature_cache_key] = result
...
def optimize_features_with_reward_feedback(
    ...
    Args:
        ohlc_data: OHLCデータ
        features_config: 特徴量設定
        reward_optimizer: 報酬関数最適化器

    Returns:
        最適化結果
    """
    # まず特徴量を最適化
    base_result = optimize_features(ohlc_data, features_config)

    if reward_optimizer is None:
        return base_result

    # 報酬関数最適化器を使用して特徴量パラメータを調整
    optimized_configs = {}
    for feature_name, config in features_config.items():
        if feature_name in ["KAMA", "ADX", "KalmanFilter"]:
            # 報酬関数に基づいてパラメータを最適化
            optimized_params = _optimize_feature_params_with_reward(
                feature_name, config, ohlc_data, reward_optimizer
            )
            optimized_configs[feature_name] = {
                **config,
                "params": optimized_params,
            }
        else:
            optimized_configs[feature_name] = config

    # 最適化された設定で再度特徴量を計算
    final_result = optimize_features(ohlc_data, optimized_configs)

    return {
        **final_result,
        "reward_optimized_configs": optimized_configs,
...
def _optimize_feature_params_with_reward(
    ...
    # 報酬関数最適化器を使用してパラメータを調整
    # ダミー実装 - 実際には報酬関数に基づいてパラメータを最適化
    optimized_params = base_params.copy()

    # 例: KAMAのパラメータを報酬に基づいて調整
    if feature_name == "KAMA":
        # 報酬関数から学習したパラメータを使用
        if hasattr(reward_optimizer, 'get_optimized_feature_params'):
            reward_based_params = reward_optimizer.get_optimized_feature_params(feature_name)
            optimized_params.update(reward_based_params)
