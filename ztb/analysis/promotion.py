...
class YamlPromotionEngine(PromotionEngine):
    ...
    def _create_criterion(self, criterion_config: ObjectMap) -> Criterion:
        ...
        if criterion_type == "duration":
            return DurationCriterion(name, operator, value, weight)
        if criterion_type == "distribution":
    ...
    def _optimize_required_score(
        self, category: str, historical_results: ObjectRecords
    ) -> float:
        """Optimize required score based on historical promotion success patterns"""
        categories = _as_object_map(self.config.get("categories", {}))
        category_map = _as_object_map(categories.get(category))
        default_required = _as_float(category_map.get("required_score", 0.7), 0.7)
        ...
        for result in historical_results:
            if str(result.get("promotion_result", "")) == "promote":
                score = _coerce_metric(result.get("achieved_score", 0.0))
        ...
        optimal_threshold = float(percentile_result)
    ...
    def evaluate_promotion(
        ...
        required_score = _as_float(category_config.get("required_score", 0.5), 0.5)
        ...
        hard_requirement_mode = str(
            staging_config.get("hard_requirement_mode", "strict")
        )
        ...
        if logic == "OR":
            should_promote = bool(passed_criteria) and normalized_score >= required_score
        else:
            should_promote = (not failed_criteria) and normalized_score >= required_score

        if current_status == "pending":
            result = PromotionResult.PROMOTE if should_promote else PromotionResult.KEEP
        ...
        if result == PromotionResult.PROMOTE:
            ...
            hard_req_failures = [
                d for d in hard_requirement_details if not d["passed"]
            ]
