...
def analyze_tensorboard_events(event_file_path, session_name):
    ...
    # 主要メトリクスを抽出
    metrics = {}
    key_metrics = ['train/critic_loss', 'train/actor_loss', 'train/ent_coef', 
...
def compare_sessions(metrics1, metrics2, name1, name2):
    ...
    for metric in sorted(common_metrics):
        ...
        if vals1 and vals2:
            ...
            print(f"\n【{metric}】")
            print(f"  {name1}: 平均 {avg1:.6f}, 範囲 [{min(vals1):.6f}, {max(vals1):.6f}]")
            print(f"  {name2}: 平均 {avg2:.6f}, 範囲 [{min(vals2):.6f}, {max(vals2):.6f}]")
