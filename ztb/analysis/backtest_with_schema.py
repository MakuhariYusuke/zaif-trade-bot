...
def run_backtest_with_schema(
    ...
    logger.info(
        f"Environment created with {env.observation_space.shape[0] if hasattr(env, 'observation_space') and env.observation_space else 0} features"
    )

    # 環境の特徴量リストを確認
    if hasattr(env, "feature_names") and env.feature_names is not None:
        logger.info(
            f"Environment features ({len(env.feature_names)}): {env.feature_names[:5]}..."
        )

    # データの利用可能な特徴量を確認
    available_features = set(df.columns)
    required_features = (
        set(env.feature_names)
        if hasattr(env, "feature_names") and env.feature_names is not None
        else set()
    )
    ...
    required_count = (
        ...
        if required_features
            ...
            if hasattr(env, "observation_space") and env.observation_space
            else 0
        )
    )
    logger.info(
        f"Data has {len(df.columns)} columns, environment needs {required_count} features"
    )
    ...
    for ep in range(episodes):
        ...
        return_pct = (
            (final_portfolio - initial_portfolio_value) / initial_portfolio_value
        ) * 100
...
def main() -> None:
    ...
    parser.add_argument(
        "--data", default="ml-dataset-enhanced.csv", help="Data path (.csv)"
    )
    ...
    try:
        results = run_backtest_with_schema(
            model_path=args.model, data_path=args.data, episodes=args.episodes
        )
