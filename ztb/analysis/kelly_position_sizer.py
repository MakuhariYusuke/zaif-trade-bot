...
class KellyPositionSizer:
    ...
    def calculate_kelly_parameters(
        ...
        if len(trades) < self.min_trades:
            self.logger.warning(f"Kelly計算に十分なトレードがありません: {len(trades)} < {self.min_trades}")
            return None
    ...
    def calculate_dynamic_position_size(
        ...
        reasoning = self._generate_reasoning(kelly_params, position_size_fraction, risk_tolerance)
    ...
    def _calculate_confidence_score(self, kelly_params: KellyParameters) -> float:
        """信頼度スコアを計算"""
        # トレード数の影響
        trade_confidence = min(1.0, kelly_params.total_trades / 50)  # 50トレードで最大信頼度

        # 勝率の安定性
        win_rate_confidence = 1.0 - abs(kelly_params.win_rate - 0.5) * 2  # 50%に近いほど信頼度高

        # 勝ち負け比率の安定性
        ratio_confidence = min(1.0, kelly_params.win_loss_ratio / 2.0)  # 2.0以上で最大信頼度
    ...
    def optimize_portfolio_allocation(
        ...
        if total_allocation > 1.0:
            for symbol in allocations:
                ...
                allocations[symbol].risk_amount = total_balance * allocations[symbol].position_size_fraction
...
class KellyCriterionValidator:
    ...
    def validate_kelly_effectiveness(
        ...
        for fraction in kelly_fractions:
            ...
            total_return = (simulated_balance[-1] - simulated_balance[0]) / simulated_balance[0]
...
if __name__ == "__main__":
    ...
    if kelly_params:
        print(f"Kellyパラメータ: 勝率={kelly_params.win_rate:.1%}, "
    ...
    print(f"ポジションサイズ決定: {decision.position_size_fraction:.2%} "
    ...
    print(f"最適性能: Sharpe={validation_result['optimal_performance']['sharpe_ratio']:.2f}, "
