...
def main() -> None:
    ...
    try:
        ...
        logger.info("  ⚠️  This step takes ~8 seconds - environment initialization is slow")
        ...
        policy_kwargs = {
            "net_arch": [dict(pi=[32, 32], vf=[32, 32])]  # Much smaller than default
        }
        ...
        logger.info(f"  Memory increase: {mem_after_rollout - start_rollout_mem:.1f} MB")
        ...
        logger.info(f"  Memory increase during training: {mem_after_train - mem_before_train:.1f} MB")
        ...
        logger.info(f"  After data load:      {mem_after_data:.1f} MB (+{mem_after_data - start_mem:.1f} MB)")
        logger.info(f"  After environment:    {mem_after_env:.1f} MB (+{mem_after_env - mem_after_data:.1f} MB)")
        logger.info(f"  After model:          {mem_after_model:.1f} MB (+{mem_after_model - mem_after_env:.1f} MB)")
        logger.info(f"  After training:       {mem_after_train:.1f} MB (+{mem_after_train - mem_after_model:.1f} MB)")
        ...
        logger.error(f"  System memory available: {psutil.virtual_memory().available / 1024 / 1024:.1f} MB")
        logger.error("=" * 80)
        return False
