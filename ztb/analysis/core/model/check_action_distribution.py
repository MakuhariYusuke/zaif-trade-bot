...
def check_action_distribution(
    ...
    trade_pct = (
        ((action_counts["BUY"] + action_counts["SELL"]) / total * 100)
        if total > 0
        else 0
    )
    ...
    return {
        ...
        "action_percentages": {
            k: (v / total * 100) if total > 0 else 0 for k, v in action_counts.items()
        },
        "total_steps": total,
        "hold_pct": hold_pct,
        "trade_pct": trade_pct,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, default="ml-dataset-enhanced.csv", help="Data path"
    )
