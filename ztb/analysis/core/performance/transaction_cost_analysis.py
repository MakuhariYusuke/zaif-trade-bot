...
from ztb.evaluation.unified_evaluation import EvaluationType, UnifiedEvaluator  # noqa: E402
...
def _simulate_transaction_costs_impl(
    ...
    plt.tight_layout()
    save_plot(
        output_dir / "transaction_cost_analysis.png"
    )
    plt.close()
...
def main() -> int:
    ...
    for cost, metrics in results.items():
        if "error" not in metrics:
            ...
            LOGGER.info(
                f"Cost {cost}: Return={total_return:.4f}, " f"Trades={total_trades}"
            )
