...
class CoverageMetadata(TypedDict, total=False):
    ...
    total_staging: int
    total_pending: int
    total_pending_due_to_gate_fail: int
    total_failed: int
...
class CoverageValidator:
    ...
    @staticmethod
    def load_coverage_files(base_path: str = "ztb/coverage") -> CoverageData:
        """Load and merge multiple coverage files"""
        import glob
        from pathlib import Path

        base_path_obj = Path(base_path)
        merged_coverage: CoverageData = {
    ...
    def _merge_coverage_data(
        target: CoverageData, source: CoverageData, source_file: str
    ) -> None:
    ...
    def archive_coverage_data(
        coverage_data: CoverageData, archive_dir: str = "ztb/coverage/archive"
    ) -> None:
        ...
        write_json(archive_file, archive_data, indent=2, ensure_ascii=False)
...
class CoverageReporter:
    ...
    @staticmethod
    def generate_json_report(coverage_data: CoverageData) -> str:
        """Generate JSON format report"""
        return json.dumps(coverage_data, indent=2, ensure_ascii=False)

    @staticmethod
    def generate_markdown_report(coverage_data: CoverageData) -> str:
    ...
    @staticmethod
    def generate_text_report(coverage_data: CoverageData) -> str:
