...
def compare_three_versions(log_dir):
    ...
    for session_id, name in versions.items():
        ...
        metrics = ['train/ent_coef', 'train/critic_loss', 'train/actor_loss', 'train/ent_coef_loss']
    ...
    for metric, name in metrics_to_compare.items():
        print(f"\n🔍 {name} ({metric}):")
        print("-" * 100)
        print(f"{'Version':<30} {'Initial':>15} {'Final':>15} {'Min':>15} {'Max':>15} {'Mean':>15}")
        print("-" * 100)
        ...
        for version_name in versions.values():
            if version_name in results and metric in results[version_name]:
                ...
                print(f"{version_name:<30} {data['initial']:>15.4e} {data['final']:>15.4e} "
    ...
    for version_name in versions.values():
        if version_name in results and 'train/ent_coef' in results[version_name]:
            ...
            print(f"  {version_name}: {final_ent:.2f} (distance from 1.25: {distance_from_ideal:.2f})")
    ...
    # Calculate scores
    scores = {name: 0 for name in versions.values()}
