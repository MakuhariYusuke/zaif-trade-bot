...
def main() -> None:
    ...
    for col in feature_cols:
        ...
        elif any(x in col for x in ['MACD', 'EMA', 'SMA', 'TEMA', 'KAMA', 'VWAP', 'PSAR', 'ADX', 'trend', 'Trend', 'Slope', 'slope']):
            categories['トレンド系'].append(col)
        elif any(x in col for x in ['RSI', 'Stochastic', 'CCI', 'Williams', 'MFI', 'DI', 'ROC']):
            categories['オシレーター系'].append(col)
        elif any(x in col for x in ['Volume', 'OBV', 'CMF', 'volume', 'liquidity', 'tick']):
    ...
    for cat_name, features in categories.items():
        if features:
            ...
            for i, feat in enumerate(features, 1):
                ...
                variance = df[feat].var() if pd.api.types.is_numeric_dtype(df[feat]) else None
                
                var_str = f"分散={variance:.6f}" if variance is not None else "非数値"
                print(f"  {i:2d}. {feat:50s} | 有効値={non_null:4d}/{len(df)} | ユニーク={unique:4d} | {var_str}")
    ...
    if ichimoku_features:
        ...
        print("  - 価格と雲の距離")
        print("  → これらは既に Ichimoku_Composite_Signal, Ichimoku_Price_Cloud_Distance などで実装済み")
    ...
    if high_corr_pairs:
        ...
        for feat1, feat2, corr in sorted(high_corr_pairs, key=lambda x: x[2], reverse=True)[:20]:
    ...
    # 平均足のOHLC(色連続以外)
    for feat in ha_features:
        if feat in ['HeikinAshi_Open', 'HeikinAshi_High', 'HeikinAshi_Low', 'HeikinAshi_Close']:
    ...
    if 'Ichimoku_Chikou' in ichimoku_features:
        delete_candidates.append(f"  - Ichimoku_Chikou (単独では無意味、他の線との組み合わせが必要)")
    ...
    print("\n3. 保持すべき重要特徴:")
    important = [
        'close', 'open', 'high', 'low', 'volume',  # 基本
        'RSI', 'MACD', 'ADX', 'ATR', 'CCI',  # 主要指標
        'BB_Position', 'Bollinger_Percent_B',  # ボリンジャー
        'Ichimoku_Composite_Signal', 'Ichimoku_Price_Cloud_Distance',  # 一目(組み合わせ)
        'Supertrend_Direction', 'Supertrend_Strength',  # スーパートレンド
    ]
