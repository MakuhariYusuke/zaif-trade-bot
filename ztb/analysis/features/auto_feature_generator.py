...
class FeatureCalculator:
    ...
    def _calculate_simple_kama(
        price: NDArray[np.float64], slow: int, er_period: int
    ) -> NDArray[np.float64]:
...
class AutoFeatureGenerator:
    ...
    def _calculate_simple_kama(
        self, price: NDArray[np.float64], slow: int, er_period: int
    ) -> NDArray[np.float64]:
    ...
    def evaluate_and_promote_features(
        ...
        """
        results: FeatureEvalResult = {}  # type: ignore[typeddict-item]
        promoted_features = {}
        temporary_features = {}
