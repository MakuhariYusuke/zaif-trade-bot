...
class FeatureCorrelationAnalyzer:
    ...
    def create_correlation_report(
        ...
        try:
            ...
            if high_corr_pairs:
                lines.append("高相関特徴量ペア:")
                for pair in sorted(
                    high_corr_pairs, key=lambda x: abs(x["pearson"]), reverse=True
                )[:10]:
                    lines.append(f"  {pair['feature1']} ↔ {pair['feature2']}")
                    lines.append(f"    Pearson: {pair['pearson']:.3f}")
                    lines.append(f"    Spearman: {pair['spearman']:.3f}")
                    lines.append(f"    Mutual Info: {pair['mutual_info']:.3f}")
                    lines.append("")
            else:
                lines.append("高相関特徴量ペアは見つかりませんでした。")
            ...
            if len(pearson_corr) > 0:
                lines.append("特徴量ごとの平均相関:")
                for i, name in enumerate(feature_names):
                    avg_corr = np.mean(np.abs(pearson_corr[i, :]))
                    lines.append(f"  {name}: {avg_corr:.3f}")
    ...
    def visualize_correlations(
        ...
        try:
            ...
            if linkage_matrix is not None:
                ...
                plt.tight_layout()
                save_plot(
                    f"{output_dir}/feature_dendrogram.png"
                )
                plt.close()
