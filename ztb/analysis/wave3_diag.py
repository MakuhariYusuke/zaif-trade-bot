...
def calculate_correlations(df: pd.DataFrame, target_col: str = 'return') -> Dict[str, pd.DataFrame]:
    """相関計算"""
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    feature_cols = [c for c in numeric_cols if c not in ['ts', 'episode_id', target_col]]
...
def calculate_mutual_info(df: pd.DataFrame, horizons: List[int]) -> Dict[str, pd.DataFrame]:
    """相互情報量計算"""
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    feature_cols = [c for c in numeric_cols if c not in ['ts', 'episode_id', 'return']]
...
def check_leaks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in feature_cols:
        ...
        # 警告判定
        warning = (
            isinstance(corr_future, float) and not np.isnan(corr_future) and
            isinstance(corr_current, float) and not np.isnan(corr_current) and
            abs(corr_future) > abs(corr_current) * 1.5
        )
        reason = "Future correlation significantly higher than current" if warning else ""
...
def main() -> None:
    ...
    mapping = {
        'Ichimoku': {
            'old': ['ichimoku_tenkan', 'ichimoku_kijun', 'ichimoku_senkou_a', 'ichimoku_senkou_b', 'ichimoku_chikou'],
            'new': ['ichimoku_diff_norm', 'ichimoku_cross']
        },
        'Donchian': {
            'old': ['donchian_position', 'donchian_width_rel'],
            'new': ['donchian_pos_20', 'donchian_slope_20', 'donchian_pos_55', 'donchian_slope_55']
        },
        'KalmanFilter': {
            'old': ['kalman_filtered'],
    ...
    mapping_df = pd.DataFrame([
        {'feature': k, 'old_columns': ', '.join(v['old']), 'new_columns': ', '.join(v['new'])}
