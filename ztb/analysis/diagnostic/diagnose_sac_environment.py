...
class EnvironmentDiagnostics:
    ...
    def __init__(self, config_path: str) -> None:
        ...
        self.env = None
        self.diagnostics = {"config": config_path, "episodes": [], "summary": {}}
    ...
    def setup_environment(self) -> None:
        ...
        factory = AlgorithmFactory(algorithm=algorithm, config=unified_config, df=df)
    ...
    def generate_summary(self, all_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        avg_zero_pct = np.mean(
            [a["rewards"]["num_zero"] / a["total_steps"] for a in all_analyses]
        )
        if avg_zero_pct > 0.5:
            summary["issues"].append(
                {
    ...
    def save_diagnostics(
        self, output_path: str = "sac_environment_diagnostics.json"
    ) -> None:
        """診断結果を保存"""
        write_json(output_path, self.diagnostics, indent=2, ensure_ascii=False)
        print(f"\n診断結果を保存: {output_path}")
    ...
    def run(self, num_episodes: int = 3, num_steps_per_episode: int = 100) -> None:
        ...
        for ep in range(1, num_episodes + 1):
            ...
            self.diagnostics["episodes"].append(
                {"episode_data": episode_data, "analysis": analysis}
            )
