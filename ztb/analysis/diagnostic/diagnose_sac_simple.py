...
def diagnose_environment(
    ...
    df = DataLoader.load_csv_optimized(dataset_path)
    ...
    print(
        f"  正の報酬: {(rewards > 0).sum()} ({(rewards > 0).sum()/len(rewards)*100:.1f}%)"
    )
    print(
        f"  負の報酬: {(rewards < 0).sum()} ({(rewards < 0).sum()/len(rewards)*100:.1f}%)"
    )
    print(
        f"  ゼロ: {(rewards == 0).sum()} ({(rewards == 0).sum()/len(rewards)*100:.1f}%)"
    )
    ...
    if len(large_std_indices) > 0:
        ...
        for idx in large_std_indices[:5]:  # 最初の5個だけ表示
            print(
                f"      特徴量 {idx}: std={obs_std[idx]:.2f}, range=[{obs_min[idx]:.2f}, {obs_max[idx]:.2f}]"
            )
    ...
    for action, count in zip(unique_disc, counts_disc):
        print(
            f"    {action_names.get(action, f'Action {action}')}: {count} ({count/len(actions_disc)*100:.1f}%)"
        )
    ...
    if rewards.max() > 10.0 or rewards.min() < -10.0:
        issues.append(
            f"⚠️ 報酬範囲が広すぎる: [{rewards.min():.4f}, {rewards.max():.4f}]"
        )
    ...
    if max_action_pct > 0.8:
        ...
        issues.append(
            f"⚠️ 行動が偏りすぎている: {action_names.get(max_action, f'Action {max_action}')} が {max_action_pct*100:.1f}%"
        )
    ...
    output = {
        ...
        "actions": {
            ...
            "discrete_distribution": {
                int(k): int(v) for k, v in zip(unique_disc, counts_disc)
            },
        },
        "issues": issues,
    }
...
if __name__ == "__main__":
    diagnose_environment(
        config_path="configs/sac_v395h_normalized.json", num_episodes=3, num_steps=100
    )
