...
class V444RegimeClassifier:
    ...
    def _calculate_trend_strength(
        ...
        try:
            ...
            logger.debug(
                "DEBUG TREND: momentum=%.6f, volatility=%.6f, trend_strength=%.6f, bull_strength=%.6f, bear_strength=%.6f",
                float(momentum.iloc[-1]) if not momentum.empty else 0.0,
                float(scaled_volatility),
                float(result_trend),
                float(result_bull),
                float(result_bear),
            )
