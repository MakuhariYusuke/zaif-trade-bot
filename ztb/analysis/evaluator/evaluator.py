...
from ztb.types.common import Action
...
class TradingEvaluator:
    ...
    def __init__(
        ...
        super().__init__()
        warnings.warn(
            "TradingEvaluator is deprecated. "
            "Use ztb.evaluation.unified_evaluation.UnifiedEvaluator with "
            "EvaluationType.BACKTEST instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def _load_data(self) -> pd.DataFrame:
        ...
        if (
            ...
            df = DataLoader.load_csv_strict(self.data_path)
            # キャッシュ保存
    ...
    def _load_model(self) -> Optional[BaseAlgorithm]:
        ...
        try:
            # Backward-compatible: if the legacy evaluation entrypoint exposes a
            # PPO loader, prefer it so tests can patch it (tests patch
            # `ztb.evaluation.evaluate.PPO.load`). Fall back to BaseAlgorithm.load.
            try:
                from ztb.evaluation.evaluate import PPO as EvalPPO  # type: ignore

                model = EvalPPO.load(str(self.model_path))
                logger.info(
                    f"Model loaded via ztb.evaluation.evaluate.PPO from {self.model_path} (type={type(model).__name__})"
                )
                return model
            except Exception:
                # Fallback to the generic SB3 BaseAlgorithm loader
    ...
    def _normalize_action(self, raw_action: Action) -> int | float:
