    ...
    def run_sac(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            output: AnalysisOutput = {
                "results": results,
                "summary": {"analysis_type": "sac_model", "samples": getattr(args, "samples", 10000)},
                "metadata": {"model_path": getattr(args, "model", None)}
            }
...
class ComparativeAnalysis(BaseAnalyzer):
    ...
    def run_backtest(self, args: argparse.Namespace) -> int:
        ...
        write_json(output_path, result, indent=2, ensure_ascii=False)
    ...
    def run_paper_trade(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            evaluator = UnifiedEvaluator(
                config={
                    "paper_trading_num_episodes": num_episodes,
                    "paper_trading_output_path": output_path,
                }
            )
            evaluation = evaluator.evaluate_model(
                model_path=model_path,
                data_path=data_path,
                evaluation_type=EvaluationType.PAPER_TRADING,
            )
            ...
            print("=" * 60)
            print(
                f"Average Reward: {results.get('avg_reward', 0.0):.2f} "
                f"± {results.get('std_reward', 0.0):.2f}"
            )
            print(
                f"Average Portfolio Value: {results.get('avg_portfolio_value', 0.0):.2f} "
                f"± {results.get('std_portfolio_value', 0.0):.2f}"
            )
            print("=" * 60)
            return 0
        except Exception as e:
        ...
        try:
            ...
            if getattr(args, "output", None):
                output_content = (
                    ...
                    f"Confidence interval: {result.confidence_interval}\n"
                )
                write_text(args.output, output_content)
