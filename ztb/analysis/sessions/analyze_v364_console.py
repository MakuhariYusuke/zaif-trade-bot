...
def analyze_distribution(counts: List[Tuple[int, int, int]]) -> None:
    ...
    for i, (hold, buy, sell) in enumerate(counts, 1):
        ...
        print(f"  Iteration {i}: [{hold:2d}, {buy:2d}, {sell:2d}]  "
    ...
    print(f"  Gap:               {(target_rate - sell_rate):.1%} ({(target_rate - sell_rate)*100:.1f} percentage points)")
    ...
    if hold_rate > 0.60:
        ...
        print("      → Consider reducing HOLD incentives or increasing action penalties")
    ...
    print(f"      With BUY={buy_rate:.1%}, SELL cannot sustainably exceed {buy_rate:.1%}")
    print(f"      Current SELL={sell_rate:.1%} is {sell_rate/buy_rate:.1f}x BUY rate")
    
    if sell_rate > buy_rate * 0.8:
        print(f"      → SELL is close to BUY - position turnover is high")
        print(f"      → To increase SELL further, must increase BUY first")
