...
def check_entry_conditions(price, prev_price, market_condition, config):
    ...
    if market_condition == "high_vol" and entry_config.get(
    ...
    if entry_config.get("support_resistance_filter", False):
        ...
        if (
            market_condition in ("sideways", "neutral")
            and trend_strength < min_trend * 1.2
        ):
            return False
...
def run_backtest_simulation_v432_1(config, num_steps=10000, initial_capital=10000.0):
    ...
    print(
        "Features: Negative HOLD penalty, Advanced position management, Dynamic sizing"
    )
    ...
    for step in range(1, num_steps):
        ...
        position_size = calculate_dynamic_position_size(
            config, market_condition, confidence, volatility
        )
...
def main():
    ...
    results = run_backtest_simulation_v432_1(
    ...
    output_file = (
        ...
        / "evaluation"
        / "v432"
        / "sac_v432_1_advanced_position_management_results.json"
    )
