...
class BacktestAnalyzer:
    ...
    def __init__(self, results_path: str, training_report_path: Optional[str] = None):
        ...
        self.training_report_path = Path(training_report_path) if training_report_path else None
        self.data = self._load_data()
        self.training_data = self._load_training_data() if training_report_path else None
        self._validate_data()
    ...
    def _load_training_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Training report file not found: {self.training_report_path}")
            return None
        except json.JSONDecodeError as e:
            logger.warning(f"Invalid JSON format in training report {self.training_report_path}: {e}")
            return None
        ...
        with self.performance_monitor:
            try:
                ...
                if self.training_data:
                    ...
                    # アクション分布比較
                    training_actions = self.training_data.get('training_stats', {}).get('action_distribution', {})
                    ...
                    if training_actions and backtest_actions:
                        ...
                        for action in ['HOLD', 'BUY', 'SELL']:
                            ...
                            report_lines.append(f"  {action}: 訓練 {train_pct:.1f}% → テスト {backtest_pct:.1f}% (差: {diff:+.1f}%)")
                        report_lines.append("")

                    # トレーニング情報
                    training_stats = self.training_data.get('training_stats', {})
                    report_lines.append("トレーニング情報:")
                    report_lines.append(f"  トレーニング時間: {training_stats.get('training_time', 0):.1f}秒")
                    report_lines.append(f"  ステップ/秒: {training_stats.get('steps_per_second', 0):.2f}")
                    report_lines.append(f"  総ステップ数: {training_stats.get('total_timesteps', 0)}")
                    report_lines.append("")

                    # 環境設定比較
                    env_config = self.training_data.get('configuration', {}).get('environment', {})
                    report_lines.append("環境設定:")
                    report_lines.append(f"  初期残高: ¥{env_config.get('initial_balance', 0):,.0f}")
                    report_lines.append(f"  取引コスト: {env_config.get('transaction_cost', 0):.2e}")
                    report_lines.append(f"  最大ポジションサイズ: {env_config.get('max_position_size', 0)}")
                    report_lines.append(f"  連続アクション: {env_config.get('use_continuous_actions', False)}")
                    report_lines.append(f"  標準化観測: {env_config.get('use_standardized_observations', False)}")
                    report_lines.append("")
                ...
                elif 'portfolio_history' in self.data and len(self.data['portfolio_history']) > 1:
                ...
                if pnl_returns is not None and len(pnl_returns) > 0:
                    ...
                    report_lines.append(f"総リターン: {metrics.get('total_return', 0):.2%}")
                    report_lines.append(f"年間リターン: {metrics.get('annual_return', 0):.2%}")
                    report_lines.append(f"シャープレシオ: {metrics.get('sharpe_ratio', 0):.3f}")
                    report_lines.append(f"ソルティーノレシオ: {metrics.get('sortino_ratio', 0):.3f}")
                    report_lines.append(f"カルマーレシオ: {metrics.get('calmar_ratio', 0):.3f}")
                    report_lines.append(f"最大ドローダウン: {metrics.get('max_drawdown', 0):.2%}")
                    report_lines.append(f"勝率: {metrics.get('win_rate', 0):.1%}")
                    report_lines.append(f"プロフィットファクター: {metrics.get('profit_factor', 0):.3f}")
                    report_lines.append(f"期待値: {metrics.get('expected_value', 0):.3f}")
                    report_lines.append(f"回復力: {metrics.get('recovery_factor', 0):.3f}")
                    report_lines.append(f"ボラティリティ: {metrics.get('volatility', 0):.2%}")
                    report_lines.append("")
                    ...
                    try:
                        ...
                        if len(pnl_returns) > 0:
                            geometric_mean = np.exp(np.mean(np.log(1 + pnl_returns))) - 1
                            report_lines.append("=== P平均法分析 (幾何平均) ===")
                            report_lines.append(f"P平均リターン (幾何平均): {geometric_mean:.4f}")
                            report_lines.append("")
                    ...
                    try:
                        ...
                        if regime_result is not None:
                            ...
                            if hasattr(regime_result, 'iloc'):
                                current_regime = regime_result.iloc[-1] if len(regime_result) > 0 else 'unknown'
                            else:
                    ...
                    try:
                        ...
                        report_lines.append(f"リターン標準偏差: {np.std(pnl_returns):.6f}")
                        report_lines.append(f"歪度: {pd.Series(pnl_returns).skew():.3f}")
                        report_lines.append(f"尖度: {pd.Series(pnl_returns).kurtosis():.3f}")
                        report_lines.append("")
                    except Exception as e:
                    ...
                    if 'timestamps' in self.data and self.data['timestamps']:
                        try:
                            seasonality_results = seasonality_analysis(pnl_returns, self.data['timestamps'])
                            if seasonality_results:
                                report_lines.append("=== 季節性分析 ===")
                                report_lines.append(f"日次季節性: {seasonality_results.get('daily_seasonality', 'N/A')}")
                                report_lines.append(f"週次季節性: {seasonality_results.get('weekly_seasonality', 'N/A')}")
                                report_lines.append(f"月次季節性: {seasonality_results.get('monthly_seasonality', 'N/A')}")
                                report_lines.append(f"季節性強度: {seasonality_results.get('seasonality_strength', 0):.2%}")
                                report_lines.append("")
                        except Exception as e:
                    ...
                    # 多市場分析
                    if 'price_history' in self.data and self.data['price_history'] and len(self.data['price_history']) > len(pnl_returns):
                        try:
                            market_analysis = multi_market_backtest_analysis(pnl_returns, self.data['price_history'])
                            if market_analysis:
                                report_lines.append("=== 多市場分析 ===")
                                report_lines.append(f"市場相関: {market_analysis.get('market_correlation', 0):.3f}")
                                report_lines.append(f"ベータ値: {market_analysis.get('beta', 0):.3f}")
                                report_lines.append(f"アルファ値: {market_analysis.get('alpha', 0):.4f}")
                                report_lines.append(f"情報比率: {market_analysis.get('information_ratio', 0):.3f}")
                                report_lines.append("")
                        except Exception as e:
                ...
                if 'action_distribution' in self.data:
                    ...
                    if isinstance(actions, dict) and all(isinstance(k, str) for k in actions.keys()):
                        for action_name, count in actions.items():
                            pct = count * 100 if isinstance(count, float) and count <= 1.0 else count / total_actions * 100
                ...
                try:
                    ...
                    if 'timestamps' in self.data and self.data['timestamps']:
                        analysis_df['timestamp'] = pd.to_datetime(self.data['timestamps'])
                    ...
                    if price_data and portfolio_data:
                        ...
                        # ポートフォリオリターンを計算
                        analysis_df['returns'] = pd.Series(analysis_df['portfolio']).pct_change()
                    ...
                    if len(analysis_df) > 0:
                        ...
                        if time_bias.get('bias_detected', False):
                            report_lines.append(f"⚠️  時間周期バイアス検出: {time_bias.get('bias_score', 0):.2f}")
                            report_lines.append(f"   カバー期間: {time_bias.get('time_range_days', 0)}日")
                            report_lines.append(f"   カバー月数: {time_bias.get('months_covered', 0)}ヶ月")
                        else:
                        ...
                        if trend_bias.get('bias_detected', False):
                            report_lines.append(f"⚠️  トレンドバイアス検出: {trend_bias.get('bias_score', 0):.2f}")
                            report_lines.append(f"   陽線比率: {trend_bias.get('positive_ratio', 0):.1%}")
                        else:
                        ...
                        if vol_bias.get('bias_detected', False):
                            report_lines.append(f"⚠️  ボラティリティバイアス検出: {vol_bias.get('bias_score', 0):.2f}")
                            report_lines.append(f"   ボラティリティ: {vol_bias.get('volatility', 0):.4f}")
                        else:
                        ...
                        if regime_bias.get('bias_detected', False):
                            report_lines.append(f"⚠️  市場レジームバイアス検出: {regime_bias.get('bias_score', 0):.2f}")
                            reg_dist = regime_bias.get('regime_distribution', {})
                            report_lines.append(f"   強気相場: {reg_dist.get('bull_pct', 0):.1%}")
                            report_lines.append(f"   弱気相場: {reg_dist.get('bear_pct', 0):.1%}")
                            report_lines.append(f"   横ばい相場: {reg_dist.get('sideways_pct', 0):.1%}")
                        else:
                        ...
                        if btc_bias.get('bias_detected', False):
                            report_lines.append(f"⚠️  BTC特有バイアス検出: {btc_bias.get('bias_score', 0):.2f}")
                ...
                try:
                    ...
                    if 'error' not in robustness:
                        ...
                        report_lines.append(f"総合ロバストネススコア: {robustness.get('robustness_score', 0):.3f}")

                        # ボラティリティレジーム分析
                        vol_analysis = robustness.get('volatility_analysis', {})
                        if vol_analysis:
                            report_lines.append("ボラティリティレジーム分析:")
                            high_vol = vol_analysis.get('high_volatility_performance', {})
                            low_vol = vol_analysis.get('low_volatility_performance', {})
                            report_lines.append(f"  高ボラティリティ: シャープ {high_vol.get('sharpe_ratio', 0):.3f}, 勝率 {high_vol.get('win_rate', 0):.1%}")
                            report_lines.append(f"  低ボラティリティ: シャープ {low_vol.get('sharpe_ratio', 0):.3f}, 勝率 {low_vol.get('win_rate', 0):.1%}")
                            report_lines.append(f"  レジーム間一貫性: {vol_analysis.get('volatility_regime_consistency', 0):.3f}")
                        ...
                        if trend_analysis:
                            ...
                            report_lines.append(f"  上昇トレンド: シャープ {uptrend.get('sharpe_ratio', 0):.3f}, 勝率 {uptrend.get('win_rate', 0):.1%}")
                            report_lines.append(f"  下降トレンド: シャープ {downtrend.get('sharpe_ratio', 0):.3f}, 勝率 {downtrend.get('win_rate', 0):.1%}")
                            report_lines.append(f"  横ばい: シャープ {sideways.get('sharpe_ratio', 0):.3f}, 勝率 {sideways.get('win_rate', 0):.1%}")
                            report_lines.append(f"  レジーム間一貫性: {trend_analysis.get('trend_regime_consistency', 0):.3f}")

                        # 季節性分析
                        seasonal = robustness.get('seasonal_analysis', {})
                        if seasonal and 'error' not in seasonal:
                            report_lines.append("季節性分析:")
                            report_lines.append(f"  時間帯別一貫性: {seasonal.get('seasonal_consistency_score', 0):.3f}")

                            # 最も良い/悪い時間帯を表示
                            hourly_perf = seasonal.get('hourly_performance', {})
                            if hourly_perf:
                                best_hour = max(hourly_perf.items(), key=lambda x: x[1].get('sharpe_ratio', 0) if x[1] else 0)
                                worst_hour = min(hourly_perf.items(), key=lambda x: x[1].get('sharpe_ratio', 0) if x[1] else 0)
                                report_lines.append(f"  最適時間帯: {best_hour[0]}時 (シャープ {best_hour[1].get('sharpe_ratio', 0):.3f})")
                                report_lines.append(f"  最悪時間帯: {worst_hour[0]}時 (シャープ {worst_hour[1].get('sharpe_ratio', 0):.3f})")

                        report_lines.append("")
                    else:
                        report_lines.append(f"ロバストネス分析エラー: {robustness['error']}")
                        report_lines.append("")
                ...
                try:
                    ...
                    if correlation_analysis:
                        ...
                        report_lines.append(f"価格-ポートフォリオ相関: {correlation_analysis.get('price_portfolio_correlation', 0):.3f}")

                        # ラグ相関のトップ3
                        lag_corrs = correlation_analysis.get('lag_correlations', {})
                        if lag_corrs:
                            sorted_lags = sorted(lag_corrs.items(), key=lambda x: abs(x[1]), reverse=True)[:3]
                            report_lines.append("ラグ相関 (トップ3):")
                            for lag, corr in sorted_lags:
                                report_lines.append(f"  {lag}: {corr:.3f}")

                        report_lines.append(f"ベータ値: {correlation_analysis.get('beta', 0):.3f}")

                        # アクションと価格の関係
                        action_rels = correlation_analysis.get('action_price_relationships', {})
                        if action_rels:
                ...
                try:
                    ...
                    if cost_impact:
                        ...
                        report_lines.append(f"総取引コスト: ¥{cost_impact.get('total_transaction_cost', 0):,.0f}")
                        report_lines.append(f"取引ごとの平均コスト: ¥{cost_impact.get('average_cost_per_trade', 0):,.2f}")
                        report_lines.append(f"コスト対リターン比: {cost_impact.get('cost_to_return_ratio', 0):.4f}")
                        report_lines.append(f"ステップあたり取引数: {cost_impact.get('trades_per_step', 0):.3f}")
                        report_lines.append(f"コスト効率スコア: {cost_impact.get('cost_efficiency_score', 0):.3f}")
                        report_lines.append("")
                except Exception as e:
                ...
                try:
                    ...
                    if stress_tests:
                        ...
                        for test_name, results in stress_tests.items():
                            ...
                            report_lines.append(f"  ストレス下リターン: {results.get('stressed_return', 0):.2%}")
                            if 'survival_probability' in results:
                                report_lines.append(f"  生存確率: {results['survival_probability']:.1%}")
                            if 'volatility_multiplier' in results:
                                report_lines.append(f"  ボラティリティ倍率: {results['volatility_multiplier']:.1f}")
                            if 'cost_multiplier' in results:
                                report_lines.append(f"  コスト倍率: {results['cost_multiplier']:.1f}")
                            if 'affected_periods' in results:
                                report_lines.append(f"  影響期間数: {results['affected_periods']}")
                        report_lines.append("")
                except Exception as e:
                    ...
                    if wf_efficiency:
                        ...
                        for window_name, metrics in wf_efficiency.items():
                            if window_name.startswith('window_'):
                                report_lines.append(f"{window_name.replace('_', ' ').title()}:")
                                report_lines.append(f"  平均リターン: {metrics.get('mean_return', 0):.4f}")
                                report_lines.append(f"  ボラティリティ: {metrics.get('volatility', 0):.4f}")
                                report_lines.append(f"  シャープレシオ: {metrics.get('sharpe_ratio', 0):.3f}")
                                report_lines.append(f"  一貫性スコア: {metrics.get('consistency_score', 0):.3f}")
                            elif window_name == 'adaptation_analysis':
                                report_lines.append("学習適応分析:")
                                report_lines.append(f"  前半リターン: {metrics.get('first_half_return', 0):.4f}")
                                report_lines.append(f"  後半リターン: {metrics.get('second_half_return', 0):.4f}")
                                report_lines.append(f"  適応比率: {metrics.get('adaptation_ratio', 0):.3f}")
                                report_lines.append(f"  学習効率: {metrics.get('learning_efficiency', 0):.3f}")
                        report_lines.append("")
                except Exception as e:
                ...
                try:
                    ...
                    if microstructure:
                        ...
                        if price_impact:
                            ...
                            report_lines.append(f"  平均インパクト: {price_impact.get('average_impact', 0):.4f}")
                            report_lines.append(f"  インパクトボラティリティ: {price_impact.get('impact_volatility', 0):.4f}")
                            report_lines.append(f"  逆選択コスト: {price_impact.get('adverse_selection_cost', 0):.4f}")

                        # 市場の深さ
                        market_depth = microstructure.get('market_depth', {})
                        if market_depth:
                            report_lines.append("市場の深さ:")
                            report_lines.append(f"  価格ボラティリティ: {market_depth.get('price_volatility', 0):.4f}")
                            report_lines.append(f"  流動性プロキシ: {market_depth.get('liquidity_proxy', 0):.3f}")
                            report_lines.append(f"  取引効率: {market_depth.get('trading_efficiency', 0):.3f}")

                        # スプレッド分析
                        spread_analysis = microstructure.get('spread_analysis', {})
                        if spread_analysis:
                            report_lines.append("スプレッド分析:")
                            report_lines.append(f"  推定スプレッド: ¥{spread_analysis.get('estimated_spread', 0):,.2f}")
                            report_lines.append(f"  スプレッド/価格比: {spread_analysis.get('spread_to_price_ratio', 0):.4f}")
                            report_lines.append(f"  スリッページリスク: ¥{spread_analysis.get('slippage_risk', 0):,.0f}")

                        # 行動パターン
                        behavioral = microstructure.get('behavioral_patterns', {})
                        if behavioral:
                            report_lines.append("行動パターン:")
                            report_lines.append(f"  アクション自己相関: {behavioral.get('action_autocorrelation', 0):.3f}")
                            report_lines.append(f"  モメンタム効果: {behavioral.get('momentum_effect', 0):.3f}")
                            report_lines.append(f"  平均回帰傾向: {behavioral.get('mean_reversion_tendency', 0):.3f}")
                        report_lines.append("")
                except Exception as e:
    ...
    def analyze_robustness(self) -> Dict[str, Any]:
        ...
        with self.performance_monitor:
            ...
            # 様々な市場条件下での分析
            robustness_metrics['overall_performance'] = self._calculate_performance_metrics(pnls)

            # ボラティリティ別の分析
            try:
                robustness_metrics['volatility_analysis'] = self._analyze_by_volatility_regimes(pnls)
            except Exception as e:
                robustness_metrics['volatility_analysis'] = {'error': f'Volatility analysis failed: {str(e)}'}

            # トレンド別の分析
            try:
                robustness_metrics['trend_analysis'] = self._analyze_by_trend_regimes(pnls)
            except Exception as e:
                robustness_metrics['trend_analysis'] = {'error': f'Trend analysis failed: {str(e)}'}

            # ドローダウン別の分析
            try:
                robustness_metrics['drawdown_analysis'] = self._analyze_by_drawdown_periods(pnls)
            except Exception as e:
                robustness_metrics['drawdown_analysis'] = {'error': f'Drawdown analysis failed: {str(e)}'}

            # 季節性分析
            try:
                robustness_metrics['seasonal_analysis'] = self._analyze_seasonal_performance(pnls)
            except Exception as e:
                robustness_metrics['seasonal_analysis'] = {'error': f'Seasonal analysis failed: {str(e)}'}

            # ロバストネススコアの計算
            robustness_metrics['robustness_score'] = self._calculate_robustness_score(robustness_metrics)
    ...
    def _analyze_by_volatility_regimes(self, pnls: np.ndarray) -> Dict[str, Any]:
        ...
        high_vol_returns = pnls[50:][high_vol_mask.values]  # 最初の50期間をスキップし、有効なマスクを適用
        low_vol_returns = pnls[50:][low_vol_mask.values]

        return {
            'high_volatility_performance': self._calculate_performance_metrics(high_vol_returns),
            'low_volatility_performance': self._calculate_performance_metrics(low_vol_returns),
    ...
    def _analyze_by_trend_regimes(self, pnls: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            'downtrend_performance': self._calculate_performance_metrics(downtrend_returns),
            'sideways_performance': self._calculate_performance_metrics(sideways_returns),
    ...
    def _analyze_by_drawdown_periods(self, pnls: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            'drawdown_performance': self._calculate_performance_metrics(drawdown_returns),
            'recovery_performance': self._calculate_performance_metrics(recovery_returns),
    ...
    def _analyze_seasonal_performance(self, pnls: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            'seasonal_consistency_score': self._calculate_seasonal_consistency(hourly_returns, weekday_returns, monthly_returns)
        }
    ...
    def _calculate_regime_consistency(self, *regime_metrics) -> float:
        ...
        # シャープレシオと勝率の変動係数を計算
        sharpe_cv = np.std(sharpe_ratios) / np.mean(sharpe_ratios) if np.mean(sharpe_ratios) != 0 else 1
        win_rate_cv = np.std(win_rates) / np.mean(win_rates) if np.mean(win_rates) != 0 else 1

        # 一貫性スコア（低い変動係数 = 高い一貫性）
        consistency_score = 1 / (1 + sharpe_cv + win_rate_cv)
        return consistency_score

    def _calculate_seasonal_consistency(self, hourly: Dict, weekday: Dict, monthly: Dict) -> float:
        """季節性の一貫性を計算"""
        seasonal_scores = []
        ...
        if hourly:
            ...
            if len(hourly_sharpes) > 1:
                seasonal_scores.append(1 / (1 + np.std(hourly_sharpes) / max(np.mean(hourly_sharpes), 0.01)))

        # 曜日別一貫性
        if weekday:
            weekday_sharpes = [m.get('sharpe_ratio', 0) for m in weekday.values() if m]
            if len(weekday_sharpes) > 1:
                seasonal_scores.append(1 / (1 + np.std(weekday_sharpes) / max(np.mean(weekday_sharpes), 0.01)))

        # 月別一貫性
        if monthly:
            monthly_sharpes = [m.get('sharpe_ratio', 0) for m in monthly.values() if m]
            if len(monthly_sharpes) > 1:
                seasonal_scores.append(1 / (1 + np.std(monthly_sharpes) / max(np.mean(monthly_sharpes), 0.01)))
    ...
    def _calculate_robustness_score(self, robustness_metrics: Dict) -> float:
        ...
        # レジーム間一貫性スコア
        vol_consistency = robustness_metrics.get('volatility_analysis', {}).get('volatility_regime_consistency', 0)
        trend_consistency = robustness_metrics.get('trend_analysis', {}).get('trend_regime_consistency', 0)
        drawdown_consistency = robustness_metrics.get('drawdown_analysis', {}).get('drawdown_recovery_consistency', 0)
        seasonal_consistency = robustness_metrics.get('seasonal_analysis', {}).get('seasonal_consistency_score', 0)

        scores.extend([vol_consistency, trend_consistency, drawdown_consistency, seasonal_consistency])
    ...
    def analyze_correlation_and_dependencies(self) -> Dict[str, Any]:
        ...
        if np.std(price_returns) > 0:
            beta = np.cov(portfolio_returns, price_returns)[0, 1] / np.var(price_returns)
        else:
            beta = 0.0
        ...
        if 'actions' in self.data:
            ...
            if np.sum(hold_mask) > 0:
                action_price_correlations['hold_price_change'] = np.mean(price_returns_trimmed[hold_mask])
            if np.sum(buy_mask) > 0:
                action_price_correlations['buy_price_change'] = np.mean(price_returns_trimmed[buy_mask])
            if np.sum(sell_mask) > 0:
                action_price_correlations['sell_price_change'] = np.mean(price_returns_trimmed[sell_mask])
    ...
    def analyze_transaction_cost_impact(self) -> Dict[str, Any]:
        ...
        transaction_cost = self.training_data.get('configuration', {}).get('environment', {}).get('transaction_cost', 0.00001)

        # 取引が発生したステップを特定
        trade_mask = (actions == 1) | (actions == 2)  # BUY or SELL
        ...
        if 'portfolio_history' in self.data:
            ...
            cost_ratio = total_transaction_cost / abs(gross_return) if gross_return != 0 else float('inf')
        else:
            cost_ratio = 0.0
        ...
        return {
            ...
            'average_cost_per_trade': np.mean(trade_costs) if len(trade_costs) > 0 else 0,
            'cost_to_return_ratio': cost_ratio,
            'trades_per_step': trades_per_step,
            'cost_efficiency_score': 1.0 / (1.0 + cost_ratio) if cost_ratio != float('inf') else 0.0
        }
    ...
    def perform_stress_tests(self) -> Dict[str, Any]:
        ...
        for drop_pct in [0.1, 0.2, 0.3]:
            ...
            stressed_return = (stressed_portfolio[-1] - stressed_portfolio[0]) / stressed_portfolio[0]
            stress_tests[f'price_drop_{int(drop_pct*100)}pct'] = {
                'stressed_return': stressed_return,
                'survival_probability': 1.0 if stressed_portfolio[-1] > stressed_portfolio[0] * 0.5 else 0.5
            }
        ...
        if len(portfolio_values) > 20:
            ...
            stressed_return = (stressed_portfolio[-1] - portfolio_values[0]) / portfolio_values[0]
        ...
        if transaction_impact:
            ...
            if 'portfolio_history' in self.data:
                ...
                stressed_return = (final_value - increased_cost - self.data['portfolio_history'][0]) / self.data['portfolio_history'][0]
    ...
    def analyze_walk_forward_efficiency(self) -> Dict[str, Any]:
        ...
        for window_size in window_sizes:
            ...
            if window_returns:
                ...
                walk_forward_metrics[f'window_{window_size}'] = {
                    ...
                    'consistency_score': 1.0 / (1.0 + np.std(window_returns) / max(abs(np.mean(window_returns)), 0.001))
                }
        ...
        if len(portfolio_values) > 10:
            ...
            adaptation_ratio = second_half_return / first_half_return if first_half_return != 0 else 0
            ...
            walk_forward_metrics['adaptation_analysis'] = {
                ...
                'learning_efficiency': max(0, adaptation_ratio)  # 学習効率：改善した場合のみ正
            }
    ...
    def analyze_microstructure_effects(self) -> Dict[str, Any]:
        ...
        if price_impacts:
            microstructure_analysis['price_impact'] = {
                ...
                'adverse_selection_cost': abs(np.mean(price_impacts))  # 逆選択コストの推定
            }
        ...
        if len(prices) > 10:
            ...
            microstructure_analysis['market_depth'] = {
                ...
                'liquidity_proxy': 1.0 / (1.0 + price_volatility),  # ボラティリティが高いほどliquidityが低い
                'trading_efficiency': 1.0 / (1.0 + price_volatility * len(trade_indices))  # 取引頻度を考慮
            }
        ...
        if len(prices) > 1:
            ...
            spread_estimate = np.mean(np.abs(price_changes)) * 0.1  # 簡易的な推定
            microstructure_analysis['spread_analysis'] = {
                ...
                'slippage_risk': spread_estimate * len(trade_indices)  # 総スリッページリスク
            }
    ...
    def create_market_condition_balanced_dataset(self, output_path: str = "data/btc_balanced_dataset.csv"):
    ...
    def enhance_feature_engineering(self):
        ...
        feature_improvements = {
            ...
            "volatility_regime_features": {
                ...
                "features": ["volatility_ratio", "volatility_trend", "regime_stability"],
            ...
            "correlation_aware_features": {
                ...
                "features": ["price_position_corr", "action_price_corr", "regime_alignment"],
    ...
    def generate_v425_improvement_plan(self):
        ...
        print(f"\n総工期: 10-15日")
        print("既存活用率: 85%（BTCDataAugmentor, BTCBiasDetector, RewardCalculator, analyze_backtest.py）")
