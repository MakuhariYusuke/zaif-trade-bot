...
def analyze_observations(
    config_path: str, config_name: str, num_samples: int = 10
) -> Dict[str, Any]:
    """観測値を解析"""
    print(f"\n{'='*80}")
    ...
    print("環境作成成功")
    print(
        f"  - use_standardized_observations: {getattr(config_obj, 'use_standardized_observations', False)}"
    )
    ...
    if hasattr(env.observation_builder, "scaler_mean"):
        if (
            env.observation_builder.scaler_mean is not None
            and env.observation_builder.scaler_std is not None
        ):
            print("\n✅ スケーラー設定あり")
            print(
                f"  - scaler_mean: min={env.observation_builder.scaler_mean.min():.6f}, max={env.observation_builder.scaler_mean.max():.6f}"
            )
            print(
                f"  - scaler_std: min={env.observation_builder.scaler_std.min():.6f}, max={env.observation_builder.scaler_std.max():.6f}"
            )
        else:
            print("\n❌ スケーラー設定なし（正規化されません）")
    else:
    ...
    for i, stat in enumerate(sorted_by_max[:5], 1):
        print(
            f"  {i}. Feature {stat['index']}: [{stat['min']:.6f}, {stat['max']:.6f}], mean={stat['mean']:.6f}, std={stat['std']:.6f}"
        )
    ...
    for i, stat in enumerate(sorted_by_std[:5], 1):
        print(
            f"  {i}. Feature {stat['index']}: std={stat['std']:.6f}, mean={stat['mean']:.6f}, range=[{stat['min']:.6f}, {stat['max']:.6f}]"
        )

    return {
        "config_name": config_name,
        "use_standardized_observations": getattr(
            config_obj, "use_standardized_observations", False
        ),
        "has_scaler": env.observation_builder.scaler_mean is not None
        if hasattr(env.observation_builder, "scaler_mean")
        else False,
...
def main() -> None:
    ...
    result_v395g = analyze_observations(
        "configs/sac_v395g_micro_reward.json", "v395g (正規化なし)"
    )

    # v395i（正規化あり）
    result_v395i = analyze_observations(
        "configs/sac_v395i_complete_fix.json", "v395i (正規化あり)"
    )
    ...
    print("【v395g（正規化なし）】")
    print(
        f"  use_standardized_observations: {result_v395g['use_standardized_observations']}"
    )
    print(f"  スケーラー: {result_v395g['has_scaler']}")
    print(
        f"  観測値範囲: [{result_v395g['overall_stats']['min']:.6f}, {result_v395g['overall_stats']['max']:.6f}]"
    )
    ...
    print("\n【v395i（正規化あり）】")
    print(
        f"  use_standardized_observations: {result_v395i['use_standardized_observations']}"
    )
    print(f"  スケーラー: {result_v395i['has_scaler']}")
    print(
        f"  観測値範囲: [{result_v395i['overall_stats']['min']:.6f}, {result_v395i['overall_stats']['max']:.6f}]"
    )
    ...
    print("\n【改善効果】")
    max_reduction = (
        (result_v395g["overall_stats"]["max"] - result_v395i["overall_stats"]["max"])
        / result_v395g["overall_stats"]["max"]
        * 100
    )
    std_reduction = (
        (result_v395g["overall_stats"]["std"] - result_v395i["overall_stats"]["std"])
        / result_v395g["overall_stats"]["std"]
        * 100
    )
    ...
    with open("observation_comparison.json", "w", encoding="utf-8") as f:
        json.dump(
            {"v395g": result_v395g, "v395i": result_v395i},
            f,
            indent=2,
            ensure_ascii=False,
        )
