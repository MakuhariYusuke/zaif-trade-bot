...
class BenchmarkComparisonAnalyzer:
    ...
    def rolling_comparison_analysis(
        ...
        for window_size in window_sizes:
            if (
                len(strategy_returns) < window_size
                or len(benchmark_returns) < window_size
            ):
            ...
            for i in range(window_size, len(strategy_returns) + 1):
                ...
                try:
                    alpha, beta, _, tracking_error = self.calculate_regression_metrics(
                        strat_win, bench_win
                    )
    ...
    def analyze(self, metrics: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_result: Dict[str, Any] = {
                "benchmark_score": result.comparisons[0].information_ratio
                ...
                "information_ratio": result.comparisons[0].information_ratio
                if result.comparisons
                else 0.0,
                "tracking_error": result.comparisons[0].tracking_error
                if result.comparisons
                else 0.0,
                "r_squared": result.comparisons[0].r_squared
                if result.comparisons
                else 0.0,
            ...
            if result.rolling_comparisons:
                for rolling_comp in result.rolling_comparisons:
                    analysis_result["rolling_alpha"].extend(
                        rolling_comp.rolling_alpha.tolist()
                    )
                    analysis_result["rolling_beta"].extend(
                        rolling_comp.rolling_beta.tolist()
                    )
                    analysis_result["rolling_tracking_error"].extend(
                        rolling_comp.rolling_tracking_error.tolist()
                    )
            ...
            if result.multi_benchmark_summary:
                ...
                ] = result.multi_benchmark_summary
...
R²: {comparison.r_squared:.4f}"""
    ...
    def plot_rolling_comparison(
        self,
        rolling_comparisons: List[RollingComparison],
    ...
    def export_results(self, result: BenchmarkComparisonResult, output_path: str):
        ...
        if result.multi_benchmark_summary:
            export_data["multi_benchmark_summary"] = result.multi_benchmark_summary[
                "summary_stats"
            ]
...
def main():
    ...
    parser.add_argument(
        "--strategy-data", required=True, help="Path to strategy returns (CSV)"
    )
    parser.add_argument(
        "--benchmark-data",
        required=True,
        help="Path to benchmark data (JSON with benchmark names and CSV paths)",
    )
    ...
    if result.rolling_comparisons:
        rolling_plot_path = os.path.join(
            args.output_dir, "rolling_benchmark_comparison.png"
        )
        analyzer.plot_rolling_comparison(
            result.rolling_comparisons, save_path=rolling_plot_path
        )
