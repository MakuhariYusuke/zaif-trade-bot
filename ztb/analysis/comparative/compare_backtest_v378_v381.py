...
def run_backtest_for_model(
    ...
    for episode in range(episodes):
        ...
        print(
            f"  Episode {episode + 1:2d}: Reward={episode_reward:8.2f}, PnL={episode_pnl:10.6f}, Steps={steps:4d}"
        )
    ...
    results = {
        ...
        "action_distribution": {
            "HOLD": (action_counts["HOLD"] / total_actions * 100)
            if total_actions > 0
            else 0,
            "BUY": (action_counts["BUY"] / total_actions * 100)
            if total_actions > 0
            else 0,
            "SELL": (action_counts["SELL"] / total_actions * 100)
            if total_actions > 0
            else 0,
        },
...
def compare_models(results1: Dict[str, Any], results2: Dict[str, Any]) -> None:
    ...
    print("\n📊 Average Reward:")
    print(
        f"  {model1_name:20s}: {results1['avg_reward']:8.2f} ± {results1['std_reward']:.2f}"
    )
    print(
        f"  {model2_name:20s}: {results2['avg_reward']:8.2f} ± {results2['std_reward']:.2f}"
    )
    reward_diff = results2["avg_reward"] - results1["avg_reward"]
    reward_pct = (
        (reward_diff / abs(results1["avg_reward"]) * 100)
        if results1["avg_reward"] != 0
        else 0
    )
    ...
    print("\n💰 Average PnL:")
    print(
        f"  {model1_name:20s}: {results1['avg_pnl']:10.6f} ± {results1['std_pnl']:.6f}"
    )
    print(
        f"  {model2_name:20s}: {results2['avg_pnl']:10.6f} ± {results2['std_pnl']:.6f}"
    )
    ...
    print("\n🎯 Action Distribution:")
    print(
        f"  {'Action':<10s} {model1_name:>15s} {model2_name:>15s} {'Difference':>12s}"
    )
    print(f"  {'-'*10} {'-'*15} {'-'*15} {'-'*12}")
    ...
    if (
        results2["action_distribution"]["HOLD"]
        < results1["action_distribution"]["HOLD"]
    ):
        criteria_scores[model2_name] += 1
