...
def extract_metrics(log_dir: str) -> Optional[Any]:
    ...
    if "rollout/ep_rew_mean" in ea.Tags()["scalars"]:
        ...
        metrics["reward_trend_last10"] = (
            [r.value for r in rewards[-10:]] if len(rewards) >= 10 else []
        )
    ...
    if "train/explained_variance" in ea.Tags()["scalars"]:
        ...
        metrics["explained_variance"] = [
            {"step": e.step, "value": e.value} for e in evs
        ]
        metrics["final_ev"] = evs[-1].value if evs else None
        metrics["avg_ev_last10"] = (
            np.mean([e.value for e in evs[-10:]]) if len(evs) >= 10 else None
        )
...
def main() -> None:
    ...
    for name, log_dir in log_dirs.items():
        ...
        if metrics:
            ...
            print(
                f"✅ Final Policy Gradient Loss: {metrics.get('final_pg_loss', 'N/A'):.4f}"
            )
    ...
    if "v378_scale" in results and "v381_revised" in results:
        ...
        reward_improvement = (
            (v381["final_reward"] - v378["final_reward"]) / abs(v378["final_reward"])
        ) * 100
        ev_improvement = (
            (v381["final_ev"] - v378["final_ev"]) / max(abs(v378["final_ev"]), 0.01)
        ) * 100
        ...
        print("\n📊 Explained Variance:")
        print(
            f"  v378:  {v378['final_ev']:.4f} (avg last10: {v378['avg_ev_last10']:.4f})"
        )
        print(
            f"  v381:  {v381['final_ev']:.4f} (avg last10: {v381['avg_ev_last10']:.4f})"
        )
