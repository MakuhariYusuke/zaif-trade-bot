...
class RegimeEvaluator:
    ...
    def calculate_regime_metrics(
        ...
        regime_labels: NDArray[np.str_],
        regime: str,
        actions: Optional[NDArray[np.int_]] = None,
        ...
        volatility = float(
            np.std(regime_returns_array) * np.sqrt(TRADING_DAYS_PER_YEAR)
        )  # type: ignore

        # Sharpe比率
        risk_free_rate = 0.02  # 仮定の無リスク金利
        sharpe_ratio = (
            (regime_returns.mean() - risk_free_rate / TRADING_DAYS_PER_YEAR)
            / volatility
            if volatility > 0
            else 0
        )
    ...
    def analyze_regime_performance(
        ...
        regime_labels: NDArray[np.str_],
        actions_dict: Optional[Dict[str, Tuple[NDArray[np.int_], int]]] = None,
    ...
    def plot_regime_analysis(
        self, result: RegimeAnalysisResult, save_path: Optional[str] = None
    ) -> None:
        """レジーム分析結果を可視化"""
        ...
        for model in model_names:
            winrate_row = [
                result.regime_metrics[model][regime].win_rate for regime in regimes
            ]
...
def main() -> None:
    ...
    result = evaluator.analyze_regime_performance(
        models, price_data, regime_labels, actions_dict
    )
    ...
    # 結果を辞書に変換して保存
    result_dict = {"regime_counts": result.regime_counts, "regime_metrics": {}}
