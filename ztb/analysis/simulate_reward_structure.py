...
def simulate_rewards():
    ...
    for name, base_profit, trend_mult in scenarios:
        ...
        print(f"  HOLD:  profit={hold_profit_bonus:+.3f} - penalty={hold_action_penalty:.3f} + loss={hold_loss_penalty:+.3f} = {hold_reward:+.3f}")
        print(f"  BUY:   profit={buy_profit_bonus:+.3f} - penalty={buy_action_penalty:.3f} + loss={buy_loss_penalty:+.3f} = {buy_reward:+.3f}")
        print(f"  SELL:  profit={sell_profit_bonus:+.3f} - penalty={sell_action_penalty:.3f} + loss={sell_loss_penalty:+.3f} = {sell_reward:+.3f}")
        
        best = max([("HOLD", hold_reward), ("BUY", buy_reward), ("SELL", sell_reward)], key=lambda x: x[1])
