...
class MultiTimeframeDataPipeline:
    ...
    def synchronize_timeframes(
        ...
        if base_timeframe not in data_dict:
            logger.warning(
                f"Base timeframe {base_timeframe.value} not found, using first available"
            )
        ...
        for timeframe, df in data_dict.items():
            ...
            try:
                synced_df = self._synchronize_to_base(
                    df, base_df, timeframe, base_timeframe
                )
    ...
    def _synchronize_to_base(
        ...
        """Synchronize dataframe to base timeframe timestamps using proper time alignment."""
        ...
        # This finds the nearest previous timestamp in df for each base_df timestamp
        merged = pd.merge_asof(
            base_df[["timestamp"]],  # Keep only timestamp from base
            df,  # All columns from source df
            on="timestamp",
            direction="backward",  # Use previous available data
            tolerance=pd.Timedelta(
                minutes=self._get_timeframe_minutes(source_timeframe)
            ),  # Allow some tolerance
        )

        # Forward fill any missing values within reasonable time gaps
        merged = merged.set_index("timestamp")
        # Fill gaps of up to the source timeframe duration
        max_gap = pd.Timedelta(
            minutes=self._get_timeframe_minutes(source_timeframe) * 2
        )
        merged = merged.ffill(limit=1)  # Only fill immediate gaps

        # Reset index
        merged = merged.reset_index()

        # Remove rows where we couldn't find matching data
        merged = merged.dropna()

        logger.debug(
            f"Synchronized {len(df)} source rows to {len(merged)} aligned rows for {source_timeframe.value}"
        )
    ...
    def generate_missing_timeframes(
        ...
        finest_timeframe = min(
            available_timeframes, key=lambda tf: self._get_timeframe_minutes(tf)
        )
        ...
        for target_tf in target_timeframes:
            ...
            try:
                resampled_df = self._resample_timeframe(
                    base_df, finest_timeframe, target_tf
                )
    ...
    def _resample_timeframe(
        ...
        # Resample OHLCV data
        resampled = (
            df_indexed.resample(target_freq)
            .agg(
                {
                    "open": "first",
                    "high": "max",
    ...
    def get_data_quality_report(
        ...
        for timeframe, df in data_dict.items():
            tf_report = {
                ...
                "data_quality": {
                    ...
                    "positive_prices": (df[["open", "high", "low", "close"]] > 0)
                    .all()
                    .all(),
                },
            }
        ...
        report["summary"] = {
            ...
            "avg_rows_per_timeframe": np.mean(
                [r["row_count"] for r in report["timeframes"].values()]
            ),
            "data_completeness": np.mean(
                [r["data_quality"]["valid_ohlc"] for r in report["timeframes"].values()]
            ),
        }
