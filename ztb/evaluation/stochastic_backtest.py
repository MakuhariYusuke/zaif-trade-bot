...
def run_stochastic_backtest(
    ...
    for ep in range(episodes):
        ...
        while True:
            ...
            if bool(cast(Any, done)[0]):
                ...
                final_value = info_list[0].get('portfolio_value', initial_portfolio_value)
                trades = int(info_list[0].get('total_trades', 0))
                total_trades += trades
                
                returns = ((final_value - initial_portfolio_value) / initial_portfolio_value) * 100
    ...
    print(f"{'='*80}")
    print(f"Average Reward:   {episode_rewards_array.mean():7.2f} ± {episode_rewards_array.std():6.2f}")
    print(f"Average Return:   {episode_returns_array.mean():6.2f}% ± {episode_returns_array.std():5.2f}%")
    ...
    print(f"  HOLD: {action_counts[0]:4d}/{total_actions} ({action_counts[0]/total_actions*100:5.1f}%)")
    print(f"  BUY:  {action_counts[1]:4d}/{total_actions} ({action_counts[1]/total_actions*100:5.1f}%)")
    print(f"  SELL: {action_counts[2]:4d}/{total_actions} ({action_counts[2]/total_actions*100:5.1f}%)")
    print(f"{'='*80}")
