...
from ztb.evaluation.preprocess import SmartPreprocessor, prepare_ohlc_data
...
def evaluate_feature_class(
    ...
    try:
        ...
        aligned_data = pd.concat(
            [feature_df, returns.rename("returns")], axis=1
        ).dropna()
        ...
        cv_results = evaluate_with_cv(
            feature_df, aligned_data["returns"], n_splits=3, test_size=30
        )
                    ...
                    try:
                        ...
                        if len(top_periods) > 5 and len(bottom_periods) > 5:
                            ...
                            feature_performances[col] = {
                                ...
                                "quantile_spread": top_metrics["sharpe_ratio"]
                                - bottom_metrics["sharpe_ratio"],
        ...
        for fname, perf in feature_performances.items():
            if perf["type"] == "signal":
                ...
                delta_sharpe = abs(
                    float(
                        delta_sharpe_val
                        if isinstance(delta_sharpe_val, (int, float))
                        else 0.0
                    )
                )
            else:
                ...
                delta_sharpe = abs(
                    float(
                        quantile_spread_val
                        if isinstance(quantile_spread_val, (int, float))
                        else 0.0
                    )
                )
...
class ComprehensiveFeatureReEvaluator:
        ...
        try:
            if self.price_data_path.endswith(".parquet"):
                ...
                try:
                    ...
                    if columns_to_load:
                        df = pd.read_parquet(
                            self.price_data_path, columns=columns_to_load
                        )
                        print(
                            f"Loaded {len(columns_to_load)}/{len(self.essential_columns)} essential columns from parquet"
                        )
                        return df
                    else:
                        print(
                            "No essential columns found in parquet, loading all columns"
                        )
    ...
    def evaluate_experimental_features(self, ohlc_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for module_path in self.experimental_modules:
            ...
            print(
                f"Found {len(discovered_features)} feature classes: {list(discovered_features.keys())}"
            )
    ...
    def classify_experimental_feature_status(
        ...
        if computation_time > 1000:  # > 1 second
            return (
                "🟡",
                "slow_computation",
                f"Computation time too high ({computation_time:.0f}ms)",
            )
        ...
        if avg_correlation > 0.8:
            return (
                "🔴",
                "high_corr",
                f"High correlation with returns ({avg_correlation:.3f})",
            )
        ...
        if best_delta_sharpe > 0.15:
            return (
                "🟢",
                "strong_performance",
                f"Strong delta Sharpe: {best_delta_sharpe:.4f}",
            )
        elif best_delta_sharpe > 0.05:
            return (
                "🟡",
                "moderate_performance",
                f"Moderate delta Sharpe: {best_delta_sharpe:.4f}",
            )
        elif best_delta_sharpe < -0.05:
            return (
                "🔴",
                "negative_performance",
                f"Negative delta Sharpe: {best_delta_sharpe:.4f}",
            )
        else:
            return (
                "🟡",
                "weak_performance",
                f"Weak delta Sharpe: {best_delta_sharpe:.4f}",
            )
        ...
        if composite:
            ...
            if delta_sharpe > 0.15 and trade_periods > 50:
                return (
                    "🟢",
                    "strong_composite",
                    "Remove from harmful list - Strong composite performance",
                )
            elif delta_sharpe > 0.05 and trade_periods > 30:
                return (
                    "🟡",
                    "moderate_composite",
                    "Conditional use - Moderate composite improvement",
                )

        # Check individual feature performance
        if best_individual:
            best_score = (
                abs(best_individual[0][1]) if best_individual[0][1] is not None else 0
            )
            ...
            if best_score > 0.2:
                return (
                    "🟡",
                    "strong_individual",
                    "Conditional use - Strong individual features",
                )
            elif best_score > 0.1:
                return (
                    "🟡",
                    "moderate_individual",
                    "Monitor closely - Some promising features",
                )

        return (
            "🔴",
            "no_improvement",
            "Keep in harmful list - No significant improvement",
        )
    ...
    def evaluate_harmful_feature(
        ...
        try:
            ...
            if feature_name == "KalmanFilter":
                extended_features = evaluator(
                    ohlc_data, process_variance=0.001, measurement_variance=0.1
                )
            else:
                extended_features = evaluator(ohlc_data)

            print(
                f"Calculated {len(extended_features.columns)} features for {feature_name}"
            )
            ...
            # Align features with returns
            aligned_data = pd.concat(
                [extended_features, returns.rename("returns")], axis=1
            ).dropna()
            ...
            for feature_col, result in feature_results.items():
                if result["type"] == "signal" and result["delta_sharpe"] > 0.05:
                    ...
                    result["type"] == "quantile"
                    and abs(result["quantile_spread"]) > 0.1
                ):
                    best_features.append(feature_col)
                    ...
                    if feature_col in aligned_data.columns:
                        if aligned_data[feature_col].nunique() <= 5:
                            ...
                            composite_signals.append(
                                aligned_data[feature_col]
                                > aligned_data[feature_col].median()
                            )
                        else:
                ...
                if len(composite_signals) >= 2:
                    ...
                    if len(composite_periods) > 20:
                        ...
                        composite_result = {
                            ...
                            "baseline_sharpe": baseline_metrics["sharpe_ratio"],
                            "delta_sharpe": composite_metrics["sharpe_ratio"]
                            - baseline_metrics["sharpe_ratio"],
    ...
    def generate_comprehensive_report(self, evaluation_results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"- **Experimental/Wave Features Evaluated**: {len(experimental_results)}"
        )

        # Count by status
        all_results = {**harmful_results, **experimental_results}
        green_count = sum(
            1 for r in all_results.values() if r.get("status_emoji") == "🟢"
        )
        yellow_count = sum(
            1 for r in all_results.values() if r.get("status_emoji") == "🟡"
        )
        ...
        if harmful_results:
            ...
            report.append(
                "| Feature | Status | Delta Sharpe | Reason | Recommendation |"
            )
            report.append(
                "|---------|--------|--------------|--------|-----------------|"
            )
            ...
            for feature_name, result in harmful_results.items():
                ...
                report.append(
                    f"| {feature_name} | {status_emoji} | {delta_sharpe:+.4f} | {reason_tag} | {recommendation} |"
                )
        ...
        if experimental_results:
            ...
            report.append(
                "| Feature | Module | Status | NaN Rate | Time (ms) | Delta Sharpe | Reason | Recommendation |"
            )
            report.append(
                "|---------|--------|--------|----------|-----------|--------------|--------|-----------------|"
            )
            ...
            for feature_name, result in experimental_results.items():
                ...
                report.append(
                    f"| {feature_name} | {module} | {status_emoji} | {nan_rate:.1%} | {time_ms:.0f} | {delta_sharpe:+.4f} | {reason_tag} | {recommendation} |"
                )
        ...
        if harmful_results:
            ...
            for feature_name, result in harmful_results.items():
                ...
                report.append(
                    f"**Status**: {result.get('status_emoji', '⚪')} {result.get('recommendation', '')}"
                )
                report.append(
                    f"**Extended Features**: {result.get('total_features', 'N/A')}"
                )
                report.append(
                    f"**Evaluation Periods**: {result.get('evaluation_periods', 'N/A')}"
                )
                report.append("")
                ...
                if composite:
                    ...
                    report.append(
                        f"- Features Used: {', '.join(composite.get('features_used', []))}"
                    )
                    report.append(
                        f"- Composite Sharpe: {composite.get('composite_sharpe', 0):.4f}"
                    )
                    report.append(
                        f"- Baseline Sharpe: {composite.get('baseline_sharpe', 0):.4f}"
                    )
                    report.append(
                        f"- **Improvement**: {composite.get('delta_sharpe', 0):+.4f}"
                    )
                    report.append(
                        f"- Trade Periods: {composite.get('trade_periods', 0)}"
                    )
        ...
        green_harmful = [
            name
            for name, result in harmful_results.items()
            if result.get("status_emoji") == "🟢"
        ]
        green_experimental = [
            name
        ...
        if green_experimental:
            ...
            for feature in green_experimental:
                report.append(
                    f"- **{feature}**: {experimental_results[feature].get('recommendation', '')}"
                )
            report.append("")
    ...
    def _generate_unverified_features_section(self) -> List[str]:
        ...
        try:
            ...
            section.append(
                f"**Total Verified**: {metadata.get('total_verified', 0)} | "
            ...
            if pending:
                ...
                section.append(
                    "| Feature | Reason | Required Samples | Current Samples | Last Attempt |"
                )
                section.append(
                    "|---------|--------|------------------|-----------------|--------------|"
                )
                ...
                for feature_name, data in pending.items():
                    ...
                    section.append(
                        f"| {feature_name} | {reason} | {required} | {current} | {last_attempt} |"
                    )
            ...
            if failed:
                ...
                section.append(
                    "| Feature | Reason | Sharpe | Win Rate | Max Drawdown | Last Attempt |"
                )
                section.append(
                    "|---------|--------|--------|----------|--------------|--------------|"
                )
                ...
                for feature_name, data in failed.items():
                    ...
                    section.append(
                        f"| {feature_name} | {reason} | {sharpe} | {win_rate} | {max_dd} | {last_attempt} |"
                    )
    ...
    def generate_report(self, evaluation_results: Dict[str, Any]) -> str:
        ...
        for feature_name, result in evaluation_results["results"].items():
            ...
            report.append(
                f"| {feature_name} | {result['status_emoji']} | {delta_sharpe:+.4f} | {result['recommendation']} |"
            )
        ...
        for feature_name, result in evaluation_results["results"].items():
            report.append(f"### {feature_name}")
            report.append(
                f"**Status**: {result['status_emoji']} {result['recommendation']}"
            )
            ...
            if composite:
                report.append("#### Composite Strategy Performance")
                report.append(
                    f"- Features Used: {', '.join(composite['features_used'])}"
                )
                report.append(
                    f"- Composite Sharpe: {composite['composite_sharpe']:.4f}"
                )
...
def main() -> None:
    ...
    for feature_name, result in harmful_results.items():
        print(
            f"  {feature_name}: {result.get('status_emoji', '⚪')} ({result.get('reason_tag', 'unknown')})"
        )
    ...
    if yellow_features:
        print(
            f"  🟡 Monitor/conditional use ({len(yellow_features)}): {', '.join(yellow_features)}"
        )
    if red_features:
