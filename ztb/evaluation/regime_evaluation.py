...
class RegimeEvaluator:
    ...
    def classify_market_regime(self, price_data: pd.DataFrame) -> Tuple[NDArray[np.str_], Dict[str, int]]:
        ...
        # ボラティリティの計算（標準偏差）
        volatility = returns.rolling(window=self.volatility_window).std() * np.sqrt(TRADING_DAYS_PER_YEAR)  # 年率化
    ...
    def calculate_regime_metrics(self, returns: pd.Series, regime_labels: NDArray[np.str_],
        ...
        volatility = float(np.std(regime_returns_array) * np.sqrt(TRADING_DAYS_PER_YEAR))  # type: ignore

        # Sharpe比率
        risk_free_rate = 0.02  # 仮定の無リスク金利
        sharpe_ratio = (regime_returns.mean() - risk_free_rate/TRADING_DAYS_PER_YEAR) / volatility if volatility > 0 else 0
