...
def load_config():
    ...
    config_path = get_project_root() / "ztb" / "configs" / "v432" / "sac_v432_0_ensemble_optimized.json"
...
def simulate_sac_v432_action(price, prev_price, position, config, market_condition):
    ...
    specialization_overrides = {
        ...
        "sideways": {
            "reward_overrides": {"HOLD": 0.01}  # 2.0x
        },
        "high_vol": {
            "reward_overrides": {"BUY": 0.33, "SELL": 0.33}  # 1.1x each
        },
        "low_vol": {
            "reward_overrides": {"HOLD": 0.0075}  # 1.5x
        }
    }
...
def main():
    ...
    results = run_backtest_simulation_v432(config, num_steps=10000, initial_capital=10000.0)

    # Save results
    output_file = get_project_root() / "ztb" / "evaluation" / "v432" / "sac_v432_backtest_results.json"
