...
def load_config():
    ...
    config_path = get_project_root() / "ztb" / "configs" / "v432" / "sac_v432_1_advanced_position_management.json"
...
def simulate_sac_v432_1_action(
    ...
    """Simulate SAC v432.1 action with Advanced Position Management"""
    ...
    # Get base rewards (v432.1: negative HOLD bonus)
    base_rewards = {
        'SELL': config['reward_function']['sell_bonus'],  # 0.3
        'HOLD': config['reward_function']['hold_bonus'],  # -0.02 (NEGATIVE!)
...
def run_backtest_simulation_v432_1(config, num_steps=10000, initial_capital=10000.0):
    ...
    print("=== SAC v432.1 Advanced Position Management Backtest ===")
    print(f"Steps: {num_steps}, Initial Capital: ${initial_capital}")
    print("Features: Negative HOLD penalty, Advanced position management, Dynamic sizing")
    ...
    for step in range(1, num_steps):
        ...
        # Get SAC v432.1 action with advanced position management
        action, reward, market_condition, confidence = simulate_sac_v432_1_action(
            current_price,
            prev_price,
            position,
            hold_periods,
        ...
        position_size = calculate_dynamic_position_size(config, market_condition, confidence, volatility)
...
def main():
    ...
    results = run_backtest_simulation_v432_1(config, num_steps=10000, initial_capital=10000.0)

    # Save results
    output_file = get_project_root() / "ztb" / "evaluation" / "v432" / "sac_v432_1_advanced_position_management_results.json"
