...
class ParameterCombinationGenerator:
    ...
    def generate_combinations(
        *param_lists: List[Any], max_combinations: Optional[int] = None
    ) -> Iterator[Tuple[Any, ...]]:
        ...
        if max_combinations is not None and len(combinations) > max_combinations:
            print(
                f"Warning: Generated {len(combinations)} combinations, limiting to {max_combinations}"
            )
        ...
        if feature_type:
            if feature_type in ["kama", "ema_cross"]:
                if len(combination) >= 2:
                    ...
                    if not isinstance(fast, (int, float)) or not isinstance(
                        slow, (int, float)
                    ):
                        return False
...
class FeatureCalculator:
    ...
    def calculate_ema_cross(
        ohlc_data: pd.DataFrame, fast_period: int, slow_period: int
    ) -> pd.DataFrame:
    ...
    @staticmethod
    def calculate_kama(
        ohlc_data: pd.DataFrame, fast: int, slow: int, er_period: int
    ) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {f"kama_{fast}_{slow}_{er_period}": kama}, index=ohlc_data.index
        )

    @staticmethod
    def _calculate_simple_kama(
        price: np.ndarray[Any, np.dtype[Any]], slow: int, er_period: int
...
class AutoFeatureGenerator:
    ...
    def generate_ema_cross_features(
        ...
        # Ensure we have valid lists
        assert isinstance(fast_periods, list) and all(
            isinstance(x, int) for x in fast_periods
        )
        assert isinstance(slow_periods, list) and all(
            isinstance(x, int) for x in slow_periods
        )
        ...
        for combo in all_combinations:
            if ParameterCombinationGenerator.validate_combination(
                combo, feature_type="ema_cross"
            ):
            ...
            for slow in slow_periods:
                for er in efficiency_periods:
                    ...
                    try:
                        ...
                        features[name] = pd.DataFrame(
                            {f"kama_{fast}_{slow}_{er}": kama}, index=ohlc_data.index
                        )
    ...
    def _calculate_simple_kama(
        self, price: np.ndarray[Any, np.dtype[Any]], slow: int, er_period: int
    ) -> np.ndarray[Any, np.dtype[Any]]:
    ...
    def evaluate_and_promote_features(
        ...
        for feature_name, feature_df in features.items():
            try:
                ...
                for col in feature_df.columns:
                    if col in aligned_data.columns:
                        ...
                        if aligned_data[col].nunique() <= 5:
                            signal_returns = aligned_data[
                                aligned_data[col] > aligned_data[col].median()
                            ]["returns"]
                            if len(signal_returns) > 10:
                                signal_metrics = self._calculate_basic_metrics(
                                    np.asarray(signal_returns.values)
                                )
                                feature_results[col] = {
                ...
                temporary_features[feature_name] = {"status": "error", "error": str(e)}
    ...
    def _check_promotion_criteria(self, feature_results: Dict[str, Any]) -> bool:
        ...
        for col, results in feature_results.items():
            ...
            if (
                sharpe >= self.promotion_criteria["min_sharpe_ratio"]
                and win_rate >= self.promotion_criteria["min_win_rate"]
                and max_dd >= self.promotion_criteria["max_drawdown_limit"]
            ):
                return True
    ...
    def save_promoted_features(
        self,
        promoted_features: Dict[str, Any],
    ...
    def generate_comprehensive_feature_set(
        ...
        if evaluation_results["promoted"]:
            ...
            print(
                f"Promoted {len(evaluation_results['promoted'])} features to permanent catalog"
            )
