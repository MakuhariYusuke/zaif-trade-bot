...
def validate_evaluation_success_rate(logger: EvaluationLogger,
    ...
    recent_evaluations = [r for r in history if datetime.fromisoformat(r['timestamp']) >= recent_cutoff]
    older_evaluations = [r for r in history if datetime.fromisoformat(r['timestamp']) < recent_cutoff]

    recent_success_rate = len([r for r in recent_evaluations if r['status'] == 'success']) / len(recent_evaluations) if recent_evaluations else 0
    older_success_rate = len([r for r in older_evaluations if r['status'] == 'success']) / len(older_evaluations) if older_evaluations else 0
...
def test_feature_computation_stability(feature_func: Callable,
    ...
    return {
        'status': 'success' if consistent and success_rate >= 0.9 else 'warning' if success_rate >= 0.7 else 'failure',
        'success_rate': success_rate,
...
def test_evaluation_pipeline_robustness(evaluation_func: Callable,
    ...
    for i, test_case in enumerate(test_cases):
        try:
            ...
            test_result = {
                'test_case': i,
                'status': 'success' if has_expected_keys and not has_error else 'failure',
...
def detect_performance_regressions(logger: EvaluationLogger,
    ...
    baseline_metrics = {
        'computation_time': np.mean([r['computation_time_ms'] for r in baseline_successful]) if baseline_successful else 0,
        'nan_rate': np.mean([r['nan_rate'] for r in baseline_successful]) if baseline_successful else 0,
        'success_rate': len(baseline_successful) / len(baseline_data) if baseline_data else 0
    }

    # Calculate recent metrics
    recent_successful = [r for r in recent_data if r['status'] == 'success']
    recent_metrics = {
        'computation_time': np.mean([r['computation_time_ms'] for r in recent_successful]) if recent_successful else 0,
        'nan_rate': np.mean([r['nan_rate'] for r in recent_successful]) if recent_successful else 0,
...
def generate_test_report(test_results: Dict[str, Any],
    ...
    if 'test_results' in test_results:
        ...
        for result in test_results['test_results']:
            ...
            status_emoji = {'success': '✅', 'failure': '❌', 'exception': '💥'}.get(status, '❓')
            ...
            report_lines.append(f"| {result.get('test_case', 'N/A')} | {status_emoji} {status} | {details_str} |")
    ...
    if 'regressions' in test_results and test_results.get('has_regressions'):
        ...
        for metric, data in test_results['regressions'].items():
            ...
            report_lines.append(f"| {metric} | {data['baseline']:.3f} | {data['recent']:.3f} | {change_pct:+.1f}% |")
...
def run_comprehensive_evaluation_tests(logger: EvaluationLogger,
    ...
    edge_cases = [
        {
            'args': {'feature_class': lambda: None, 'ohlc_data': ohlc_data.head(10), 'feature_name': 'test'},
            'expected_keys': ['status']
        },
        {
            'args': {'feature_class': lambda: None, 'ohlc_data': pd.DataFrame(), 'feature_name': 'empty'},
            'expected_keys': ['status']
        }
    ]

    results['robustness'] = test_evaluation_pipeline_robustness(evaluation_func, edge_cases)
