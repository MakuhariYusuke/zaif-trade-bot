...
def load_config():
    ...
    config_path = Path(__file__).parent.parent.parent / "configs" / "v431" / "sac_v431_1_enhanced.json"
...
def analyze_market_adaptability(results, config):
    """Analyze market adaptability of the strategy"""
    print("\n" + "=" * 80)
    print("市場適応性分析")
    print("=" * 80)

    # バックテストは合成データなので、理論的な適応性分析
    reward_config = config['reward_function']
    actions = results['actions_taken']
...
【理論的な市場適応性】")
    ...
    sideways_mult = reward_config['market_adaptive']['sideways_multiplier']

    print("強気市場での期待:")
    bull_sell_prob = 0.3  # 理論的な確率
    bull_hold_prob = 0.2
    bull_buy_prob = 0.5
    bull_expected_reward = (sell_bonus * bull_sell_prob +
                           hold_bonus * bull_hold_prob +
                           buy_bonus * bull_buy_prob)
    print(".3f"
    print("弱気市場での期待:")
    bear_sell_prob = 0.5
    bear_hold_prob = 0.2
    bear_buy_prob = 0.3
    bear_expected_reward = (sell_bonus * bear_sell_prob +
                           hold_bonus * bear_hold_prob +
                           buy_bonus * bear_buy_prob)
    print(".3f"
    print("サイドウェイズ市場での期待:")
    side_sell_prob = 0.3
    side_hold_prob = 0.4  # HOLDが増える
    side_buy_prob = 0.3
    side_expected_reward = ((sell_bonus * side_sell_prob +
                           hold_bonus * side_hold_prob +
                           buy_bonus * side_buy_prob) * sideways_mult)
    print(".3f"
    print("
【適応性評価】")
    print("-" * 50)
    ...
    # 市場条件による期待値の差
    market_diff = max(bull_expected_reward, bear_expected_reward, side_expected_reward) - \
                 min(bull_expected_reward, bear_expected_reward, side_expected_reward)

    if market_diff > 0.1:
        print("✅ 市場条件による適応性が高い")
    else:
        print("⚠️ 市場条件による適応性が低い")

    # HOLD適応性
    if sideways_mult > 1.2:
        print("✅ サイドウェイズ市場でのHOLD適応性が高い")
    else:
        print("⚠️ サイドウェイズ市場でのHOLD適応性が不十分")
...
def main():
    ...
    try:
        ...
        analyze_market_adaptability(results, config)
