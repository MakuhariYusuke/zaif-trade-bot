...
class DataLoadingValidator:
    ...
    def simulate_missing_data_scenarios(self) -> List[tuple[str, pd.DataFrame]]:
        ...
        base_data["bb_upper"] = (
            base_data["close"].rolling(20).mean()
            + 2 * base_data["close"].rolling(20).std()
        )
        base_data["bb_lower"] = (
            base_data["close"].rolling(20).mean()
            - 2 * base_data["close"].rolling(20).std()
        ...
        data1.loc[
            missing_mask, ["close", "volume", "rsi", "macd", "bb_upper", "bb_lower"]
        ] = np.nan
        ...
        for hour in maintenance_hours:
            ...
            data3.loc[
                hour_mask, ["close", "volume", "rsi", "macd", "bb_upper", "bb_lower"]
            ] = np.nan
    ...
    def apply_missing_data_fill(
        self, data: pd.DataFrame, method: str = "ffill_bfill"
    ) -> pd.DataFrame:
        ...
        if method == "ffill_bfill":
            ...
            for col in numeric_cols:
                filled_data[col] = filled_data[col].fillna(
                    filled_data[col].rolling(24, min_periods=1).mean()
                )
    ...
    def run_validation(self):
        ...
        for scenario_name, original_data in scenarios:
            ...
            for method in fill_methods:
                ...
                result = {"method": method, "impact": impact}

                scenario_results["fill_method_results"].append(result)

                print(
                    f"  {method:12}: Reward change: {impact['reward_change_percent']:+.2f}%, "
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        for result in self.validation_results:
            if "scenario" in result:
                ...
                best = min(
                    methods,
                    key=lambda x: abs(x["impact"]["reward_change_percent"])
                    + x["impact"]["correlation_change"],
                )
