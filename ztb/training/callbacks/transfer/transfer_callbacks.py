...
class FineTuningCallback(MemoryOptimizedCallback):
    """
    Fine-tuning callback for transfer learning.

    Monitors fine-tuning progress, layer-wise learning rates,
    and catastrophic forgetting during transfer learning.
    """
...
class TransferPerformanceCallback(MemoryOptimizedCallback):
    ...
    def __init__(self, compute_frequency: int = 1, evaluation_metrics: Optional[List[str]] = None):
        super().__init__(cache_size=1000)
        self.compute_frequency = compute_frequency
