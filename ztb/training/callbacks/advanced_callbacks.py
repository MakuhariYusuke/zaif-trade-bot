        ...
        if self.steps_without_improvement >= self.patience:
            ...
            if len(self.metric_history) >= self.window_size:
                ...
                if cv < self.cv_threshold:
                    ...
                    logger.info("="*80)
                    logger.info("🛑 Early Stopping: Training stopped due to convergence")
                    logger.info("="*80)
                    ...
                    logger.info(f"  Steps without improvement: {self.steps_without_improvement}")
                    logger.info(f"  Coefficient of Variation: {cv:.4f} (< {self.cv_threshold})")
...
class BestModelCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if is_best:
            ...
            # モデル保存
            model_path = self.save_path / f"{self.model_name}_best_{self.metric_name}_{current_metric:.6f}_step_{self.n_calls}.zip"
