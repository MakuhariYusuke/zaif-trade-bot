...
class StratifiedSampler:
    ...
    def classify_regime(self, prices: NDArray[np.floating[Any]], indices: NDArray[np.integer[Any]]) -> NDArray[np.integer[Any]]:
        """
    ...
    def bucket_indices(
        ...
        prev_actions: NDArray[np.integer[Any]]
    ...
    def sample_batch(
        ...
        prev_actions: NDArray[np.integer[Any]],
        batch_size: int,
        available_indices: Optional[NDArray[np.integer[Any]]] = None
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_samples_per_bucket": self.bucket_samples_drawn.sum() / max(1, self.total_batches_sampled),
        }
...
def test_stratified_sampler() -> None:
    ...
    sell_ratio_original = (prev_actions == 2).sum() / len(prev_actions)
    sell_ratio_batch = action_counts[2] / len(batch_indices)
