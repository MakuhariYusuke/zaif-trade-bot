...
def predict_with_masks(
    ...
    This is a unified interface that automatically handles action masks
    for MaskablePPO models and falls back to standard prediction for others.

    Args:
        model: The trained model (MaskablePPO or other SB3 model)
        observation: Current observation
        env: Environment that provides action masks (required for MaskablePPO)
    ...
    if isinstance(model, MaskablePPO):
        if env is None:
            raise ValueError(
                "MaskablePPO requires 'env' parameter to get action masks. "
                "Pass the environment instance to enable mask retrieval."
            )
        ...
        action, state = model.predict(
            observation, action_masks=action_masks, deterministic=deterministic
        )

        logger.debug(f"MaskablePPO prediction: action={action}, masks={action_masks}")
    else:
        # Standard prediction for non-MaskablePPO models
        action, state = model.predict(observation, deterministic=deterministic)
        logger.debug(f"Standard prediction: action={action}")
...
def neutralize_policy_bias(model: MaskablePPO | None) -> None:
    ...
    if not bias_neutralized and hasattr(policy, "action_net"):
        ...
        if (
            hasattr(action_net, "bias")
            and getattr(action_net, "bias", None) is not None
        ):
    ...
    if not bias_neutralized and hasattr(policy, "mlp_extractor"):
        if hasattr(policy.mlp_extractor, "policy_net"):
            ...
            if (
                last_layer
                and hasattr(last_layer, "bias")
                and getattr(last_layer, "bias", None) is not None
            ):
...
def apply_cosine_decay_entropy(
    ...
    if current_step >= total_steps:
        ...
        new_ent_coef = (
            final_ent_coef + (initial_ent_coef - final_ent_coef) * cosine_decay
        )
