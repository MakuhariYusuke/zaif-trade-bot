...
class SACPruner:
    ...
    def analyze_pruning_opportunities(self, model: nn.Module) -> Dict:
        ...
        if analysis['total_parameters'] > 0:
            analysis['estimated_pruning_ratio'] = analysis['prunable_parameters'] / analysis['total_parameters']

        return analysis

    def _is_prunable_module(self, module: nn.Module) -> bool:
        """Check if a module is prunable."""
        return type(module) in self.config['target_modules'] and \
               type(module) not in self.config['skip_modules']
    ...
    def apply_pruning(self, model: nn.Module) -> Tuple[nn.Module, Dict]:
        ...
        try:
            if self.config['pruning_schedule'] == 'one_shot':
                ...
                raise ValueError(f"Unknown pruning schedule: {self.config['pruning_schedule']}")

            # Collect pruning statistics
            stats = self._collect_pruning_stats(model, pruned_model, analysis)

            logger.info(f"Pruning completed. Removed {stats['parameters_removed']} parameters")
    ...
    def _collect_pruning_stats(self,
        ...
        stats = {
            ...
            'compression_ratio': pruned_params / original_params if original_params > 0 else 0,
...
class LowRankApproximator:
    ...
    def analyze_low_rank_opportunities(self, model: nn.Module) -> Dict:
                ...
                if hasattr(module, 'weight') and module.weight is not None:
                    ...
                    if weight.dim() >= 2:
                        ...
                        try:
                            ...
                            # Find effective rank
                            effective_rank = np.searchsorted(cumulative_energy,
                                                           self.config['rank_selection_threshold']) + 1
                            ...
                            analysis['rank_distributions'][name] = {
                                'singular_values': s[:20].tolist(),  # Top 20 singular values
        ...
        if analysis['compressible_modules']:
            avg_compression = np.mean([m['compression_ratio']
                                     for m in analysis['compressible_modules']])
    ...
    def _is_lra_module(self, module: nn.Module) -> bool:
        ...
        return type(module) in self.config['target_modules'] and hasattr(module, 'weight')
    ...
    def apply_low_rank_approximation(self, model: nn.Module) -> Tuple[nn.Module, Dict]:
        ...
        try:
            ...
            logger.info(f"Low-rank approximation completed. Compression: {stats['compression_ratio']:.2%}")
    ...
    def _apply_svd_compression(self, model: nn.Module, analysis: Dict) -> nn.Module:
        ...
        for module_info in analysis['compressible_modules']:
            ...
            if hasattr(module, 'weight') and module.weight is not None:
                ...
                module.weight.data = torch.from_numpy(approximated).to(weight.device, weight.dtype)

        return model

    def _collect_lra_stats(self,
                          original_model: nn.Module,
...
class CompositeCompressor:
    ...
    def run_compression_pipeline(self, model: nn.Module) -> Dict:
        ...
        try:
            ...
            # Final statistics
            total_stats['final_parameters'] = sum(p.numel() for p in current_model.parameters())
            ...
            logger.info(f"Composite compression completed. "
