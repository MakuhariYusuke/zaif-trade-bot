...
import logging
from typing import Optional, Tuple, Any, Dict, Union, cast
...
class PerActionAdvantageNormalizer:
    ...
    ) -> Union[NDArray[np.floating[Any]], Tuple[NDArray[np.floating[Any]], Tuple[NDArray[np.floating[Any]], NDArray[np.floating[Any]], NDArray[np.integer[Any]]]]]:
        """
...
def normalize_advantages_per_action(
    ...
    return cast(NDArray[np.floating[Any]], normalizer.normalize(advantages, actions, return_statistics=False))
...
def test_pan_basic() -> None:
    ...
    advantages = np.array([
        1.0, 1.2, 0.8, 1.1,  # HOLD (4 samples)
        0.5, 0.6,            # BUY (2 samples)
        -0.3, -0.2           # SELL (2 samples) - minority with negative values
    ])
    ...
    print("\n=== Original Advantages ===")
    print(f"HOLD: {advantages[actions == 0]}")
    print(f"BUY:  {advantages[actions == 1]}")
    print(f"SELL: {advantages[actions == 2]}")
    
    print("\n=== Traditional Normalization ===")
    print(f"HOLD: {traditional[actions == 0]}")
    print(f"BUY:  {traditional[actions == 1]}")
    print(f"SELL: {traditional[actions == 2]}")
    
    print("\n=== PAN Normalization ===")
    print(f"HOLD: {pan_normalized[actions == 0]}")
    print(f"BUY:  {pan_normalized[actions == 1]}")
    print(f"SELL: {pan_normalized[actions == 2]}")
    ...
    sell_pan_std = np.std(pan_normalized[actions == 2])
    print(f"\n✓ SELL std after PAN: {sell_pan_std:.4f}")
    print(f"  (Should be ~1.0 due to normalization)")
