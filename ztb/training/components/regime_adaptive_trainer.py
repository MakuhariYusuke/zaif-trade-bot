...
class RegimeAdaptiveTrainerMixin(ABC):
    ...
    def detect_market_regime(self, data: pd.DataFrame, current_index: int = -1) -> Optional[RegimeDetectionResult]:
        """
    ...
    def get_regime_specific_parameters(self, regime: RegimeType) -> Dict[str, Any]:
        ...
        return base_params.get(regime_key, {
            'ent_coef': 0.02,
            'learning_rate': 0.0003,
            'reward_scale': 1.0
        })
    ...
    def adapt_hyperparameters_for_regime(self, regime: RegimeType) -> Dict[str, Any]:
        ...
        """
        if not self.regime_config.get('adaptation_rules', {}).get('hyperparameter_adjustment', True):
            return {}
        ...
        logger.info(f"Adapted hyperparameters for regime {regime.value}: {adapted_params}")
    ...
    def get_regime_aware_reward_scale(self, regime: RegimeType) -> float:
        ...
        """
        if not self.regime_config.get('adaptation_rules', {}).get('reward_scaling', True):
            return 1.0
    ...
    def update_regime_performance(self, regime: RegimeType, reward: float, step_count: int):
        """
    ...
    def get_adaptation_suggestions(self) -> List[str]:
        ...
        if avg_rewards:
            ...
            if avg_rewards[best_regime] > 0 and avg_rewards[worst_regime] < 0:
                suggestions.append(f"Consider adjusting parameters for {worst_regime} regime")
        ...
        if len(regime_counts) < 3:
            suggestions.append("Limited regime diversity detected - consider adjusting detection thresholds")
