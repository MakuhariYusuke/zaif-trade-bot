...
def perform_statistical_tests(session_data: Dict[int, Dict[str, List[float]]],
                            metrics: List[str],
                            alpha: float = DEFAULT_ALPHA) -> Dict[str, Dict[str, Any]]:
    """
    セッション間の統計的検定を実行
...
def compare_sac_sessions(log_dir: str,
                       session_ids: List[int],
                       metrics: List[str],
                       output_file: str,
                       alpha: float,
                       verbose: bool = False) -> Tuple[Dict[int, Dict[str, Dict[str, float]]], Dict[str, Any]]:
    """
    複数のSACセッションを比較
