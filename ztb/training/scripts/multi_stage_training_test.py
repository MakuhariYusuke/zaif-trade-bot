...
def simulate_training_stage(stage_name, timesteps, learning_rate, exploration_rate, config):
    """Simulate training for one stage"""
    print(f"\n=== Stage: {stage_name} ===")
    print(f"Timesteps: {timesteps}, Learning Rate: {learning_rate}, Exploration: {exploration_rate}")
    ...
    print(f"Rewards: Sell={rewards['sell']}, Hold={rewards['hold']}, Buy={rewards['buy']}")
...
def main():
    ...
    for stage in stages:
        stage_reward, avg_reward, actions, elapsed = simulate_training_stage(
            stage['name'], stage['timesteps'], stage['learning_rate'],
            stage['exploration_rate'], config
        )
