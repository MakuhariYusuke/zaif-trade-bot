...
class FeatureSchemaManager:
    ...
    def __init__(
        ...
        self.models_dir = models_dir if models_dir is not None else self.DEFAULT_MODELS_DIR
    ...
    async def save_schema_async(
        ...
        """モデルの特徴量スキーマを非同期で保存"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self.save_schema, features, config, scaler_data)
    ...
    def verify_compatibility(self, other_model_name: str) -> bool:
        ...
        try:
            ...
            compatible = (
                my_metadata.num_features == other_metadata.num_features and
                my_metadata.feature_names == other_metadata.feature_names
            )
            
            if compatible:
                logger.info(f"✅ {self.model_name} and {other_model_name} are compatible")
            else:
                logger.warning(f"⚠️  {self.model_name} and {other_model_name} are NOT compatible")
                logger.warning(f"   {self.model_name}: {my_metadata.num_features} features")
                logger.warning(f"   {other_model_name}: {other_metadata.num_features} features")
            
            return compatible
            
        except FileNotFoundError:
            logger.error(f"Cannot verify compatibility: {other_model_name} schema not found")
            return False
    ...
    def _save_scaler(self, scaler_data: Dict[str, NDArray[np.floating[Any]]], schema_hash: str) -> None:
        """scaler.npzを保存"""
        scaler_path = self.model_schema_dir / "scaler.npz"
...
def migrate_legacy_schema(
    ...
    if legacy_schema_path is None:
        legacy_schema_path = FeatureSchemaManager.DEFAULT_MODELS_DIR / "features_schema.json"
