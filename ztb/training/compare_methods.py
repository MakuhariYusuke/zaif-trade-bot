...
def compare_optimization_methods(
        ...
        if param_space.param_type.value == "categorical":
            ...
            if param_space.param_type.value == "log_uniform":
                ...
                log_values = np.linspace(
                    np.log10(param_space.low), np.log10(param_space.high), 3
                )
            ...
            objective_function=lambda params: objective_func(
                {
                    **{
                        k: v.default
                        for k, v in param_spaces_dict.items()
                        if k != "learning_rate"
                    },
                    **params,
                }
        ...
        comparison.append(
            {
                ...
                "avg_time_per_trial": result.total_duration_seconds / result.n_trials
    ...
    # 表形式で表示
    print(
        f"{'手法':<25} {'ベスト目的値':>15} {'試行回数':>10} {'成功率':>10} {'総時間(秒)':>12} {'試行当たり(秒)':>15}"
    )
    print("-" * 100)
...
def main() -> None:
    ...
    parser.add_argument("--n-trials", type=int, default=20, help="各手法の試行回数")
    parser.add_argument(
        "--output-dir",
        type=str,
