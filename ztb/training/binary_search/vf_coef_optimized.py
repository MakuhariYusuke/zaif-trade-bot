...
class VfCoefOptimizer(HyperparameterOptimizer):
    ...
    def update_ppo_params(self, value: Union[int, float]) -> None:
        """Update PPO parameters with vf_coef value."""
        self.ppo_params["vf_coef"] = float(value)


def main() -> None:
    parser = BinarySearchArgumentParser.create_parser('Optimize vf_coef parameter for PPO')
