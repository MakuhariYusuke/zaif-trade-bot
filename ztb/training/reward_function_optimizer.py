...
from ztb.optimization.hyperparameter_optimizer import ParameterSpace
...
class RewardFunctionOptimizer:
    ...
    def _robust_evaluate_parameters(
        ...
        if cache_key in self.evaluation_cache:
            self.logger.debug(f"Using cached evaluation for trial {trial_number}")
            return self.evaluation_cache[cache_key].copy()
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Validate scores
                if all(obj in scores for obj in objectives):
                    # Cache successful evaluation
                    self.evaluation_cache[cache_key] = scores.copy()
                    return scores

            except Exception as e:
                self.logger.warning(
                    f"Evaluation attempt {attempt + 1} failed for trial {trial_number}: {e}"
                )
        ...
        # All retries failed
        error_msg = f"All evaluation attempts failed for trial {trial_number}"
        self._handle_error(Exception(error_msg), f"trial_{trial_number}_evaluation")
        return {obj: -999 for obj in objectives}
    ...
    def optimize_reward_function(
        ...
        # Print header
        self._print_header(
            f"Reward Function Optimization - {stage.title()} Stage",
            f"Optimizing {len(self.parameter_spaces[stage])} parameters over {n_trials} trials",
        )
        ...
        def objective(trial):
            """Optuna objective function for reward optimization."""
            ...
            # Evaluate parameters using robust evaluation with retry and caching
            scores = self._robust_evaluate_parameters(params, objectives, trial.number)
            ...
            # Update progress
            completed_trials += 1
            if self.verbose and completed_trials % max(1, n_trials // 10) == 0:
                ...
                try:
                    ...
                    best_score = 0.0
                print(
                    f"  📊 Trial {completed_trials}/{n_trials} completed | "
                    f"Best: {best_score:.4f} | ETA: {eta:.1f}s"
                )

            # Update dynamic weights periodically during optimization
            if trial.number > 0 and trial.number % max(1, n_trials // 20) == 0:
                self._update_dynamic_weights_from_history()

            # Get dynamic weights based on current conditions
            dynamic_weights = self.get_dynamic_objective_weights(objectives)

            composite_score = 0.0
            for obj in objectives:
                if obj in scores and obj in dynamic_weights:
                    # Normalize scores (assuming higher is better)
                    score = scores[obj]
                    if obj == "max_drawdown":  # Lower is better for drawdown
                        score = -score
                    composite_score += dynamic_weights.get(obj, 0.0) * score

            return composite_score

        # Create Optuna study with improved Bayesian optimization
        study = optuna.create_study(
            direction="maximize",
            sampler=optuna.samplers.GPSampler(
                ...
                n_startup_trials=max(
                    10, n_trials // 10
                ),  # More random exploration initially
        ...
        try:
            ...
            self._handle_error(e, "optimization process")
            raise
        ...
        # Calculate final scores for best parameters
        try:
            best_scores = evaluation_function(best_params)
        except Exception as e:
            self._handle_error(e, "final evaluation")
            best_scores = {obj: 0.0 for obj in objectives}
        ...
        result = RewardOptimizationResult(
            ...
            convergence_info={
                ...
                "study_best_params": best_trial.params,
            },
        )
        ...
        self.logger.info(f"Reward function optimization completed for stage '{stage}'")
        self.logger.info(f"Best composite score: {best_score:.4f}")
