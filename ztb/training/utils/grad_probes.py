...
class ActionGradientProbe:
    ...
    def probe(
        ...
        # Check health
        is_healthy = (
            grad_norm_ma > self.grad_norm_threshold
            and advantage_ma > self.advantage_threshold
        )
...
def create_failsafe_dump(
    ...
    with open(stats_path, "w") as f:
        ...
        if stats["advantage_mean"] <= probe.advantage_threshold:
            f.write(
                f"- {probe.target_action} advantage non-positive (policy not learning value)\n"
            )
