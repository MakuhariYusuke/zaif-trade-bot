#!/usr/bin/env python3
"""
Simple reward function training test
"""


from ztb.training.utils.training_utils import (
    setup_project_path, create_ppo_model, 
    load_training_data, save_model_with_path, 
    print_training_results, print_training_start
)
from ztb.trading.environment.environment import HeavyTradingEnv
from ztb.training.config.ppo_config import DEFAULT_INITIAL_PORTFOLIO_VALUE
from ztb.training.callbacks.callbacks import SimpleTrainingCallback


def train_simple_reward(
    config_name: str = "default",
    reward_scaling: float = 1.0,
    entropy_coef: float = 0.01,
    learning_rate: float = 3e-4,
) -> str:
    """Train with simple portfolio reward for 100k steps with configurable parameters"""

    # Setup project path
    setup_project_path()

    # Load data
    df = load_training_data()

    # Get trading environment config from common configuration
    from ztb.trading.env_config import get_trading_env_config
    from typing import cast, Any

    env_config = get_trading_env_config({
        "reward_scaling": reward_scaling,  # Override with parameter
        "risk_free_rate": 0.02,
        "feature_set": "full",
        "initial_portfolio_value": DEFAULT_INITIAL_PORTFOLIO_VALUE,
        "curriculum_stage": "simple_portfolio",  # Use simple reward
    })

    env = HeavyTradingEnv(
        df=df,
        config=cast(dict[str, Any], env_config),
        streaming_pipeline=None,
        stream_batch_size=1000,
        max_features=68,
    )

    # Create PPO model with custom config
    model_config = {
        "ent_coef": entropy_coef,
        "learning_rate": learning_rate,
    }
    model = create_ppo_model(env, model_config)

    # Create callback
    callback = SimpleTrainingCallback()

    print_training_start(config_name, reward_scaling, entropy_coef, learning_rate)

    # Train for 100k steps
    model.learn(total_timesteps=100000, callback=callback)

    print_training_results(callback.episode_rewards)

    # Save model with config name
    model_path = save_model_with_path(model, f"aggressive_{config_name}")
    print(f"\nModel saved to: {model_path}")

    env.close()
    return model_path


if __name__ == "__main__":
    # Define fine-tuned reward scaling configurations
    configs = [
        # Fine-tune reward_scaling around optimal value (7.5)
        {
            "name": "reward_scale_6_0",
            "reward_scaling": 6.0,
            "entropy_coef": 0.03,
            "learning_rate": 1e-3,
        },
        {
            "name": "reward_scale_6_5",
            "reward_scaling": 6.5,
            "entropy_coef": 0.03,
            "learning_rate": 1e-3,
        },
        {
            "name": "reward_scale_7_0",
            "reward_scaling": 7.0,
            "entropy_coef": 0.03,
            "learning_rate": 1e-3,
        },
        {
            "name": "reward_scale_7_5",
            "reward_scaling": 7.5,
            "entropy_coef": 0.03,
            "learning_rate": 1e-3,
        },
        {
            "name": "reward_scale_8_0",
            "reward_scaling": 8.0,
            "entropy_coef": 0.03,
            "learning_rate": 1e-3,
        },
        {
            "name": "reward_scale_8_5",
            "reward_scaling": 8.5,
            "entropy_coef": 0.03,
            "learning_rate": 1e-3,
        },
        {
            "name": "reward_scale_9_0",
            "reward_scaling": 9.0,
            "entropy_coef": 0.03,
            "learning_rate": 1e-3,
        },
    ]

    trained_models = []

    for config in configs:
        print(f"\n{'='*60}")
        print(f"Training configuration: {config['name']}")
        print(f"{'='*60}")

        try:
            from ztb.utils.config import get_config_value

            model_path = train_simple_reward(
                config_name=get_config_value(config, "name", str, "default"),
                reward_scaling=get_config_value(config, "reward_scaling", float, 1.0),
                entropy_coef=get_config_value(config, "entropy_coef", float, 0.01),
                learning_rate=get_config_value(config, "learning_rate", float, 3e-4),
            )
            trained_models.append((config["name"], model_path))
            print(f"✅ Successfully trained: {config['name']}")

        except Exception as e:
            print(f"❌ Failed to train {config['name']}: {e}")
            continue

    print(f"\n{'='*60}")
    print("Training completed! Models trained:")
    for name, path in trained_models:
        print(f"  - {name}: {path}")
    print(f"{'='*60}")
