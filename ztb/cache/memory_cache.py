...
class MemoryManager:
    ...
    def __init__(self, max_memory_mb: float = 500.0, enable_monitoring: bool = True) -> None:
        ...
        self._custom_ttl_caches: OrderedDict[float, TTLCache] = OrderedDict()
        self.max_custom_ttl_caches = 16
    ...
    def cache_feature_data(self, key: str, data: Any, ttl: float | None = None) -> None:
        ...
        if ttl is not None and ttl != self.feature_cache.ttl:
            # For custom TTL, store in a separate bounded cache keyed by TTL.
            self._get_custom_ttl_cache(ttl)[key] = data
        else:
            self.feature_cache[key] = data

    def _get_custom_ttl_cache(self, ttl: float) -> TTLCache:
        """Resolve or create a bounded custom-TTL cache bucket."""
        ttl_key = float(ttl)
        cache = self._custom_ttl_caches.get(ttl_key)
        if cache is not None:
            self._custom_ttl_caches.move_to_end(ttl_key)
            return cache

        cache = TTLCache(maxsize=1000, ttl=ttl_key)
        self._custom_ttl_caches[ttl_key] = cache
        while len(self._custom_ttl_caches) > self.max_custom_ttl_caches:
            _, evicted_cache = self._custom_ttl_caches.popitem(last=False)
            evicted_cache.clear()
        return cache
    ...
    def get_cached_feature_data(self, key: str) -> Any | None:
        ...
        for ttl_key, cache in list(self._custom_ttl_caches.items()):
            try:
                value = cache[key]
            except KeyError:
                if len(cache) == 0:
                    self._custom_ttl_caches.pop(ttl_key, None)
                continue
            self._custom_ttl_caches.move_to_end(ttl_key)
            return value
