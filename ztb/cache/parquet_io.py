...
def analyze_column_dependencies(features_config: Dict[str, Any], 
                              target_features: Optional[List[str]] = None) -> Set[str]:
    """
    ...
    for category, feature_list in features.items():
        ...
        for feature_info in feature_list:
            ...
            if any(keyword in feature_name.lower() for keyword in ['ma', 'sma', 'ema', 'moving']):
            ...
            elif any(keyword in feature_name.lower() for keyword in ['volume', 'vwap', 'mfi']):
                required_columns.update(['close', 'volume'])
            elif any(keyword in feature_name.lower() for keyword in ['bb', 'bollinger', 'atr']):
                required_columns.update(['close', 'high', 'low'])
            elif any(keyword in feature_name.lower() for keyword in ['ichimoku', 'donchian']):
...
def smart_column_detection(parquet_path: Path, 
                          required_columns: Optional[Set[str]] = None) -> List[str]:
    """
    ...
    try:
        ...
        if required_columns:
            ...
            columns_to_read = [col for col in available_columns if col in required_columns]
...
def read_parquet(path: Path, config: Optional[Dict] = None, columns: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    if mem_after > peak_memory_limit:
        print(f"WARNING: Memory usage exceeded limit. Current: {mem_after/(1024*1024):.1f}MB, Limit: {peak_memory_limit/(1024*1024):.1f}MB")
...
def convert_to_parquet(input_path: Path, output_path: Path, compression: Optional[str] = None) -> None:
