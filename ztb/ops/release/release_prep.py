...
def main():
    ...
    parser.add_argument(
        "--canary-duration", type=int, default=3, help="Canary test duration in minutes"
    )
    parser.add_argument(
        "--output-dir", default="artifacts", help="Output directory for release bundle"
    )
    parser.add_argument("--skip-canary", action="store_true", help="Skip canary test")
    parser.add_argument(
        "--force", action="store_true", help="Force release prep even if checks fail"
    )
