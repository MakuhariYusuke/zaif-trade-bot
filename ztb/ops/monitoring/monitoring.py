...
class PrometheusExporter:
    ...
    def __init__(self, port: int = 8000):
        ...
        self.data_fetch_total = Counter(
            "ztb_data_fetch_total", "Total number of data fetch operations", ["status"]
        )
        self.data_fetch_duration = Histogram(
            "ztb_data_fetch_duration_seconds", "Time spent fetching data", ["source"]
        )
        ...
        self.job_executions_total = Counter(
            "ztb_job_executions_total", "Total number of ML job executions", ["status"]
        )
        self.job_execution_duration = Histogram(
            "ztb_job_execution_duration_seconds", "Time spent executing ML jobs"
        )
        self.active_jobs = Gauge("ztb_active_jobs", "Number of currently active jobs")
        ...
        self.portfolio_balance = Gauge(
            "ztb_portfolio_balance", "Current portfolio balance"
        )
        self.portfolio_pnl = Gauge("ztb_portfolio_pnl", "Current portfolio P&L")
        self.risk_drawdown = Gauge(
            "ztb_risk_drawdown_percent", "Current drawdown percentage"
        )
        ...
        self.system_errors_total = Counter(
            "ztb_system_errors_total", "Total number of system errors", ["component"]
        )
    ...
    def record_data_fetch(
        self, status: str, duration: float, source: str = "binance"
    ) -> None:
    ...
    def update_portfolio_metrics(
        self, balance: float, pnl: float, drawdown: float
    ) -> None:
    ...
    def record_model_performance_drift(
        self, metric_type: str, drift_value: float
    ) -> None:
    ...
    def record_quality_gate_result(
        self, gate_type: str, passed: bool, reason: str = ""
    ) -> None:
