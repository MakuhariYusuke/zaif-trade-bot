...
class ConfigLinter:
    ...
    def _validate_venue_config(
        ...
        if not connectivity_skip and config.get("validation", {}).get(
            ...
            try:
                ...
                if response.status_code != 200:
                    warnings.append(
                        f"API endpoint {venue['api_url']} returned status {response.status_code}"
                    )
        ...
        if fees.get("maker_fee_percent", 0) > fees.get("taker_fee_percent", 0):
            warnings.append(
                "Maker fee is higher than taker fee - unusual configuration"
            )
    ...
    def lint_configs(
        ...
        for config_file in config_files:
            ...
            result = self.validate_config(
                config_file, schema_name, strict_mode, connectivity_skip
            )
...
def main():
    ...
    parser.add_argument(
        "--schema-dir", type=Path, default=Path("config"), help="Schema directory"
    )
    parser.add_argument(
        "--files", nargs="*", type=Path, help="Specific config files to validate"
    )
    parser.add_argument(
        "--strict", action="store_true", help="Treat warnings as errors"
    )
    parser.add_argument(
        "--connectivity-skip", action="store_true", help="Skip API connectivity checks"
    )
    parser.add_argument(
