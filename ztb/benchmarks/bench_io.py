...
def measure_io_performance(df: pd.DataFrame, config: Dict, temp_dir: Path) -> Dict[str, Any]:
    ...
    parquet_path = temp_dir / f"test_{config['parquet']['compression']}_{config['parquet']['row_group_size']}.parquet"
...
def run_benchmark(n_rows: int = 100000, output_dir: Path = Path("reports")) -> pd.DataFrame:
...
def generate_plots(results_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    grouped = results_df.groupby(['compression', 'row_group_size', 'use_columns']).agg({
    ...
    axes[0,0].set_xticklabels([f"{row['compression']}\n{row['row_group_size']}\n{'subset' if row['use_columns'] else 'all'}" for _, row in top_configs.iterrows()], fontsize=8)
    ...
    axes[0,1].set_xticklabels([f"{row['compression']}\n{row['row_group_size']}\n{'subset' if row['use_columns'] else 'all'}" for _, row in top_configs.iterrows()], fontsize=8)
    
    # CPU usage
    axes[1,0].bar(range(len(top_configs)), top_configs['write_cpu_percent'], label='Write', alpha=0.7)
    axes[1,0].bar(range(len(top_configs)), top_configs['read_cpu_percent'], label='Read', alpha=0.7)
    axes[1,0].set_title('CPU Usage (%)')
    axes[1,0].legend()
    
    # Memory usage
    axes[1,1].bar(range(len(top_configs)), top_configs['write_mem_mb'], label='Write', alpha=0.7)
    axes[1,1].bar(range(len(top_configs)), top_configs['read_mem_mb'], label='Read', alpha=0.7)
...
def generate_weekly_report_section(results: pd.DataFrame) -> str:
    ...
    report_lines.append(f"- **Best Read Throughput**: {results['read_throughput_mbps'].max():.1f} MB/s")
    report_lines.append(f"- **Best Write Throughput**: {results['write_throughput_mbps'].max():.1f} MB/s")
    report_lines.append(f"- **Lowest Memory Usage**: {results[['write_mem_mb', 'read_mem_mb']].min().min():.1f} MB")
    ...
    report_lines.append("| Rank | Compression | Row Group | Columns | Read MB/s | Write MB/s | Memory MB |")
    report_lines.append("|------|-------------|-----------|---------|-----------|------------|-----------|")
    ...
    columns_rec = "Use column projection (essential columns only)" if best_config['use_columns'] else "Read all columns"
    report_lines.append(f"- Column Strategy: {columns_rec}")
    report_lines.append(f"- Expected Performance: {best_config['read_throughput_mbps']:.1f} MB/s read, {best_config['write_throughput_mbps']:.1f} MB/s write")
    report_lines.append("")
    ...
    compression_perf = results.groupby('compression')['read_throughput_mbps'].mean().sort_values(ascending=False)
    best_compression = compression_perf.index[0]
    report_lines.append(f"- **Best compression for speed**: {best_compression} ({compression_perf.iloc[0]:.1f} MB/s avg)")
    ...
    if 'use_columns' in results.columns:
        ...
        if not proj_results.empty and not no_proj_results.empty:
            ...
            report_lines.append(f"- **Column projection savings**: {mem_savings:.1f}% memory reduction")
    
    report_lines.append("")
    return "\n".join(report_lines)


def update_weekly_report(benchmark_results: pd.DataFrame, weekly_report_path: str = "weekly_report.md") -> None:
    """
    ...
    if "## Notes" in content:
        ...
        updated_content = content[:insert_pos] + benchmark_section + "\n" + content[insert_pos:]
    else:
...
def main():
    ...
    parser.add_argument("--rows", "-r", type=int, default=100000, help="Number of test rows")
    parser.add_argument("--output", "-o", default="reports", help="Output directory")
    parser.add_argument("--update-weekly", action="store_true", help="Update weekly report with results")
    ...
    for _, row in top3.iterrows():
        columns_desc = "All" if not row['use_columns'] else "Essential"
        print(f"  {row['compression']}/{row['row_group_size']//1000}K/{columns_desc}: {row['read_throughput_mbps']:.1f} MB/s read")
