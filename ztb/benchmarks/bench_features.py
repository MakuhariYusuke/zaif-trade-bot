...
def load_real_data(sample_path: Path, n_rows: Optional[int] = None) -> pd.DataFrame:
    ...
    if sample_path.exists():
        ...
        if all(col in df.columns for col in required):
            ...
            print(f"Warning: {sample_path} missing required columns, using synthetic data")
    else:
        print(f"Warning: {sample_path} not found, using synthetic data")
    return generate_synthetic_data(n_rows or 1000)


def benchmark_feature(feature_name: str, manager, df: pd.DataFrame, n_runs: int = 5) -> Dict[str, float]:
    """単一特徴量のベンチマーク"""
    times = []
    memories = []
    ...
    return {
        'ms_real': float(np.median(times)),
        'peak_MB': float(np.max(memories))
    }


def benchmark_bundle(manager, df: pd.DataFrame, waves, n_runs: int = 5) -> Dict[str, float]:
...
def main():
    ...
    parser.add_argument('--sample', type=str, default='data/features/2025/09/sample_09.parquet',
                       help='Path to sample data')
    parser.add_argument('--waves', type=str, default='1', help='Comma-separated waves to benchmark')
    parser.add_argument('--output', type=str, default=None, help='Output CSV path')
    parser.add_argument('--n-runs', type=int, default=5, help='Number of benchmark runs')
    parser.add_argument('--n-rows', type=int, default=1000, help='Number of rows to use from sample')
    parser.add_argument('--n-synth-rows', type=int, default=10000, help='Number of rows to use for synthetic data')
    ...
    sorted_results = df_results[df_results['feature'] != 'Wave1_bundle'].sort_values('ms_real', ascending=False)
