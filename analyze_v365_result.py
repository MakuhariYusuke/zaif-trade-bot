...
def main() -> None:
    ...
    for i, (hold, buy, sell) in enumerate(counts, 1):
        ...
        print(f"  Iteration {i}: HOLD={hold:2d} ({hold/total:5.1%})  "
    ...
    print(f"                v3.6.4    v3.6.5    Change")
    print(f"  HOLD:         {v364_hold:5.1f}%    {v365_hold:5.1f}%    {v365_hold - v364_hold:+6.1f}pp")
    print(f"  BUY:          {v364_buy:5.1f}%    {v365_buy:5.1f}%    {v365_buy - v364_buy:+6.1f}pp")
    print(f"  SELL:         {v364_sell:5.1f}%    {v365_sell:5.1f}%    {v365_sell - v364_sell:+6.1f}pp")
    ...
    if v365_sell < v364_sell:
        ...
        print(f"     {v364_sell:.1f}% → {v365_sell:.1f}% ({v365_sell - v364_sell:+.1f}pp)")
    
    if v365_hold > 80:
        print("  ❌ HOLD dominance WORSENED")
        print(f"     {v364_hold:.1f}% → {v365_hold:.1f}% ({v365_hold - v364_hold:+.1f}pp)")
