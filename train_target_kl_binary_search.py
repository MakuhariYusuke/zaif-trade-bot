...
Tests different target_kl values to find optimal early stopping threshold.
"""
...
def train_target_kl_test(target_kl: float = 0.01):
    """Train PPO model with specified target_kl value."""

    print(f"\n=== Training with target_kl={target_kl} ===")
...
Action distribution:""")
        ...
        # Save model
        target_kl_str = f"{target_kl:.4f}".replace('.', '')
        model_path = f"models/target_kl_test_target_kl_{target_kl_str}.zip"
        os.makedirs("models", exist_ok=True)
        model.save(model_path)
        print(f"Model saved to: {model_path}")

        print(f"✅ Successfully trained: target_kl_{target_kl_str}")
...
def main():
    parser = argparse.ArgumentParser(description='Train PPO with specific target_kl value')
    parser.add_argument('--target_kl', type=float, default=0.01,
                       help='Target KL divergence for early stopping (default: 0.01)')
    ...
    # Train model
    train_target_kl_test(args.target_kl)
