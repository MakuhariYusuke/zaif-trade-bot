...
class SignalGuidanceDebugger:
        ...
        for episode in self.results.get('results', []):
            for signal in episode.get('guidance_signals', []):
                if 'guidance_score' in signal:
                    ...
                    rewards.append(signal.get('portfolio_value', 0))  # ポートフォリオ価値を報酬の代わりに使用

        if not scores:
            logger.warning("No SIGNAL_GUIDANCE scores found in results")
            return {
                'score_stats': {'mean': 50.0, 'std': 0.0, 'min': 50.0, 'max': 50.0, 'median': 50.0},
    ...
    def analyze_score_action_correlation(self) -> Dict[str, Any]:
            ...
            for signal in episode.get('guidance_signals', []):
                if 'guidance_score' in signal:
                    score_action_data.append({
                        ...
                        'reward': signal.get('portfolio_value', 0),  # ポートフォリオ価値を使用
                        'next_reward': signal.get('portfolio_value', 0)  # 次ステップのポートフォリオ価値（簡易的に同じ値）
                    })
        ...
        if not score_action_data:
            ...
            return {
                'range_analysis': {},
                'action_analysis': {},
                'correlations': {'score_reward': 0.0, 'score_next_reward': 0.0, 'action_reward': 0.0},
                'total_samples': 0
            }
        ...
        for min_score, max_score in score_ranges:
            ...
            if len(range_data) > 0:
                range_analysis[f"{min_score}-{max_score}"] = {
                    ...
                    'action_distribution': range_data['action'].value_counts().to_dict()
                }
    ...
    def analyze_score_effectiveness(self) -> Dict[str, Any]:
        ...
        if not score_action_data:
            ...
            return {
                'quartile_analysis': {},
                'effectiveness_indicators': {'high_score_better': False, 'score_reward_trend': 'unknown'}
            }
        ...
        df['score_quartile'] = pd.cut(df['score'],
                                    bins=[df['score'].min(), quartiles[0], quartiles[1], quartiles[2], df['score'].max()],
        ...
        for quartile in quartile_labels:
            ...
            if len(quartile_data) > 0:
                quartile_analysis[quartile] = {
                    ...
                    'positive_reward_ratio': float((quartile_data['reward'] > quartile_data['reward'].iloc[0]).mean())  # 初期値より増加した割合
                }

        # スコアの有効性指標
        effectiveness = {
            'high_score_better': quartile_analysis.get('Q4 (Highest)', {}).get('avg_reward', 0) >
                               quartile_analysis.get('Q1 (Low)', {}).get('avg_reward', 0),
            'score_reward_trend': 'increasing' if all(
                quartile_analysis.get(q1, {}).get('avg_reward', 0) <= quartile_analysis.get(q2, {}).get('avg_reward', 0)
...
"""
        ...
        if not effectiveness['effectiveness_indicators']['high_score_better']:
            issues.append("高いスコアが低いスコアよりも悪い結果 - スコアリングロジックの逆転")
        if effectiveness['effectiveness_indicators']['score_reward_trend'] == 'not_increasing':
...
def main():
    """メイン実行関数"""
    debugger = SignalGuidanceDebugger("signal_guidance_backtest_results_20251112_135639.json")

    try:
        report = debugger.generate_report()

        # レポートをファイルに保存
        with open("signal_guidance_debug_analysis_report.md", 'w', encoding='utf-8') as f:
            f.write(report)

        print("デバッグ分析レポートを生成しました: signal_guidance_debug_analysis_report.md")
        print("\n=== 分析サマリー ===")
        print(report.split('\n')[:20])  # 最初の20行を表示
