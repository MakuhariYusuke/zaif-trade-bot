...
class SimpleTradingEnv(gym.Env):
    def __init__(self, data):
        ...
        self.data = data
        self.current_step = 0
        self.action_space = gym.spaces.Box(low=-1, high=1, shape=(1,), dtype=np.float32)  # Continuous action
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(5,), dtype=np.float32)  # OHLCV
        self.position = 0
        self.entry_price = 0
        self.total_pnl = 0
    ...
    def _get_obs(self):
        return self.data.iloc[self.current_step][['open', 'high', 'low', 'close', 'volume']].values.astype(np.float32)
    ...
    for model_path in v396_models:
        if os.path.exists(model_path):
            try:
                ...
                print(f"Buy/Sell/Hold: {result['buy_actions']}/{result['sell_actions']}/{result['hold_actions']}")
                print("---")
