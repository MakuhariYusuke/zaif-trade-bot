...
def create_sac_config_with_params(
    ...
    # Learning parameters
    sac_params["learning_rate"] = params.get(
    ...
    sac_params["ent_coef_init"] = params.get(
        "ent_coef_init", sac_params["ent_coef_init"]
    )
    ...
    config["training"]["learning_starts"] = params.get(
        "learning_starts", config["training"]["learning_starts"]
    )
    config["training"]["sac_hyperparameters"]["gradient_steps"] = params.get(
        "gradient_steps", config["training"]["sac_hyperparameters"]["gradient_steps"]
    )
    ...
    env_config["reward_scale"] = params.get(
        "reward_scale", env_config.get("reward_scale", 100.0)
    )
    ...
    behavior_config["entropy_regularization"] = params.get(
        "entropy_regularization", behavior_config.get("entropy_regularization", 0.0)
    )
    behavior_config["action_smoothing"] = params.get(
        "action_smoothing", behavior_config.get("action_smoothing", 0.15)
    )

    return config


def run_sac_training_with_config(
    config: Dict[str, Any], max_steps: int = 5000
) -> Dict[str, Any]:
    """
...
"""
            ...
            training_script = training_script.replace(
                "MAX_STEPS_PLACEHOLDER", str(max_steps)
            )
...
def sac_parameter_optimization_objective(params: Dict[str, Any]) -> float:
    ...
    try:
        ...
        training_results = run_sac_training_with_config(
            config, max_steps=1000
        )  # Reduced from 2000 for faster optimization
...
def main():
    ...
    search_space = {
        ...
        "batch_size": {
            ...
            "high": 512,
            "step": 64,
        },  # Narrowed from 64  # Narrowed from 1024
        "buffer_size": {
            "type": "int",
        ...
        "tau": {"type": "float", "low": 0.001, "high": 0.1, "log": True},
        "gamma": {"type": "float", "low": 0.90, "high": 0.999},
        "ent_coef_init": {"type": "float", "low": 0.1, "high": 2.0},
        # Training parameters
        "learning_starts": {
            "type": "int",
        ...
        "entropy_regularization": {"type": "float", "low": 0.0, "high": 0.1},
        "action_smoothing": {"type": "float", "low": 0.0, "high": 0.5},
    }
    ...
    logger.info(".4f" f"Execution time: {result.execution_time:.2f}s")
