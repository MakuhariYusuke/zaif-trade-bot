...
class RegimeDetectionTester:
    ...
    def __init__(
        self,
        config_path: str = "config/sac_v444_advanced_regime_adaptation_config.json",
    ):
        """
    ...
    def generate_synthetic_data(
        self, regime: RegimeType, length: int = 500
    ) -> pd.DataFrame:
        """
        ...
        print(
            f"DEBUG SYNTHETIC: Regime {regime.value}, drift={regime_params['drift']}, noise={regime_params['noise']}"
        )

        # Generate price series using geometric Brownian motion
        regime_params = self._get_regime_generation_params(regime)
        prices = self._generate_geometric_brownian_series(
            length, regime_params["drift"], regime_params["noise"]
        )
        ...
        print(
            f"DEBUG RETURNS: Mean return: {np.mean(returns)}, Expected drift: {regime_params['drift']}"
        )
        ...
        for i, price in enumerate(prices):
            ...
            spread = abs(
                np.random.normal(0, regime_noise * 2)
            )  # Spread proportional to noise
            high = price * (1 + spread)
            low = price * (1 - spread)
    ...
    def _generate_geometric_brownian_series(
        self, n: int, drift: float, noise: float, seed: int = 42
    ) -> np.ndarray:
    ...
    def _get_regime_generation_params(self, regime: RegimeType) -> Dict[str, float]:
        ...
        print(
            f"DEBUG PARAMS: Input regime: {regime}, type: {type(regime)}, value: {regime.value}"
        )
        params = {
            RegimeType.STRONG_BULL_TREND: {"drift": 0.15, "noise": 0.005},
            RegimeType.MODERATE_BULL_TREND: {"drift": 0.001, "noise": 0.001},
            RegimeType.WEAK_BULL_TREND: {"drift": 0.0005, "noise": 0.001},
            RegimeType.STRONG_BEAR_TREND: {"drift": -0.15, "noise": 0.005},
            RegimeType.MODERATE_BEAR_TREND: {"drift": -0.001, "noise": 0.001},
            RegimeType.WEAK_BEAR_TREND: {"drift": -0.0005, "noise": 0.001},
            RegimeType.HIGH_VOLATILITY_RANGING: {"drift": 0.0, "noise": 0.02},
            RegimeType.MODERATE_VOLATILITY_RANGING: {"drift": 0.0, "noise": 0.015},
            RegimeType.LOW_VOLATILITY_RANGING: {"drift": 0.0, "noise": 0.01},
            RegimeType.EXTREME_VOLATILITY: {"drift": 0.0, "noise": 0.03},
            RegimeType.CONSOLIDATION: {"drift": 0.0, "noise": 0.005},
            RegimeType.BREAKOUT_SETUP: {"drift": 0.0005, "noise": 0.005},
            RegimeType.BREAKDOWN_SETUP: {"drift": -0.0005, "noise": 0.005},
        }
    ...
    def test_regime_detection_accuracy(self) -> Dict[str, Any]:
        ...
        for regime in all_regimes:
            ...
            detections = []
            for i in range(
                self.min_data_length // 2, len(test_data)
            ):  # Test every bar from midpoint
                try:
                    result = self.classifier.detect_regime(test_data, i)
                    detections.append(result)
                    if len(detections) <= 3:  # Debug first few detections
                        print(
                            f"    Debug: Index {i}, Detected {result.primary_regime.value}, Confidence {result.confidence:.3f}"
                        )
                        print(
                            f"    Debug: Metrics - trend_strength: {result.metrics.trend_strength:.3f}, volatility: {result.metrics.volatility:.3f}, momentum: {result.metrics.momentum:.3f}"
                        )
            ...
            if detections:
                primary_regime_matches = sum(
                    1 for d in detections if d.primary_regime == regime
                )
                high_confidence_detections = sum(
                    1 for d in detections if d.confidence >= self.confidence_threshold
                )

                accuracy = primary_regime_matches / len(detections)
                avg_confidence = np.mean([d.confidence for d in detections])
                secondary_regime_diversity = np.mean(
                    [len(d.secondary_regimes) for d in detections]
                )

                results[regime.value] = {
                    "accuracy": accuracy,
                    "avg_confidence": avg_confidence,
                    "high_confidence_ratio": high_confidence_detections
                    / len(detections),
                ...
                print(
                    f"    High Confidence Ratio: {results[regime.value]['high_confidence_ratio']:.3f}"
                )
            else:
    ...
    def _test_flat_market(self) -> Dict[str, Any]:
        ...
        try:
            ...
            flat_prices = np.full(300, 100.0) + np.random.normal(
                0, 0.01, 300
            )  # Very low volatility
            ...
            return {
                "success": True,
                "detected_regime": result.primary_regime.value,
                "expected_consolidation": result.primary_regime
                in [RegimeType.CONSOLIDATION, RegimeType.LOW_VOLATILITY_RANGING],
                "confidence": result.confidence,
            }
        except Exception as e:
            return {"success": False, "error": str(e)}
    ...
    def generate_accuracy_report(
        self, accuracy_results: Dict[str, Any], edge_case_results: Dict[str, Any]
    ) -> str:
        """
        ...
        if accuracy_results:
            accuracies = [
                r["accuracy"] for r in accuracy_results.values() if "accuracy" in r
            ]
        ...
        for regime_name, metrics in accuracy_results.items():
            if "accuracy" in metrics:
                ...
                report.append(
                    f"  High Confidence Ratio: {metrics['high_confidence_ratio']:.3f}"
                )
        ...
        for test_name, result in edge_case_results.items():
            report.append(
                f"{test_name}: {'✅ PASS' if result.get('success', False) else '❌ FAIL'}"
            )
        ...
        failed_edge_cases = [
            name
            for name, result in edge_case_results.items()
