...
def test_parallel_processing():
    ...
    from ztb.training.utils.parallel_utils import CPUParallelProcessor
    ...
    batch_results = processor.parallel_batch_process(
        batch_func, test_data, batch_size=3
    )
    expected_batch = [
        [x * 2 for x in test_data[i : i + 3]] for i in range(0, len(test_data), 3)
    ]
    assert (
        batch_results == expected_batch
    ), f"Batch processing failed: {batch_results} != {expected_batch}"
