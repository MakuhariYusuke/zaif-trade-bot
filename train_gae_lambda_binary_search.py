...
def train_gae_lambda_test(gae_lambda: float, reward_scaling: float = 6.0, entropy_coef: float = 0.03) -> str:
    ...
    gae_lambda_str = f"{gae_lambda:.4f}".replace('.', '')
    config_name = f"gae_lambda_{gae_lambda_str}"

    print(f"Starting training with config: {config_name}")
    print(f"Reward scaling: {reward_scaling}, Entropy coef: {entropy_coef}, GAE lambda: {gae_lambda}")
    ...
    model_path = PROJECT_ROOT / "models" / f"gae_lambda_test_{config_name}.zip"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO model with specified GAE lambda")
    ...
    # Create config name from gae_lambda
    gae_lambda_str = f"{args.gae_lambda:.4f}".replace('.', '')
    config_name = f"gae_lambda_{gae_lambda_str}"

    print(f"\n{'='*60}")
    print(f"Training with gae_lambda = {args.gae_lambda}")
    print(f"Config: {config_name}")
    print(f"{'='*60}")

    try:
        model_path = train_gae_lambda_test(
