...
def main():
    parser = argparse.ArgumentParser(
        description="Run balance-focused AB tests"
    )
    parser.add_argument(
        "--base-config",
    ...
    parser.add_argument(
        "--run",
        action="store_true",
        help="Run AB tests after generating configs"
    )
    ...
    if args.run:
        ...
        print(f"  python tools/ab_test_runner.py \\")
        print(f"    --configs {' '.join(config_paths)} \\")
        print(f"    --seeds {args.seeds} --jobs {args.jobs} --timesteps {args.timesteps}")
