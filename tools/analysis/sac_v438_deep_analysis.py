...
class SACv438DeepAnalyzer:
    ...
    def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        # 4. リスク調整リターン分析
        self.analysis_results['risk_adjusted_analysis'] = self._analyze_risk_adjusted_returns()

        # 5. 行動パターン分析
        self.analysis_results['behavioral_analysis'] = self._analyze_behavioral_patterns()

        # 6. 統計的意義検定
        self.analysis_results['statistical_significance'] = self._analyze_statistical_significance()

        # 7. 改善点の抽出
        self.analysis_results['improvement_insights'] = self._extract_improvement_insights()
    ...
    def _analyze_market_regimes(self) -> Dict[str, Any]:
        ...
        for regime_name, regime_info in regimes.items():
            ...
            multipliers = regime_multipliers.get(regime_name, {'return': 1.0, 'win_rate': 1.0, 'drawdown': 1.0})

            regime_performance[regime_name] = {
                'total_return': base_performance.get('total_return', 0) * multipliers['return'],
                'win_rate': min(1.0, base_performance.get('win_rate', 0) * multipliers['win_rate']),
                'max_drawdown': base_performance.get('max_drawdown', 0) * multipliers['drawdown'],
                'sharpe_ratio': base_performance.get('sharpe_ratio', 0) * multipliers['return'],
                'regime_weight': regime_info['weight'],
                'trade_count': int(base_performance.get('total_trades', 0) * regime_info['weight'])
            }
    ...
    def _calculate_regime_adaptability(self, regime_performance: Dict[str, Any]) -> float:
        """レジーム適応性スコアの計算"""
    ...
    def _calculate_stability_score(self, return_vol: float, win_rate_vol: float, sharpe_vol: float) -> float:
        """安定性スコアの計算"""
    ...
    def _analyze_risk_adjusted_returns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'omega_ratio': omega,
            'risk_adjusted_rank': self._rank_risk_adjusted_metrics(calmar, sortino, omega),
            'volatility_adjusted_return': total_return / volatility if volatility > 0 else 0
        }
    ...
    def _rank_risk_adjusted_metrics(self, calmar: float, sortino: float, omega: float) -> str:
        """リスク調整指標のランキング"""
    ...
    def _analyze_statistical_significance(self) -> Dict[str, Any]:
        ...
        # 全体的な統計的意義スコア
        overall_significance = sum(test['significant'] for test in significance_tests.values()) / len(significance_tests)
        ...
        return {
            ...
            'recommendations': self._generate_statistical_recommendations(significance_tests)
        }

    def _test_return_significance(self, p_avg_results: Dict[str, Any]) -> Dict[str, Any]:
        """リターンの統計的意義検定"""
        p_avg_return = p_avg_results.get('p_average_return', 0)
        return_volatility = p_avg_results.get('return_volatility', 0)

        # t検定の簡易版（帰無仮説: リターンが0）
        t_statistic = p_avg_return / (return_volatility / np.sqrt(p_avg_results.get('sample_size', 1)))
        ...
        return {
            ...
            'effect_size': p_avg_return / return_volatility if return_volatility > 0 else 0
        }

    def _test_win_rate_significance(self, p_avg_results: Dict[str, Any]) -> Dict[str, Any]:
        """勝率の統計的意義検定"""
        ...
        t_statistic = (p_avg_win_rate - expected_win_rate) / (win_rate_volatility / np.sqrt(p_avg_results.get('sample_size', 1)))
    ...
    def _test_stability_significance(self, p_avg_results: Dict[str, Any]) -> Dict[str, Any]:
        """安定性の統計的意義検定"""
    ...
    def _generate_statistical_recommendations(self, significance_tests: Dict[str, Any]) -> List[str]:
        """統計的検定結果に基づく推奨事項の生成"""
        recommendations = []

        if not significance_tests['return_significance']['significant']:
            recommendations.append("リターンの統計的意義が不十分 - サンプルサイズを増やすか、戦略を改善")

        if not significance_tests['win_rate_significance']['significant']:
            recommendations.append("勝率がランダムと統計的に区別できない - シグナル品質の改善が必要")
    ...
    def _extract_improvement_insights(self) -> Dict[str, Any]:
        ...
        if perf.get('sharpe_ratio', 0) > 1.5:
            insights['strengths'].append("優れたリスク調整リターン（Sharpe Ratio > 1.5）")
        ...
        if perf.get('max_drawdown', 0) > 0.15:
            insights['weaknesses'].append("ドローダウンが大きい（リスク管理の改善が必要）")
    ...
    def _generate_action_items(self, insights: Dict[str, Any]) -> List[str]:
        ...
        for weakness in insights['weaknesses']:
            if 'ドローダウン' in weakness:
                action_items.append("リスク管理強化: ストップロス最適化、ポジションサイジング改善")
            if '安定性' in weakness:
        ...
        for opportunity in insights['opportunities']:
            ...
            if 'アクション' in opportunity:
                action_items.append("行動最適化: アクション分布の動的調整メカニズム導入")

        # 脅威に対するアクション
        for threat in insights['threats']:
            if '統計的意義' in threat:
                action_items.append("堅牢性確保: クロスバリデーション導入、統計的検定強化")
...
def main():
    ...
    try:
        ...
        print(f"  レジーム適応性スコア: {regime.get('regime_adaptability_score', 0):.3f}")
        ...
        print(f"  信頼区間: {p_avg.get('confidence_interval', (0, 0))[0]:.3f} - {p_avg.get('confidence_interval', (0, 0))[1]:.3f}")
