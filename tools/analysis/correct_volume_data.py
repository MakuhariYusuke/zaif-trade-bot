...
class VolumeDataCorrector:
    ...
    def correct_volume_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        missing_columns = [col for col in required_columns if col not in corrected_df.columns]
        if missing_columns:
        ...
        for idx, row in corrected_df.iterrows():
            try:
                ...
                if 1e10 <= volume_btc <= 1e16:
                    ...
                    logger.debug(f"Corrected volume at {idx}: {volume_btc:.2f} JPY -> {volume_btc_corrected:.6f} BTC")
                else:
    ...
    def validate_correction(self, original_df: pd.DataFrame, corrected_df: pd.DataFrame) -> dict:
        """
        ...
        for idx, (orig_vol, corr_vol, close_price) in enumerate(zip(
            original_df['volume'], corrected_df['volume'], corrected_df['close']
        )):
            if orig_vol > 0 and close_price > 0:
                ...
                if 1e10 <= orig_vol <= 1e16:
                    ...
                    ratio_diff = abs(ratio - expected_ratio) / expected_ratio if expected_ratio > 0 else 0
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Correct Yahoo Finance BTC-JPY volume data')
    parser.add_argument('--input', '-i', required=True, help='Input CSV file path')
    parser.add_argument('--output', '-o', required=True, help='Output CSV file path')
    parser.add_argument('--validate', action='store_true', help='Perform validation after correction')
    ...
    try:
        ...
        logger.info(f"Original volume range: {df['volume'].min():.2e} - {df['volume'].max():.2e}")
        ...
        logger.info(f"Corrected volume range: {corrected_df['volume'].min():.2e} - {corrected_df['volume'].max():.2e}")
        ...
        if args.validate:
            ...
            logger.info(f"Original volume range: {validation_results['original_volume_range']}")
            logger.info(f"Corrected volume range: {validation_results['corrected_volume_range']}")
            logger.info(f"Correction statistics: {validation_results['correction_stats']}")

            if validation_results['volume_ratio_check']:
                logger.warning(f"Found {len(validation_results['volume_ratio_check'])} suspicious volume ratios")
                for check in validation_results['volume_ratio_check'][:5]:  # Show first 5
                    logger.warning(f"Index {check['index']}: ratio diff {check['ratio_difference_pct']:.1f}%")
            else:
