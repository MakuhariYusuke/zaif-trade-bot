    ...
    for report_file in report_files:
        try:
            ...
            results.append({
                ...
                "timesteps": data.get("training_stats", {}).get("total_timesteps", 0)
            })
...
def main():
    ...
    print("-" * 80)
    print(f"{'Rank':<5} {'Balance':<8} {'BUY':<8} {'SELL':<8} {'HOLD':<8} {'Shaping':<10} {'Penalty':<8}")
    print("-" * 80)
    ...
    if target_results:
        ...
        for r in sorted(target_results, key=lambda x: x["balance_score"], reverse=True)[:5]:
            print(
                f"  BUY={r['buy']:.1f}%, SELL={r['sell']:.1f}%, HOLD={r['hold']:.1f}% "
                f"(Balance={r['balance_score']:.2f})"
            )
            if r["balance_shaping"] != "N/A":
                print(f"    → balance_shaping={r['balance_shaping']}, penalty={r['balance_penalty']}")
    else:
    ...
    print(f"   BUY={best['buy']:.1f}%, SELL={best['sell']:.1f}%, HOLD={best['hold']:.1f}%")
    if best["balance_shaping"] != "N/A":
        print(f"   Config: balance_shaping={best['balance_shaping']}, penalty={best['balance_penalty']}")
    ...
    if target_results:
        ...
        shaping_values = [
            r["balance_shaping"] for r in target_results
            if r["balance_shaping"] != "N/A"
        ]
        if shaping_values:
            avg_shaping = sum(shaping_values) / len(shaping_values)
            print(f"1. Target distributions found with avg balance_shaping={avg_shaping:.3f}")
        ...
        print("3. Run: python tools/run_balance_ab_tests.py --balance-values 0.04 0.05 0.06 --run")
