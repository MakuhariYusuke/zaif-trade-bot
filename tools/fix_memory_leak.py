...
def add_cache_cleanup_to_ab_test():
    ...
    if "from ztb.cache.memory_cache import default_memory_manager" not in content:
        ...
        lines.insert(import_end_idx + 1, "from ztb.cache.memory_cache import default_memory_manager")
        content = "\n".join(lines)
...
"""
            content = content[:func_body_start] + cleanup_code + content[func_body_start:]
