...
def main() -> None:
    ...
    logger.info(f"{'='*80}")
    logger.info("v384 Backtest - 68 Curated Features")
    logger.info(f"{'='*80}")
    ...
    logger.info(
        f"Environment observation space: {env.observation_space.shape[0]} features"
    )
    ...
    logger.info(
        f"  Average PnL: {np.mean(all_pnls):,.0f} ± {np.std(all_pnls):,.0f} JPY"
    )
    logger.info(f"  Total PnL: {np.sum(all_pnls):,.0f} JPY")
    logger.info(
        f"  Average return: {np.mean(all_returns):.2f}% ± {np.std(all_returns):.2f}%"
    )
    ...
    if len(trades) > 0:
        ...
        logger.info(
            f"  Winning trades: {len(winning)} ({len(winning)/len(trades)*100:.1f}%)"
        )
        logger.info(
            f"  Losing trades: {len(losing)} ({len(losing)/len(trades)*100:.1f}%)"
        )
        if winning:
    ...
    if len(all_step_pnls) > 0:
        ...
        if np.std(all_step_pnls) > 0:
            ...
            sharpe = (np.mean(all_step_pnls) / np.std(all_step_pnls)) * np.sqrt(
                periods_per_year
            )
        else:
            sharpe = 0.0
