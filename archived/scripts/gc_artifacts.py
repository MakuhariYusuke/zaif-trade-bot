...
class CheckpointGarbageCollector:
    ...
    def mark_keepers(self, checkpoints: List[CheckpointInfo]) -> None:
        ...
        if self.keep_best > 0:
            ...
            with_sharpe.sort(
                key=lambda x: x.sharpe_ratio or float("-inf"), reverse=True
            )
            # Mark top M
        ...
        logger.info(f"Retention policy: {keep_count} to keep, {delete_count} to delete")
    ...
    def run(self) -> None:
        ...
        logger.info(
            f"Policy: keep_last={self.keep_last}, keep_best={self.keep_best}, ttl_days={self.ttl_days}"
        )
