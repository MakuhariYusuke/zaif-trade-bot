...
def run_paper_trading(
    model: SAC, env: HeavyTradingEnv, max_steps: int = 5000
) -> Dict[str, Any]:
    ...
    while step_count < max_steps:
        ...
        if step_count % 1000 == 0:
            logger.info(
                f"Step {step_count}/{max_steps}, Portfolio: {env.portfolio_value:.2f} JPY"
            )
    ...
    results = {
        ...
        "total_return_percent": (
            (env.portfolio_value - env.initial_balance) / env.initial_balance
        )
        * 100,
