...
def main():
    ...
    try:
        ...
        print(
            "Features: Win rate bonus (20pts for 80%+, 10pts for 70%+, 5pts for 60%+)"
        )
        ...
        if result and result.get("success"):
            ...
            print(
                f"   Error: {result.get('error', 'Unknown error') if result else 'No result returned'}"
            )

        print("\n✅ Training completed successfully!")
        print(
            f"Model saved to: checkpoints/sac_session/{config.get('model_name')}_final.zip"
        )
