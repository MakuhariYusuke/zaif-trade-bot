...
class SACBiasAnalyzer:
    ...
    def analyze_action_distribution(self, n_samples: int = 10000) -> Dict[str, Any]:
        ...
        for i in range(n_samples):
            ...
            buy_threshold = self.config.get("reward_settings", {}).get(
                "action_threshold_buy", 0.05
            )
            sell_threshold = self.config.get("reward_settings", {}).get(
                "action_threshold_sell", -0.3
            )
            ...
            reward = self.reward_calculator.calculate_reward(
                ...
                reward_scaling=self.config.get("reward_settings", {}).get(
                    "reward_scale", 8000.0
                ),
    ...
    def analyze_reward_bias(self, n_episodes: int = 100) -> Dict[str, Any]:
        ...
        for episode in range(n_episodes):
            ...
            while not done and step < 1000:  # Limit episode length
                ...
                # Step environment
                obs, actual_reward, terminated, truncated, info = self.env.step(
                    discrete_action
                )
    ...
    def generate_report(self, action_analysis: Dict, reward_analysis: Dict) -> str:
        ...
        for stage, action_rewards in reward_analysis["stage_rewards"].items():
            ...
            for action, rewards in action_rewards.items():
                if rewards:
                    ...
                    report.append(
                        f"  {action_name}: {avg_reward:.4f} (n={len(rewards)})"
                    )
        ...
        report.append(
            f"  SELL threshold: {sell_threshold} (coverage: {sell_ratio:.1%})"
        )

        if sell_ratio > buy_ratio * 2:
            report.append(
                "⚠️  SELL threshold allows much more coverage than BUY threshold"
            )
