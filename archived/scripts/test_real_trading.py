    ...
    if response.status_code == 200:
        ...
        if "insufficient" in error_msg.lower() or "balance" in error_msg.lower():
            raise InsufficientFundsError(
                f"Insufficient funds for buy order: {error_msg}"
            )
...
def place_sell_order(
    api_key: str, api_secret: str, price: float, amount: float
) -> dict:
...
def main():
    ...
    parser.add_argument(
        ...
        default="coincheck",
        help="Exchange to use (default: coincheck)",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Run in dry-run mode (no real orders)"
    )
    ...
    try:
        ...
        if args.dry_run:
            ...
            print(
                f"   JPY: ¥{initial_balance['jpy']['free']:,.0f} free, ¥{initial_balance['jpy']['locked']:,.0f} locked"
            )
            print(
                f"   BTC: {initial_balance['btc']['free']:.6f} free, {initial_balance['btc']['locked']:.6f} locked"
            )
        else:
            if args.exchange == "coincheck":
            ...
            try:
                if args.exchange == "coincheck":
                    buy_result = place_buy_order(
                        api_key, api_secret, test_buy_price, test_amount
                    )
                else:  # bitflyer
                    buy_result = place_buy_order_bitflyer(
                        api_key, api_secret, test_buy_price, test_amount
                    )
                ...
                if isinstance(e, InsufficientFundsError):
                    print(
                        "⚠️  Buy order failed due to insufficient funds (expected for testing)"
                    )
                ...
                buy_result = {"error": type(e).__name__.lower(), "message": str(e)}
                buy_order_id = None
            except Exception:
            ...
            try:
                if args.exchange == "coincheck":
                    sell_result = place_sell_order(
                        api_key, api_secret, high_sell_price, sell_amount
                    )
                else:  # bitflyer
                    sell_result = place_sell_order_bitflyer(
                        api_key, api_secret, high_sell_price, sell_amount
                    )
                ...
                if isinstance(e, InsufficientFundsError):
                    print(
                        "⚠️  Sell order failed due to insufficient funds (expected for testing)"
                    )
                elif isinstance(e, MinimumSizeError):
                    print("⚠️  Sell order failed due to minimum size requirements")
                print("🔄 Continuing with test simulation...")
                sell_result = {"error": type(e).__name__.lower(), "message": str(e)}
                sell_order_id = None
            except Exception:
                # Re-raise unexpected errors
                raise
            ...
            if args.dry_run:
                ...
                try:
                    if args.exchange == "coincheck":
                        ...
                        cancel_result = cancel_order_bitflyer(
                            api_key, api_secret, sell_order_id
                        )
                    print("✅ Sell order cancelled successfully!")
                except Exception as e:
                    error_msg = str(e)
                    if (
                        "Order not found" in error_msg
                        or "already cancelled" in error_msg.lower()
                        or "not found" in error_msg.lower()
                    ):
                        print(
                            "⚠️  Order cancellation failed (order may have already executed or been cancelled)"
                        )
        ...
        if args.dry_run:
            ...
            print(
                f"   JPY: ¥{final_balance['jpy']['free']:,.0f} free, ¥{final_balance['jpy']['locked']:,.0f} locked"
            )
            print(
                f"   BTC: {final_balance['btc']['free']:.6f} free, {final_balance['btc']['locked']:.6f} locked"
            )
        else:
            if args.exchange == "coincheck":
