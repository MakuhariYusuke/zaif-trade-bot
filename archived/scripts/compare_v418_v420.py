...
def run_comparison():
    ...
    if len(successful_configs) == 2:
        print(
            "Both configurations completed successfully. Compare the training reports and metrics to determine which performs better."
        )
    elif len(successful_configs) == 1:
        print(
            f"Only {successful_configs[0]} completed successfully. This appears to be the better configuration."
        )
    else:
        print(
            "Both configurations failed. Check the error messages above and fix the issues."
        )
