...
def estimate_action_distribution(sell_rate: float) -> Dict[str, float]:
    ...
    return {"hold": hold_rate, "buy": buy_rate, "sell": sell_rate}


def diagnose_sell_avoidance(
    sell_rate: float, lambda_val: float, constraint_active: bool
) -> SellAvoidanceDiagnosis:
    """SELL回避問題の詳細診断"""
    ...
    if lambda_val >= 1.9:  # 上限近く
        ...
        recommendations.append(
            "1. Lambda上限を大幅に引き上げる: lambda_max 2.0 → 10.0 以上"
        )
    ...
    print(
        f"  推定エントロピー: {entropy:.4f} / {max_entropy:.4f} ({entropy/max_entropy*100:.1f}%)"
    )
    print()
    ...
    # 原因1: Lagrange制約の設計問題
    print("\n【原因1】Lagrange制約の設計問題")
    print("  - 現在のλ上限: 2.0 → 完全に飽和")
    print("  - 現在のη（学習率）: 0.05 → 遅すぎる可能性")
    print("  - 制約の強さが不十分でSELL誘導できていない")
...
def main():
    ...
    diagnosis = diagnose_sell_avoidance(
        info["sell_rate"], info.get("lambda", 0.0), info.get("constraint_active", False)
    )
