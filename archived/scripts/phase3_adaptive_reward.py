...
class AdaptiveRewardSystem:
    ...
    def __init__(
        self, data_path: str = "data/btc_jpy_correlation_aware_v426_dataset.csv"
    ):
    ...
    def calculate_adaptive_reward(
        self, row: pd.Series, config_name: str = "correlation_focused"
    ) -> float:
        """
        適応型報酬を計算
    ...
    def create_reward_curriculum(self, df: pd.DataFrame) -> Dict[str, Dict]:
        ...
        for config_name, config in self.reward_configs.items():
            ...
            curriculum[config_name] = {
                ...
                "regime_stats": {
                    regime: {"mean": np.mean(rewards), "count": len(rewards)}
        ...
        for corr_bonus in param_grid["correlation_bonus"]:
            for regime_mult in param_grid["regime_multiplier"]:
                for vol_penalty in param_grid["volatility_penalty"]:
                    ...
                    for idx, row in df.iterrows():
                        reward = self.calculate_adaptive_reward_with_config(
                            row, test_config
                        )
    ...
    def calculate_adaptive_reward_with_config(
        self, row: pd.Series, config: Dict
    ) -> float:
        """指定された設定で適応型報酬を計算"""
        # 基本報酬
        base_reward = config["base_penalty"]
    ...
    def generate_reward_report(self, curriculum: Dict, optimization: Dict) -> None:
        ...
        with open(self.report_path, "w", encoding="utf-8") as f:
            ...
            for stage_name, stage_data in curriculum.items():
                ...
                f.write(
                    f"- 相関ボーナス: {stage_data['config']['correlation_bonus']}\n"
                )
                f.write(
                    f"- レジーム倍率: {stage_data['config']['regime_multiplier']}\n\n"
                )

                f.write("レジーム別平均報酬:\n")
                for regime, r_stats in stage_data["regime_stats"].items():
                    f.write(
                        f"- {regime}: {r_stats['mean']:.6f} ({r_stats['count']} サンプル)\n"
                    )
                f.write("\n")
