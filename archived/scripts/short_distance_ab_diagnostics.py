...
def generate_synthetic_uptrend(
    n_steps: int = 300, base_price: float = 100.0
) -> pd.DataFrame:
...
def generate_synthetic_downtrend(
    n_steps: int = 300, base_price: float = 100.0
) -> pd.DataFrame:
...
def run_diagnostic(
    ...
    print(
        f"Config: T={config.temperature}, tau={config.tiebreaker_tau}, "
    ...
    print(
        f"  Action distribution: HOLD={action_dist[0]:.1%}, BUY={action_dist[1]:.1%}, SELL={action_dist[2]:.1%}"
    )
    print(
        f"  Legal SELL rate: {legal_sell_stats['legal_sell_rate']:.1%} (target: ≥15%)"
    )
    print(f"  Probability std (mean): {np.mean(prob_stds):.4f} (target: >0)")
    print(
        f"  Margin (mean±std): {np.mean(margins_array):.4f}±{np.std(margins_array):.4f}"
    )
    print(
        f"  Tiebreaker activations: {np.sum(tiebreaker_array)}/{len(tiebreaker_array)} ({np.mean(tiebreaker_array):.1%})"
    )
    ...
    print(
        f"  Cost gate triggered: {np.sum(cost_gate_array)}/{len(cost_gate_array)} ({np.mean(cost_gate_array):.1%})"
    )  # NEW

    # Print calibration diagnostics
    print("\n  Calibration Diagnostics:")
    print(
        f"    Brier score (overall): {calibration_report['brier_score']['overall']:.4f} (lower is better)"
    )
    print("    Brier score per action:")
...
def main():
    ...
    parser.add_argument(
        "--steps", type=int, default=300, help="Number of steps per dataset"
    )
    parser.add_argument(
        "--real-data",
        type=str,
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.7, help="Softmax temperature"
    )
    parser.add_argument(
        "--tiebreaker-tau", type=float, default=0.05, help="Tiebreaker margin threshold"
    )
    parser.add_argument(
        "--disable-tiebreaker", action="store_true", help="Disable tiebreaker"
    )
    parser.add_argument(
        "--deterministic",
        action="store_true",
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="diagnostics_results.json",
    ...
    result_up_det = run_diagnostic(
        model_file, synth_up, "Synthetic_Uptrend_Det", config_det, args.steps
    )
    ...
    print("=" * 60)
    result_down_det = run_diagnostic(
        model_file, synth_down, "Synthetic_Downtrend_Det", config_det, args.steps
    )
    ...
    if real_data is not None:
        ...
        print("=" * 60)
        result_real_det = run_diagnostic(
            model_file, real_data, "Real_Data_Det", config_det, args.steps
        )
    ...
    result_up_stoch = run_diagnostic(
        model_file, synth_up, "Synthetic_Uptrend_Stoch", config_stoch, args.steps
    )
    ...
    for result in all_results:
        ...
        print(
            f"    - Legal SELL rate: {result['legal_sell_stats']['legal_sell_rate']:.1%}"
        )
        print(
            f"    - Prob std (mean): {result['probability_variance']['mean_std']:.4f}"
        )
