...
class PerformanceAttributionAnalyzer:
    ...
    def __init__(self, annualization_factor: int = TRADING_DAYS_PER_YEAR):
        """
        Args:
    ...
    def create_market_factors(
        ...
        if risk_free_rate is not None:
            ...
            factors["interest_rate"] = pd.Series(
                0.02 / TRADING_DAYS_PER_YEAR, index=returns.index
            )  # 2%年率
    ...
    def single_period_attribution(
        ...
        explained_return = (
            1 + pd.Series(y_pred, index=strategy_returns.index)
        ).prod() - 1
    ...
    def multi_period_attribution(
        ...
        factor_returns: Dict[str, pd.Series],
        window_size: int = 60,
    ) -> Dict[str, PerformanceAttributionResult]:
        """
        複数期間のパフォーマンス帰属分析
    ...
    def rolling_attribution(
        ...
        for i in range(window_size, len(strategy_returns) + 1):
            ...
            try:
                ...
                warnings.warn(
                    f"Failed rolling analysis at {strategy_returns.index[i-1]}: {e}"
                )
                continue

        return pd.DataFrame(rolling_data).set_index("date")

    def run_performance_attribution_analysis(
        self,
        strategy_returns: pd.Series,
        ...
        if include_multi_period:
            multi_period = self.multi_period_attribution(
        ...
        if include_rolling:
            rolling_attribution = self.rolling_attribution(
                strategy_returns, factor_returns
            )
    ...
    def analyze(self, metrics) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_result: Dict[str, Any] = {
                "attribution_score": result.single_period.r_squared
                if result.single_period
                else 0.0,
                "factor_contributions": {},
            ...
            if result.single_period and result.single_period.factors:
                for factor in result.single_period.factors:
                    analysis_result["factor_contributions"][
                        factor.name
                    ] = factor.contribution
            ...
            if result.single_period and result.single_period.factor_contributions:
                for (
                    factor_name,
                    contribution,
                ) in result.single_period.factor_contributions.items():
            ...
            if result.rolling_attribution is not None:
                ...
                if hasattr(result.rolling_attribution, "r_squared"):
                    analysis_result["rolling_r_squared"] = (
                        result.rolling_attribution.r_squared.tolist()
                        if hasattr(result.rolling_attribution.r_squared, "tolist")
                        else []
                    )
            ...
            if result.multi_period is not None:
                for period_name, period_result in result.multi_period.items():
                    if hasattr(period_result, "factor_contributions"):
                        analysis_result["multi_period_factors"][period_name] = dict(
                            period_result.factor_contributions
                        )
    ...
    def plot_attribution_analysis(
        self, result: AttributionAnalysisResult, save_path: Optional[str] = None
    ):
        """パフォーマンス帰属分析結果を可視化"""
        ...
        values = [
            result.single_period.explained_return,
            result.single_period.unexplained_return,
        ]
        ...
        # 残差分析
        axes[1, 0].plot(
            result.single_period.residuals.index,
            result.single_period.residuals.values,
            "b-",
            alpha=0.7,
        )
        ...
        # ローリングR²（利用可能な場合）
        if (
            result.rolling_attribution is not None
            and not result.rolling_attribution.empty
        ):
            axes[1, 1].plot(
                result.rolling_attribution.index,
                result.rolling_attribution["r_squared"],
                "g-",
                linewidth=2,
                label="R²",
            )
            ...
            axes[1, 1].text(
                0.5,
                0.5,
                "Rolling analysis not available",
                ha="center",
                va="center",
    ...
    def export_results(self, result: AttributionAnalysisResult, output_path: str):
        ...
        if result.rolling_attribution is not None:
            export_data["rolling_attribution"] = result.rolling_attribution.to_dict(
                "index"
            )  # type: ignore
...
def main():
    ...
    parser.add_argument(
        "--returns-path", required=True, help="Path to strategy returns (CSV)"
    )
    parser.add_argument(
        "--market-returns-path", help="Path to market returns (CSV, optional)"
    )
    parser.add_argument("--output-dir", default="results", help="Output directory")
    parser.add_argument(
        "--no-multi-period", action="store_true", help="Skip multi-period analysis"
    )
    parser.add_argument(
        "--no-rolling", action="store_true", help="Skip rolling analysis"
    )
    ...
    if "returns" in strategy_df.columns:
        strategy_returns = strategy_df.set_index(
            "date" if "date" in strategy_df.columns else strategy_df.columns[0]
        )["returns"]
    else:
    ...
    if args.market_returns_path:
        ...
        if "returns" in market_df.columns:
            market_returns = market_df.set_index(
                "date" if "date" in market_df.columns else market_df.columns[0]
            )["returns"]
        else:
