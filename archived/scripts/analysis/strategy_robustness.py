...
class StrategyRobustnessAnalyzer:
    ...
    def __init__(
        self, n_simulations: int = 100, confidence_level: float = 0.95
    ) -> None:
        """
        Args:
    ...
    def evaluate_strategy_performance(
        self,
        strategy_func: Callable[..., Dict[str, float]],
        ...
        try:
            ...
            volatility = float(returns.std() * np.sqrt(TRADING_DAYS_PER_YEAR))  # type: ignore
    ...
    def calculate_parameter_sensitivity(
        ...
        # 信頼区間の計算
        ci_lower = float(
            np.percentile(performances, (1 - self.confidence_level) * 100 / 2)
        )
        ci_upper = float(
            np.percentile(performances, (1 + self.confidence_level) * 100 / 2)
        )
    ...
    def calculate_robustness_score(
        self, performances: List[float], baseline_performance: float
    ) -> float:
        """
        堅牢性スコアの計算
    ...
    def find_optimal_parameters(
        ...
        # 初期値
        initial_guess = [
            np.mean(param_range) for param_range in parameter_ranges.values()
        ]
        ...
        try:
            ...
            # デフォルト値を使用
            return {
                name: float(np.mean(param_range))
    ...
    def run_robustness_analysis(
        ...
        """
        # 基準パフォーマンス
        baseline_perf = self.evaluate_strategy_performance(
            strategy_func, base_parameters, data
        )
            ...
            all_performances.extend(
                [
                    self.evaluate_strategy_performance(
                        strategy_func, {**base_parameters, param_name: val}, data
                    )[metric]
        ...
        robustness_score = self.calculate_robustness_score(
            all_performances, baseline_value
        )

        # 最適パラメータの探索
        optimal_parameters = self.find_optimal_parameters(
            strategy_func, parameter_ranges, data, metric
        )
    ...
    def run_monte_carlo_stability_test(
        self,
        strategy_func: Callable[..., Dict[str, float]],
        parameters: Dict[str, float],
        parameter_ranges: Dict[str, Tuple[float, float]],
        ...
        for _ in range(n_monte_carlo):
            ...
            perf = self.evaluate_strategy_performance(
                strategy_func, random_params, data
            )
            performances.append(perf[metric])
    ...
    def run_comprehensive_robustness_analysis(
        ...
        """
        # 基準パフォーマンス
        baseline_performance = self.evaluate_strategy_performance(
            strategy_func, base_parameters, data
        )
    ...
    def analyze(self, metrics) -> Dict[str, Any]:
        ...
        try:
            ...
            sharpe_ratio = (
                ...
                if hasattr(metrics, "sharpe_ratio") and metrics.sharpe_ratio != 0
                else returns_mean / returns_std
                if returns_std > 0
                else 0.0
            )
            ...
            avg_positive_streak = (
                float(np.mean(positive_streaks)) if positive_streaks else 0.0
            )
            avg_negative_streak = (
                abs(float(np.mean(negative_streaks))) if negative_streaks else 0.0
            )

            # 総合ロバストネススコア
            robustness_score = (
                0.3 * stability_score
                + 0.3 * max(0, sharpe_ratio / 2.0)
                + 0.2 * (1.0 / (1.0 + abs(max_drawdown)))  # シャープレシオを正規化
                + 0.2
                * max(
                    0, min(1, (skewness / 2.0 + 0.5))
                )  # 最大ドローダウンを正規化  # 歪度を0-1に正規化
            )
    ...
    def plot_robustness_analysis(
        self, result: StrategyRobustnessResult, save_path: Optional[str] = None
    ):
        """堅牢性分析結果を可視化"""
        ...
        # パラメータ感度
        param_names = [
            s.parameter_name for s in result.robustness_analysis.parameter_sensitivities
        ]
        sensitivity_scores = [
            s.sensitivity_score
        ...
        bars = axes[0, 0].bar(
            param_names, sensitivity_scores, color="skyblue", alpha=0.7
        )
        ...
        stability_values = [
            result.robustness_analysis.stability_score,
            result.robustness_analysis.robustness_score,
        ]

        bars2 = axes[0, 1].bar(
            stability_labels, stability_values, color="lightgreen", alpha=0.7
        )
        ...
        if len(result.robustness_analysis.parameter_sensitivities) >= 2:
            ...
            if len(matrix.columns) >= 3:  # パラメータ + 指標
                ...
                scatter = axes[0, 2].scatter(
                    x_vals, y_vals, c=z_vals, cmap="viridis", alpha=0.7
                )
        ...
        bars4 = axes[1, 1].bar(
            opt_param_names, opt_param_values, color="purple", alpha=0.7
        )
        ...
        bars5 = axes[1, 2].bar(
            baseline_labels, baseline_values, color="orange", alpha=0.7
        )
    ...
    def export_results(self, result: StrategyRobustnessResult, output_path: str):
        """分析結果を JSON ファイルにエクスポート"""
        export_data = {
            "baseline_performance": result.baseline_performance,
            "parameter_ranges": {
                k: list(v) for k, v in result.parameter_ranges.items()
            },
            "robustness_analysis": {
...
def main():
    ...
    parser.add_argument(
        "--strategy-function", required=True, help="Strategy function name"
    )
    parser.add_argument(
        "--parameters", required=True, help="Base parameters (JSON string)"
    )
    parser.add_argument(
        "--parameter-ranges", required=True, help="Parameter ranges (JSON string)"
    )
    parser.add_argument("--output-dir", default="results", help="Output directory")
    parser.add_argument("--metric", default="sharpe_ratio", help="Evaluation metric")
    parser.add_argument(
        "--no-monte-carlo", action="store_true", help="Skip Monte Carlo test"
    )
    ...
    parameter_ranges = {
        k: tuple(v) for k, v in json.loads(args.parameter_ranges).items()
    }
