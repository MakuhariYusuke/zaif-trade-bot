...
env_spec = importlib.util.spec_from_file_location(
    "environment", project_root / "ztb" / "trading" / "environment" / "environment.py"
)
...
def analyze_forced_diversity_penalty() -> None:
    ...
    for counts in action_counts_scenarios:
        ...
        print(
            f"Action counts {counts}: ratios={[f'{r:.1f}' for r in action_ratios]}, unused_penalty={unused_penalty:.2f}, ratio_penalty={ratio_penalty:.2f}, balance_bonus={balance_bonus:.2f}"
        )
...
def analyze_data_distribution() -> None:
    ...
    if "close" in df.columns:
        ...
        print(
            f"価格変化の統計: mean={price_changes.mean():.6f}, std={price_changes.std():.6f}"
        )
        print(
            f"上昇日数: {(price_changes > 0).sum()}, 下落日数: {(price_changes < 0).sum()}"
        )

    # 特徴量のSELL関連性の分析
    sell_related_features = [
        col
        for col in df.columns
        if any(keyword in col.lower() for keyword in ["sell", "short", "bear", "down"])
    ]
    print(f"SELL関連特徴量: {sell_related_features}")

    buy_related_features = [
        col
        for col in df.columns
        if any(keyword in col.lower() for keyword in ["buy", "long", "bull", "up"])
    ]
