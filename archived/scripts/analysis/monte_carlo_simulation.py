...
class MonteCarloSimulator:
    ...
    def __init__(
        ...
        time_horizon: int = TRADING_DAYS_PER_YEAR,
    ...
    def fit_distribution(
        self, returns: pd.Series, distribution: str = "normal"
    ) -> Dict[str, Any]:
        """
        リターンの分布をフィット
        ...
        if distribution == "normal":
            ...
            params = {"type": "t", "df": df, "loc": loc, "scale": scale}
    ...
    def generate_random_returns(
        self, params: Dict[str, Any], n_periods: int
    ) -> np.ndarray:
        """
        ...
        if params["type"] == "normal":
            ...
            returns = stats.t.rvs(
                params["df"], loc=params["loc"], scale=params["scale"], size=n_periods
            )
    ...
    def simulate_portfolio_path(
        self, initial_value: float, returns: np.ndarray, transaction_costs: float = 0.0
    ) -> np.ndarray:
        """
        ポートフォリオの価値推移をシミュレート
    ...
    def stress_test_scenarios(
        self, returns: pd.Series, initial_value: float = 10000
        ...
        scenarios["high_volatility"] = self.run_monte_carlo_simulation(
            high_vol_returns, initial_value
        )

        # 低リターンスシナリオ
        low_return_returns = returns * 0.7
        scenarios["low_returns"] = self.run_monte_carlo_simulation(
            low_return_returns, initial_value
        )
        ...
        scenarios["market_crash"] = self.run_monte_carlo_simulation(
            crash_returns, initial_value
        )
        ...
        scenarios["high_costs"] = self.run_monte_carlo_simulation(
            returns,
            initial_value,
            transaction_costs=0.005,  # 0.5%コスト
        )

        return scenarios

    def analyze_scenarios(
        self, scenario_results: Dict[str, MonteCarloResult]
    ...
    def analyze(self, metrics) -> Dict[str, Any]:
        ...
        if BenchmarkMetrics is None:
            raise ImportError(
                "BenchmarkMetrics not available. Please ensure comprehensive_benchmark.py is in the path."
            )
        ...
        # 日付インデックスを作成（仮定）
        dates = pd.date_range(
            start="2023-01-01", periods=len(metrics.returns), freq="D"
        )
        ...
        try:
            ...
            result = self.run_monte_carlo_simulation(
                returns=returns, initial_value=10000.0
            )

            # 結果を辞書形式に変換
            analysis_result: Dict[str, Any] = {
                "simulation_score": 1.0
                - abs(result.var_95) / 0.5,  # VaRが小さいほど良い
                ...
                if hasattr(result.final_values, "tolist")
                else [],
                "simulation_paths": result.simulation_paths.tolist()
                if hasattr(result.simulation_paths, "tolist")
                else [],
            }
    ...
    def plot_monte_carlo_results(
        self, result: MonteCarloAnalysisResult, save_path: Optional[str] = None
    ):
        """モンテカルロ分析結果を可視化"""
        ...
        # 最終価値の分布
        axes[0, 0].hist(
            baseline.final_values, bins=50, alpha=0.7, color="skyblue", density=True
        )
        axes[0, 0].axvline(
            np.mean(baseline.final_values),
            color="red",
            linestyle="--",
            linewidth=2,
            label=f"Mean: ${np.mean(baseline.final_values):.0f}",
        )
        axes[0, 0].axvline(
            np.percentile(baseline.final_values, 5),
            color="orange",
            linestyle=":",
            linewidth=2,
            label=f"5th percentile: ${np.percentile(baseline.final_values, 5):.0f}",
        )
        ...
        axes[0, 1].axhline(
            y=0.05, color="r", linestyle="--", alpha=0.7, label="5% probability"
        )
        axes[0, 1].axvline(
            x=np.percentile(baseline.final_values, 5),
            color="r",
            linestyle="--",
            alpha=0.7,
        )
        ...
        if result.scenario_analysis:
            ...
            expected_values = [
                result.scenario_analysis[s]["expected_value"] for s in scenarios
            ]
            ...
            # 確率比較
            loss_probs = [
                result.scenario_analysis[s]["probability_of_loss"] * 100
                for s in scenarios
            ]
    ...
    def export_results(self, result: MonteCarloAnalysisResult, output_path: str):
...
def main():
    ...
    parser.add_argument(
        "--returns-path", required=True, help="Path to returns data (CSV)"
    )
    parser.add_argument(
        "--initial-value", type=float, default=10000, help="Initial portfolio value"
    )
    parser.add_argument(
        "--n-simulations", type=int, default=1000, help="Number of simulations"
    )
    parser.add_argument(
        "--time-horizon",
        type=int,
        default=TRADING_DAYS_PER_YEAR,
        help="Time horizon (days)",
    )
    parser.add_argument(
        "--distribution",
        choices=["normal", "t", "lognormal"],
        default="normal",
        help="Return distribution",
    )
    parser.add_argument("--output-dir", default="results", help="Output directory")
    parser.add_argument(
        "--no-stress-tests", action="store_true", help="Skip stress tests"
    )
    ...
    if "returns" in returns_df.columns:
        returns = returns_df.set_index(
            "date" if "date" in returns_df.columns else returns_df.columns[0]
        )["returns"]
    else:
    ...
    simulator = MonteCarloSimulator(
        n_simulations=args.n_simulations, time_horizon=args.time_horizon
    )
