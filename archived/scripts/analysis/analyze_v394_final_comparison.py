...
def analyze_v394_comparison() -> None:
    ...
    v394d_data = {"HOLD": 228, "BUY": 12, "SELL": 16, "entropy": 0.610}

    v394f_data = {"HOLD": 228, "BUY": 15, "SELL": 13, "entropy": 0.597}
    ...
    for action in ["HOLD", "BUY", "SELL"]:
        ...
        print(
            f"{action:<30} | {d_count:>3} ({d_pct:>5.1f}%){'':<8} | {f_count:>3} ({f_pct:>5.1f}%){'':<8} | {winner}"
        )
