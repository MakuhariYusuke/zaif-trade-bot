...
class CostSensitivityAnalyzer:
    ...
    def apply_trading_costs(
        self, returns: pd.Series, fee_per_trade: float, trades_per_day: float = 1.0
    ) -> pd.Series:
        """
        リターン系列に取引コストを適用
    ...
    def calculate_cost_sensitivity_metrics(
        self, returns: pd.Series, fee_levels: List[float], trades_per_day: float = 1.0
        ...
        for fee in fee_levels:
            # コスト適用後のリターン
            cost_adjusted_returns = self.apply_trading_costs(
                returns, fee, trades_per_day
            )
    ...
    def _calculate_performance_metrics(self, returns: pd.Series) -> Dict[str, float]:
        ...
        volatility = float(np.std(returns_array) * np.sqrt(TRADING_DAYS_PER_YEAR))  # type: ignore

        # Sharpe比率
        risk_free_rate = 0.02  # 仮定の無リスク金利
        sharpe_ratio = (
            (returns.mean() - risk_free_rate / TRADING_DAYS_PER_YEAR) / volatility
            if volatility > 0
            else 0
        )
    ...
    def calculate_sensitivity_scores(
        self, metrics_by_fee: Dict[float, Dict[str, float]]
    ) -> Dict[str, float]:
        """
        ...
        for metric_name in ["sharpe_ratio", "total_return", "profit_factor"]:
            ...
            for fee, metrics in metrics_by_fee.items():
                if metric_name == "sharpe_ratio" and metrics[metric_name] > 0:
                    ...
                    and metrics[metric_name] > benchmark_return
                ):
                    valid_fees.append(fee)
    ...
    def run_cost_sensitivity_analysis(
        self, returns: pd.Series, trades_per_day: float = 1.0
    ...
    def analyze(self, metrics: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_result: Dict[str, Any] = {
                "cost_sensitivity_score": result.sensitivity_scores.get(
                    "overall_sensitivity", 0.0
                ),
                "breakeven_fee": result.breakeven_analysis.get("breakeven_fee", 0.0),
                "profit_erosion_rate": result.breakeven_analysis.get(
                    "profit_erosion_rate", 0.0
                ),
    ...
    def plot_cost_sensitivity_analysis(
        self, result: CostSensitivityResult, save_path: Optional[str] = None
    ):
        """手数料感度分析結果を可視化"""
        ...
        # Sharpe比率の感度
        sharpe_ratios = [
            result.metrics_by_fee[fee]["sharpe_ratio"] for fee in fee_levels
        ]
        axes[0, 0].plot(
            fee_percentages, sharpe_ratios, "b-o", linewidth=2, markersize=4
        )
        axes[0, 0].axvline(
            x=self.base_fee * 100,
            color="r",
        ...
        # 総リターンの感度
        total_returns = [
            result.metrics_by_fee[fee]["total_return"] * 100 for fee in fee_levels
        ]
        axes[0, 1].plot(
            fee_percentages, total_returns, "g-o", linewidth=2, markersize=4
        )
        axes[0, 1].axvline(
            x=self.base_fee * 100,
            color="r",
        ...
        axes[0, 2].plot(
            fee_percentages, win_rates, "purple", marker="o", linewidth=2, markersize=4
        )
        axes[0, 2].axvline(
            x=self.base_fee * 100,
            color="r",
        ...
        # プロフィットファクターの感度
        profit_factors = [
            result.metrics_by_fee[fee]["profit_factor"] for fee in fee_levels
        ]
        axes[1, 0].plot(
            fee_percentages,
            profit_factors,
        ...
        axes[1, 0].axhline(
            y=1.0, color="k", linestyle="-", alpha=0.3, label="Break-even"
        )
        ...
        # 最大ドローダウンの感度
        max_drawdowns = [
            result.metrics_by_fee[fee]["max_drawdown"] * 100 for fee in fee_levels
        ]
        axes[1, 1].plot(
            fee_percentages, max_drawdowns, "r-o", linewidth=2, markersize=4
        )
        axes[1, 1].axvline(
            x=self.base_fee * 100,
            color="r",
        ...
        bars = axes[1, 2].bar(
            metric_names, sensitivity_values, color="skyblue", alpha=0.7
        )
    ...
    def print_cost_sensitivity_summary(self, result: CostSensitivityResult):
        ...
        positive_sharpe_fees = [
            fee
            for fee, metrics in result.metrics_by_fee.items()
        ...
        if positive_sharpe_fees:
            ...
            print(
                f"Positive Sharpe ratio range: {min_fee_val*100:.2f}% to {max_fee_val*100:.2f}%"
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="results/cost_sensitivity",
        help="Output directory for results",
    )
    parser.add_argument(
        "--trades-per-day", type=float, default=1.0, help="Average trades per day"
    )
    parser.add_argument(
        "--base-fee",
        type=float,
    ...
    parser.add_argument(
        "--fee-max", type=float, default=0.01, help="Maximum fee to analyze (0.01 = 1%)"
    )
    parser.add_argument(
        "--fee-steps", type=int, default=20, help="Number of fee levels to analyze"
    )
