...
def analyze_action_distribution(model, env, max_steps=1000):
    ...
    print(
        f"  BUY (> {threshold}): {len(buy_actions)} actions ({100*len(buy_actions)/len(actions):.2f}%)"
    )
    print(
        f"  HOLD ({-threshold} to {threshold}): {len(hold_actions)} actions ({100*len(hold_actions)/len(actions):.2f}%)"
    )
    print(
        f"  SELL (< {-threshold}): {len(sell_actions)} actions ({100*len(sell_actions)/len(actions):.2f}%)"
    )
...
def trace_buy_sell_logic(env, max_steps=100):
    ...
    threshold = (
        getattr(config, "continuous_to_discrete_threshold", 0.10)
        if hasattr(config, "continuous_to_discrete_threshold")
        else 0.10
    )
    max_position = (
    ...
    initial_balance = (
        getattr(config, "initial_portfolio_value", 200000)
        if hasattr(config, "initial_portfolio_value")
        else 200000
    )
    ...
    for step in range(max_steps):
        action, _ = (
            env.model.predict(obs, deterministic=True)
        ...
        if action_value <= threshold:
            can_buy = False
            buy_reasons.append(
                f"action_value {action_value:.4f} <= threshold {threshold}"
            )
        ...
        if current_position >= max_position:
            can_buy = False
            buy_reasons.append(
                f"current_position {current_position:.4f} >= max_position {max_position}"
            )
        ...
        if current_position + affordable_size > max_position:
            can_buy = False
            buy_reasons.append(
                f"would exceed max_position: {current_position:.4f} + {affordable_size:.4f} > {max_position}"
            )
    ...
    print("🎯 BUY/SELL Logic Analysis:")
    print(f"  Steps Analyzed: {len(trace_data)}")
    print(
        f"  BUY Conditions Met: {buy_attempts} times ({100*buy_attempts/len(trace_data):.1f}%)"
    )
    print(
        f"  SELL Conditions Met: {sell_attempts} times ({100*sell_attempts/len(trace_data):.1f}%)"
    )
    ...
    for reason, count in sorted(
        buy_failure_reasons.items(), key=lambda x: x[1], reverse=True
    ):
...
def analyze_reward_components(env, max_steps=100):
    ...
    print("\n📊 Component Breakdown:")
    print(
        f"  PnL Component: mean={df_rewards['pnl_component'].mean():.4f}, std={df_rewards['pnl_component'].std():.4f}"
    )
    print(
        f"  Trade Bonus: mean={df_rewards['trade_bonus'].mean():.4f}, count={df_rewards['trade_bonus'].gt(0).sum()}"
    )
    print(
        f"  Inactivity Penalty: mean={df_rewards['inactivity_penalty'].mean():.4f}, count={df_rewards['inactivity_penalty'].lt(0).sum()}"
    )
...
def main():
    ...
    print("1. 🎯 THRESHOLD ISSUE:")
    print(
        f"   - BUY actions: {buy_actions}/{len(actions)} ({100*buy_actions/len(actions):.2f}%)"
    )
    print(f"   - Model never outputs action > {threshold}")
    print(f"   - Max action value: {np.max(actions):.4f} < threshold {threshold}")

    print("\n2. 🧠 LEARNING ISSUE:")
    print("   - Model converged to HOLD strategy")
    print("   - No exploration of BUY action space")
    print("   - Entropy coefficient: -4.37 (very low exploration)")

    print("\n3. 💰 REWARD STRUCTURE ISSUE:")
    print("   - BUY has no immediate reward signal")
    print("   - Only future PnL provides feedback")
    print("   - HOLD avoids inactivity penalty")

    print("\n4. ⚙️ ENVIRONMENT LOGIC ISSUE:")
    print("   - max_position_size=0.01 may be too restrictive")
    print("   - BUY execution requires multiple conditions")
