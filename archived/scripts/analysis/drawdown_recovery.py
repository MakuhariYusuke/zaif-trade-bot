...
class DrawdownRecoveryAnalyzer:
    ...
    def __init__(
        self,
        min_drawdown_threshold: float = 0.05,
        recovery_threshold: float = 0.99,  # 5%以上のドローダウン
    ):  # ピークの99%まで回復
        """
        Args:
    ...
    def identify_drawdown_periods(self, returns: pd.Series) -> List[DrawdownPeriod]:
        ...
        for i in range(len(drawdown_series)):
            ...
            if not in_drawdown and current_drawdown <= -self.min_drawdown_threshold:
                ...
                peak_date = (
                    peak_idx[-1] if len(peak_idx) > 0 else drawdown_series.index[0]
                )
                ...
                if current_cumulative >= recovery_target:
                    ...
                    current_period.recovery_time = (
                        current_date - current_period.start_date
                    ).days
    ...
    def calculate_recovery_statistics(
        ...
        recovery_times = [
            p.recovery_time for p in recovered_periods if p.recovery_time is not None
        ]
    ...
    def calculate_risk_metrics(
        self, drawdown_periods: List[DrawdownPeriod], returns: pd.Series
    ) -> Dict[str, float]:
        """
        リスク指標を計算
        ...
        for period in drawdown_periods:
            ...
            if period.recovery_date is not None and period.recovery_time is not None:
                ...
                if period.recovery_value is not None:
                    excess_return = (
                        period.recovery_value - period.peak_value
                    ) / period.peak_value
                    patterns["recovery_ratios"].append(excess_return)

        return patterns

    def run_drawdown_recovery_analysis(
    ...
    def plot_drawdown_recovery_analysis(
        ...
        axes[0, 0].plot(
            drawdown_series.index, drawdown_series.values, "r-", linewidth=1
        )
        ...
        for period in result.drawdown_periods:
            axes[0, 0].axvspan(
                ...
                color="red",
                alpha=0.2,
                label="Drawdown Period" if period == result.drawdown_periods[0] else "",
            )

        # 回復時間の分布
        if result.recovery_patterns["recovery_times"]:
            axes[0, 1].hist(
                result.recovery_patterns["recovery_times"],
                bins=20,
                color="skyblue",
                alpha=0.7,
                edgecolor="black",
            ...
            axes[0, 1].axvline(
                np.mean(result.recovery_patterns["recovery_times"]),
                color="red",
                linestyle="--",
                linewidth=2,
                label="Mean",
        ...
        if (
            result.recovery_patterns["recovery_times"]
            and result.recovery_patterns["drawdown_depths"]
        ):
            recovered_depths = []
            recovered_times = []
            ...
            for period in result.drawdown_periods:
                if (
                    period.recovery_date is not None
                    and period.recovery_time is not None
                ):
            ...
            if recovered_depths and recovered_times:
                axes[0, 2].scatter(
                    recovered_depths, recovered_times, color="blue", alpha=0.6, s=50
                )
        ...
        bars2 = axes[1, 1].bar(
            recovery_labels, recovery_values, color="lightgreen", alpha=0.7
        )
        ...
        if result.recovery_patterns["drawdown_depths"]:
            axes[1, 2].hist(
                result.recovery_patterns["drawdown_depths"],
                bins=15,
                color="orange",
                alpha=0.7,
                edgecolor="black",
            ...
            axes[1, 2].axvline(
                np.mean(result.recovery_patterns["drawdown_depths"]),
                color="red",
                linestyle="--",
                linewidth=2,
                label="Mean",
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="results/drawdown_recovery",
            ...
            "drawdown_periods": [
                {
                    ...
                    "recovery_date": p.recovery_date.isoformat()
                    if p.recovery_date
                    else None,
