...
class TradeAnalyzer:
    ...
    def __init__(self, data_path: Path, output_dir: Path = Path("trade_analysis")):
        """
    ...
    def analyze_model_actions(
        ...
        # Analyze actions
        result = self._analyze_action_patterns(
            all_actions, action_sequences, model_name
        )
    ...
    def _analyze_action_patterns(
        self, all_actions: List[str], action_sequences: List[List[str]], model_name: str
    ) -> TradeAnalysisResult:
    ...
    def compare_models(
        self, model_configs: List[Dict[str, Any]], n_episodes: int = 5
        ...
        for config in model_configs:
            ...
            result = self.analyze_model_actions(model_path, model_name, n_episodes)
            results.append(result)

        return results

    def generate_analysis_report(
        self, results: List[TradeAnalysisResult], output_prefix: str = "trade_analysis"
    ) -> Path:
        """
    ...
    def _generate_action_distribution_plot(
        self, results: List[TradeAnalysisResult], prefix: str
    ) -> None:
        ...
        for i, (h, b, s) in enumerate(zip(hold_pct, buy_pct, sell_pct)):
            ax.text(
                i - width, h + 0.01, f"{h:.1%}", ha="center", va="bottom", fontsize=8
            )
            ax.text(i, b + 0.01, f"{b:.1%}", ha="center", va="bottom", fontsize=8)
            ax.text(
                i + width, s + 0.01, f"{s:.1%}", ha="center", va="bottom", fontsize=8
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="trade_analysis", help="Output directory"
    )
    parser.add_argument(
        "--episodes",
        type=int,
