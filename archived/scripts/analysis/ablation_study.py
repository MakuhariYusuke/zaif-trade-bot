...
class AblationStudy:
    ...
    def __init__(
        ...
        LOGGER.info(
            f"Initialized ablation study with {len(self.eval_data)} data points"
        )
    ...
    def run_ablation_study(
        self, n_episodes: int = 5, configs: List[AblationConfig] = None
    ) -> List[AblationResult]:
        """
        Run ablation study.
    ...
    def _evaluate_config(
        self, config: AblationConfig, n_episodes: int
    ) -> AblationResult:
        """
    ...
    def _calculate_metrics(self, returns: List[float]) -> Dict[str, float]:
        ...
        # Sharpe ratio
        risk_free_rate = 0.02 / TRADING_DAYS_PER_YEAR  # Daily risk-free rate
    ...
    def generate_ablation_report(
        self, results: List[AblationResult], output_prefix: str = "ablation_study"
    ) -> Path:
        """
    ...
    def _generate_ablation_plot(
        self, results: List[AblationResult], prefix: str
    ) -> None:
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="ablation_results", help="Output directory"
    )
    parser.add_argument(
        "--episodes",
        type=int,
