...
def analyze_training_logs():
    ...
    if not metrics:
        ...
        for tag in scalars:
            ...
            if len(events) > 0:
                print(
                    f"    Sample values: {events[0].value:.6f} -> {events[-1].value:.6f}"
                )
    ...
    if metrics:
        ...
        print("Training logs analysis saved to: training_logs_analysis.png")
    ...
    for tag in metrics.keys():
        if "entropy" in tag.lower():
            entropy_found = True
            final_entropy = (
                np.mean(metrics[tag]["values"][-10:])
                if len(metrics[tag]["values"]) >= 10
                else np.mean(metrics[tag]["values"])
            )
            print(".4f")
            if abs(final_entropy) < 0.1:
                ...
                print(
                    "⚠️  WARNING: Entropy is relatively low - may indicate limited exploration"
                )
            else:
            ...
            if abs(final_entropy) < 0.1:
                print(
                    "Current ent_coef is too low. Try increasing to 0.1-0.2 for more exploration."
                )
            elif abs(final_entropy) > 1.5:
                print(
                    "Current ent_coef might be too high. Try decreasing to 0.01-0.05 for more exploitation."
                )
            else:
