...
class WalkforwardAnalyzer:
    ...
    def __init__(
        self,
        initial_train_days: int = TRADING_DAYS_PER_YEAR,  # 約1年
    ...
    def create_walkforward_windows(self, data: pd.DataFrame) -> List[WalkforwardWindow]:
        ...
        while (
            current_train_end + timedelta(days=self.validation_days + self.test_days)
            <= end_date
        ):
            # 訓練期間
            train_start = (
                start_date
                if window_id == 0
                else windows[-1].test_end + timedelta(days=1)
            )
    ...
    def evaluate_window_performance(
        self, returns: pd.Series, window: WalkforwardWindow
    ) -> Dict[str, float]:
        """
        指定ウィンドウの性能を評価
        ...
        # ボラティリティ（年率化）
        volatility = float(np.std(test_returns_array) * np.sqrt(TRADING_DAYS_PER_YEAR))

        # Sharpe比率
        risk_free_rate = 0.02  # 仮定の無リスク金利
        sharpe_ratio = (
            (test_returns.mean() - risk_free_rate / TRADING_DAYS_PER_YEAR) / volatility
            if volatility > 0
            else 0
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--input", "-i", type=str, required=True, help="入力データファイル（CSV）"
    )
    parser.add_argument(
        "--output", "-o", type=str, default="output", help="出力ディレクトリ"
    )
    parser.add_argument(
        "--initial-train-days",
        type=int,
    ...
    parser.add_argument(
        "--validation-days", type=int, default=21, help="検証期間の日数"
    )
    parser.add_argument("--test-days", type=int, default=21, help="テスト期間の日数")
    parser.add_argument(
        "--step-days", type=int, default=21, help="ウィンドウシフトの日数"
    )
    ...
    try:
        ...
        if result.rolling_sharpe:
            ...
            plt.subplot(2, 1, 2)
            plt.plot(
                result.rolling_volatility, marker="s", linestyle="-", color="orange"
            )
