...
def evaluate(
    models: List[PPO], df: pd.DataFrame, args: argparse.Namespace, model_name: str
) -> EvaluationResult:
    env_config = {
        "reward_scaling": 1.0,
    ...
    total_return = (
        float(np.prod([1 + r for r in episode_returns]) - 1) if episode_returns else 0.0
    )
    ...
    if len(episode_returns) > 1 and np.std(episode_returns) > 0:
        sharpe = (np.mean(episode_returns) / np.std(episode_returns)) * np.sqrt(
            TRADING_DAYS_PER_YEAR
        )

    win_rate = (
        sum(1 for r in episode_returns if r > 0) / len(episode_returns)
        if episode_returns
        else 0.0
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare ensemble PPO against single model"
    )
    parser.add_argument(
        "--single-model",
        type=Path,
...
def main() -> int:
    ...
    ensemble_result = evaluate(
        ensemble_models, data_frame, args, model_name="ensemble_mean_prob"
    )
