...
class ComprehensiveBenchmark:
        ...
        if evaluation_modules_available:
            try:
                ...
                if "cost_sensitivity" in evaluation_analyzers:
                    ...
                    evaluation_analyzers["cost_sensitivity"] = CostSensitivityAnalyzer(
                        base_fee=transaction_cost
                    )
    ...
    def _create_env(self, model_path: Optional[Path] = None) -> HeavyTradingEnv:
        ...
        if model_path:
            try:
                ...
                LOGGER.info(
                    f"Using max_features={max_features} to match model observation space"
                )
            except Exception as e:
                LOGGER.warning(
                    f"Could not determine model observation space, using default max_features=26: {e}"
                )
    ...
    def evaluate_single_model(
        self, model_path: Path, model_name: str = "single_model"
    ) -> BenchmarkResult:
        """
        ...
        for episode in tqdm(
            range(self.n_episodes), desc=f"Evaluating {model_name}", unit="episode"
        ):
            env = self._create_env(model_path)
            obs, info = env.reset()  # Unpack obs and info
            initial_portfolio_value = info.get(
            ...
            while not done:
                ...
                final_portfolio_value = info.get(
    ...
    def evaluate_ensemble_model(
        ...
        """
        LOGGER.info(
            f"Evaluating ensemble: {ensemble_name} with {len(model_paths)} models"
        )
        ...
        for episode in tqdm(
            range(self.n_episodes), desc=f"Evaluating {ensemble_name}", unit="episode"
        ):
    ...
    def _calculate_metrics(
        self, rewards: List[float], returns: List[float], actions: List[str]
    ) -> BenchmarkMetrics:
        """
    ...
    def _run_extended_evaluation(
        self, metrics: BenchmarkMetrics, model_name: str
    ) -> Dict[str, Any]:
        ...
        for analyzer_name, analyzer in self.evaluation_analyzers.items():
            try:
                if hasattr(analyzer, "analyze"):
                    ...
                    LOGGER.warning(
                        f"Analyzer {analyzer_name} does not have analyze method"
                    )
            except Exception as e:
    ...
    def cross_validate(
        self, model_paths: List[Path], n_splits: int = 5, model_type: str = "single"
    ) -> BenchmarkResult:
        """
        ...
        for fold, (train_idx, val_idx) in tqdm(
            enumerate(kf.split(self.eval_data)),
            ...
            try:
                if model_type == "single":
                    result = self.evaluate_single_model(
                        model_paths[0], f"cv_fold_{fold}"
                    )
                else:  # ensemble
                    result = self.evaluate_ensemble_model(
                        model_paths, f"cv_fold_{fold}"
                    )
        ...
        metrics = BenchmarkMetrics(cv_scores=cv_scores, cv_mean=cv_mean, cv_std=cv_std)
    ...
    def ablation_study(
        ...
        for ablation_name, config_diff in ablation_configs.items():
            ...
            results[ablation_name] = self.evaluate_single_model(
                model_path, f"ablation_{ablation_name}"
            )
            ...
            detailed_results.append(
                {
                    ...
                    "metrics": {
                        ...
                        "comprehensive_score": getattr(
                            result.metrics, "comprehensive_score", None
                        ),
                        "risk_adjusted_score": getattr(
                            result.metrics, "risk_adjusted_score", None
                        ),
                        "robustness_score": getattr(
                            result.metrics, "robustness_score", None
                        ),
                    },
    ...
    def _generate_comparison_plots(
        self, results: List[BenchmarkResult], prefix: str
    ) -> None:
        ...
        for result in results:
            if result.metrics.cumulative_returns:
                ax.plot(
                    result.metrics.cumulative_returns,
        ...
        for result in results:
            if result.metrics.drawdowns:
                ax.plot(result.metrics.drawdowns, label=result.model_name, linewidth=2)
    ...
    def _run_extended_evaluation(
        ...
        for analyzer_name, analyzer in tqdm(
            self.evaluation_analyzers.items(),
    ...
    def _calculate_robustness_score(self, metrics: BenchmarkMetrics) -> float:
        ...
        base_robustness = (
            1.0 - abs(metrics.max_drawdown)
        ) * 0.5 + metrics.consistency_score * 0.5  # Lower drawdown = higher robustness
        ...
        return (
            float(np.mean(robustness_scores)) if robustness_scores else base_robustness
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--single-model", type=str, help="Path to single model for evaluation"
    )
    parser.add_argument("--ensemble-models", nargs="+", help="Paths to ensemble models")
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    if args.single_model:
        result = benchmark.evaluate_single_model(
            Path(args.single_model), "single_model"
        )

        # Cross-validation for single model
        cv_result = benchmark.cross_validate(
            [Path(args.single_model)], n_splits=args.cv_folds, model_type="single"
        )
    ...
    if args.ensemble_models:
        ...
        result = benchmark.evaluate_ensemble_model(ensemble_paths, "ensemble")
        results.append(result)

        # Cross-validation for ensemble
        cv_result = benchmark.cross_validate(
            ensemble_paths, n_splits=args.cv_folds, model_type="ensemble"
        )
