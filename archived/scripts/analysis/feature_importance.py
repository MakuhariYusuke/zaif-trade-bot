...
class FeatureImportanceAnalyzer:
    ...
    def aggregate_shap_importance(
        self, shap_values: NDArray[np.floating]
    ...
    def plot_feature_importance(
        self, result: FeatureImportanceResult, save_path: Optional[str] = None
    ):
        """特徴量重要度を可視化"""
        ...
        if result.mean_shap_importance is not None:
            ...
            std_sorted = (
                result.std_shap_importance[sorted_idx]
                if result.std_shap_importance is not None
                else None
            )
            ...
            if result.shap_values is not None and SHAP_AVAILABLE:
                ...
                axes[0, 1].barh(
                    range(len(result.feature_names)),
                    shap_importance,
                    color="lightgreen",
                    alpha=0.7,
                )
        ...
        if result.mean_permutation_importance is not None:
            ...
            std_sorted = (
                result.std_permutation_importance[sorted_idx]
                if result.std_permutation_importance is not None
                else None
            )
            ...
            if result.mean_shap_importance is not None:
                # 正規化して比較
                shap_norm = (
                    result.mean_shap_importance - np.min(result.mean_shap_importance)
                ) / (
                    np.max(result.mean_shap_importance)
                    - np.min(result.mean_shap_importance)
                )
                perm_norm = (
                    result.mean_permutation_importance
                    - np.min(result.mean_permutation_importance)
                ) / (
                    np.max(result.mean_permutation_importance)
                    - np.min(result.mean_permutation_importance)
                )
                ...
                axes[1, 1].plot(
                    [min_val, max_val], [min_val, max_val], "r--", alpha=0.5
                )
        else:
            axes[1, 0].text(
                0.5,
                0.5,
                "Permutation Importance not calculated",
                ha="center",
                va="center",
    ...
    def export_results(self, result: FeatureImportanceResult, output_path: str):
        ...
        if result.shap_values is not None:
            ...
            export_data["mean_shap_importance"] = (
                result.mean_shap_importance.tolist()
                if result.mean_shap_importance is not None
                else None
            )
            export_data["std_shap_importance"] = (
                result.std_shap_importance.tolist()
                if result.std_shap_importance is not None
                else None
            )
        ...
        if result.permutation_importance is not None:
            export_data["permutation_importance"] = {
                "importances_mean": result.mean_permutation_importance.tolist()
                if result.mean_permutation_importance is not None
                else None,
                "importances_std": result.std_permutation_importance.tolist()
                if result.std_permutation_importance is not None
                else None,
            }
...
def main():
    ...
    parser.add_argument(
        "--model-path", required=True, help="Path to trained model (pickle file)"
    )
    parser.add_argument(
        "--data-path", required=True, help="Path to feature data (CSV or pickle)"
    )
    parser.add_argument(
        "--target-column", help="Target column name (for permutation importance)"
    )
