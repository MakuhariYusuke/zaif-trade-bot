...
class BootstrapConfidenceAnalyzer:
    ...
    def calculate_bootstrap_confidence(
        ...
        for _ in range(self.n_bootstrap):
            ...
            bootstrap_data = resample(
                data, n_samples=n_samples, random_state=self.random_state
            )
    ...
    def _calculate_bca_interval(
        ...
        alpha2 = stats.norm.cdf(
            z0 + (z0 + z_1_minus_alpha) / (1 - a * (z0 + z_1_minus_alpha))
        )
    ...
    def analyze_trading_metrics(
        ...
        def sharpe_ratio(data):
            if len(data) == 0 or np.std(data) == 0:
                return 0.0
            return (
                np.mean(data) / np.std(data) * np.sqrt(TRADING_DAYS_PER_YEAR)
            )  # 年率化
        ...
        def annual_return(data):
            ...
            return total_return ** (TRADING_DAYS_PER_YEAR / len(data)) - 1
        ...
        def volatility(data):
            if len(data) == 0:
                return 0.0
            return np.std(data) * np.sqrt(TRADING_DAYS_PER_YEAR)
    ...
    def plot_bootstrap_analysis(
        self, result: BootstrapAnalysisResult, save_path: Optional[str] = None
    ):
        """ブートストラップ分析結果を可視化"""
        ...
        fig.suptitle(
            f"Bootstrap Confidence Intervals ({result.confidence_level:.0%})",
            fontsize=16,
        )
        ...
        for i, (metric_name, metric_result) in enumerate(result.results.items()):
            ...
            # ヒストグラム
            ax.hist(
                metric_result.bootstrap_samples,
                bins=50,
                alpha=0.7,
                color="skyblue",
                edgecolor="black",
            ...
            ax.axvline(
                ...
                linestyle="--",
                linewidth=2,
                label=f"Original: {metric_result.original_value:.4f}",
            )

            # 信頼区間
            ax.axvline(
                metric_result.confidence_interval[0],
                color="green",
                linestyle=":",
                linewidth=2,
                label=f"Lower: {metric_result.confidence_interval[0]:.4f}",
            )
            ax.axvline(
                metric_result.confidence_interval[1],
                color="green",
                linestyle=":",
                linewidth=2,
                label=f"Upper: {metric_result.confidence_interval[1]:.4f}",
            )
    ...
    def export_results(self, result: BootstrapAnalysisResult, output_path: str):
...
def main():
    ...
    parser.add_argument(
        "--confidence-level", type=float, default=0.95, help="Confidence level (0-1)"
    )
