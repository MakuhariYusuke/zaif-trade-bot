...
def generate_synthetic_price_patterns(
    base_price: float, n_rows: int, pattern_type: str
) -> np.ndarray:
    ...
    if pattern_type == "uptrend":
        ...
        chunk_size = n_rows // 4
        prices[0:chunk_size] = generate_synthetic_price_patterns(
            base_price, chunk_size, "uptrend"
        )
        prices[chunk_size : 2 * chunk_size] = generate_synthetic_price_patterns(
            base_price, chunk_size, "volatile"
        )
        prices[2 * chunk_size : 3 * chunk_size] = generate_synthetic_price_patterns(
            base_price, chunk_size, "downtrend"
        )
        prices[3 * chunk_size :] = generate_synthetic_price_patterns(
            base_price, n_rows - 3 * chunk_size, "stable"
        )
...
def augment_dataset(df: pd.DataFrame, target_rows: int = 2000) -> pd.DataFrame:
    ...
    for pattern in patterns:
        ...
        if "close" in df.columns:
            ...
            prices = generate_synthetic_price_patterns(
                base_price, n_per_pattern, pattern
            )
            ...
            new_segment["open"] = (
                new_segment["close"].shift(1).fillna(new_segment["close"].iloc[0])
            )
            new_segment["high"] = new_segment[["open", "close"]].max(axis=1) * (
                1 + np.random.uniform(0, 0.01, n_per_pattern)
            )
            new_segment["low"] = new_segment[["open", "close"]].min(axis=1) * (
                1 - np.random.uniform(0, 0.01, n_per_pattern)
            )
            ...
            if "timestamp" in df.columns:
                ...
                new_segment["timestamp"] = pd.date_range(
                    start=last_timestamp, periods=n_per_pattern + 1, freq="1h"
                )[1:]
    ...
    if remaining > 0:
        ...
        final_segment["open"] = (
            final_segment["close"].shift(1).fillna(final_segment["close"].iloc[0])
        )
        final_segment["high"] = final_segment[["open", "close"]].max(axis=1) * (
            1 + np.random.uniform(0, 0.01, remaining)
        )
        final_segment["low"] = final_segment[["open", "close"]].min(axis=1) * (
            1 - np.random.uniform(0, 0.01, remaining)
        )
