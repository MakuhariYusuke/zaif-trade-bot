...
def main():
    ...
    report = {
        ...
        "summary": {
            ...
            "unique_keys_files": len(
                [f for f, k in consistency["unique_keys"].items() if k]
            ),
    ...
    print(
        f"📈 要改善項目: {len([f for f, k in consistency['unique_keys'].items() if k]) + len(consistency['type_inconsistencies'])}件"
    )
