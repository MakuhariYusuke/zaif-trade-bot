...
def analyze_action_distribution(model, df, n_samples=10000):
    ...
    temp_env = HeavyTradingEnv(
        df=df.head(1000), config=temp_config
    )  # Small dataset for temp env
...
def plot_action_distribution(actions, results):
    ...
    ax1.axvline(
        results["thresholds"]["buy_threshold"],
        color="green",
        linestyle="--",
        label=f'BUY threshold ({results["thresholds"]["buy_threshold"]})',
    )
    ax1.axvline(
        results["thresholds"]["sell_threshold"],
        color="red",
        linestyle="--",
    ...
    counts = [
        results["discrete_distribution"].get(0, 0),
    ...
    plt.tight_layout()
    plt.savefig(
        "results/sac_action_distribution_detailed.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def investigate_bias_causes(results, actions):
    ...
    print(
        f"  HOLD range:     [{thresholds['sell_threshold']}, {thresholds['buy_threshold']}]"
    )

    print("\nAction Distribution:")
    print(
        f"  BUY:  {bias_analysis['buy_ratio']*100:.1f}% ({results['discrete_distribution'][1]})"
    )
    print(
        f"  SELL: {bias_analysis['sell_ratio']*100:.1f}% ({results['discrete_distribution'][2]})"
    )
    print(
        f"  HOLD: {bias_analysis['hold_ratio']*100:.1f}% ({results['discrete_distribution'][0]})"
    )
    ...
    if bias_analysis["is_sell_biased"]:
        ...
        print("\nRange Analysis:")
        print(
            f"  SELL range width: {sell_range_width:.4f} (from {action_stats['min']:.4f} to {thresholds['sell_threshold']})"
        )
        print(
            f"  HOLD range width: {hold_range_width:.4f} (from {thresholds['sell_threshold']} to {thresholds['buy_threshold']})"
        )
        print(
            f"  BUY range width:  {buy_range_width:.4f} (from {thresholds['buy_threshold']} to {action_stats['max']:.4f})"
        )

        # Check if thresholds are the problem
        if sell_range_width > buy_range_width * 1.5:
            print(
                "🔍 CAUSE IDENTIFIED: SELL range is significantly wider than BUY range"
            )
            print(
                "   This makes SELL actions much more likely due to threshold asymmetry"
            )
        ...
        if action_stats["mean"] < -0.1:
            ...
            print(
                f"   Mean action value ({action_stats['mean']:.4f}) favors SELL range"
            )
    ...
    if bias_analysis["is_sell_biased"]:
        ...
        print("  1. Adjust thresholds for balance:")
        print(
            f"     BUY threshold: {thresholds['buy_threshold']} → {optimal_buy_threshold:.4f}"
        )
        print(
            f"     SELL threshold: {thresholds['sell_threshold']} → {optimal_sell_threshold:.4f}"
        )

        print(
            f"  2. Alternative: Make thresholds symmetric around median ({action_stats['median']:.4f})"
        )
        symmetric_range = min(
            abs(action_stats["median"] - action_stats["q25"]),
