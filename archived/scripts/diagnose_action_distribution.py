...
def collect_action_distribution() -> Optional[Dict[str, float]]:
    ...
    try:
        ...
        return {"hold": hold, "buy": buy, "sell": sell}
...
def diagnose_distribution(dist: Dict[str, float]) -> Dict[str, any]:
    ...
    if sell > (ideal + tolerance_critical):
        ...
        root_causes.extend(["Lagrange制約の調整不足", "エントロピー係数が低い"])
        recommendations.extend(
            [
                "1. Lagrange制約を微調整:",
    ...
    if hold > (ideal + tolerance_critical):
        ...
        severity.append("WARNING")
        root_causes.extend(["保守的な報酬設定", "エントロピー係数の不足"])
        recommendations.extend(
            [
                "1. エントロピー係数を増加:",
                "   - ent_coef: 0.1 → 0.15",
    ...
    if buy > (ideal + tolerance_critical):
        ...
        severity.append("WARNING")
        root_causes.extend(["報酬関数の微調整不足"])
        recommendations.extend(
            ["1. profit_bonus_multipliersを調整:", "   - [1.0, 0.9, 1.0] (BUY=0.9)"]
        )
    ...
    if imbalance_ratio > 10.0:
        ...
        severity.append("CRITICAL")
        root_causes.extend(["根本的な報酬設計の問題", "環境設定の深刻なミス"])
        recommendations.extend(
            [
                "1. カリキュラム学習を強制:",
                "   - curriculum_stage: 'forced_balance'",
    ...
    print(
        f"  エントロピー: {entropy:.4f} / {max_entropy:.4f} ({entropy_ratio*100:.1f}%)"
    )

    if entropy_ratio < 0.7:
        issues.append(
            f"🟡 低いアクションエントロピー: {entropy:.3f} (最大: {max_entropy:.3f})"
        )
...
def print_diagnosis(diagnosis: Dict[str, any]):
    ...
    print("⚠️ 検出された問題:")
    print("=" * 70)
    for i, (issue, sev) in enumerate(
        zip(diagnosis["issues"], diagnosis["severity"]), 1
    ):
        print(f"{i}. {issue} [{sev}]")
...
def generate_config_fix(diagnosis: Dict[str, any]) -> Dict[str, any]:
    ...
    if dist["hold"] > 0.50:
        ...
        fix.update({"ent_coef": 0.15, "action_penalty_scale": 0.008})
