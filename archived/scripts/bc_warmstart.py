        ...
        for features, actions in train_loader:
            ...
            with torch.no_grad():
                ...
                if model.policy.share_features_extractor:
                    ...
                    latent_pi = model.policy.mlp_extractor.forward_actor(
                        features_extracted
                    )
...
def verify_sell_logits(
    ...
    print("\nLogit verification:")
    print(
        f"  Mean logits: HOLD={mean_logits[0]:.3f}, BUY={mean_logits[1]:.3f}, SELL={mean_logits[2]:.3f}"
    )
    ...
    if is_competitive:
        print(
            f"  ✅ SELL logit is competitive (gap: {max_logit - sell_logit:.3f} ≤ 0.1)"
        )
    else:
...
def main():
    ...
    test_indices = np.random.choice(
        len(features), size=min(1000, len(features)), replace=False
    )
