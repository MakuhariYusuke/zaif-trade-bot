...
def analyze_reward_components(
    env: HeavyTradingEnv, action: int, pnl: float, position: float, old_position: float
) -> Dict[str, Any]:
...
def run_comprehensive_analysis():
    ...
    test_scenarios = [
        ...
        },
        {
            ...
            "position": 0.0,
            "old_position": 0.01,
            "description": "SELL with profit",
        },
        {
            "pnl": 0.0,
            "position": 0.01,
            "old_position": 0.01,
            "description": "HOLD with no change",
        },
        # Loss scenarios
        {
            "pnl": -100.0,
        ...
        # Position changes
        {
            ...
            "position": 0.02,
            "old_position": 0.01,
            "description": "Increase position with profit",
        },
        {
            "pnl": 50.0,
            "position": 0.0,
            "old_position": 0.01,
            "description": "Close position with profit",
        },
    ]
    ...
    for scenario in test_scenarios:
        ...
        if abs(reward_diff_buy_sell) > bias_threshold:
            if reward_diff_buy_sell > 0:
                print(
                    f"⚠️  BUY bias detected: BUY gets {abs(reward_diff_buy_sell):.4f} more reward"
                )
            else:
                print(
                    f"⚠️  SELL bias detected: SELL gets {abs(reward_diff_buy_sell):.4f} more reward"
                )
    ...
    for result in results:
        ...
        if abs(diff) > 0.1:
            ...
            if diff > 0:
                total_buy_bias += diff
                print(
                    f"BUY bias in scenario '{result['scenario']}': BUY gets {diff:.4f} more reward"
                )
            else:
                total_sell_bias += abs(diff)
                print(
                    f"SELL bias in scenario '{result['scenario']}': SELL gets {abs(diff):.4f} more reward"
                )
