...
def test_action_distribution(config_path: str):
    ...
    model_name = (
        "sac_v414_balanced_trading_final"  # Use the existing working v414 model
    )
    ...
    for episode in range(10):
        ...
        print(
            f"Episode {episode+1}: Actions - BUY:{episode_discrete.count(ACTION_BUY)}, SELL:{episode_discrete.count(ACTION_SELL)}, HOLD:{episode_discrete.count(ACTION_HOLD)}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test action distribution for SAC model"
    )
    parser.add_argument(
