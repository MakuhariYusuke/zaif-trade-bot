...
def compare_checkpoints(
    ...
    checkpoint_metrics_list.sort(
        key=lambda x: x.get_score(primary_metric), reverse=True
    )
...
Examples:
  ...
  python scripts/compare_checkpoints.py --checkpoint-dir checkpoints/ensemble_B_100k_test --export results.csv
    ...
    parser.add_argument(
        "--metrics", nargs="+", help="Metrics to compare (default: predefined metrics)"
    )
    parser.add_argument("--top", type=int, help="Show only top N checkpoints")
    parser.add_argument(
        "--primary-metric",
