...
from .types import SafetyAction, SafetyEvent, SafetyLevel
...
class IntegratedSafetyManager:
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            if self.config.enable_anomaly_detection:
                self.anomaly_manager = AnomalyDetectionManager(self.safety_manager)
                self.anomaly_manager.add_anomaly_callback(self._on_anomaly_detected)

            # リカバリーマネージャーの初期化
            if (
                self.config.enable_recovery_system
                and self.fallback_manager
                and self.anomaly_manager
            ):
                self.recovery_manager = RecoveryManager(
                    self.safety_manager, self.fallback_manager, self.anomaly_manager
                )
    ...
    def _monitor_system_state(self) -> None:
        ...
        try:
            ...
            if self.anomaly_manager and self.config.enable_anomaly_detection:
                ...
                # 自動フォールバック対応
                if (
                    self.config.auto_fallback_on_anomaly
                    and self.fallback_manager
                    and self._should_activate_fallback(anomalies)
                ):
        ...
        try:
            ...
            if self.system_health_score < 50.0:
                self._trigger_critical_alert(
                    "System health critically low", health_status
                )
    ...
    def _on_fallback_activated(self, fallback_mode: FallbackMode) -> None:
        ...
        try:
            ...
            # 自動リカバリー
            if (
                self.config.auto_recovery_on_fallback
                and self.recovery_manager
                and fallback_mode
                in [FallbackMode.CIRCUIT_BREAKER, FallbackMode.EMERGENCY_SHUTDOWN]
            ):
    ...
    def _should_activate_fallback(self, anomalies: List[AnomalyResult]) -> bool:
        ...
        try:
            ...
            critical_anomalies = [
                a
                for a in anomalies
                if a.severity == SafetyLevel.CRITICAL
                or a.confidence_score >= self.config.critical_anomaly_threshold
            ]
    ...
    def _activate_emergency_fallback(self, anomalies: List[AnomalyResult]) -> None:
        ...
        try:
            ...
            max_severity = max(
                (a.severity for a in anomalies), default=SafetyLevel.INFO
            )
            ...
            if max_severity == SafetyLevel.CRITICAL or max_confidence >= 0.9:
                self.fallback_manager.activate_fallback_mode(
                    FallbackMode.EMERGENCY_SHUTDOWN
                )
            elif max_confidence >= 0.8:
                self.fallback_manager.activate_fallback_mode(
                    FallbackMode.CIRCUIT_BREAKER
                )
            else:
                self.fallback_manager.activate_fallback_mode(FallbackMode.CONSERVATIVE)

            logger.warning(
                f"Emergency fallback activated due to {len(anomalies)} anomalies"
            )
    ...
    def _initiate_automatic_recovery(self, fallback_mode: FallbackMode) -> None:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Automatic recovery initiated with strategy: {strategy.value}"
                )
            else:
    ...
    def _check_fallback_manager_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "current_mode": self.fallback_manager.current_mode.value
                if self.fallback_manager.current_mode
                else "normal",
    ...
    def _check_recovery_manager_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "active"
                if not self.recovery_manager.is_recovery_active
                else "recovery_in_progress",
                "recovery_history": len(self.recovery_manager.recovery_history),
                "last_recovery": self.recovery_manager.recovery_history[
                    -1
                ].timestamp.isoformat()
    ...
    def _record_event(self, event: SafetyEventRecord) -> None:
        ...
        try:
            ...
            logger.info(
                f"Safety event recorded: {event.event_id} - {event.description}"
            )
    ...
    def _trigger_critical_alert(self, message: str, details: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._trigger_callbacks(
                "critical_alert", {"message": message, "details": details}
            )
    ...
    def get_safety_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.anomaly_manager:
                component_stats[
                    "anomaly_detection"
                ] = self.anomaly_manager.get_anomaly_statistics(hours)
                ...
                component_stats["fallback_system"] = {
                    "fallback_count": len(
                        [
                            ...
                            if e.event_type.value == "FALLBACK_ACTIVATED"
                        ]
                    ),
                    "current_mode": self.fallback_manager.current_mode.value
                    if self.fallback_manager.current_mode
            ...
            if self.recovery_manager:
                component_stats[
                    "recovery_system"
                ] = self.recovery_manager.get_recovery_statistics(hours)
    ...
    def _generate_recommendations(self, events: List[SafetyEventRecord]) -> List[str]:
        ...
        try:
            ...
            # 異常検知の推奨
            anomaly_events = [
                e for e in events if e.event_type.value == "ANOMALY_DETECTED"
            ]
            if len(anomaly_events) > 10:
                recommendations.append(
                    "Consider adjusting anomaly detection thresholds - high frequency of anomalies detected"
                )

            # フォールバックの推奨
            fallback_events = [
                e for e in events if e.event_type.value == "FALLBACK_ACTIVATED"
            ]
            if len(fallback_events) > 5:
                recommendations.append(
                    "Review system stability - frequent fallback activations indicate potential issues"
                )
            ...
            if len(failed_recoveries) > 2:
                recommendations.append(
                    "Investigate recovery procedures - multiple recovery failures detected"
                )

            # 正常性の推奨
            if self.system_health_score < 70:
                recommendations.append(
                    "System health is degraded - consider maintenance or resource scaling"
                )
