...
class ConceptDriftManager:
    ...
    def detect_drift(
        ...
        if (
            ...
            and (current_time - self.last_detection_time).seconds
            < self.config.detection_interval_seconds
        ):
            # 前回の結果を返す
            return (
                self.drift_history[-1]
                if self.drift_history
                else DriftDetectionResult(
        ...
        try:
            ...
            logger.info(
                f"Drift detection completed: {final_result.drift_detected}, "
    ...
    def _detect_parallel(
        self, data: np.ndarray, error_data: Optional[np.ndarray]
        ...
        with ThreadPoolExecutor(max_workers=min(len(self.detectors), 4)) as executor:
            future_to_detector = {
                executor.submit(
                    self._call_detector, name, detector, data, error_data
                ): name
                for name, detector in self.detectors.items()
            }

            for future in as_completed(
                future_to_detector, timeout=30.0
            ):  # 30秒タイムアウト
    ...
    def _call_detector(
        self, name: str, detector, data: np.ndarray, error_data: Optional[np.ndarray]
    ):
        """検知器を適切なパラメータで呼び出す"""
    ...
    def _aggregate_results(
        self, results: List[DriftDetectionResult]
    ) -> DriftDetectionResult:
        """複数の検知結果を集約"""
        if not results:
        ...
        # 平均p値
        p_values = [
            r.p_value for r in results if r.drift_detected and r.p_value is not None
        ]
        avg_p_value = np.mean(p_values) if p_values else 0.5

        # 平均統計量
        statistics = [
            r.statistic for r in results if r.drift_detected and r.statistic is not None
        ]
    ...
    def get_detector_stats(self) -> Dict[str, Any]:
        ...
        for name, detector in self.detectors.items():
            ...
            stats[name] = {
                ...
                "avg_score": np.mean([score for _, score in history])
                if history
                else 0.0,
            }
