...
def test_adaptive_feature_selection():
    ...
    all_features = [col for col in df_with_features.columns if col not in ['open', 'high', 'low', 'close', 'volume']]
    ...
    assert len(selected_features) <= len(all_features), "Selected more features than available"
    assert all(f in all_features for f in selected_features), "Selected features not in original list"
...
def test_multi_timeframe_integration():
    ...
    for tf_name, tf_config in TIMEFRAME_DEFINITIONS.items():
        ...
        prices = base_price + np.random.randn(n_samples).cumsum() * volatility_scale * 0.1
        prices += np.linspace(0, trend_strength * 10, n_samples)  # Add trend

        data = {
            'open': prices[:-1],
            'high': prices[:-1] + abs(np.random.randn(n_samples-1)) * volatility_scale * 0.05,
            'low': prices[:-1] - abs(np.random.randn(n_samples-1)) * volatility_scale * 0.05,
            'close': prices[1:],
    ...
    for tf_name, df in timeframes_data.items():
        try:
            ...
            logger.info(f"Calculated features for {tf_name}: {len(features_df.columns)} features")
    ...
    if len(multi_tf_features) >= 2:
        ...
        if primary_tf in multi_tf_features and secondary_tf in multi_tf_features:
            ...
            logger.info(f"Combined multi-timeframe features: {len(combined_features.columns)} total features")
...
def test_feature_importance_analysis():
    ...
    # Get all features
    all_features = [col for col in df_with_features.columns if col not in ['open', 'high', 'low', 'close', 'volume', 'reward']]
