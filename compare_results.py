...
def main():
    ...
    print(f'SIGNAL_GUIDANCE平均リターン: {sg_results["avg_total_return_pct"]:.2f}% ± {sg_results["std_total_return_pct"]:.2f}%')
    print(f'ベースライン平均リターン: {bl_results["avg_return_pct"]:.2f}% ± {bl_results["std_return_pct"]:.2f}%')
    print(f'パフォーマンス差: {sg_results["avg_total_return_pct"] - bl_results["avg_return_pct"]:.2f}%')
    print()
