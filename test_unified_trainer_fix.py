...
def test_config_curriculum_stage():
    ...
    for config_file in config_files:
        ...
        curriculum_stage = config.get("training", {}).get("curriculum_learning", {}).get("curriculum_stage", "NOT SET")
        balance_penalty = config.get("environment", {}).get("behavior_optimization", {}).get("balance_penalty", "NOT SET")
        ...
        supported_stages = ("forced_balance", "balanced_penalty", "balance_optimization", "balance_penalty")
        if curriculum_stage in supported_stages:
            logger.info(f"  ✅ curriculum_stage is supported")
        else:
            logger.warning(f"  ⚠️  curriculum_stage '{curriculum_stage}' might not be recognized")
...
def test_reward_calculator_logic():
    ...
    try:
        ...
        if "balance_penalty_enabled_stages" in source:
            logger.info(f"  ✅ balance_penalty_enabled_stages tuple found in RewardCalculator")
        elif "in balance_penalty_enabled_stages" in source:
            logger.info(f"  ✅ Support for multiple curriculum stages found")
        else:
            logger.warning(f"  ⚠️  No multi-stage support found in RewardCalculator")
...
def test_environment_config():
    ...
    try:
        ...
        if 'curriculum_stage' in fields:
            ...
            logger.info(f"  ✅ curriculum_stage is defined in EnvironmentConfig")
            logger.info(f"     Type: {field.type}")
            logger.info(f"     Default: {field.default}")
        else:
            logger.warning(f"  ❌ curriculum_stage is NOT defined in EnvironmentConfig")
...
def test_config_manager():
    ...
    try:
        ...
        if 'curriculum_stage' in source:
            logger.info(f"  ✅ curriculum_stage is handled in ConfigManager")
            
            # Check if it's being set from curriculum_learning
            if 'curriculum_learning' in source and 'curriculum_stage' in source:
                logger.info(f"  ✅ curriculum_stage is extracted from curriculum_learning")
            else:
                logger.warning(f"  ⚠️  curriculum_stage extraction logic might be incomplete")
        else:
            logger.warning(f"  ❌ curriculum_stage is NOT handled in ConfigManager")
...
def main():
    ...
    logger.info("  python quick_train_v444_configurable.py --config config/sac_v444_3_balanced_penalty_scale_200.json --verbose")
