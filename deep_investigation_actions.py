...
def test_environment_initialization():
    ...
    print(
        f"✓ use_standardized_observations: {env_config.get('use_standardized_observations')}"
    )
    ...
    for step in range(5):
        ...
        if hasattr(env, "_reward_calculator") and hasattr(
            env._reward_calculator, "_recent_actions"
        ):
            ...
            if recent:
                ...
                print(
                    f"    Last discrete action: {last_action} ({['HOLD', 'BUY', None, None, None, None, None, None, 'SELL'].get(last_action, 'UNKNOWN')})"
                )
