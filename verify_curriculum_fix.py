...
original_stage = config.get('training', {}).get('curriculum_learning', {}).get('curriculum_stage')
...
print(f"   (Previously was 'forced_balance')")

# Step 4: Simulate environment creation
test_cfg = EnvironmentConfig(
    curriculum_stage=converted_stage,
    exchange='coincheck'
)
...
if converted_stage == original_stage and converted_stage == 'balanced_penalty':
    ...
    print(f"   Config value 'balanced_penalty' correctly propagates to environment")
else:
    print(f"❌ FAILED: curriculum_stage mismatch")
