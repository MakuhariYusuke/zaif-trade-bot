...
def generate_comprehensive_test_data():
    ...
    for scenario in scenarios:
        ...
        # Create OHLCV data
        high_prices = prices * (
            1 + np.random.uniform(0, scenario["volatility"] * 2, n_points)
        )
        low_prices = prices * (
            1 - np.random.uniform(0, scenario["volatility"] * 2, n_points)
        )
        open_prices = prices + np.random.normal(
            0, scenario["volatility"] * 0.5, n_points
        )
        close_prices = prices + np.random.normal(
            0, scenario["volatility"] * 0.5, n_points
        )
    ...
    for threshold in thresholds:
        ...
        for i in range(100, len(data), 20):  # Sample every 20 points
            ...
            for signal in filtered_signals:
                pattern = (
                    signal.source_patterns[0] if signal.source_patterns else "unknown"
                )
...
def generate_optimal_configuration(analysis_results, threshold_analysis, correlations):
    ...
    optimal_config["recommendations"].append(
        f"Global strength threshold: {optimal_threshold}"
    )
    ...
    for pattern, stats in pattern_stats.items():
        ...
        if strength_stats:
            consistency = 1 / (
                strength_stats.get("std", 1) + 0.1
            )  # Lower std = higher consistency
            signal_count = stats.get("signals_generated", 0)
            weight = consistency * min(
                signal_count / 100, 1
            )  # Balance consistency and volume
...
def run_comprehensive_analysis():
    ...
    for scenario_name, data in test_datasets.items():
        ...
        for i in range(10, len(data)):
            ...
            period_signals = [
                ...
                if s.timestamp >= data.index[max(0, i - 10)]
                and s.timestamp <= data.index[i]
            ]
    ...
    aggregated_analysis = signal_guide.analyze_pattern_effectiveness(
        all_trading_results
    )

    # Analyze strength thresholds
    primary_data = test_datasets["trending_bull"]  # Use trending scenario as primary
    threshold_analysis = analyze_strength_thresholds(
        signal_guide, primary_data, all_trading_results
    )
    ...
    optimal_config = generate_optimal_configuration(
        aggregated_analysis, threshold_analysis, correlations
    )
