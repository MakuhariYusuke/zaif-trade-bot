...
def analyze_action_distribution(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    analysis = {"action_balance": {}, "trade_frequency": {}, "reward_stability": {}}
    ...
    if "backtest" in results:
        ...
        if episodes:
            ...
            analysis["trade_frequency"] = {
                ...
                "zero_trade_episodes": sum(
                    1 for ep in episodes if ep.get("trades", 0) == 0
                ),
            }
...
def generate_statistical_report(results: Dict[str, Any]) -> str:
    ...
    if "backtest" in results:
        ...
        report_lines.append(".2f")
        report_lines.append(
            f"- Total Trades: {action_analysis.get('trade_frequency', {}).get('total_trades', 'N/A')}"
        )
    else:
    ...
    report_lines.append(
        "1. **Extended Training**: Run longer training sessions (50k-100k timesteps)"
    )
    report_lines.append(
        "2. **Hyperparameter Tuning**: Optimize reward scaling and action thresholds"
    )
    report_lines.append(
        "3. **Curriculum Learning**: Implement gradual difficulty increase"
    )
    report_lines.append(
        "4. **Ensemble Methods**: Consider model ensembling for robustness"
    )
    report_lines.append("")
...
def main():
    ...
    report_file = (
        f"v440_statistical_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    )
