...
from ztb.utils.logging_utils import (
    log_analysis_start,
    log_data_loading,
    log_error,
    log_model_loading,
...
def run_detailed_backtest(model_name, config_path, output_dir):
    ...
    if not Path(data_file).exists():
        ...
        if not Path(data_file).exists():
            ...
            save_csv_data(synthetic_df, data_file)
    ...
    try:
        model = SAC.load(model_path)  # type: ignore[attr-defined]
    ...
    for i in range(start_idx, end_idx):
        ...
        if i % 1000 == 0:
            logger.info(
                f"Processed {i}/{end_idx} steps. PV: {step_data['portfolio_value']:.2f}"
            )

    # Save History
    history_df = pd.DataFrame(history)
    csv_path = output_dir / "backtest_detailed_results.csv"
    save_csv_data(history_df, str(csv_path), index=False)
...
def analyze_results(df, output_dir):
    ...
    plt.legend()
    plt.grid(True)
    save_plot(output_dir / "pnl_curve.png")
    plt.close()
    ...
    plt.grid(True)
    save_plot(output_dir / "action_distribution.png")
    plt.close()
    ...
    plt.grid(True)
    save_plot(output_dir / "drawdown.png")
    plt.close()
    ...
    sharpe = sharpe_ratio(
        returns, period_per_year=525600
    )  # 1m data: 525600 periods per year
...
- [Drawdown](drawdown.png)
    ...
    log_success(
        logger, "Analysis", f"Report saved to {output_dir / 'analysis_report.md'}"
    )
    print(report)
