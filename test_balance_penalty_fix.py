...
def test_balance_penalty_calculation():
    ...
    target_ratio = env_config.get("behavior_optimization", {}).get(
        "action_balance_target", 0.333
    )
    balance_penalty_scale = env_config.get("behavior_optimization", {}).get(
    ...
    print(
        f"{'Step':>4} {'Actions':25} {'BUY':>7} {'SELL':>7} {'HOLD':>7} | Deviations"
    ...
    for step, (action, action_name) in enumerate(test_actions, 1):
        ...
        if total > 0:
            ...
            if step % 5 == 0 or total in [10, 20]:
                print(
                    f"{step:4d} {action_name:25} {buy_ratio:7.1%} {sell_ratio:7.1%} "
