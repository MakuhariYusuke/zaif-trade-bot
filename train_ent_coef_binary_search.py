...
Tests different ent_coef values to find optimal entropy coefficient.
"""
...
def train_ent_coef_test(ent_coef: float = 0.01):
    """Train PPO model with specified ent_coef value."""
...
Action distribution:""")
        ...
        # Save model
        ent_coef_str = f"{ent_coef:.4f}".replace('.', '')
        model_path = f"models/ent_coef_test_ent_coef_{ent_coef_str}.zip"
        os.makedirs("models", exist_ok=True)
        model.save(model_path)
        print(f"Model saved to: {model_path}")

        print(f"✅ Successfully trained: ent_coef_{ent_coef_str}")
...
def main():
    parser = argparse.ArgumentParser(description='Train PPO with specific ent_coef value')
    ...
    # Train model
    train_ent_coef_test(args.ent_coef)
