...
class MemoryMonitoringBenchmark:
    ...
    def benchmark_memory_monitoring(self, total_steps: int = 10000,
                                   monitoring_interval: int = 10) -> Dict[str, Any]:
        """Benchmark memory monitoring overhead"""

        print(f"🔍 Benchmarking memory monitoring (steps: {total_steps}, interval: {monitoring_interval})")
        ...
        result = {
            ...
            'avg_step_time_with_monitoring': np.mean(step_times_total) if step_times_total else 0,
    ...
    def analyze_dynamic_threshold(self, memory_history: List[float]) -> Dict[str, Any]:
        ...
        return {
            ...
            'threshold_ratio': dynamic_threshold / fixed_threshold if fixed_threshold > 0 else 0,
            'trend_increase_percent': trend_increase,
            'recommendation': 'dynamic' if dynamic_threshold < fixed_threshold else 'fixed'
        }

    def save_results(self, output_file: str = "reports/memory_monitoring_benchmark.json"):
        ...
        if self.results:
            ...
            if 'memory_history' in latest_result and latest_result['memory_history']:
                threshold_analysis = self.analyze_dynamic_threshold(latest_result['memory_history'])
    ...
    def _find_optimal_interval(self) -> int:
        ...
        candidates = [r for r in self.results if r['gc_time_percentage'] < 1.0 and r['monitoring_interval'] <= 100]

        if candidates:
            # Prefer more frequent monitoring within acceptable overhead
            return min(candidates, key=lambda x: x['monitoring_interval'])['monitoring_interval']
