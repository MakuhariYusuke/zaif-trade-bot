...
def get_function_hash(func_node: ast.FunctionDef) -> str:
    ...
    for node in func_node.body:
        if not isinstance(node, (ast.Expr, ast.Pass)) or not isinstance(getattr(node, 'value', None), ast.Str):
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                functions[func_hash].append((file_path, node.name, len(ast.unparse(node))))
