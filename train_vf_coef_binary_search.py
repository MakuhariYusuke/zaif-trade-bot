#!/usr/bin/env python3
"""
Binary search for GAE lambda parameter
"""
...
def train_vf_coef_test(vf_coef=0.75, reward_scaling=6.0, entropy_coef=0.03):
    """Train with GAE lambda test for 100k steps"""
    ...
    vf_coef_str = f"{vf_coef:.4f}".replace('.', '')
    config_name = f"vf_coef_{vf_coef_str}"
    ...
    model = PPO(
        ...
        clip_range=0.3,  # Fixed optimal clip_range
        ent_coef=entropy_coef,
        vf_coef=vf_coef,
        max_grad_norm=0.5,
        verbose=1,
        tensorboard_log=f"runs/vf_coef_test_vf_coef_{vf_coef_str}",
    )
    ...
    print(f"Reward scaling: {reward_scaling}, Entropy coef: {entropy_coef}, VF coef: {vf_coef}")
    ...
    model_path = PROJECT_ROOT / "models" / f"vf_coef_test_{config_name}.zip"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO model with specified VF coef")
    parser.add_argument("--vf_coef", type=float, default=0.75, help="VF coefficient parameter for PPO")
    ...
    # Create config name from vf_coef
    vf_coef_str = f"{args.vf_coef:.4f}".replace('.', '')
    config_name = f"vf_coef_{vf_coef_str}"

    print(f"\n{'='*60}")
    print(f"Training with vf_coef = {args.vf_coef}")
    print(f"Config: {config_name}")
    print(f"{'='*60}")

    try:
        model_path = train_vf_coef_test(
