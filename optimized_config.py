...
from ztb.trading.strategies.action_signal_guide.action_signal_guide import (
    ActionSignalGuide, ActionSignalGuideConfig, RecognizerConfig
)
...
def create_optimized_config() -> ActionSignalGuideConfig:
    ...
    config = ActionSignalGuideConfig(
        ...
        fibonacci_patterns=[
            RecognizerConfig(name='fibonacci_retracement', enabled=True, weight=7.3, config=pattern_configs['fibonacci_retracement'], group='fibonacci'),
            RecognizerConfig(name='fibonacci_extension', enabled=True, weight=7.3, config=pattern_configs['fibonacci_extension'], group='fibonacci'),
            RecognizerConfig(name='fibonacci_projection', enabled=True, weight=7.3, config=pattern_configs['fibonacci_projection'], group='fibonacci'),
        ],

        gann_patterns=[
            RecognizerConfig(name='gann_angle', enabled=True, weight=8.0, config=pattern_configs['gann_angle'], group='gann'),
            RecognizerConfig(name='gann_square', enabled=True, weight=8.0, config=pattern_configs['gann_square'], group='gann'),
            RecognizerConfig(name='gann_time_cluster', enabled=True, weight=8.0, config=pattern_configs['gann_time_cluster'], group='gann'),
        ],

        wave_patterns=[
            RecognizerConfig(name='impulse_wave', enabled=True, weight=6.3, config=pattern_configs['impulse_wave'], group='wave'),
            RecognizerConfig(name='corrective_wave', enabled=True, weight=6.3, config=pattern_configs['corrective_wave'], group='wave'),
            RecognizerConfig(name='wave_extension', enabled=True, weight=6.3, config=pattern_configs['wave_extension'], group='wave'),
            RecognizerConfig(name='wave_i', enabled=True, weight=6.3, config=pattern_configs['wave_i'], group='wave'),
            RecognizerConfig(name='wave_v', enabled=True, weight=6.3, config=pattern_configs['wave_v'], group='wave'),
            RecognizerConfig(name='wave_y', enabled=True, weight=6.3, config=pattern_configs['wave_y'], group='wave'),
            RecognizerConfig(name='wave_p', enabled=True, weight=6.3, config=pattern_configs['wave_p'], group='wave'),
            RecognizerConfig(name='wave_n', enabled=True, weight=6.3, config=pattern_configs['wave_n'], group='wave'),
            RecognizerConfig(name='wave_s', enabled=True, weight=6.3, config=pattern_configs['wave_s'], group='wave'),
        ],

        harmonic_patterns=[
            RecognizerConfig(name='gartley', enabled=True, weight=5.0, config=pattern_configs['gartley'], group='harmonic'),
            RecognizerConfig(name='butterfly', enabled=True, weight=5.0, config=pattern_configs['butterfly'], group='harmonic'),
            RecognizerConfig(name='bat', enabled=True, weight=5.0, config=pattern_configs['bat'], group='harmonic'),
            RecognizerConfig(name='crab', enabled=True, weight=5.0, config=pattern_configs['crab'], group='harmonic'),
        ],

        oscillator_patterns=[
            RecognizerConfig(name='cci', enabled=True, weight=4.0, config=pattern_configs['cci'], group='oscillator'),
            RecognizerConfig(name='stochastic', enabled=True, weight=4.0, config=pattern_configs['stochastic'], group='oscillator'),
            RecognizerConfig(name='williams_r', enabled=True, weight=4.0, config=pattern_configs['williams_r'], group='oscillator'),
            RecognizerConfig(name='mfi', enabled=True, weight=4.0, config=pattern_configs['mfi'], group='oscillator'),
        ],

        volume_patterns=[
            RecognizerConfig(name='chaikin_ad', enabled=True, weight=3.0, config=pattern_configs['chaikin_ad'], group='volume'),
        ],

        bollinger_patterns=[
            RecognizerConfig(name='bollinger_bands', enabled=True, weight=3.9, config=pattern_configs['bollinger_bands'], group='bollinger'),
        ],

        adx_patterns=[
            RecognizerConfig(name='adx', enabled=True, weight=2.1, config=pattern_configs['adx'], group='adx'),
        ],
...
def validate_optimized_config():
    ...
    if len(sorted_patterns) > 10:
        ...
        others_percentage = (others_count / total_signals) * 100 if total_signals > 0 else 0
...
def main():
    ...
    enabled_groups = []
    if config.enable_fibonacci_patterns: enabled_groups.append("Fibonacci")
    if config.enable_gann_patterns: enabled_groups.append("Gann")
    if config.enable_wave_patterns: enabled_groups.append("Wave")
    if config.enable_harmonic_patterns: enabled_groups.append("Harmonic")
    if config.enable_oscillator_patterns: enabled_groups.append("Oscillator")
    if config.enable_volume_patterns: enabled_groups.append("Volume")
    if config.enable_bollinger_patterns: enabled_groups.append("Bollinger")
    if config.enable_adx_patterns: enabled_groups.append("ADX")
    ...
    disabled_groups = []
    if not config.enable_candlestick_patterns: disabled_groups.append("Candlestick")
    if not config.enable_granville_patterns: disabled_groups.append("Granville")
    if not config.enable_heikin_ashi_patterns: disabled_groups.append("Heikin-Ashi")
    if not config.enable_dow_theory_patterns: disabled_groups.append("Dow Theory")
    ...
    print("1. Global strength threshold set to 0.1 to maximize signal volume while maintaining quality")
