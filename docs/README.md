...
## 🚀 クイックスタート - Unified Trainer

**推奨**: 学習にはUnified Trainerを使用してください。旧スクリプト（`run_smoke_test.py`等）は非推奨です。
...
- **学習/環境**: ztb/training/（Unified Trainer、PPO、コールバック、評価）
  - `trainers/` - 専門トレーナー（SELLBiasMitigationPPOTrainer等）
  - `callbacks_lib/` - トレーニングコールバック（SELLBiasMitigationCallback等）
  - `optimization/` - ハイパーパラメータ最適化ツール
  - `unified_trainer.py` - メインのトレーニングオーケストレータ
  → 詳細: [ztb/training/README.md](./ztb/training/README.md)
- **設定ファイル**: configs/training/（JSONベースの学習設定）
  - `ppo_100k_optimized.json` - 100kステップ最適化済み設定
  - カスタム設定を追加可能
...
- **非推奨スクリプト**: _deprecated/（旧トレーニングスクリプト）
  - 移行ガイド: [_deprecated/README.md](./_deprecated/README.md)
- **開発者向け**: アーキテクチャ/セットアップ/テスト
  → 詳細: [docs/contributing/architecture.md](./docs/contributing/architecture.md),
           ...
           [docs/contributing/testing.md](./docs/contributing/testing.md)_training.py --config my_config.json --algorithm curriculum
```

### 設定ファイルの例

最適化済みパラメータを使用した100kステップのトレーニング:
...
- ✅ **統一されたインターフェース**: 全てのアルゴリズム（PPO, Curriculum, Ensemble）を単一のエントリーポイントから実行
- ✅ **設定ファイルベース**: JSONで全パラメータを管理、バージョン管理が容易
- ✅ **保守性**: バグ修正とテストが一箇所に集約
- ✅ **拡張性**: 新しいアルゴリズムや機能を簡単に追加可能
...
旧スクリプトは`_deprecated/`ディレクトリに移動しました。移行ガイドは[こちら](./_deprecated/README.md)を参照してください。
...
- **reward_scaling最適化**: 二分探索で6.0を最適値として最適値として特定（71.10%リターン達成）
...
- **型安全性向上**: mypyエラーを237個から154個に削減（35%改善）、cast()使用による安全な型変換、設定キャッシュ機能の実装、Phase 1-4の包括的なコード品質改善計画完了（ドキュメンテーション、型安全性、インターフェース統一）
- **Ta-Lib統合**: 高性能テクニカル指標ライブラリの統合により、SMA/EMA/RSI/MACDなどの計算が高速化
- **共通ユーティリティ拡充**: データ検証、型定義、設定管理などの再利用可能なユーティリティを整備
- **系統統合テスト**: エンドツーエンドの動作を検証する包括的なテストスイートを実装運用を見据えた、**強化学習 + バックテスト + ペーパートレード**一体型の取引基盤。
...
- Ta-Libライブラリの統合により、テクニカル指標計算を高速化
- フォールバック実装により、Ta-Lib未インストール環境でも動作
- 包括的な入力検証とエラーハンドリングを実装
- パフォーマンス最適化とメモリ使用量の改善

効果:

- テクニカル指標計算が5-10倍高速化
- メモリ使用量の最適化により大規模データ処理が可能
- 堅牢なエラーハンドリングにより安定性が向上

#### 共通ユーティリティの拡充

実施内容:

- `data_validation.py`: DataFrameと数値データの検証ユーティリティ
- `types.py`: Protocol、TypedDict、型エイリアスによる高度な型定義
- 既存ユーティリティの強化（file_utils.py、logging_utils.py、config.py）

効果:

- コードの再利用性と保守性が向上
- 型安全性がさらに強化され、開発効率が改善
- データ検証によりランタイムエラーが減少
...
# ⚠️ 非推奨: 旧スクリプト（_deprecated/に移動済み）
...
```bash
# TensorBoard起動
tensorboard --logdir checkpoints/ppo_100k_optimized/tensorboard

# ログファイルの確認
type checkpoints\ppo_100k_optimized\training.log
```
...
# トレーニングモジュールのみテスト
python -m pytest ztb/tests/unit/training/ --cov=ztb.training --cov-report=term-missing -v

# カバレッジレポート生成
python -m pytest --cov=ztb --cov-report=html --cov-report=term-missing
```

### カバレッジ改善の進捗

- **unified_trainer.py**: 30% → 59% (29%改善)
  - 5つのトレーニングアルゴリズム全てのテスト実装
  - プロパティベーステスト（hypothesis）導入
  - エラーハンドリングとエッジケースの包括的テスト

- **ppo_trainer.py**: 87% (安定)
  - 既存の包括的なテストスイート

- **その他のモジュール**: テスト拡張中
  - ensemble.py: 予測と確率計算のテスト追加
  - curriculum_learning.py: 段階的学習のテスト
  - base_trainer.py: 抽象基底クラスのテスト

### テストインフラ

- **並列実行**: pytest-xdist使用（-n auto）
- **プロパティベーステスト**: hypothesisライブラリ導入
- **カバレッジレポート**: HTMLレポート自動生成
- **継続的インテグレーション**: 品質ゲート設定

### テスト戦略

1. **ユニットテスト**: 個別関数/メソッドの動作検証
2. **統合テスト**: エンドツーエンドのワークフロー検証
3. **プロパティベーステスト**: 自動エッジケース生成
4. **パフォーマンステスト**: トレーニング効率の検証
...
| パラメータ | 最適値 | 改善幅 | 信頼度 |
|-----------|--------|--------|--------|
| `n_steps` | **1024** | +11 | ✅ 高 (差分>10) |
| `vf_coef` | **0.1** | +1 | ✅ 高 (明確な傾向) |
| `reward_multipliers` | **5.0** | +152 | ✅ 高 (劇的改善) |
| `risk_free_rate` | **0.0** | - | ✅ 完全収束 |

#### 中信頼度パラメータ (要再検証)

| パラメータ | 暫定最適値 | 差分 | 推奨アクション |
|-----------|----------|------|---------------|
| `gamma` | **0.8475** | 1~2 | 100k再走 or マルチシード |
| `n_epochs` | **16** | <1 | 100k×1 or 50k×2 |

#### 統計的基盤

- **標準誤差**: ±3.62 (50kステップ、n=51エピソード)
- **95%信頼区間**: ±7.1
- **必要ステップ数の理論推定**:
  - 差分≥10検出: 26k以上
  - 差分≥5検出: 103k以上
  - 差分≥3検出: 285k以上 (または3シード×100k)

詳細は [`docs/binary_search_optimization/`](./docs/binary_search_optimization/) を参照してください。
...
| パラメータ | 最適値 | 成果 |
|-----------|--------|------|
| `r_target` | **0.175** | 31-34% SELL比率達成 |
| `tolerance` | **0.042625** | 制約違反 <0.01 |
| `eta` | **0.062875** | 安定した収束 |
| `lambda_max` | **3.875** | 適切な制約圧力 |
| `warmup_steps` | **3874** | 初期学習の安定化 |

詳細は [`docs/LAGRANGE_OPTIMIZATION_RESULTS.md`](./docs/LAGRANGE_OPTIMIZATION_RESULTS.md) を参照してください。

### 🔄 継続的な最適化

**📁 二分探索パラメータ最適化ドキュメント集**: [`docs/binary_search_optimization/`](./docs/binary_search_optimization/)

このディレクトリには以下のドキュメントが含まれます:
- 📊 **包括的な結果分析** - 過去の二分探索結果と統計的評価
- ✅ **パラメータ検証進捗管理** - 14パラメータの追跡シート
- 🚀 **クイックリファレンス** - 今すぐ実行できるコマンド集
- � **実行手順ガイド** - 初心者向け詳細ガイド
- � **Windowsコマンド集** - コピペ実行可能なコマンド

**クイックスタート**:
- 初めて実行する方 → [`BINARY_SEARCH_EXECUTION_GUIDE.md`](./docs/binary_search_optimization/BINARY_SEARCH_EXECUTION_GUIDE.md)
- 経験者・即実行したい方 → [`PARAMETER_VALIDATION_QUICK_REFERENCE.md`](./docs/binary_search_optimization/PARAMETER_VALIDATION_QUICK_REFERENCE.md)
- 進捗確認したい方 → [`PARAMETER_VALIDATION_TRACKING.md`](./docs/binary_search_optimization/PARAMETER_VALIDATION_TRACKING.md)

**推奨アクション**: 高優先度パラメータ (batch_size, learning_rate, max_grad_norm) から順次検証を実施してください。
