...
**レビュー結果**: 週末全停止(skip_days_of_week)は過剰対応として撤回。
buy_priority=0 の単独効果を先に観測し、データ駆動で段階的に判断する方針に変更。
...
### 5-1. ranging_buy_priority 無効化 (config) ✅ 適用中
...
### 5-2. 週末スキップ (code + config) — ⚠️ レビューにより撤回
...
```yaml
time_filter:
  skip_days_of_week: []    # 661# レビューにより全停止は撤回
```

#### 撤回理由 (独立レビューによる指摘)

1. **交絡因子の未分離**: 週末損失の63%は buy bias (ranging_buy_priority=3) 由来。
   buy_priority=0 適用後の週末パフォーマンスが**未知のまま**全停止するのは過剰
2. **水曜との非対称扱い**: 水曜 VWAP spread -25.45 bps は土曜(-29.66)の86%だが放置
   → 「週末だから」という曜日ラベルへの認知バイアスの疑い
3. **W10 の黒字週末**: W10 WE +0.03 bps → 全週末が赤字の前提は不成立
4. **取引量34%消失**: Sat+Sun fills=1,844（全体の34%）を全放棄する機会損失が未定量
5. **効果帰属の不可能**: 2変更同時投入で Change 1 単独の効果測定が不可能に
6. **spread品質の矛盾**: 週末 avg_spread (Sat 2.24, Sun 2.08) は平日より**タイト**

## 5-3. 週末損失の深堀り分析結果

### Buy/Sell バランスの曜日別分布

| 曜日 | Buy | Sell | Surplus | Buy比率 |
|------|-----|------|---------|---------|
| Mon | 268 | 254 | +14 | 51.3% |
| Tue | 262 | 257 | +5 | 50.5% |
| Wed | 326 | 303 | +23 | 51.8% |
...
**週末 surplus = +88 (全140の63%)** → buy_priority=0 で大幅改善が期待される。

### VWAP Spread (matched pairs, bias除去後)

| 曜日 | Matched BTC | VWAP Spread | Spread bps | Spread PnL (mBTC) |
|------|-------------|-------------|------------|-------------------|
| Mon | 0.254 | -10,971 JPY | -10.07 | -0.256 |
| Tue | 0.257 | -7,989 JPY | -7.42 | -0.191 |
| **Wed** | **0.303** | **-27,610 JPY** | **-25.45** | **-0.771** |
| Thu | 0.426 | -6,380 JPY | -5.82 | -0.248 |
| Fri | 0.525 | +759 JPY | +0.70 | **+0.037** |
| **Sat** | **0.505** | **-31,986 JPY** | **-29.66** | **-1.498** |
| **Sun** | **0.373** | **-24,373 JPY** | **-22.58** | **-0.842** |

**注目**: 水曜も -25.45 bps で日曜(-22.58)より悪い。金曜のみが黒字。

### PnL30 Side別

| 曜日 | Buy Mean | Sell Mean | 特徴 |
|------|----------|-----------|------|
| Mon | +0.18 | +0.09 | **両サイド黒字** |
| Sat | -0.43 | -0.43 | 両サイド均等に赤 |
| Sun | -0.42 | -0.66 | 両サイド赤（sell側が悪い） |

### 週別 平日 vs 週末

| Week | 平日 Mean | 平日 B/S | 週末 Mean | 週末 B/S |
|------|-----------|----------|-----------|----------|
| W8 | -0.01 | 271/275 | -0.43 | 171/167 |
| W9 | -0.20 | 355/348 | -1.18 | 108/104 |
| **W10** | -0.24 | 195/192 | **+0.03** | 146/148 |
| W11 | -0.03 | 286/289 | -0.44 | 150/151 |
| W12 | -0.23 | 338/329 | -0.50 | 181/105 |

**W10 週末は黒字** — 一律停止すべきでない根拠。

### テスト
- `test_661_skip_days_of_week.py`: 新規テスト 7 件（全パス、コード基盤の品質保証）
- `test_336`: KNOWN_YAML_OVERRIDES 追加 (ranging_buy_priority_max_consecutive)
...
| 変更 | 期待効果 | 根拠 |
|------|----------|------|
| buy_priority=0 | buy surplus 解消 → VWAP逆鞘改善 | +140 surplus の根本原因除去 |
| (週末停止は撤回) | — | レビューにより保留 |

**次のステップ**: buy_priority=0 の単独効果を1-2週間観測。
もし週末が依然赤字なら、以下の段階的対応を検討:
1. 週末 offset 拡大 (`weekend_offset_multiplier`)
2. 週末 lot 削減 (`weekend_lot_factor`)
3. worst window 限定停止 (Sat 01:00 等のピンポイント)
4. 週末全停止 (最終手段として `skip_days_of_week: [5, 6]`)
...
### 計測待ち
- **661# 効果観測**: buy_priority=0 の単独効果 → 1-2週間観測
- **週末再評価**: buy_priority=0 後の週末 BTC PnL を再測定し、段階的対応を判断
