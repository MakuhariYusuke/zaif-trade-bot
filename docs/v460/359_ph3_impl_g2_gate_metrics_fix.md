...
| ID | 内容 | 優先度 | 次ステップ |
|---|---|---|---|
| S-2 | `env_info["obs_dim"]` 型不整合 (int\|str → range) | MED | 次回 phg |
| S-3 | `sac_cfg` shadowing (dict コピーで上書き) | LOW | コメント追加済、実害なし |
...
| ファイル | 種別 | 内容 |
|---|---|---|
| `scripts/v460/lib/tasks/sac_train.py` | MOD | L-3: checkpoint ROI, L-5: eval gross_roi, `_extract_roi_from_env` 新規 |
| `scripts/v460/run_experiment.py` | MOD | (前回 commit) C-1~C-2, S-1, S-4, L-1/L-2 |
| `configs/v460/experiments/g2_sac_train.yaml` | MOD | (前回 commit) C-3: 356a# 参照修正 |
| `tests/unit/v460/test_356_g2_sac_blockers.py` | MOD | 新規テスト 4 件追加 |
