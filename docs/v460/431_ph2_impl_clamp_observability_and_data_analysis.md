...
| **Sell ceiling clamp rate** | **332/332 (100%)** |
| Buy AS rate | 22.6% |
| Sell AS rate | 28.9% |
...
1. **Ceiling clamp rate 100%** — buy/sell 双方で全非 skip 注文が ceiling に切り詰め。offset chain（base→regime→VG→kyle→amihud→EV mult）は **完全に装飾的**。
...
```python
ceiling = cfg.resolve_offset_ceiling(record.get("side", "buy"))
effective = record.get("effective_offset_used", -1)
if abs(effective - ceiling) < 1e-6:
...
**100% ceiling clamp rate は fill_test の最重要構造問題**。offset chain の全最適化・EV モデル・VG boost が ceiling で打ち消され、全注文が同一価格で出る。431# で clamp 発火率のリアルタイム可視化を実装したことで、ceiling 調整や Sidecar 蘇生の効果を定量評価できる基盤を整えた。
