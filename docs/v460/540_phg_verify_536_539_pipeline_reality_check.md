...
## §9 541# コード実装（Phase 1.5）

540# の分析を受けて実施した即時実装:

### 9.1 Lazy import のモジュールレベル引き上げ
...
### 9.2 Pre-order disabled stage のスキップ最適化
...
- `test_168_low_vol_offset_boost.py`: FFD テストに `fast_fill_defense_enabled=True` を追加（541# のスキップ最適化に対応）

### 9.4 spread_adapt 挙動の解明
...
## §10 542# 実装: ceiling 0.25→0.30 + 残存 identity 段分析

### 10.1 ceiling 0.25→0.30 の根拠

540# §2.1 で spread_adapt が offset を ~0.15→0.30 に倍増させ、ceiling 0.25 が 76.7% のサイクルで情報を切り捨てていることを発見。541# §9.4 で spread_adapt の narrow_spread_bps=2.5 がほぼ毎サイクル発火していることを確認。
