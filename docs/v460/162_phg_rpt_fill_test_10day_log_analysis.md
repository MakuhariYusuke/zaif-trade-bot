...
## §7 追記レビュー（163# 対象を含む・多角点検）

> 163# の「姑息的手段カタログ」を踏まえ、162# のログ分析をレビュー視点で再点検。

### 7.1 追加で重要な指摘（見落とし）

1. **分析粒度の混在リスク（run_id/commit混合）**
	- `temp/analyze_logs.py` は `fill_records_202602*.jsonl` を横断集計しており、run_id 境界で分離していない。
	- `temp/analyze_logs2.py` も日付帯で集計するが同様に run_id 固定がない。
	- 結果として、`git_sha` 変更や設定変更前後が混ざり、改善/劣化の因果推定がブレる。

2. **「10日間分析」と再現条件の未固定**
	- 対象ファイル指定がワイルドカード依存で、将来データ追加時に結果が変わる。
	- 再実行時の再現性確保には、`run_id`, `git_sha`, `date_from/to` を固定した分析モードが必要。

3. **163# の stopgap 棚卸しは有用だが、廃止判定基準が未定義**
	- 163# は「何が姑息か」の分類が強い一方、各機能を「いつOFFにするか」の SLO/ガード条件が不足。
	- 特に `time_filter`, `trending_sell_skip`, `balance_forced_skip` は解除順序を誤ると再悪化しやすい。

4. **Sell防御の“層”を減らす順番が未明確**
	- 163# の6層防御指摘は妥当。ただし即削減ではなく、`Inventory Skewing` → `per-regime判定` → `time_filter縮退` の順でないとリスクが高い。

### 7.2 過去成果の活用（未活用/活用不足の指摘）

- **107# の動的ゲーティング設計**は既に存在。time_filter の代替方針として、162/163 の改善提案に「実装入口（どのモジュールへ統合するか）」まで落とし込むべき。
- **159# / 160# の判定資産（3指標A/B + per-regime）**を、163# の stopgap 廃止判定にそのまま流用可能。現状は棚卸しと判定基盤が分断。
- **162# で言及済みの Inventory Skewing 実装（enabled=false）**は、最優先で「段階有効化手順 + ロールバック条件」を明文化すべき。ここが未定義だと balance 系 stopgap が残存する。

### 7.3 具体的な追補提案（レビュー）

| 優先 | 提案 | 目的 | 受入基準 |
|---|---|---|---|
| P0 | 分析スクリプトに `--run-id` / `--git-sha` / `--date-from,to` を追加 | 因果混在の排除 | 同一条件で再実行して同値結果 |
| P0 | stopgap 廃止の「退出基準表」を163#へ追記 | 廃止時の事故防止 | 機能ごとにON/OFF条件・監視指標を定義 |
| P1 | 160# 判定ロジックを stopgap 検証に統合 | 過去成果の再利用 | 3指標 + per-regime を日次で出力 |
| P1 | Inventory Skewing の段階有効化計画を161/163に接続 | 根本対策の運用化 | Stage1/2 各24hでKPI劣化なし |

### 7.4 最終レビュー判断

- 162# はログからの症状把握として質が高い。
- 163# は stopgap の構造化として有効で、根本対策への視座も妥当。
- ただし両者を接続する「**再現可能な評価軸**」と「**廃止の退出条件**」が未完成。
- 次アクションは新規機能追加よりも、既存成果（107/159/160/161/163）を1本の判定パイプラインに統合することが費用対効果最大。
