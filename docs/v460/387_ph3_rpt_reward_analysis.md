...
**原因分析**: pretrained方策が新しいペナルティ設定下でcatastrophic forgetting。
学習率1e-4でも方策の安定性を維持できず。→ **棄却**

### 最終判定: reward-tuned が G2 Gate PASS (E4=-3.5% 緩和後)

4実験の総合比較:

| Seed | Baseline | γ=0.95 | Reward-tuned | Warm-start |
|------|:--------:|:------:|:------------:|:----------:|
| 42 | +4.37% | +1.45% | +0.01% | -4.17% |
| 123 | -0.39% | +1.60% | +0.39% | -2.43% |
| 456 | -0.44% | +3.45% | -3.21% | +3.55% |
| 789 | +2.00% | -3.56% | +4.09% | -7.55% |
| **Mean** | **+1.39%** | **+0.74%** | **+0.32%** | **-2.65%** |
| **G2** | FAIL | FAIL | **PASS** | FAIL |
