...
- `MTFWeightManager` に apply() メソッドを提供して Candidate を実行時に反映
  ...
  - `set_weights(...)` の戻り値はブール値で、変更が拒否された場合は `False` を返します。呼び出し側は戻り値をチェックし、失敗時は再試行またはアラートすることを推奨します。`set_weights()` は内部でロックや copy-on-write を利用して原子的に適用する設計を採用してください。適用時には `applied_at` タイムスタンプと `applied_candidate_id` を telemetry に記録することを推奨します。
- `RewardCalculator` / `BehavioralPenaltyCalculator` に候補関連 telemetry を出力
 - `BalanceCurriculumManager` の stage-change listener を利用して、MTFScheduler の最適化/適用を実行できるようにする
  - 例: `bcm.add_stage_change_listener(mtf_scheduler.create_stage_change_callback(stage_filter=["balanced_transition"]))`
    - BalanceCurriculumManager の stage-change listener は callback をキーワード引数（kwargs）付きで呼び出します。Event のキーは `previous_stage`, `new_stage`, `step`, `emergency` です。`create_stage_change_callback()` が生成する callback は `new_stage` を使って判断するため、callback は `kwargs.get('new_stage')` を参照するように設計されています。
  ...
  - Gate 設定: `behavior.mtf.weight_optimizer.gate_min_reports` / `gate_composite_score` を設定すると、MTFScheduler は候補適用前に (1) `report_count` が指定値以上か、(2) `composite_score` が下限値以上か を検証します。未設定時は Scheduler 側のデフォルトが使われます。
  - `HeavyTradingEnv` は `mtf_scheduler` プロパティとして初期化済みインスタンスを公開します。テストやデバッグでは `env.mtf_scheduler.configure_gates(...)` のように直接ゲートを調整できます。
  - CLI 連携: `tools/training/run_mtf_scheduler.py` / `confirm_candidate.py` が `--gate-min-reports` と `--gate-composite-score` を受け取り、構成ファイルを介さずに Gate を上書きできます。CI / manual run で閾値を調整する際に利用してください。
...
- Artifacts: `reports/`、`logs/`、`best_model/` を保管
  ...
  - Scheduler integration: `run_mtf_scheduler.py` の CLI ゲート (`--gate-min-reports`, `--gate-composite-score`) と behavior config の `gate_min_reports` / `gate_composite_score` は同じ論理で評価されます。CI 側で CLI フラグを渡すと、設定ファイルを変更せずとも nightly job 内で閾値を tighten/relax できます。
...
- `HeavyTradingEnv` で `MTFScheduler` を自動生成するパスは `mtf_optimizer` と `behavior.mtf.weight_optimizer` の両方をサポートします。設定が dict か attribute オブジェクトかの両方に対応するための `_read_val()` ヘルパーを使う実装が入っています。
  ...
  - `BalanceCurriculumManager._revert_to_forced_balance()` は `previous_stage` を正しく設定してイベントを発行します。リスナーはイベントを受け取る際、kwargs として `previous_stage`, `new_stage`, `step`, `emergency` を受け取ることを想定してください（例: `def cb(**kwargs): stage = kwargs.get('new_stage')`).
...
4. CI の dry-run と gating（1-2日）
  ...
  - Scheduler gate を `gate_min_reports` (report 完了数) と `gate_composite_score` (安定化指標) の 2 軸で評価する。CPU 限定環境で seed ごとの diff が出やすいので、report gate は `seeds-1` 以上を推奨。
