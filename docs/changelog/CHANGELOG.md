...
#### SACモデルライブ取引バグの修正
- **live_trade.py - SACモデルタイプフラグのリセットバグ**:
  - **問題**: `_load_model()`で`_is_sac=True`を設定した後、初期化中にFalseにリセットされる
  - **影響**: SACモデルがPPOとして誤認識され、連続アクションが離散アクションとして処理されて取引エラー
  - **根本原因**: `__init__`でPositionManager初期化後に重複して`_is_sac = False`を設定していた
  - **修正**: 重複した`_is_sac = False`設定を削除（Line 297）
  - **結果**: SACモデルが正しく認識され、連続アクションが適切に処理されるようになった

- **アクション処理のDeprecationWarning修正**:
  - **問題**: `action[0]`でNumPy配列の要素アクセス時にDeprecationWarningが発生
  - **修正**: `action.item()`を使用した安全なスカラー変換
  - **効果**: 警告が解消され、SAC/PPO両方のアクション処理が安定

#### ライブ取引機能の改善
- **SACモデル対応の完全実装**:
  - 連続アクションから離散アクションへの変換ロジックを修正
  - dry-runモードでのSACモデルテストが可能になった
  - モデルタイプ検出の信頼性が向上

### ✅ テスト結果
- SACモデル（sac_v420_hold_relaxed.zip）のdry-runが正常動作
- 連続アクションの適切な処理を確認
- DeprecationWarningの完全解消
- ユニットテストおよび統合テストが通過
...
- **extract_model_info.py**: モデルZIPファイル解析ツール
  - Stable-Baselines3モデルの内容を表示
  - スキーマファイルが存在しない古いモデルの調査用

#### ドキュメント更新
- **MODEL_EVALUATION_REPORT_v4.0.1.md**:
