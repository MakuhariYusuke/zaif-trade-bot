...
sharpe_ratio = (mean_return / std_return) * sqrt(TRADING_DAYS_PER_YEAR)
```
...
```python
TRADING_DAYS_PER_YEAR = 252
excess_returns = returns - risk_free_rate / TRADING_DAYS_PER_YEAR  # 日次無リスク金利
sharpe_ratio = (mean(excess_returns) / std(excess_returns)) * sqrt(TRADING_DAYS_PER_YEAR)
```
