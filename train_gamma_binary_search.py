#!/usr/bin/env python3
"""
Binary search for gamma parameter
"""
...
def train_gamma_test(config_name="default", reward_scaling=6.0, entropy_coef=0.03, gamma=0.95):
    """Train with learning_rate test for 100k steps"""
    ...
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=5e-4,  # Fixed optimal learning_rate
        n_steps=2048,
        batch_size=64,
        n_epochs=10,
        gamma=gamma,  # Variable gamma for binary search
        gae_lambda=0.95,
        clip_range=0.2,
    ...
    print(f"Reward scaling: {reward_scaling}, Entropy coef: {entropy_coef}, Gamma: {gamma}")
    ...
    model_path = PROJECT_ROOT / "models" / f"gamma_test_{config_name}.zip"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train PPO model with specified learning rate")
    parser.add_argument("--gamma", type=float, default=0.95, help="Gamma parameter for PPO training")
    ...
    # Create config name from gamma
    gamma_str = f"{args.gamma:.3f}".replace(".", "")
    config_name = f"gamma_{gamma_str}"

    print(f"\n{'='*60}")
    print(f"Training with gamma = {args.gamma}")
    print(f"Config: {config_name}")
    print(f"{'='*60}")

    try:
        model_path = train_gamma_test(
